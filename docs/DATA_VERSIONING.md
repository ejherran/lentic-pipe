...
5. Run `scripts/dvc_data_assistant.sh push`.
6. Commit only code, configs, docs, manifests, reports, and `.dvc` pointers.

## Integrity Rules

- Any raw file change invalidates the current freeze.
- Any adapter, panel, target, split, fuzzy state, PIPE sequence, or model change
  requires updated derived hashes before results are used.
