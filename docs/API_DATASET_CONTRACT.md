...
`pipe_grud` is registered as `pipe_grud_reference_workflow_v0` with two explicit
...
inference. `parameters.execution_mode="artifact_reference"` validates and
reports the reviewed adaptive PIPE-GRU-D artifacts. Neither mode performs
dataset-specific external inference yet.
