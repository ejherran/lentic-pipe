...
def test_real_pending_contract_loads_without_opening_payloads() -> None:
    ...
    assert contract.h16_cert_commit == certification.H16_CERT_COMMIT
    ...
    assert contract.raw["test_certification"][
        "public_junit_redaction_policy"
    ] == contract.raw["isolation"]["public_junit_redaction_policy"]
    assert dict(contract.public_junit_redaction_policy) == contract.raw[
        "isolation"
    ]["public_junit_redaction_policy"]
    ...
    assert contract.raw["topology"]["H-CERT17"]["direct_parent"] == (
        "p16_cert_commit"
    )
    assert contract.raw["topology"]["P-CERT17"]["supersedes_P_CERT16"] is True
    assert contract.raw["topology"]["R-CERT17"] == {
        "role": "superseded_failed_final_doctoral_software_and_restorability_evidence",
    ...
    assert contract.raw["topology"]["H-CERT18"]["direct_parent"] == (
        "p17_cert_commit"
    )
    ...
    assert contract.raw["topology"]["H-CERT19"]["commit"] == "h19_cert_commit"
    assert contract.raw["topology"]["P-CERT19"] == {
        ...
        "authority_bytes": certification.P19_CANDIDATE_AUTHORITY_BYTES,
        "authority_sha256": certification.P19_CANDIDATE_AUTHORITY_SHA256,
        "companion_manifest_bytes": certification.P19_CANDIDATE_MANIFEST_BYTES,
        "companion_manifest_sha256": (
            certification.P19_CANDIDATE_MANIFEST_SHA256
        ),
    ...
    assert contract.raw["topology"]["H-CERT20"] == {
        "role": (
...
def test_schema_seals_scopes_suite_dvc_and_manifest_last() -> None:
    ...
    authorities = properties["authorities"]["properties"]
    ...
    assert properties["contract_version"] == {
        "const": certification.CONTRACT_VERSION
    }
    ...
    assert topology["P-CERT19"]["properties"][
        "generated_authority_matches_effective_reconstruction"
    ] == {"const": False}
    assert topology["P-CERT19"]["properties"][
        "generated_manifest_matches_effective_reconstruction"
    ] == {"const": False}
    assert topology["H-CERT20"]["properties"]["direct_parent"] == {
        "const": "h19_cert_commit"
    }
    assert topology["P-CERT20"]["properties"]["role"] == {
    ...
    assert scopes["H-CERT20"]["allOf"][1]["properties"]["modifications"][
        "const"
    ] == 11
    assert scopes["P-CERT20"]["allOf"][1]["properties"]["additions"][
        "const"
    ] == 2
    ...
    assert boundary["superseded_p16_retry_authorized"] == {"const": False}
    ...
    expected_junit_diagnostics = (
        certification.expected_public_tests_junit_diagnostic_policy()
    )
    assert expected_junit_diagnostics["duplicate_testcase_nodeid_policy"] == (
        "single_record_or_exact_failure_error_pair"
    )
    assert expected_junit_diagnostics["duplicate_testcase_pair_max_records"] == 2
    assert expected_junit_diagnostics[
        "duplicate_testcase_pair_exact_outcomes"
    ] == ["failure", "error"]
    assert expected_junit_diagnostics[
        "pytest_declared_tests_teardown_error_counter_accounting"
    ] is True
    assert expected_junit_diagnostics["declared_tests_minimum"] == (
        "logical_testcase_identity_count"
    )
    assert expected_junit_diagnostics["declared_tests_maximum"] == (
...
def test_effective_authority_loader_checks_topology_and_exact_companion(
    ...
    p_commit = "f" * 40
    h_commit = "e" * 40
    redaction_policy = certification.expected_public_junit_redaction_policy()
    expected_authority = {
    ...
    assert result["h16_cert_commit"] == contract.h16_cert_commit
    ...
    assert result["authority"]["public_junit_redaction_policy"] == (
        result["authority"]["isolation"]["public_junit_redaction_policy"]
    )
    assert result["manifest"] == certification._expected_effective_manifest(  # noqa: SLF001
        contract,
...
def test_effective_authority_reconstruction_binds_exact_isolation(
    ...
    assert authority["p16_failure"] == p16_failure
    assert p16_failure["attempt"] == "R-CERT16"
    assert p16_failure["active_error"]["public_tests_failure"]["totals"] == {}
    assert p16_failure["active_error"]["public_tests_failure"][
        "unavailable_reason"
    ...
    assert authority["p17_failure"] == p17_failure
    assert p17_failure["attempt"] == "R-CERT17"
    assert p17_failure["active_error"]["public_tests_failure"]["totals"] == {
        "tests": 944,
        "passed": 895,
        "failures": 7,
        "errors": 0,
    ...
    assert authority["p19_failure"] == p19_failure
    ...
    assert p19_failure["active_error"]["extra_keys"] == []
    assert p19_failure["active_error"]["shared_values_equal"] is True
    assert p19_failure["active_error"]["precommit_returncode"] == 0
    assert p19_failure["active_error"]["effective_loader_would_accept"] is False
    assert p19_failure["candidate_authority"]["bytes"] == (
        certification.P19_CANDIDATE_AUTHORITY_BYTES
    )
    assert p19_failure["candidate_authority"]["sha256"] == (
        certification.P19_CANDIDATE_AUTHORITY_SHA256
    )
    assert p19_failure["candidate_authority"][
        "public_junit_redaction_policy_top_level_present"
    ] is False
    assert p19_failure["candidate_authority"][
        "public_junit_redaction_policy_isolation_copy_present"
    ] is True
    assert p19_failure["candidate_manifest"]["bytes"] == (
        certification.P19_CANDIDATE_MANIFEST_BYTES
    )
    assert p19_failure["candidate_manifest"]["sha256"] == (
        certification.P19_CANDIDATE_MANIFEST_SHA256
    )
    ...
    assert p19_failure["evidence_counts"]["dvc_commands"] == 0
    assert p19_failure["evidence_counts"]["public_test_runs"] == 0
    assert p19_failure["evidence_counts"]["scientific_payload_reads"] == 0
    ...
    assert authority["topology"]["h16_cert_commit"] == contract.h16_cert_commit
    ...
    assert authority["h16_component_records"] == []
    assert authority["p16_component_records"] == []
    assert authority["h16_scope"] == certification.expected_h16_scope()
    assert authority["p16_scope"] == certification.expected_p16_scope()
    assert authority["r16_scope"] == certification.expected_r16_scope()
    assert authority["h17_component_records"] == []
    assert authority["p17_component_records"] == []
    ...
    assert authority["public_junit_redaction_policy"] == authority["isolation"][
        "public_junit_redaction_policy"
    ]
    assert authority["public_junit_redaction_policy"][
        "historical_marker_bearing_nodeids_preserved_or_published"
    ...
    manifest = certification._expected_effective_manifest(  # noqa: SLF001
        contract,
        authority_bytes=authority_bytes,
        h_cert_commit="e" * 40,
    )
    ...
    assert manifest["authority"] == manifest["outputs"][0]
...
def test_historical_p3_is_byte_exact_and_failure_is_sanitized() -> None:
    ...
    complete_v16 = certification._historical_through_p16_records(  # noqa: SLF001
        contract,
        root=ROOT,
    )
    assert [len(group) for group in complete_v16] == [11, 2] * 15
    assert complete_v16[-1] == [
        {
            "path": certification.H16_AUTHORITY_PATH.as_posix(),
            "bytes": certification.H16_AUTHORITY_BYTES,
            "sha256": certification.H16_AUTHORITY_SHA256,
            "git_blob_oid": complete_v16[-1][0]["git_blob_oid"],
            "git_mode": "100644",
        },
        {
            "path": certification.H16_AUTHORITY_MANIFEST_PATH.as_posix(),
