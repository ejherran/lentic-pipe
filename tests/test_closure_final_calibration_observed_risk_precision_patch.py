...
from src.experiments import (
    closure_final_calibration_observed_risk_precision_patch as patch,
)
from src.experiments import (
    lock_closure_final_calibration_observed_risk_precision_patch as locker,
)
from src.experiments import run_closure_anfis_learning_curve as e7_runner


ROOT = Path(__file__).resolve().parents[1]
BASE_P_MCALG = "34a4578efa37d0ccf09e7fc45adf655d3b5a21c1"
H_MCALG = "ba15b7647811bbef2b63b3eb904ba427788da048"
EXPECTED_SUPERSEDED = {
...
EXPECTED_PRECISION_CONTRACT: dict[str, Any] = {
...
def _forbid_real_scientific_payload_reads(
    ...
    def forbidden(*args: Any, **kwargs: Any) -> Any:
        del args, kwargs
        raise AssertionError("MCALH governance opened a scientific payload")
...
def _precision_contract() -> dict[str, Any]:
    return json.loads(json.dumps(EXPECTED_PRECISION_CONTRACT))
    ...
    def require_snapshot(
        ...
        *,
        ...
        if expected != observed:
            raise patch.FinalCalibrationObservedRiskPrecisionPatchError(
...
def test_h_p_r_topology_and_published_bases_are_exact() -> None:
    assert patch.PATCH_GATE == "E0-MCALH"
    assert patch.BASE_P_MCALG_COMMIT == BASE_P_MCALG
    assert patch.H_MCALG_COMMIT == H_MCALG
    assert patch.H_MCALG_PARENT == "11a130809c2ad5d37c100e681d1f7a03c611603d"
...
def test_h_mcalg_partition_and_prior_p_inputs_are_exact() -> None:
    assert set(patch.H_MCALG_SUPERSEDED_PATHS) == EXPECTED_SUPERSEDED
    assert len(patch.H_MCALG_PRESERVED_PATHS) == 5
    assert not set(patch.H_MCALG_PRESERVED_PATHS) & EXPECTED_SUPERSEDED
    assert set(patch.P_MCALG_PATHS) == {
        "reports/closure_v1/00_protocol/"
        "final_calibration_ordinal_label_representation_patch_lock.json",
        "reports/closure_v1/00_protocol/"
    ...
    authority_source = inspect.getsource(patch._p_mcalg_authority)
    for token in (
        "mcalg._state_for_h(",
        "mcalg._p_mcalf_authority(repo_root=repo_root)",
...
def test_consumed_failure_is_exact_clean_non_retryable_and_documented() -> None:
    ...
    text = (
        ROOT
        / "docs/closure_v1/"
        "E0_M_FINAL_CALIBRATION_OBSERVED_RISK_PRECISION_PATCH_1.md"
    ).read_text()
    for token in (
        "E0-MCALH",
        "4M+5A",
...
def test_observed_risk_precision_contract_is_exact_and_schema_closed() -> None:
    contract = json.loads(
        patch._canonical_json_bytes(patch.OBSERVED_RISK_PRECISION_CONTRACT)
    )
    assert contract == EXPECTED_PRECISION_CONTRACT
    assert set(contract) == set(EXPECTED_PRECISION_CONTRACT)
    assert contract["calibration_bundle_dtype"] == "float64"
    assert contract["bloom_bundle_dtype"] == "float32"
    assert contract["target_join_keys"] == list(
        calibration_runner.TARGET_JOIN_COLUMNS
    )
    assert contract["target_comparison"] == "exact_float64_array_equality"
    assert contract["absolute_tolerance"] == 0.0
    assert contract["relative_tolerance"] == 0.0
    assert contract["float32_roundtrip_authorized"] is False
    assert contract["data_rewrite_performed"] is False
    assert contract["model_rewrite_performed"] is False
    assert contract["prediction_rewrite_performed"] is False
    schema = _schema()
    definition = schema["$defs"]["observedRiskPrecisionContract"]
    assert definition == {"const": EXPECTED_PRECISION_CONTRACT}


def test_runner_bundle_preserves_exact_float64_risk_and_float32_bloom(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    target_rows: list[dict[str, Any]] = []
    sequence_rows: list[dict[str, str]] = []
    expected_risk = np.empty((224, 3), dtype=np.float64)
    for origin_index in range(224):
        common_origin_id = f"origin-{origin_index:03d}"
        site_id = f"site-{origin_index:03d}"
        sequence_rows.append(
            {
                ...
                "origin_year_month": "2020-12",
            }
        )
        for horizon_index, horizon in enumerate((1, 2, 3)):
            risk = np.float64((origin_index * 3 + horizon + 0.125) / 673.0)
            expected_risk[origin_index, horizon_index] = risk
            target_rows.append(
                {
                    "source_id": "wqp",
                    "site_id": site_id,
                    ...
                    "time_role": "calibration_threshold",
                    "origin_year_month": "2020-12",
                    "target_year_month": f"2021-{horizon:02d}",
                    "horizon_months": horizon,
                    "bloom_h": bool((origin_index + horizon) % 2),
    ...
    monkeypatch.setattr(
        calibration_runner.anfis_training,
        "_tensor_from_sequence",
        lambda frame, *, model_id: np.zeros((len(frame), 3, 2), dtype=np.float32),
    )
    monkeypatch.setattr(
        calibration_runner.anfis_training,
        "apply_mask_aware_standardizer",
        lambda tensor, standardizer: tensor,
    )
    standardizer = calibration_runner.anfis_training.RawStandardizer(
        columns=(),
        counts=np.array([], dtype=np.int64),
        means=np.array([], dtype=np.float64),
        standard_deviations=np.array([], dtype=np.float64),
    )
    bundle = calibration_runner._calibration_bundle_from_sequence(
        targets,
        sequence,
        model_id="A0",
        standardizer=standardizer,
    )
    calibration_runner._require_anfis_calibration_bundle_dtypes(
        bundle, context="governance bundle"
    )
    assert bundle.x.dtype == np.float32
    assert bundle.bloom.dtype == np.float32
    assert bundle.risk.dtype == np.float64
    assert np.array_equal(bundle.risk, expected_risk)
    assert np.count_nonzero(
        bundle.risk != bundle.risk.astype(np.float32).astype(np.float64)
    ) > 0

    shape = (224, 3)
    predicted = calibration_runner._anfis_calibration_threshold_prediction_frame(
        bundle,
        model_id="A0",
        base_seed=1729,
        bloom_probability=np.full(shape, 0.5, dtype=np.float64),
        risk_mu=np.full(shape, 0.5, dtype=np.float64),
        risk_logvar=np.full(shape, -4.0, dtype=np.float64),
    )
    joined = predicted.merge(
        targets.loc[
            :,
            [*calibration_runner.TARGET_JOIN_COLUMNS, "target_risk_chla_h"],
        ],
        on=list(calibration_runner.TARGET_JOIN_COLUMNS),
        how="inner",
        validate="one_to_one",
    )
    observed = calibration_runner._require_observed_risk_precision(
        joined["observed_risk"], context="governance observed"
    )
    target = calibration_runner._require_observed_risk_precision(
        joined["target_risk_chla_h"], context="governance target"
    )
    assert len(joined) == 672 and np.array_equal(observed, target)
    assert "closure_final_calibration_observed_risk_precision_patch" in (
        inspect.getsource(calibration_runner)
    )


def test_float32_roundtrip_tolerance_and_wrong_dtypes_fail_closed() -> None:
    exact = np.array([0.0, 0.11453989988418839, 1.0], dtype=np.float64)
    assert calibration_runner._require_observed_risk_precision(
        exact, context="governance exact"
    ) is exact
    series = pd.Series(exact, dtype="float64")
    assert np.array_equal(
        calibration_runner._require_observed_risk_precision(
            series, context="governance series"
        ),
        exact,
    )
    for malformed in (
        exact.astype(np.float32),
        exact.astype(np.int64),
        exact.astype(object),
        ...
        with pytest.raises(patch.FinalCalibrationError):
            calibration_runner._require_observed_risk_precision(
                malformed, context="governance drift"
            )

    metadata = pd.DataFrame({"time_role": ["calibration_threshold"]})
    valid_bundle = calibration_runner.anfis_training.TrainingBundle(
        metadata=metadata,
        x=np.zeros((1, 3, 2), dtype=np.float32),
        bloom=np.zeros((1, 3), dtype=np.float32),
        risk=np.zeros((1, 3), dtype=np.float64),
    )
    calibration_runner._require_anfis_calibration_bundle_dtypes(
        valid_bundle, context="governance exact"
    )
    for field in ("x", "bloom", "risk"):
        values = {
            "metadata": metadata,
            "x": valid_bundle.x,
            "bloom": valid_bundle.bloom,
            "risk": valid_bundle.risk,
        }
        values[field] = values[field].astype(
            np.float64 if field != "risk" else np.float32
        )
        malformed_bundle = calibration_runner.anfis_training.TrainingBundle(**values)
        with pytest.raises(patch.FinalCalibrationError):
            calibration_runner._require_anfis_calibration_bundle_dtypes(
                malformed_bundle, context=f"governance {field} drift"
            )

    rounded = exact.astype(np.float32).astype(np.float64)
    assert not np.array_equal(exact, rounded)
    precision_source = inspect.getsource(
        calibration_runner._require_observed_risk_precision
    )
    bundle_source = inspect.getsource(
        calibration_runner._calibration_bundle_from_sequence
    )
    validator_source = inspect.getsource(calibration_runner._validate_calibration_frame)
    assert "np.empty_like(" not in bundle_source
    assert "dtype=np.float64" in bundle_source
    assert "np.array_equal(" in validator_source
    for forbidden in ("allclose", "isclose", "round("):
        assert forbidden not in precision_source + validator_source
...
def test_schema_preflight_and_semantic_payload_chain_are_closed(
    ...
    schema = _schema()
    assert schema["properties"]["gate"]["const"] == "E0-MCALH"
    result = (
        patch.preflight_final_calibration_observed_risk_precision_patch_schema()
    )
    assert result["gate"] == "E0-MCALH"
    assert result["supported_subset_verified"] is True
    assert result["schema_count"] == 1

    prior_lock_path = patch.mcalg.DEFAULT_PATCH_LOCK_PATH
    prior_manifest_path = patch.mcalg.DEFAULT_PATCH_LOCK_MANIFEST_PATH
    ...
    assert prior_lock_bytes == patch.mcal._git_blob_bytes(
        ROOT, BASE_P_MCALG, prior_lock_path
    )
    assert prior_manifest_bytes == patch.mcal._git_blob_bytes(
        ROOT, BASE_P_MCALG, prior_manifest_path
    )
    ...
    prelock = {
        "repository": {
            "base_p_mcalg_commit": BASE_P_MCALG,
            "h_patch_head": "a" * 40,
            "branch": "main",
            "remote_head": "a" * 40,
            "scope": {
        ...
        "observed_risk_precision_contract": _precision_contract(),
    ...
    payload = (
        patch.build_final_calibration_observed_risk_precision_patch_lock_payload(
    ...
    def validate_synthetic_schema(value: Any, observed_schema: Any) -> None:
        ...
        assert value["gate"] == "E0-MCALH"
        assert value["observed_risk_precision_contract"] == (
    ...
    monkeypatch.setattr(
        patch,
        "collect_final_calibration_observed_risk_precision_patch_prelock_state",
        lambda *, verify_remote, repo_root: json.loads(
            patch._canonical_json_bytes(prelock)
        ),
    )
    assert (
        patch.validate_final_calibration_observed_risk_precision_patch_lock_payload(
...
def test_companion_is_exact_16_physical_4_historical_1_output_and_canonical(
    ...
    with monkeypatch.context() as loader_patch:
        ...
        for lock_ns, manifest_ns in (
            ...
            authority = (
                patch.load_effective_final_calibration_observed_risk_precision_patch_authority(
    ...
    with monkeypatch.context() as loader_patch:
        _install_synthetic_loader(loader_patch)
        with pytest.raises(patch.FinalCalibrationObservedRiskPrecisionPatchError):
            patch.load_effective_final_calibration_observed_risk_precision_patch_authority(
                verify_remote=False, repo_root=tmp_path
            )
...
def test_exact_four_superseded_paths_adopt_mcalg_and_gate_before_io() -> None:
    assert calibration_runner.calibration is patch
    assert e7_runner.calibration is patch
    for path in EXPECTED_SUPERSEDED:
        text = (ROOT / path).read_text()
        assert "closure_final_calibration_observed_risk_precision_patch" in text
...
def test_cli_modes_are_closed_and_mutually_exclusive() -> None:
    ...
    ordered = [
        "preflight_final_calibration_observed_risk_precision_patch_schema",
        "before =",
        "run_final_calibration_observed_risk_precision_patch_verification",
        "after =",
        "if before != after:",
        "build_final_calibration_observed_risk_precision_patch_lock_payload",
        "validate_final_calibration_observed_risk_precision_patch_lock_payload",
        "publish_final_calibration_observed_risk_precision_patch_lock_bundle",
    ]
...
def test_check_only_runs_schema_before_remote_prelock_and_is_nonwriting(
    ...
    monkeypatch.setattr(
        patch,
        "preflight_final_calibration_observed_risk_precision_patch_schema",
        lambda: events.append("schema") or {"gate": "E0-MCALH"},
    )
    monkeypatch.setattr(
        patch,
        "collect_final_calibration_observed_risk_precision_patch_prelock_state",
...
def test_effective_loader_and_run_namespace_close_zero_six_eight_and_races(
    ...
    with monkeypatch.context() as completed_patch:
        ...
        for runner in ("calibration", "e7"):
            with pytest.raises(
                patch.FinalCalibrationObservedRiskPrecisionPatchError
            ):
                patch.require_final_calibration_run_namespace(
...
def test_full_publisher_is_canonical_manifest_last_frozen_and_no_clobber(
    ...
    with monkeypatch.context() as bundle_patch:
        ...
        observed, companion = (
            patch.publish_final_calibration_observed_risk_precision_patch_lock_bundle(
