...
def _target_rows() -> pd.DataFrame:
    rows: list[dict[str, object]] = []
    for origin_month in ["2022-01", "2022-02", "2022-03"]:
...
def _write_identity_neural_ode_model(path: Path) -> None:
...
def test_evaluate_pipe_neural_ode_rollouts_cli_writes_backtest_outputs(tmp_path: Path) -> None:
    ...
    _write_identity_neural_ode_model(model_path)
    ...
    assert overall_all["rows"].tolist() == [27, 27]
    assert all(math.isclose(value, 0.0, abs_tol=1e-7) for value in overall_all["rmse"])
    assert irc_alert["rows"].tolist() == [3, 3]
    assert irc_alert["positive_rows"].tolist() == [3, 3]
    ...
    assert manifest["row_counts"]["selected_origins"] == 3
    assert manifest["row_counts"]["evaluated_rollout_rows"] == 6
    assert manifest["row_counts"]["backtest_row_export_rows"] == 6
    assert manifest["script"]["path"] == "src/experiments/evaluate_pipe_neural_ode_rollouts.py"
    assert "PIPE Neural ODE Rollout Backtest Report v0" in report_path.read_text(encoding="utf-8")


def test_evaluate_pipe_neural_ode_rollouts_can_match_reference_origins(tmp_path: Path) -> None:
    pytest.importorskip("torch")
    pytest.importorskip("torchdiffeq")
    ...
    model_path = tmp_path / "model.pt"
    model_manifest_path = tmp_path / "model_manifest.json"
    reference_path = tmp_path / "reference_rows.parquet"
    metrics_path = tmp_path / "metrics.csv"
    alert_metrics_path = tmp_path / "alert_metrics.csv"
    examples_path = tmp_path / "examples.csv"
    backtest_rows_path = tmp_path / "backtest_rows.parquet"
    ...
    assert overall_all["rows"].tolist() == [9, 9]
    assert len(backtest_rows) == 2
    assert backtest_rows["origin_year_month"].tolist() == ["2022-02", "2022-02"]
