...
def _historical_records() -> tuple[list[dict[str, Any]], ...]:
    ...
    p18_records = records(locker.P18_SCOPE, 4001)
    for record in p18_records:
        if record["path"] == locker.H18_AUTHORITY_PATH.as_posix():
            record["bytes"] = locker.H18_AUTHORITY_BYTES
            record["sha256"] = locker.H18_AUTHORITY_SHA256
        else:
...
def _install_contract_stubs(
    ...
    monkeypatch.setattr(
        certification,
        "expected_h18_scope",
        lambda: dict(locker.H18_SCOPE),
    )
    monkeypatch.setattr(
        certification,
        "expected_p18_scope",
        lambda: dict(locker.P18_SCOPE),
    )
    monkeypatch.setattr(
        certification,
        "expected_r18_scope",
        lambda: dict(locker.R18_SCOPE),
    )
    monkeypatch.setattr(
        certification,
        "expected_p_scope",
    ...
    monkeypatch.setattr(
        certification,
        "expected_p18_modes",
        lambda: {path: "100644" for path in locker.P18_SCOPE},
    )
    monkeypatch.setattr(
        certification,
        "expected_r18_modes",
        lambda: {path: "100644" for path in locker.R18_SCOPE},
    )
    monkeypatch.setattr(
        certification,
        "expected_h13_modes",
...
def _make_repository(tmp_path: Path) -> tuple[Path, Path, *tuple[str, ...]]:
    ...
    for path_text in locker.H18_SCOPE:
        path = root / path_text
        path.write_text(
            path.read_text(encoding="utf-8") + "H-CERT18\n",
            encoding="utf-8",
        )
        path.chmod(int(locker.H_GIT_MODES[path_text][-3:], 8))
    _run(root, "git", "add", *locker.H18_SCOPE)
    _run(root, "git", "commit", "-m", "H-CERT18")
    h18_cert_commit = _run(root, "git", "rev-parse", "HEAD")

    for relative in (locker.H18_AUTHORITY_PATH, locker.H18_MANIFEST_PATH):
        target = root / relative
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_bytes((locker.PROJECT_ROOT / relative).read_bytes())
        target.chmod(0o644)
    _run(root, "git", "add", *locker.P18_SCOPE)
    _run(root, "git", "commit", "-m", "P-CERT18")
    p18_cert_commit = _run(root, "git", "rev-parse", "HEAD")

    for path_text in locker.H19_SCOPE:
        path = root / path_text
        path.write_text(
            path.read_text(encoding="utf-8") + "H-CERT19\n",
            encoding="utf-8",
        )
        path.chmod(int(locker.H_GIT_MODES[path_text][-3:], 8))
    _run(root, "git", "add", *locker.H19_SCOPE)
...
def test_scopes_modes_and_stop_boundary_are_exact() -> None:
    ...
    assert locker.P19_SCOPE == {
        "configs/closure_v1/phase4_final_certification_authority_v19.json": "A",
        "configs/closure_v1/phase4_final_certification_authority_manifest_v19.json": "A",
    }
    assert locker.P18_SCOPE == {
    ...
    assert locker.H18_AUTHORITY_BYTES == 166_277
    assert locker.H18_AUTHORITY_SHA256 == (
...
def test_authority_binds_every_frozen_surface_and_rejects_tampering() -> None:
    ...
    assert authority["topology"] == {
        ...
        "h18_cert_commit": locker.H18_CERT_COMMIT,
    ...
    assert authority["p18_failure"] == certification.expected_p18_failure_record()
    p18_failure = authority["p18_failure"]
    ...
    assert p18_failure["observed_cause"]["trigger"] == (
    ...
    assert "historical_marker_bearing_nodeids_serialized" not in (
        p18_failure["observed_cause"]
    )
    assert p18_failure["observed_cause"]["public_test_totals"] == {
        "tests": 944,
        "passed": 902,
        "failures": 0,
        "errors": 0,
    ...
    assert [record["path"] for record in authority["h18_component_records"]] == list(
        locker.H18_SCOPE
    )
    assert [record["path"] for record in authority["p18_component_records"]] == list(
        locker.P18_SCOPE
    )
    assert [record["path"] for record in authority["h19_component_records"]] == list(
        locker.H19_SCOPE
    )
    ...
    assert authority["isolation"]["public_junit_redaction_policy"] == (
        certification.expected_public_junit_redaction_policy()
    )
    redaction_policy = authority["isolation"]["public_junit_redaction_policy"]
...
def test_manifest_is_canonical_and_binds_only_the_authority_output() -> None:
    ...
    assert manifest["h18_cert_commit"] == locker.H18_CERT_COMMIT
    ...
    assert manifest["supersedes_p18"] is True
...
def test_check_only_accepts_pending_suite_only_for_local_h4(
    ...
    assert result["h18_cert_commit"] == h18
...
def test_generation_revalidates_state_and_publishes_exact2_manifest_last(
    ...
    original_projection = locker._require_independent_projection  # noqa: SLF001
    ...
    def observed_projection(
        ...
        publication_events.append("projection")
        original_projection(projection_root, authority)

    def observed_create(*args: Any, **kwargs: Any) -> locker._OwnedFileAt:  # noqa: SLF001
        publication_events.append(f"temporary:{args[1]}")
    ...
    def observed_link(
        ...
        publication_events.append(f"link:{name}")
        return original_link(source, parent_fd, name)

    monkeypatch.setattr(locker, "_require_independent_projection", observed_projection)
    ...
    assert publication_events[:2] == ["projection", "projection"]
    assert publication_events[2].startswith("temporary:")
    assert publication_events.index(f"link:{locker.AUTHORITY_PATH.name}") > 2
    assert _run(
