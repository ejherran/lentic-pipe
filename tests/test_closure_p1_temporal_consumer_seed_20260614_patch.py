...
from src.experiments import (
    closure_p1_temporal_consumer_seed_20260614_patch as module,
)
from src.experiments import (
    lock_closure_p1_temporal_consumer_seed_20260614_patch as locker,
)
...
def _prelock() -> dict[str, Any]:
    ...
    return {
        ...
        "progression_prelock": {
            ...
            "expected_present_count": 22,
            "expected_present_paths": [
                *(f"prior/{index}" for index in range(18)),
                *(f"current/{index}" for index in range(4)),
            ],
            "expected_present_paths_sha256": ZERO_SHA,
            "registered_absent_count": 118,
            "exact_registered_namespace_verified": True,
            "prior_seeds": [1729, 20_260_612, 20_260_613],
            "prior_present_count": 18,
            "prior_present_paths": [f"prior/{index}" for index in range(18)],
            "prior_present_paths_sha256": ZERO_SHA,
            "prior_residual_absent_count": 66,
            "current_seed": 20260614,
...
def test_physical_schema_uses_real_closure_contract_subset() -> None:
    ...
    observed = module.preflight_p1_temporal_consumer_seed_20260614_patch_schema()
...
def test_progression_prelock_requires_exact_registered_namespace(
    ...
    assert observed["expected_present_count"] == 22
    assert observed["registered_absent_count"] == 118
    assert observed["prior_present_count"] == 18
    assert observed["prior_seeds"] == [1729, 20_260_612, 20_260_613]
    assert observed["prior_residual_absent_count"] == 66
    assert observed["later_absent_count"] == 28
...
def test_historical_e0_ml_preserves_prior_publications(
    monkeypatch: pytest.MonkeyPatch,
    loader_name: str,
    seed: str,
) -> None:
    observed = module._historical_e0_ml_authority(
        execution_head=module.PATCH_BASE_COMMIT
    )
    assert observed["gate"] == "E0-ML"
    ...
    with pytest.raises(
        module.P1TemporalConsumerSeed20260614PatchError,
        match=rf"P1/{seed} publication drifted",
    ):
        module._historical_e0_ml_authority(
            execution_head=module.PATCH_BASE_COMMIT
        )


@pytest.mark.parametrize("authority_name", ["e0_mj", "e0_mk"])
def test_historical_e0_ml_rejects_nested_authority_drift(
    monkeypatch: pytest.MonkeyPatch,
    authority_name: str,
) -> None:
    original_loader = module._load_regular_json
    payload = copy.deepcopy(
        original_loader(module.e0_ml.DEFAULT_PATCH_LOCK_PATH, context="P-E0-ML lock")
    )
    payload["base_authorities"][authority_name]["gate"] = "drifted"

    def load(path: Path, *, context: str) -> dict[str, Any]:
        if path == module.e0_ml.DEFAULT_PATCH_LOCK_PATH:
            return payload
    ...
    with pytest.raises(
        ...
        module._historical_e0_ml_authority(execution_head=module.PATCH_BASE_COMMIT)
...
def test_mm_parser_preserves_both_pytest_duration_forms(
    monkeypatch: pytest.MonkeyPatch,
    summary: str,
    expected_format: str,
) -> None:
...
def test_verification_preflights_schema_before_first_command(
    ...
    observed = locker.run_p1_temporal_consumer_seed_20260614_patch_verification(
...
def test_check_only_preflights_schema_before_prelock_and_exposes_evidence(
    ...
    def collect(**kwargs: Any) -> dict[str, Any]:
        ...
        return {
            ...
            "p1_20260614_publication": {"commit": module.P1_BUNDLE_COMMIT},
...
def test_mocked_complete_transaction_uses_real_schema_and_validator(
    ...
    historical_mi = prelock["base_authorities"]["e0_mk"]
    historical_mj = prelock["base_authorities"]["e0_ml"]
    publication = prelock["p1_20260614_publication"]
    ...
    monkeypatch.setattr(module, "_historical_e0_mk_authority", lambda **kwargs: historical_mi)
    monkeypatch.setattr(module, "_historical_e0_ml_authority", lambda **kwargs: historical_mj)
    monkeypatch.setattr(module, "_published_p1_20260614_bundle", lambda **kwargs: publication)
    ...
    monkeypatch.setattr(
        locker,
        "run_p1_temporal_consumer_seed_20260614_patch_verification",
    ...
    monkeypatch.setattr(locker, "load_and_validate_p1_temporal_consumer_seed_20260614_patch_lock", lambda **kwargs: ({}, {}))
...
def test_companion_has_exact_24_physical_and_two_historical_inputs() -> None:
    ...
    mk_preserved = [
        _record(f"mk/preserved/{index}", f"mk_preserved_{index}")
        for index in range(5)
    ]
    mk_superseded = [
        _record(path, f"mk_superseded_{index}")
        for index, path in enumerate(module.PATCH_MODIFIED_PATHS)
    ]
    ml_preserved = [
        _record(f"ml/preserved/{index}", f"ml_preserved_{index}")
        for index in range(7)
    ]
    publication = [
        _record(f"p1/20260614/{index}", f"publication_{index}")
    ...
    payload = {
        ...
        "base_authorities": {
            "e0_mk": {
                "preserved_components": {"records": mk_preserved},
                "superseded_components": {"records": mk_superseded},
                "lock": _record("mk/lock.json", "mk_lock"),
                "companion_manifest": _record("mk/companion.json", "mk_companion"),
            },
            "e0_ml": {
                "preserved_components": {"records": ml_preserved},
                "lock": _record("ml/lock.json", "ml_lock"),
                "companion_manifest": _record("ml/companion.json", "ml_companion"),
            },
        },
        "p1_20260614_publication": {"records": publication},
    }
    companion = module._expected_companion(
        payload,
        lock_record=_record("mm/lock.json", "mm_lock"),
    )
    ...
    assert {record["commit"] for record in historical} == {module.E0_MK_H_COMMIT}
...
def test_locker_failure_rolls_back_empty_owner_set(
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: (_ for _ in ()).throw(
            module.P1TemporalConsumerSeed20260614PatchError("injected failure")
        ),
    )
...
def test_companion_failure_rolls_back_lock_owner_manifest_last(
    ...
    monkeypatch.setattr(locker, "_acquire_guards", lambda *args: guards)
    monkeypatch.setattr(locker.hardened, "_assert_lock_namespace", lambda *args: None)
    monkeypatch.setattr(locker, "collect_p1_temporal_consumer_seed_20260614_patch_prelock_state", lambda **kwargs: prelock)
    monkeypatch.setattr(locker, "run_p1_temporal_consumer_seed_20260614_patch_verification", lambda **kwargs: {})
    monkeypatch.setattr(locker, "p1_consumer_namespace_absence", lambda: {})
    monkeypatch.setattr(locker, "build_p1_temporal_consumer_seed_20260614_patch_lock_payload", lambda *args, **kwargs: {"patch_repository": {"head": PATCH_HEAD}})
    monkeypatch.setattr(locker, "_load_regular_json", lambda *args, **kwargs: {})
    monkeypatch.setattr(locker, "validate_p1_temporal_consumer_seed_20260614_patch_lock_payload", lambda *args, **kwargs: None)
    monkeypatch.setattr(locker, "_expected_companion", lambda *args, **kwargs: {})
...
def test_future_p_mm_publication_is_exact_two_additions(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_future_p_mm_rejects_any_third_path(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    payload = {"patch_repository": {"head": PATCH_HEAD}}
    monkeypatch.setattr(module, "_introduced_commit", lambda path: LOCK_COMMIT)
...
def test_guard_namespace_and_authorizations_remain_closed() -> None:
    assert locker.OUTPUT_GUARD_DIRECTORY.name == "closure_v1_e0_mm_locker"
    assert len(locker.OUTPUT_GUARD_NAMES) == 2
    assert all("seed_20260614_patch" in name for name in locker.OUTPUT_GUARD_NAMES)
