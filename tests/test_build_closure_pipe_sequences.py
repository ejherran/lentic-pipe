...
def test_main_stops_at_external_gate_before_state_io(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    fake_lock = types.ModuleType(
        "src.experiments.closure_p1_sequence_seed_20260614_patch"
    )
    ...
    monkeypatch.setattr(
        module,
        "parse_args",
        lambda: Namespace(model_id="P1", base_seed=20260614),
    )
    io_calls: list[str] = []
    ...
    with pytest.raises(GateStopped):
        module.main()
    assert gate_calls == [{"model_id": "P1", "base_seed": 20260614}]
    assert io_calls == []
...
def test_authorization_inputs_are_exact_and_use_generic_manifest_record_dialect(
    ...
    roles = {
        ...
        "p1_sequence_seed_20260614_patch_lock.json": (
...
def test_sequence_transaction_rolls_back_if_e0_ml_authority_mutates(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from src.experiments import build_closure_pipe_sequences as module
