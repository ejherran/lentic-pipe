...
from src.experiments.train_pipe_neural_ode_v2 import make_continuous_time_neural_ode_model
...
def _write_identity_v2_model(path: Path, *, history_length: int = 2) -> None:
...
def test_v2_direct_backtest_cli_writes_matched_outputs(tmp_path: Path) -> None:
    ...
    _write_identity_v2_model(model_path)
    ...
    assert irc_alert["rows"].tolist() == [1, 1]
    assert len(backtest_rows) == 2
    assert backtest_rows["origin_year_month"].tolist() == ["2022-02", "2022-02"]
    assert manifest["status"] == "completed"
    assert manifest["backtest_version"] == "pipe_neural_ode_continuous_direct_backtest_v2"
    assert manifest["direct_version"] == "pipe_neural_ode_continuous_direct_v2"
    assert manifest["config"]["reference_backtest_rows"] == str(reference_path)
    assert manifest["row_counts"]["selected_origins"] == 1
    assert manifest["row_counts"]["evaluated_direct_rows"] == 2
    assert "PIPE Neural ODE Continuous-Time Direct Backtest Report v2" in report
