...
from src.experiments import (
    audit_closure_p1_seed_314159_sequence_bundle as audit,
)
...
def test_physical_payload_accepts_seed_314159_and_closed_null_encoding(
...
def test_physical_payload_rejects_nonexact_seed(
    ...
    with pytest.raises(
        audit.ClosureP1Seed314159SequenceAuditError, match="base_seed"
    ):
...
def test_physical_payload_rejects_schema_null_or_finitude_drift(
    ...
    with pytest.raises(audit.ClosureP1Seed314159SequenceAuditError):
        audit._validate_physical_payload(table)
...
def _valid_namespace(*, pointer: bool = False) -> dict[str, Any]:
    data = [
        ...
        "seed_20260613.parquet.dvc",
        "seed_20260614.parquet",
        "seed_20260614.parquet.dvc",
        "seed_314159.parquet",
    ]
    if pointer:
        data.append("seed_314159.parquet.dvc")
    return {
        "data": sorted(data),
        "reports": sorted(
            (
        ...
        "model_reports": sorted(
            (
                ...
                "seed_20260613_manifest.json",
                "seed_20260613_report.md",
                "seed_20260614_manifest.json",
                "seed_20260614_report.md",
            )
        ),
...
def test_namespace_accepts_exact_ordered_progression(pointer: bool) -> None:
    ...
    assert evidence["progression_observation"]["prior_seed_present_count"] == 24
    assert evidence["progression_observation"]["prior_seeds"] == [
        1729,
        20260612,
        20260613,
        20260614,
...
@pytest.mark.parametrize(
    ...
    (
        ("data", "seed_314159.parquet.tmp", True),
        ("data", "seed_1729.parquet.dvc.tmp", True),
        ("reports", "unregistered.json", True),
        ("model_reports", "seed_314159_report.md", False),
        ("model_files", "seed_314159.pt", False),
...
def test_namespace_rejects_or_closes_forbidden_progression(
    ...
    with pytest.raises(
        audit.ClosureP1Seed314159SequenceAuditError, match="pre-consumer"
    ):
...
def test_namespace_requires_all_four_six_record_predecessors(missing: str) -> None:
    snapshot = _valid_namespace()
    cast(list[str], snapshot["model_reports"]).remove(missing)

    with pytest.raises(
        audit.ClosureP1Seed314159SequenceAuditError, match="predecessor"
    ):
        audit._validate_closed_namespace(snapshot)
...
def test_optional_dvc_pointer_binds_seed_314159_payload() -> None:
    payload = b"closed-p1-seed-314159-payload"
...
def test_optional_dvc_pointer_rejects_drift(mutation: str) -> None:
    ...
    with pytest.raises(
        audit.ClosureP1Seed314159SequenceAuditError, match="pointer"
    ):
...
def test_state_manifest_accepts_seed_314159_and_exact_state_binding() -> None:
...
def _mn_physical_records() -> dict[str, dict[str, Any]]:
    records: dict[str, dict[str, Any]] = {}
    for expected in (
        *audit.EXPECTED_E0_MN_INPUT_RECORDS,
        *audit.EXPECTED_E0_MN_PATCH_RECORDS,
    ):
        records[str(expected["path"])] = audit._physical_record(expected)
    return records


def _validate_physical_mn(
    lock: dict[str, Any],
    companion: dict[str, Any],
) -> dict[str, Any]:
    return audit._validate_e0_mn_reference(
        lock,
        companion,
        lock_record=audit._physical_record(audit.EXPECTED_INPUT_RECORDS[10]),
        physical_records=_mn_physical_records(),
    )


def test_e0_mn_reference_accepts_exact_post_consumption_authority() -> None:
    evidence = _validate_physical_mn(
        _strict_physical_json(audit.E0_MN_LOCK_PATH),
        _strict_physical_json(audit.E0_MN_COMPANION_PATH),
    )

    assert evidence["physical_input_count"] == 34
    assert evidence["physical_patch_component_count"] == 7
    assert evidence["h_commit"] == audit.E0_MN_H_COMMIT
    assert evidence["p_commit"] == audit.E0_MN_P_COMMIT
    assert evidence["published_lock_bundle_bytes_reconciled"] is True
    ...
    assert evidence["prior_p1_20260613_physically_reconciled"] is True
    assert evidence["prior_p1_20260614_physically_reconciled"] is True
...
def test_e0_mn_reference_rejects_semantic_or_binding_drift(mutation: str) -> None:
    lock = copy.deepcopy(_strict_physical_json(audit.E0_MN_LOCK_PATH))
    companion = copy.deepcopy(_strict_physical_json(audit.E0_MN_COMPANION_PATH))
    if mutation == "authorization":
        cast(dict[str, Any], lock["authorizations"])["p1_consumer_authorized"] = True
    elif mutation == "anfis":
        cast(dict[str, Any], lock["anfis_314159_state_bundle"])[
    ...
    elif mutation == "latest-predecessor":
        cast(
            list[dict[str, Any]],
            cast(dict[str, Any], lock["p1_20260613_publication"])["records"],
        )[0]["sha256"] = "0" * 64
    elif mutation == "immediate-predecessor":
        cast(
            list[dict[str, Any]],
            cast(dict[str, Any], lock["p1_20260614_publication"])["records"],
        )[0]["sha256"] = "0" * 64
    elif mutation == "input":
    ...
    with pytest.raises(audit.ClosureP1Seed314159SequenceAuditError):
        _validate_physical_mn(lock, companion)
...
def test_sequence_manifest_rejects_closed_contract_drift(mutation: str) -> None:
    ...
    with pytest.raises(
        audit.ClosureP1Seed314159SequenceAuditError, match="manifest"
    ):
        _validate_physical_sequence_manifest(payload)
...
def test_strict_json_rejects_duplicate_keys_and_noncanonical_bytes() -> None:
    with pytest.raises(
        audit.ClosureP1Seed314159SequenceAuditError, match="duplicate key"
    ):
    ...
    with pytest.raises(
        audit.ClosureP1Seed314159SequenceAuditError, match="canonical"
    ):
        audit._assert_canonical_json(
...
def test_auditor_source_has_no_mutator_process_network_or_effective_gate() -> None:
    ...
    assert audit.sys.dont_write_bytecode is True
...
@pytest.mark.skipif(
    ...
    reason="The ignored P1/314159 payload exists only in the authorized workspace",
)
def test_real_p1_seed_314159_audit_pass_is_read_only(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    def forbidden(*_args: Any, **_kwargs: Any) -> Any:
        raise AssertionError("The P1/314159 auditor attempted a mutation or external call")
    ...
    with monkeypatch.context() as guarded:
        ...
        result = audit.audit_p1_seed_314159_sequence_bundle()
...
def test_real_p1_seed_314159_late_failure_is_read_only(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    before = _real_audit_snapshot()
    descriptors_before = len(os.listdir("/proc/self/fd"))
    ...
    def injected_failure(table: pa.Table) -> dict[str, int]:
        original(table)
        raise audit.ClosureP1Seed314159SequenceAuditError(
            "injected late audit failure"
        )

    monkeypatch.setattr(audit, "_validate_physical_payload", injected_failure)
    with pytest.raises(
        audit.ClosureP1Seed314159SequenceAuditError,
        match="injected late audit failure",
    ):
        audit.audit_p1_seed_314159_sequence_bundle()
