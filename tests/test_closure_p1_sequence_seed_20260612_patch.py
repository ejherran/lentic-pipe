...
from src.experiments import (
    closure_p1_temporal_consumer_schema_subset_patch as e0_mg,
)
from src.experiments import closure_p1_sequence_seed_20260612_patch as patch
from src.experiments import lock_closure_p1_sequence_seed_20260612_patch as locker
...
def _verification(*, focused_count: int) -> dict[str, Any]:
    ...
    return {
        "schema_subset_preflight": (
            patch.preflight_p1_sequence_seed_20260612_patch_schema()
        ),
...
def test_h_patch_scope_is_exact_two_modified_plus_five_added() -> None:
    assert patch.PATCH_BASE_COMMIT == patch.P1_1729_CONSUMER_COMMIT
...
def test_authority_is_exclusive_to_the_next_p1_builder_slot() -> None:
    ...
    assert patch.AUTHORIZED_BASE_SEED == 20_260_612
...
def test_real_historical_e0_mc_authority_avoids_effective_loader(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        e0_mc,
        "load_and_validate_p1_sequence_historical_anfis_patch_lock",
        lambda: pytest.fail("effective E0-MC loader must not run"),
    )
    authority = patch._historical_e0_mc_authority(
        execution_head=patch._git("rev-parse", "HEAD")
    )
    assert authority["patch_head"] == patch.E0_MC_H_COMMIT
    assert authority["lock_commit"] == patch.E0_MC_P_COMMIT
...
def test_real_historical_e0_mg_authority_avoids_effective_loader(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        e0_mg,
        "load_and_validate_p1_temporal_consumer_schema_subset_patch_lock",
        lambda: pytest.fail("effective E0-MG loader must not run"),
    )
    authority = patch._historical_e0_mg_authority(
        execution_head=patch._git("rev-parse", "HEAD")
    )
    assert authority["patch_head"] == patch.E0_MG_H_COMMIT
    assert authority["lock_commit"] == patch.E0_MG_P_COMMIT
    assert authority["preserved_components"]["count"] == len(e0_mg.PATCH_PATHS)
    assert authority["effective_loader_called"] is False
...
def test_real_p1_1729_publication_is_bound_as_model_unavailable() -> None:
    ...
    assert publication["sequence_commit"] == e0_mg.P1_BUNDLE_COMMIT
...
def test_p1_1729_publication_rejects_semantic_manifest_drift(
    ...
    with pytest.raises(
        patch.P1SequenceSeed20260612PatchError,
...
def test_real_anfis_20260612_state_bundle_is_modern_and_available() -> None:
    state = patch._anfis_20260612_state_bundle(
        execution_head=patch._git("rev-parse", "HEAD")
    )
    assert state["publication_commit"] == patch.ANFIS_20260612_COMMIT
...
def test_anfis_20260612_manifest_does_not_activate_the_1729_adapter() -> None:
    manifest = json.loads(
        patch.ANFIS_20260612_MANIFEST_PATH.read_text(encoding="utf-8")
    )
...
def test_real_anfis_20260612_manifest_is_consumable_under_mh_authority() -> None:
    manifest = json.loads(
        patch.ANFIS_20260612_MANIFEST_PATH.read_text(encoding="utf-8")
    )
    assert sequence_builder.validate_state_slot_manifest(
        ...
        state_path=patch.ANFIS_20260612_STATE_PATH,
        consumer_authority=_effective_summary(),
    ) == (True, "", False)


def test_anfis_20260612_bundle_rejects_seed_drift(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manifest = json.loads(
        patch.ANFIS_20260612_MANIFEST_PATH.read_text(encoding="utf-8")
    ...
    with pytest.raises(
        patch.P1SequenceSeed20260612PatchError,
        match="ANFIS 20260612 manifest drifted",
    ):
        patch._anfis_20260612_state_bundle(
            execution_head=patch._git("rev-parse", "HEAD")
        )


def test_target_namespace_is_exactly_28_absent_paths() -> None:
    paths = patch.p1_seed_20260612_namespace_paths()
    contract = patch.p1_seed_20260612_namespace_absence()
    assert len(paths) == len({path.as_posix() for path in paths}) == 28
    assert contract["model_id"] == "P1"
    assert contract["base_seed"] == 20_260_612
    assert contract["paths"] == [path.as_posix() for path in paths]
    assert contract["all_absent_at_lock"] is True


def test_real_progression_has_only_the_closed_p1_1729_bundle() -> None:
    ...
    assert progression["completed_seed"] == 1729
    assert progression["completed_seed_present_count"] == 6
    assert progression["remaining_absent_count"] == 134
    assert progression["next_authorized_seed"] == 20_260_612
...
def test_gate_rejects_nonexclusive_identity_before_loading(
    ...
    with pytest.raises(
        patch.P1SequenceSeed20260612PatchError,
        match="only the one-shot P1 sequence build for seed 20260612",
    ):
        patch.require_p1_sequence_seed_20260612_authorized(model_id, base_seed)
...
def test_effective_gate_preserves_builder_only_false_seals(
    ...
    monkeypatch.setattr(patch, "p1_seed_20260612_namespace_absence", lambda: {})
    monkeypatch.setattr(patch, "closure_progression_prelock", lambda: {})
    observed = patch.require_p1_sequence_seed_20260612_authorized(
        "P1", 20_260_612
    )
...
@pytest.mark.parametrize(
    "field",
    ("p1_consumer_authorized", "p1_fit_authorized", "dvc_commands_authorized"),
)
def test_effective_gate_rejects_broadened_authority(
    ...
    with pytest.raises(
        ...
        patch.require_p1_sequence_seed_20260612_authorized("P1", 20_260_612)


def test_effective_loader_has_no_unpublished_bypass_arguments() -> None:
    assert inspect.signature(
        patch.load_and_validate_p1_sequence_seed_20260612_patch_lock
    ).parameters == {}
...
def test_schema_accepts_real_historical_authority_shapes() -> None:
    ...
    prelock = {
        ...
        "base_authorities": {
            "e0_mc": patch._historical_e0_mc_authority(
                execution_head=execution_head
            ),
            "e0_mg": patch._historical_e0_mg_authority(
                execution_head=execution_head
            ),
        },
        ...
        "anfis_20260612_state_bundle": patch._anfis_20260612_state_bundle(
            execution_head=execution_head
        ),
    ...
    payload = patch.build_p1_sequence_seed_20260612_patch_lock_payload(
        prelock,
...
def test_companion_separates_physical_and_historical_inputs() -> None:
    ...
    mc_superseded = [
        _record(path, e0_mc.PATCH_COMPONENT_ROLES[path], token)
        for path, token in zip(patch.E0_MC_SUPERSEDED_PATHS, ("c", "d"))
    ]
    publication_records = [
    ...
    state_records = [
        _record(path.as_posix(), f"anfis_20260612_{index}", "f")
        for index, path in enumerate(
            (
                patch.ANFIS_20260612_STATE_PATH,
                patch.ANFIS_20260612_POINTER_PATH,
                patch.ANFIS_20260612_MANIFEST_PATH,
            ),
            start=1,
        )
    ]
    payload = {
        ...
        "base_authorities": {
            "e0_mc": {
                ...
                "superseded_components": {"records": mc_superseded},
            },
            "e0_mg": {
                "lock": _record(
        ...
        "anfis_20260612_state_bundle": {"records": state_records},
    }
    lock_record = _record(
        patch.DEFAULT_PATCH_LOCK_PATH.as_posix(),
        "external_p1_sequence_seed_20260612_patch_lock",
    ...
    assert len(companion["inputs"]) == 16
    assert len(companion["historical_inputs"]) == 2
    assert {
        record["path"] for record in companion["historical_inputs"]
    } == set(patch.E0_MC_SUPERSEDED_PATHS)
    assert all(
        record["commit"] == patch.E0_MC_H_COMMIT
...
def test_verification_contract_is_exact_and_rejects_dvc_evidence(
    ...
    patch.validate_p1_sequence_seed_20260612_patch_verification(evidence)
...
def test_verification_preflights_schema_before_any_command(
    ...
    events: list[str] = []
    schema_preflight = patch.preflight_p1_sequence_seed_20260612_patch_schema()
    ...
    observed = locker.run_p1_sequence_seed_20260612_patch_verification(
...
def test_check_only_runs_no_verification_or_write(
    ...
    prelock = {
        ...
        "progression_prelock": {"remaining_absent_count": 134},
    }
    monkeypatch.setattr(
        locker,
        "_preflight_schema",
    ...
    monkeypatch.setattr(
        locker,
        "run_p1_sequence_seed_20260612_patch_verification",
...
def test_execute_lock_rolls_back_if_companion_publication_fails(
    ...
    guard_directory = project_root / "tmp" / "closure_v1_e0_mh_locker"
