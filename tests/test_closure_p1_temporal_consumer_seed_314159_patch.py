...
from src.experiments import (
    closure_p1_temporal_consumer_seed_314159_patch as module,
)
from src.experiments import (
    lock_closure_p1_temporal_consumer_seed_314159_patch as locker,
)
...
def test_physical_schema_uses_real_closure_contract_subset() -> None:
    ...
    def assert_required_unique(node: Any) -> None:
        if isinstance(node, Mapping):
            required = node.get("required")
            if isinstance(required, list):
                assert len(required) == len(set(required))
            for value in node.values():
                assert_required_unique(value)
        elif isinstance(node, list):
            for value in node:
                assert_required_unique(value)

    assert_required_unique(schema)
    ...
    observed = module.preflight_p1_temporal_consumer_seed_314159_patch_schema()
...
def test_progression_prelock_requires_exact_registered_namespace(
    ...
    assert observed["expected_present_count"] == 28
    assert observed["registered_absent_count"] == 112
    assert observed["prior_present_count"] == 24
    assert observed["prior_seeds"] == [1729, 20_260_612, 20_260_613, 20_260_614]
    assert observed["prior_residual_absent_count"] == 88
    assert observed["later_absent_count"] == 0
    assert observed["current_sequence_temporary_absent_count"] == 5
    assert observed["exact_registered_namespace_verified"] is True


def test_historical_e0_mm_supersedes_only_trainer_and_test() -> None:
    observed = module._historical_e0_mm_authority(
        execution_head=module.PATCH_BASE_COMMIT
    )

    assert observed["gate"] == "E0-MM"
    assert observed["patch_head"] == module.E0_MM_H_COMMIT
    assert observed["lock_commit"] == module.E0_MM_P_COMMIT
    assert observed["preserved_components"]["count"] == 5
    assert observed["superseded_components"]["count"] == 2
    assert {
        record["path"]
        for record in observed["superseded_components"]["records"]
    } == set(module.PATCH_MODIFIED_PATHS)
...
@pytest.mark.parametrize(
    ("loader_name", "seed"),
    [
        ("_published_p1_1729_bundle", "1729"),
        ("_published_p1_20260612_bundle", "20260612"),
        ("_published_p1_20260613_bundle", "20260613"),
        ("_published_p1_20260614_bundle", "20260614"),
    ],
)
def test_historical_e0_mn_preserves_prior_publications(
    monkeypatch: pytest.MonkeyPatch,
    loader_name: str,
    seed: str,
) -> None:
    observed = module._historical_e0_mn_authority(
        execution_head=module.PATCH_BASE_COMMIT
    )
    assert observed["gate"] == "E0-MN"
    ...
    with pytest.raises(
        module.P1TemporalConsumerSeed314159PatchError,
        match=rf"P1/{seed} publication drifted",
    ):
        module._historical_e0_mn_authority(
            execution_head=module.PATCH_BASE_COMMIT
        )


@pytest.mark.parametrize("authority_name", ["e0_ml", "e0_mm"])
def test_historical_e0_mn_rejects_nested_authority_drift(
    monkeypatch: pytest.MonkeyPatch,
    authority_name: str,
) -> None:
    original_loader = module._load_regular_json
    payload = copy.deepcopy(
        original_loader(module.e0_mn.DEFAULT_PATCH_LOCK_PATH, context="P-E0-MN lock")
    )
    payload["base_authorities"][authority_name]["gate"] = "drifted"

    def load(path: Path, *, context: str) -> dict[str, Any]:
        if path == module.e0_mn.DEFAULT_PATCH_LOCK_PATH:
            return payload
        return original_loader(path, context=context)

    monkeypatch.setattr(module, "_load_regular_json", load)
    with pytest.raises(
        module.P1TemporalConsumerSeed314159PatchError,
        match=rf"base_authorities\.{authority_name}\.gate",
    ):
        module._historical_e0_mn_authority(execution_head=module.PATCH_BASE_COMMIT)
...
def test_mo_parser_preserves_both_pytest_duration_forms(
    monkeypatch: pytest.MonkeyPatch,
    summary: str,
    expected_format: str,
) -> None:
...
def test_verification_preflights_schema_before_first_command(
    ...
    observed = locker.run_p1_temporal_consumer_seed_314159_patch_verification(
...
def test_check_only_preflights_schema_before_prelock_and_exposes_evidence(
    ...
    def collect(**kwargs: Any) -> dict[str, Any]:
        ...
        return {
            ...
            "p1_314159_publication": {"commit": module.P1_BUNDLE_COMMIT},
...
def test_mocked_complete_transaction_uses_real_schema_and_validator(
    ...
    historical_mm = prelock["base_authorities"]["e0_mm"]
    historical_mn = prelock["base_authorities"]["e0_mn"]
    publication = prelock["p1_314159_publication"]
    ...
    monkeypatch.setattr(module, "_historical_e0_mm_authority", lambda **kwargs: historical_mm)
    monkeypatch.setattr(module, "_historical_e0_mn_authority", lambda **kwargs: historical_mn)
    monkeypatch.setattr(module, "_published_p1_314159_bundle", lambda **kwargs: publication)
    ...
    monkeypatch.setattr(
        locker,
        "run_p1_temporal_consumer_seed_314159_patch_verification",
    ...
    monkeypatch.setattr(locker, "load_and_validate_p1_temporal_consumer_seed_314159_patch_lock", lambda **kwargs: ({}, {}))
...
def test_companion_has_exact_24_physical_and_two_historical_inputs() -> None:
    component_records = [
        _record(path, module.PATCH_COMPONENT_ROLES[path])
        for path in module.PATCH_PATHS
    ]
    mm_preserved = [
        _record(f"mm/preserved/{index}", f"mm_preserved_{index}")
        for index in range(5)
    ]
    mm_superseded = [
        _record(path, f"mm_superseded_{index}")
        for index, path in enumerate(module.PATCH_MODIFIED_PATHS)
    ]
    mn_preserved = [
        _record(f"mn/preserved/{index}", f"mn_preserved_{index}")
        for index in range(7)
    ]
    publication = [
        _record(f"p1/314159/{index}", f"publication_{index}")
    ...
    payload = {
        ...
        "base_authorities": {
            "e0_mm": {
                "preserved_components": {"records": mm_preserved},
                "superseded_components": {"records": mm_superseded},
                "lock": _record("mm/lock.json", "mm_lock"),
                "companion_manifest": _record("mm/companion.json", "mm_companion"),
            },
            "e0_mn": {
                "preserved_components": {"records": mn_preserved},
                "lock": _record("mn/lock.json", "mn_lock"),
                "companion_manifest": _record("mn/companion.json", "mn_companion"),
            },
        },
        "p1_314159_publication": {"records": publication},
    }
    companion = module._expected_companion(
        payload,
        lock_record=_record("mo/lock.json", "mo_lock"),
    )

    inputs = companion["inputs"]
    historical = companion["historical_inputs"]
    assert len(inputs) == 24
    assert len({record["path"] for record in inputs}) == 24
    assert len(historical) == 2
    assert {record["path"] for record in historical} == set(
        module.PATCH_MODIFIED_PATHS
    )
    assert {record["commit"] for record in historical} == {module.E0_MM_H_COMMIT}
    assert {record["hash_source"] for record in historical} == {
        "git_blob_at_commit"
    }
...
def test_locker_failure_rolls_back_empty_owner_set(
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: (_ for _ in ()).throw(
            module.P1TemporalConsumerSeed314159PatchError("injected failure")
        ),
    )
...
def test_companion_failure_rolls_back_lock_owner_manifest_last(
    ...
    monkeypatch.setattr(locker, "_acquire_guards", lambda *args: guards)
    monkeypatch.setattr(locker.hardened, "_assert_lock_namespace", lambda *args: None)
    monkeypatch.setattr(locker, "collect_p1_temporal_consumer_seed_314159_patch_prelock_state", lambda **kwargs: prelock)
    monkeypatch.setattr(locker, "run_p1_temporal_consumer_seed_314159_patch_verification", lambda **kwargs: {})
    monkeypatch.setattr(locker, "p1_consumer_namespace_absence", lambda: {})
    monkeypatch.setattr(locker, "build_p1_temporal_consumer_seed_314159_patch_lock_payload", lambda *args, **kwargs: {"patch_repository": {"head": PATCH_HEAD}})
    monkeypatch.setattr(locker, "_load_regular_json", lambda *args, **kwargs: {})
    monkeypatch.setattr(locker, "validate_p1_temporal_consumer_seed_314159_patch_lock_payload", lambda *args, **kwargs: None)
    monkeypatch.setattr(locker, "_expected_companion", lambda *args, **kwargs: {})
...
def test_future_p_mo_publication_is_exact_two_additions(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_future_p_mo_rejects_any_third_path(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    payload = {"patch_repository": {"head": PATCH_HEAD}}
    monkeypatch.setattr(module, "_introduced_commit", lambda path: LOCK_COMMIT)
...
def test_guard_namespace_and_authorizations_remain_closed() -> None:
    assert locker.OUTPUT_GUARD_DIRECTORY.name == "closure_v1_e0_mo_locker"
    assert len(locker.OUTPUT_GUARD_NAMES) == 2
    assert all("seed_314159_patch" in name for name in locker.OUTPUT_GUARD_NAMES)
