...
def test_closure_phase4_p_cert4_selector_accepts_git_lexical_status_order(
    ...
    assert lines[0].endswith("authority_manifest_v6.json")
    assert lines[1].endswith("authority_v6.json")
    reversed_status = "\n".join(reversed(lines)) + "\n"
    h3_commit = "a" * 40
    observed: list[str] = []
    monkeypatch.setattr(
...
def test_closure_phase4_cert_staged_double_recapture_rejects_semantic_drift(
    ...
    monkeypatch.setattr(
        precommit_artifacts,
        "_git_output",
        lambda *_args: precommit_artifacts.CLOSURE_PHASE4_P_CERT_V5_COMMIT,
    )
    monkeypatch.setattr(
        precommit_artifacts,
        "_require_closure_phase4_cert_v5_history",
