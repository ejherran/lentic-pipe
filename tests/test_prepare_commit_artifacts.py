...
def test_closure_phase4_certification_scopes_modes_and_precedence_are_exact(
    ...
    assert (
        ...
        == precommit_artifacts.CLOSURE_PHASE4_H_CERT_V13_STAGED_SCOPE
    )
    assert list(
...
def test_closure_phase4_h_cert_selector_is_exact_and_rejects_guard_extra_pending(
    ...
    monkeypatch.setattr(
        precommit_artifacts,
        "_require_closure_phase4_cert_v12_history",
        lambda **_kwargs: None,
    )
    monkeypatch.setattr(
        precommit_artifacts,
...
def test_closure_phase4_p_cert4_selector_accepts_git_lexical_status_order(
    ...
    assert lines[0].endswith("authority_manifest_v14.json")
    assert lines[1].endswith("authority_v14.json")
    reversed_status = "\n".join(reversed(lines)) + "\n"
    h3_commit = "a" * 40
    observed: list[str] = []
    monkeypatch.setattr(
...
def test_closure_phase4_cert_staged_double_recapture_rejects_semantic_drift(
    ...
    monkeypatch.setattr(
        precommit_artifacts,
        "_git_output",
        lambda *_args: precommit_artifacts.CLOSURE_PHASE4_P_CERT_V12_COMMIT,
    )
    monkeypatch.setattr(
        precommit_artifacts,
        "_require_closure_phase4_cert_v12_history",
...
def test_closure_phase4_r_cert_delegates_mutated_report_to_custom_validator(
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: {
            "p_cert_commit": head,
            "p14_cert_commit": head,
            "h_cert_commit": "b" * 40,
            "h14_cert_commit": "b" * 40,
            "p13_cert_commit": None,
            "h13_cert_commit": None,
            "p12_cert_commit": precommit_artifacts.CLOSURE_PHASE4_P_CERT_V12_COMMIT,
            "h12_cert_commit": precommit_artifacts.CLOSURE_PHASE4_H_CERT_V12_COMMIT,
...
def _phase4_r_cert_adapter_payloads() -> tuple[Any, dict[str, Any], list[Any], list[Any], dict[str, Any]]:
    ...
    manifest = {
        ...
        "p_cert_authority": {
            ...
            "p13_cert_commit": effective["p13_cert_commit"],
            "h13_cert_commit": effective["h13_cert_commit"],
