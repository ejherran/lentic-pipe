...
from src.experiments import (
    closure_p1_temporal_consumer_seed_20260614_patch as e0_mm,
)
from src.experiments import closure_p1_sequence_seed_314159_patch as patch
from src.experiments import lock_closure_p1_sequence_seed_314159_patch as locker
...
def _verification(*, focused_count: int) -> dict[str, Any]:
    ...
    return {
        "schema_subset_preflight": (
            patch.preflight_p1_sequence_seed_314159_patch_schema()
        ),
...
def test_h_patch_scope_is_exact_two_modified_plus_five_added() -> None:
    assert patch.PATCH_BASE_COMMIT == patch.P1_20260614_CONSUMER_COMMIT
...
def test_authority_is_exclusive_to_the_next_p1_builder_slot() -> None:
    ...
    assert patch.AUTHORIZED_BASE_SEED == 314159
...
def test_real_historical_e0_ml_authority_avoids_effective_loader(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        e0_ml,
        "load_and_validate_p1_sequence_seed_20260614_patch_lock",
        lambda: pytest.fail("effective E0-ML loader must not run"),
    )
    authority = patch._historical_e0_ml_authority(
        execution_head=patch._git("rev-parse", "HEAD")
    )
    assert authority["patch_head"] == patch.E0_ML_H_COMMIT
    assert authority["lock_commit"] == patch.E0_ML_P_COMMIT
...
def test_real_historical_e0_mm_authority_avoids_effective_loader(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        e0_mm,
        "load_and_validate_p1_temporal_consumer_seed_20260614_patch_lock",
        lambda: pytest.fail("effective E0-MM loader must not run"),
    )
    authority = patch._historical_e0_mm_authority(
        execution_head=patch._git("rev-parse", "HEAD")
    )
    assert authority["patch_head"] == patch.E0_MM_H_COMMIT
    assert authority["lock_commit"] == patch.E0_MM_P_COMMIT
    assert authority["preserved_components"]["count"] == len(e0_mm.PATCH_PATHS)
    assert authority["effective_loader_called"] is False
...
def test_real_p1_1729_publication_is_bound_as_model_unavailable() -> None:
    ...
    assert publication["sequence_commit"] == e0_ml.P1_1729_SEQUENCE_COMMIT
...
def test_real_p1_20260614_publication_is_bound_as_model_unavailable() -> None:
    publication = patch._published_p1_20260614_bundle(
        execution_head=patch._git("rev-parse", "HEAD")
    )
    assert publication["sequence_commit"] == patch.P1_20260614_SEQUENCE_COMMIT
    assert publication["consumer_commit"] == patch.P1_20260614_CONSUMER_COMMIT
    assert len(publication["records"]) == 6
    assert publication["model_unavailable_semantics_verified"] is True
    assert publication["completion_marker_written_last"] is True
    assert publication["future_outcomes_accessed"] is False
...
def test_p1_1729_publication_rejects_semantic_manifest_drift(
    ...
    with pytest.raises(
        patch.P1SequenceSeed314159PatchError,
...
def test_p1_20260614_publication_rejects_semantic_manifest_drift(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manifest = json.loads(
        patch.P1_20260614_MODEL_MANIFEST_PATH.read_text(encoding="utf-8")
    )
    manifest["fit_status"] = "passed"
    monkeypatch.setattr(patch, "_load_regular_json", lambda *_args, **_kwargs: manifest)
    with pytest.raises(
        ...
        patch._published_p1_20260614_bundle(
            execution_head=patch._git("rev-parse", "HEAD")
        )


def test_real_anfis_314159_state_bundle_is_modern_and_available() -> None:
    state = patch._anfis_314159_state_bundle(
        execution_head=patch._git("rev-parse", "HEAD")
    )
    assert state["publication_commit"] == patch.ANFIS_314159_COMMIT
...
def test_anfis_314159_manifest_does_not_activate_the_1729_adapter() -> None:
    manifest = json.loads(
        patch.ANFIS_314159_MANIFEST_PATH.read_text(encoding="utf-8")
    )
...
def test_real_anfis_314159_manifest_is_consumable_under_ml_authority() -> None:
    manifest = json.loads(
        patch.ANFIS_314159_MANIFEST_PATH.read_text(encoding="utf-8")
    )
    assert sequence_builder.validate_state_slot_manifest(
        ...
        state_path=patch.ANFIS_314159_STATE_PATH,
        consumer_authority=_effective_summary(),
    ) == (True, "", False)


def test_anfis_314159_bundle_rejects_seed_drift(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manifest = json.loads(
        patch.ANFIS_314159_MANIFEST_PATH.read_text(encoding="utf-8")
    ...
    with pytest.raises(
        patch.P1SequenceSeed314159PatchError,
        match="ANFIS 314159 manifest drifted",
    ):
        patch._anfis_314159_state_bundle(
            execution_head=patch._git("rev-parse", "HEAD")
        )


def test_target_namespace_is_exactly_28_absent_paths() -> None:
    paths = patch.p1_seed_314159_namespace_paths()
    contract = patch.p1_seed_314159_namespace_absence()
    assert len(paths) == len({path.as_posix() for path in paths}) == 28
    assert contract["model_id"] == "P1"
    assert contract["base_seed"] == 314159
    assert contract["paths"] == [path.as_posix() for path in paths]
    assert contract["all_absent_at_lock"] is True


def test_real_progression_has_exactly_four_closed_p1_bundles() -> None:
    ...
    assert progression["completed_seeds"] == [
        1729,
        20_260_612,
        20_260_613,
        20_260_614,
    ]
    assert progression["completed_seed_present_count"] == 24
    assert progression["remaining_absent_count"] == 116
    assert progression["prior_seed_residual_absent_count"] == 88
    assert progression["target_seed_absent_count"] == 28
    assert progression["later_seed_absent_count"] == 0
    assert progression["next_authorized_seed"] == 314159
...
def test_gate_rejects_nonexclusive_identity_before_loading(
    ...
    monkeypatch.setattr(
        patch,
        "load_and_validate_p1_sequence_seed_314159_patch_lock",
        lambda: pytest.fail("loader must not run for an unauthorized identity"),
    )
    with pytest.raises(
        patch.P1SequenceSeed314159PatchError,
        match="only the one-shot P1 sequence build for seed 314159",
    ):
        patch.require_p1_sequence_seed_314159_authorized(model_id, base_seed)
...
def test_effective_gate_preserves_builder_only_false_seals(
    ...
    monkeypatch.setattr(patch, "p1_seed_314159_namespace_absence", lambda: {})
    monkeypatch.setattr(patch, "closure_progression_prelock", lambda: {})
    observed = patch.require_p1_sequence_seed_314159_authorized(
        "P1", 314159
    )
...
def test_effective_gate_rejects_broadened_authority(
    ...
    with pytest.raises(
        ...
        patch.require_p1_sequence_seed_314159_authorized("P1", 314159)


def test_effective_loader_has_no_unpublished_bypass_arguments() -> None:
    assert inspect.signature(
        patch.load_and_validate_p1_sequence_seed_314159_patch_lock
    ).parameters == {}
    ...
    def assert_closed(node: object) -> None:
        if isinstance(node, dict):
            ...
            if isinstance(required, list):
                assert len(required) == len(set(cast(list[str], required)))
...
def test_schema_accepts_real_historical_authority_shapes() -> None:
    ...
    prelock = {
        ...
        "base_authorities": {
            "e0_ml": patch._historical_e0_ml_authority(
                execution_head=execution_head
            ),
            "e0_mm": patch._historical_e0_mm_authority(
                execution_head=execution_head
            ),
        },
        ...
        "p1_20260614_publication": patch._published_p1_20260614_bundle(
            execution_head=execution_head
        ),
        "anfis_314159_state_bundle": patch._anfis_314159_state_bundle(
            execution_head=execution_head
        ),
    ...
    payload = patch.build_p1_sequence_seed_314159_patch_lock_payload(
        prelock,
...
def test_companion_separates_physical_and_historical_inputs() -> None:
    ...
    ml_superseded = [
        _record(path, e0_ml.PATCH_COMPONENT_ROLES[path], token)
        for path, token in zip(patch.E0_ML_SUPERSEDED_PATHS, ("c", "d"))
    ]
    publication_records = [
    ...
    state_records = [
        _record(path.as_posix(), f"anfis_314159_{index}", "f")
        for index, path in enumerate(
            (
                patch.ANFIS_314159_STATE_PATH,
                patch.ANFIS_314159_POINTER_PATH,
                patch.ANFIS_314159_MANIFEST_PATH,
            ),
            start=1,
        )
    ]
    payload = {
        ...
        "base_authorities": {
            "e0_ml": {
                ...
                "superseded_components": {"records": ml_superseded},
            },
            "e0_mm": {
                "lock": _record(
        ...
        "p1_20260614_publication": {
            "records": [
                _record(path.as_posix(), f"p1_20260614_{index}", "6")
                for index, path in enumerate(
                    patch.P1_20260614_PRESENT_PATHS, start=1
                )
            ]
        },
        "anfis_314159_state_bundle": {"records": state_records},
    }
    lock_record = _record(
        patch.DEFAULT_PATCH_LOCK_PATH.as_posix(),
        "external_p1_sequence_seed_314159_patch_lock",
    ...
    assert len(companion["inputs"]) == 34
    assert len({record["path"] for record in companion["inputs"]}) == 34
    assert len(companion["historical_inputs"]) == 2
    assert {
        record["path"] for record in companion["historical_inputs"]
    } == set(patch.E0_ML_SUPERSEDED_PATHS)
    assert all(
        record["commit"] == patch.E0_ML_H_COMMIT
...
def test_verification_contract_is_exact_and_rejects_dvc_evidence(
    ...
    patch.validate_p1_sequence_seed_314159_patch_verification(evidence)
...
def test_verification_preflights_schema_before_any_command(
    ...
    events: list[str] = []
    schema_preflight = patch.preflight_p1_sequence_seed_314159_patch_schema()
    ...
    observed = locker.run_p1_sequence_seed_314159_patch_verification(
...
def test_check_only_runs_no_verification_or_write(
    ...
    prelock = {
        ...
        "progression_prelock": {"remaining_absent_count": 116},
    }
    monkeypatch.setattr(
        locker,
        "_preflight_schema",
    ...
    monkeypatch.setattr(
        locker,
        "collect_p1_sequence_seed_314159_patch_prelock_state",
        lambda **_kwargs: events.append("prelock") or prelock,
    )
    monkeypatch.setattr(
        locker,
        "run_p1_sequence_seed_314159_patch_verification",
    ...
    assert result["prior_p1_20260614_slot_completed"] is True
...
def test_execute_lock_rolls_back_if_companion_publication_fails(
    ...
    guard_directory = project_root / "tmp" / "closure_v1_e0_mn_locker"
    ...
    monkeypatch.setattr(
        locker,
        "build_p1_sequence_seed_314159_patch_lock_payload",
    ...
    monkeypatch.setattr(
        locker,
        "_load_unpublished_p1_sequence_seed_314159_patch_lock",
        lambda: ({}, {}),
    )
