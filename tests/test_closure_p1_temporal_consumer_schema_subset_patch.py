...
from src.experiments import (
    closure_p1_temporal_consumer_schema_subset_patch as module,
)
from src.experiments import (
    lock_closure_p1_temporal_consumer_schema_subset_patch as locker,
)
...
def _prelock() -> dict[str, Any]:
    ...
    return {
        ...
        "base_authority": {"e0_mf": {"gate": "E0-MF"}},
        "consumer_prelock": {
            "model_id": "P1",
            "base_seed": 1729,
            "count": 19,
...
def test_physical_schema_uses_real_closure_contract_subset() -> None:
    ...
    observed = module.preflight_p1_temporal_consumer_schema_subset_patch_schema()
...
def test_unsupported_schema_mutation_fails_before_any_downstream_action(
    ...
    with pytest.raises(
        module.P1TemporalConsumerSchemaSubsetPatchError,
...
def test_mg_parser_preserves_both_pytest_duration_forms(
    monkeypatch: pytest.MonkeyPatch,
    summary: str,
    expected_format: str,
) -> None:
...
def test_verification_preflights_schema_before_first_command(
    ...
    observed = locker.run_p1_temporal_consumer_schema_subset_patch_verification(
...
def test_mocked_complete_transaction_uses_real_schema_and_validator(
    ...
    historical = prelock["base_authority"]["e0_mf"]
    ...
    monkeypatch.setattr(module, "_historical_e0_mf_authority", lambda **kwargs: historical)
    ...
    monkeypatch.setattr(
        locker,
        "run_p1_temporal_consumer_schema_subset_patch_verification",
    ...
    monkeypatch.setattr(locker, "load_and_validate_p1_temporal_consumer_schema_subset_patch_lock", lambda **kwargs: ({}, {}))
...
def test_locker_failure_rolls_back_empty_owner_set(
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: (_ for _ in ()).throw(
            module.P1TemporalConsumerSchemaSubsetPatchError("injected failure")
        ),
    )
...
def test_companion_failure_rolls_back_lock_owner_manifest_last(
    ...
    monkeypatch.setattr(locker, "_acquire_guards", lambda *args: guards)
    monkeypatch.setattr(locker.hardened, "_assert_lock_namespace", lambda *args: None)
    monkeypatch.setattr(locker, "collect_p1_temporal_consumer_schema_subset_patch_prelock_state", lambda **kwargs: prelock)
    monkeypatch.setattr(locker, "run_p1_temporal_consumer_schema_subset_patch_verification", lambda **kwargs: {})
    monkeypatch.setattr(locker, "p1_consumer_namespace_absence", lambda: {})
    monkeypatch.setattr(locker, "build_p1_temporal_consumer_schema_subset_patch_lock_payload", lambda *args, **kwargs: {"patch_repository": {"head": PATCH_HEAD}})
    monkeypatch.setattr(locker, "_load_regular_json", lambda *args, **kwargs: {})
    monkeypatch.setattr(locker, "validate_p1_temporal_consumer_schema_subset_patch_lock_payload", lambda *args, **kwargs: None)
    monkeypatch.setattr(locker, "_expected_companion", lambda *args, **kwargs: {})
...
def test_future_p_mg_publication_is_exact_two_additions(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_future_p_mg_rejects_any_third_path(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    payload = {"patch_repository": {"head": PATCH_HEAD}}
    monkeypatch.setattr(module, "_introduced_commit", lambda path: LOCK_COMMIT)
...
def test_guard_namespace_and_authorizations_remain_closed() -> None:
    assert locker.OUTPUT_GUARD_DIRECTORY.name == "closure_v1_e0_mg_locker"
    assert len(locker.OUTPUT_GUARD_NAMES) == 2
    assert all("schema_subset_patch" in name for name in locker.OUTPUT_GUARD_NAMES)
