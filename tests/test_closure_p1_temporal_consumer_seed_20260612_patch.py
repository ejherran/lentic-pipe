...
from src.experiments import (
    closure_p1_temporal_consumer_seed_20260612_patch as module,
)
from src.experiments import (
    lock_closure_p1_temporal_consumer_seed_20260612_patch as locker,
)
...
def _prelock() -> dict[str, Any]:
    ...
    return {
        ...
        "progression_prelock": {
            ...
            "expected_present_count": 10,
            "expected_present_paths": [
                *(f"prior/{index}" for index in range(6)),
                *(f"current/{index}" for index in range(4)),
            ],
            "expected_present_paths_sha256": ZERO_SHA,
            "registered_absent_count": 130,
            "exact_registered_namespace_verified": True,
            "prior_seed": 1729,
            "prior_present_count": 6,
            "prior_present_paths": [f"prior/{index}" for index in range(6)],
            "prior_present_paths_sha256": ZERO_SHA,
            "prior_residual_absent_count": 22,
            "current_seed": 20260612,
...
def test_physical_schema_uses_real_closure_contract_subset() -> None:
    ...
    observed = module.preflight_p1_temporal_consumer_seed_20260612_patch_schema()
...
def test_progression_prelock_requires_exact_registered_namespace(
    ...
    assert observed["expected_present_count"] == 10
    assert observed["registered_absent_count"] == 130
    assert observed["prior_residual_absent_count"] == 22
...
def test_progression_prelock_rejects_registered_residuals(
    ...
    with pytest.raises(
        module.P1TemporalConsumerSeed20260612PatchError,
...
def test_mi_parser_preserves_both_pytest_duration_forms(
    monkeypatch: pytest.MonkeyPatch,
    summary: str,
    expected_format: str,
) -> None:
...
def test_verification_preflights_schema_before_first_command(
    ...
    observed = locker.run_p1_temporal_consumer_seed_20260612_patch_verification(
...
def test_check_only_preflights_schema_before_prelock_and_exposes_evidence(
    ...
    def collect(**kwargs: Any) -> dict[str, Any]:
        ...
        return {
            ...
            "p1_20260612_publication": {"commit": module.P1_BUNDLE_COMMIT},
...
def test_mocked_complete_transaction_uses_real_schema_and_validator(
    ...
    historical_mg = prelock["base_authorities"]["e0_mg"]
    historical_mh = prelock["base_authorities"]["e0_mh"]
    publication = prelock["p1_20260612_publication"]
    ...
    monkeypatch.setattr(module, "_historical_e0_mg_authority", lambda **kwargs: historical_mg)
    monkeypatch.setattr(module, "_historical_e0_mh_authority", lambda **kwargs: historical_mh)
    monkeypatch.setattr(module, "_published_p1_20260612_bundle", lambda **kwargs: publication)
    ...
    monkeypatch.setattr(
        locker,
        "run_p1_temporal_consumer_seed_20260612_patch_verification",
    ...
    monkeypatch.setattr(locker, "load_and_validate_p1_temporal_consumer_seed_20260612_patch_lock", lambda **kwargs: ({}, {}))
...
def test_locker_failure_rolls_back_empty_owner_set(
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: (_ for _ in ()).throw(
            module.P1TemporalConsumerSeed20260612PatchError("injected failure")
        ),
    )
...
def test_companion_failure_rolls_back_lock_owner_manifest_last(
    ...
    monkeypatch.setattr(locker, "_acquire_guards", lambda *args: guards)
    monkeypatch.setattr(locker.hardened, "_assert_lock_namespace", lambda *args: None)
    monkeypatch.setattr(locker, "collect_p1_temporal_consumer_seed_20260612_patch_prelock_state", lambda **kwargs: prelock)
    monkeypatch.setattr(locker, "run_p1_temporal_consumer_seed_20260612_patch_verification", lambda **kwargs: {})
    monkeypatch.setattr(locker, "p1_consumer_namespace_absence", lambda: {})
    monkeypatch.setattr(locker, "build_p1_temporal_consumer_seed_20260612_patch_lock_payload", lambda *args, **kwargs: {"patch_repository": {"head": PATCH_HEAD}})
    monkeypatch.setattr(locker, "_load_regular_json", lambda *args, **kwargs: {})
    monkeypatch.setattr(locker, "validate_p1_temporal_consumer_seed_20260612_patch_lock_payload", lambda *args, **kwargs: None)
    monkeypatch.setattr(locker, "_expected_companion", lambda *args, **kwargs: {})
...
def test_future_p_mi_publication_is_exact_two_additions(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_future_p_mi_rejects_any_third_path(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    payload = {"patch_repository": {"head": PATCH_HEAD}}
    monkeypatch.setattr(module, "_introduced_commit", lambda path: LOCK_COMMIT)
...
def test_guard_namespace_and_authorizations_remain_closed() -> None:
    assert locker.OUTPUT_GUARD_DIRECTORY.name == "closure_v1_e0_mi_locker"
    assert len(locker.OUTPUT_GUARD_NAMES) == 2
    assert all("seed_20260612_patch" in name for name in locker.OUTPUT_GUARD_NAMES)
