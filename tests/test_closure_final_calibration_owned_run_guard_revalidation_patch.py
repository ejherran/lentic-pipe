...
from src.experiments import (
    lock_closure_final_calibration_owned_run_guard_revalidation_patch as locker,
)
from src.experiments import run_closure_anfis_learning_curve as e7_runner


ROOT = Path(__file__).resolve().parents[1]
BASE_P_MCALH = "c6dbe43f01e484c7270c6a19f9a69d0b753036c7"
H_MCALH = "e107316cbe302fc806676f620af00df6244530a5"
EXPECTED_SUPERSEDED = {
...
def _forbid_real_scientific_payload_reads(
    ...
    def forbidden(*args: Any, **kwargs: Any) -> Any:
        del args, kwargs
        raise AssertionError("MCALI governance opened a scientific payload")
    ...
    def require_snapshot(
        ...
        *,
        ...
        if expected != observed:
            raise patch.FinalCalibrationOwnedRunGuardRevalidationPatchError(
...
def test_h_p_r_topology_and_published_bases_are_exact() -> None:
    assert patch.PATCH_GATE == "E0-MCALI"
    assert patch.BASE_P_MCALH_COMMIT == BASE_P_MCALH
    assert patch.H_MCALH_COMMIT == H_MCALH
    assert patch.H_MCALH_PARENT == "34a4578efa37d0ccf09e7fc45adf655d3b5a21c1"
...
def test_h_mcalh_partition_and_prior_p_inputs_are_exact() -> None:
    assert set(patch.H_MCALH_SUPERSEDED_PATHS) == EXPECTED_SUPERSEDED
    assert len(patch.H_MCALH_PRESERVED_PATHS) == 5
    assert not set(patch.H_MCALH_PRESERVED_PATHS) & EXPECTED_SUPERSEDED
    assert set(patch.P_MCALH_PATHS) == {
        "reports/closure_v1/00_protocol/"
        "final_calibration_observed_risk_precision_patch_lock.json",
        "reports/closure_v1/00_protocol/"
    ...
    authority_source = inspect.getsource(patch._p_mcalh_authority)
    for token in (
        "mcalh._state_for_h(",
        "mcalh._p_mcalg_authority(repo_root=repo_root)",
    ...
    git_bound = patch._git_record_at_commit(
        patch.P_MCALH_PATHS[0],
        role="fresh_clone_probe",
        commit=patch.BASE_P_MCALH_COMMIT,
        repo_root=ROOT,
    )
...
def test_consumed_failure_is_exact_clean_non_retryable_and_documented() -> None:
    ...
    text = (
        ROOT
        / "docs/closure_v1/"
        "E0_M_FINAL_CALIBRATION_OWNED_RUN_GUARD_REVALIDATION_PATCH_1.md"
    ).read_text()
    for token in (
        "E0-MCALI",
        "4M+5A",
        "E0-MCALH effective coordination/temporary namespace is occupied",
        "calibration_bundle_publication_authority_revalidation",
        "six provisional calibration finals",
        "manifest last",
        "active_guard",
        "post_release",
        "retry_authorized=false",
...
def test_owned_run_guard_revalidation_contract_is_exact_and_schema_closed() -> None:
    contract = json.loads(
    ...
    schema = _schema()
    definition = schema["$defs"]["ownedRunGuardRevalidationContract"]
    assert definition == {"const": EXPECTED_GUARD_CONTRACT}
...
def test_public_loader_rejects_guards_and_private_api_is_capability_bound(
    ...
    with monkeypatch.context() as public_patch:
        ...
        with pytest.raises(
            ...
            patch.load_effective_final_calibration_owned_run_guard_revalidation_patch_authority(
...
def test_schema_preflight_and_semantic_payload_chain_are_closed(
    ...
    schema = _schema()
    assert schema["properties"]["gate"]["const"] == "E0-MCALI"
    result = (
        patch.preflight_final_calibration_owned_run_guard_revalidation_patch_schema()
    )
    assert result["gate"] == "E0-MCALI"
    assert result["supported_subset_verified"] is True
    assert result["schema_count"] == 1

    prior_lock_path = patch.mcalh.DEFAULT_PATCH_LOCK_PATH
    prior_manifest_path = patch.mcalh.DEFAULT_PATCH_LOCK_MANIFEST_PATH
    ...
    assert prior_lock_bytes == patch.mcal._git_blob_bytes(
        ROOT, BASE_P_MCALH, prior_lock_path
    )
    assert prior_manifest_bytes == patch.mcal._git_blob_bytes(
        ROOT, BASE_P_MCALH, prior_manifest_path
    )
    ...
    prelock = {
        "repository": {
            "base_p_mcalh_commit": BASE_P_MCALH,
            "h_patch_head": "a" * 40,
            "branch": "main",
            "remote_head": "a" * 40,
            "scope": {
    ...
    payload = (
        patch.build_final_calibration_owned_run_guard_revalidation_patch_lock_payload(
    ...
    monkeypatch.setattr(
        patch,
        "collect_final_calibration_owned_run_guard_revalidation_patch_prelock_state",
...
def test_companion_is_exact_16_physical_4_historical_1_output_and_canonical(
    ...
    with monkeypatch.context() as loader_patch:
        _install_synthetic_loader(loader_patch)
        with pytest.raises(patch.FinalCalibrationOwnedRunGuardRevalidationPatchError):
            patch.load_effective_final_calibration_owned_run_guard_revalidation_patch_authority(
                verify_remote=False, repo_root=tmp_path
            )
...
def test_exact_four_superseded_paths_adopt_mcalh_and_gate_before_io() -> None:
    assert calibration_runner.calibration is patch
    assert e7_runner.calibration is patch
    for path in EXPECTED_SUPERSEDED:
        text = (ROOT / path).read_text()
        assert "closure_final_calibration_owned_run_guard_revalidation_patch" in text
...
def test_cli_modes_are_closed_and_mutually_exclusive() -> None:
    ...
    ordered = [
        "preflight_final_calibration_owned_run_guard_revalidation_patch_schema",
        "before =",
        "run_final_calibration_owned_run_guard_revalidation_patch_verification",
        "after =",
        "if before != after:",
        "build_final_calibration_owned_run_guard_revalidation_patch_lock_payload",
        "validate_final_calibration_owned_run_guard_revalidation_patch_lock_payload",
        "publish_final_calibration_owned_run_guard_revalidation_patch_lock_bundle",
    ]
...
def test_check_only_runs_schema_before_remote_prelock_and_is_nonwriting(
    ...
    monkeypatch.setattr(
        patch,
        "preflight_final_calibration_owned_run_guard_revalidation_patch_schema",
        lambda: events.append("schema") or {"gate": "E0-MCALI"},
    )
    monkeypatch.setattr(
        patch,
        "collect_final_calibration_owned_run_guard_revalidation_patch_prelock_state",
...
def test_effective_loader_and_run_namespace_close_zero_six_eight_and_races(
    ...
    for malformed in (
        ...
        with pytest.raises(
            patch.FinalCalibrationOwnedRunGuardRevalidationPatchError
        ):
            patch._validate_owned_guard_capability(
    ...
    loader_source = inspect.getsource(
        patch._load_effective_final_calibration_owned_run_guard_revalidation_patch_authority
    )
