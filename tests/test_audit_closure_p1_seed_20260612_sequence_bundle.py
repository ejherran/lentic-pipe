...
from src.experiments import (
    audit_closure_p1_seed_20260612_sequence_bundle as audit,
)
...
def test_physical_payload_accepts_seed_20260612_and_closed_null_encoding(
...
def test_physical_payload_rejects_nonexact_seed(
    ...
    with pytest.raises(
        audit.ClosureP1Seed20260612SequenceAuditError, match="base_seed"
    ):
...
def test_physical_payload_rejects_schema_null_or_finitude_drift(
    ...
    with pytest.raises(audit.ClosureP1Seed20260612SequenceAuditError):
        audit._validate_physical_payload(table)
...
def _valid_namespace(*, pointer: bool = False) -> dict[str, Any]:
    ...
    if pointer:
        data.append("seed_20260612.parquet.dvc")
    return {
        "data": sorted(data),
        "reports": sorted(
            (
        ...
        "model_reports": sorted(
            ("seed_1729_manifest.json", "seed_1729_report.md")
        ),
...
def test_namespace_accepts_exact_ordered_progression(pointer: bool) -> None:
    ...
    assert evidence["progression_observation"]["prior_seed_present_count"] == 6
    assert (
...
@pytest.mark.parametrize(
    ...
    (
        ("data", "seed_20260612.parquet.tmp", True),
        ("data", "seed_1729.parquet.dvc.tmp", True),
        ("reports", "unregistered.json", True),
        ("model_reports", "seed_20260612_report.md", False),
        ("data", "seed_20260613.parquet", False),
...
def test_namespace_rejects_or_closes_forbidden_progression(
    ...
    with pytest.raises(
        audit.ClosureP1Seed20260612SequenceAuditError, match="pre-consumer"
    ):
        audit._require_pre_consumer_progression_clear(evidence)


def test_namespace_requires_all_six_predecessor_records() -> None:
    snapshot = _valid_namespace()
    cast(list[str], snapshot["model_reports"]).remove("seed_1729_report.md")

    with pytest.raises(
        audit.ClosureP1Seed20260612SequenceAuditError, match="predecessor"
    ):
        audit._validate_closed_namespace(snapshot)
...
def test_optional_dvc_pointer_binds_seed_20260612_payload() -> None:
    payload = b"closed-p1-seed-20260612-payload"
...
def test_optional_dvc_pointer_rejects_drift(mutation: str) -> None:
    ...
    with pytest.raises(
        audit.ClosureP1Seed20260612SequenceAuditError, match="pointer"
    ):
...
def test_state_manifest_accepts_seed_20260612_and_exact_state_binding() -> None:
...
def _mh_physical_records() -> dict[str, dict[str, Any]]:
    records: dict[str, dict[str, Any]] = {}
    for expected in (
        *audit.EXPECTED_E0_MH_INPUT_RECORDS,
        *audit.EXPECTED_E0_MH_PATCH_RECORDS,
    ):
        records[str(expected["path"])] = audit._physical_record(expected)
    return records


def _validate_physical_mh(
    lock: dict[str, Any],
    companion: dict[str, Any],
) -> dict[str, Any]:
    return audit._validate_e0_mh_reference(
        lock,
        companion,
        lock_record=audit._physical_record(audit.EXPECTED_INPUT_RECORDS[10]),
        physical_records=_mh_physical_records(),
    )


def test_e0_mh_reference_accepts_exact_post_consumption_authority() -> None:
    evidence = _validate_physical_mh(
        _strict_physical_json(audit.E0_MH_LOCK_PATH),
        _strict_physical_json(audit.E0_MH_COMPANION_PATH),
    )

    assert evidence["physical_input_count"] == 16
...
@pytest.mark.parametrize(
    "mutation",
    ("authorization", "anfis", "predecessor", "input", "historical", "script"),
)
def test_e0_mh_reference_rejects_semantic_or_binding_drift(mutation: str) -> None:
    lock = copy.deepcopy(_strict_physical_json(audit.E0_MH_LOCK_PATH))
    companion = copy.deepcopy(_strict_physical_json(audit.E0_MH_COMPANION_PATH))
    if mutation == "authorization":
        cast(dict[str, Any], lock["authorizations"])["p1_consumer_authorized"] = True
    elif mutation == "anfis":
        cast(dict[str, Any], lock["anfis_20260612_state_bundle"])[
    ...
    with pytest.raises(audit.ClosureP1Seed20260612SequenceAuditError):
        _validate_physical_mh(lock, companion)
...
def test_sequence_manifest_rejects_closed_contract_drift(mutation: str) -> None:
    ...
    with pytest.raises(
        audit.ClosureP1Seed20260612SequenceAuditError, match="manifest"
    ):
        _validate_physical_sequence_manifest(payload)
...
def test_strict_json_rejects_duplicate_keys_and_noncanonical_bytes() -> None:
    with pytest.raises(
        audit.ClosureP1Seed20260612SequenceAuditError, match="duplicate key"
    ):
    ...
    with pytest.raises(
        audit.ClosureP1Seed20260612SequenceAuditError, match="canonical"
    ):
        audit._assert_canonical_json(
...
@pytest.mark.skipif(
    ...
    reason="The ignored P1/20260612 payload exists only in the authorized workspace",
)
def test_real_p1_seed_20260612_audit_pass_is_read_only(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    def forbidden(*_args: Any, **_kwargs: Any) -> Any:
        raise AssertionError("The P1/20260612 auditor attempted a mutation or external call")
    ...
    with monkeypatch.context() as guarded:
        ...
        result = audit.audit_p1_seed_20260612_sequence_bundle()
...
def test_real_p1_seed_20260612_late_failure_is_read_only(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    before = _real_audit_snapshot()
    descriptors_before = len(os.listdir("/proc/self/fd"))
    ...
    def injected_failure(table: pa.Table) -> dict[str, int]:
        original(table)
        raise audit.ClosureP1Seed20260612SequenceAuditError(
            "injected late audit failure"
        )

    monkeypatch.setattr(audit, "_validate_physical_payload", injected_failure)
    with pytest.raises(
        audit.ClosureP1Seed20260612SequenceAuditError,
        match="injected late audit failure",
    ):
        audit.audit_p1_seed_20260612_sequence_bundle()
