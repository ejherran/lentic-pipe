...
from src.experiments import (
    closure_p1_temporal_consumer_seed_20260612_patch as e0_mi,
)
from src.experiments import closure_p1_sequence_seed_20260613_patch as patch
from src.experiments import lock_closure_p1_sequence_seed_20260613_patch as locker
...
def _verification(*, focused_count: int) -> dict[str, Any]:
    ...
    return {
        "schema_subset_preflight": (
            patch.preflight_p1_sequence_seed_20260613_patch_schema()
        ),
...
def test_h_patch_scope_is_exact_two_modified_plus_five_added() -> None:
    assert patch.PATCH_BASE_COMMIT == patch.P1_20260612_CONSUMER_COMMIT
...
def test_authority_is_exclusive_to_the_next_p1_builder_slot() -> None:
    ...
    assert patch.AUTHORIZED_BASE_SEED == 20_260_613
...
def test_real_historical_e0_mh_authority_avoids_effective_loader(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        e0_mh,
        "load_and_validate_p1_sequence_seed_20260612_patch_lock",
        lambda: pytest.fail("effective E0-MH loader must not run"),
    )
    authority = patch._historical_e0_mh_authority(
        execution_head=patch._git("rev-parse", "HEAD")
    )
    assert authority["patch_head"] == patch.E0_MH_H_COMMIT
    assert authority["lock_commit"] == patch.E0_MH_P_COMMIT
...
def test_real_historical_e0_mi_authority_avoids_effective_loader(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        e0_mi,
        "load_and_validate_p1_temporal_consumer_seed_20260612_patch_lock",
        lambda: pytest.fail("effective E0-MI loader must not run"),
    )
    authority = patch._historical_e0_mi_authority(
        execution_head=patch._git("rev-parse", "HEAD")
    )
    assert authority["patch_head"] == patch.E0_MI_H_COMMIT
    assert authority["lock_commit"] == patch.E0_MI_P_COMMIT
    assert authority["preserved_components"]["count"] == len(e0_mi.PATCH_PATHS)
    assert authority["effective_loader_called"] is False
...
def test_real_p1_1729_publication_is_bound_as_model_unavailable() -> None:
    ...
    assert publication["sequence_commit"] == e0_mh.P1_1729_SEQUENCE_COMMIT
...
def test_p1_1729_publication_rejects_semantic_manifest_drift(
    ...
    with pytest.raises(
        patch.P1SequenceSeed20260613PatchError,
...
def test_real_anfis_20260613_state_bundle_is_modern_and_available() -> None:
    state = patch._anfis_20260613_state_bundle(
        execution_head=patch._git("rev-parse", "HEAD")
    )
    assert state["publication_commit"] == patch.ANFIS_20260613_COMMIT
...
def test_anfis_20260613_manifest_does_not_activate_the_1729_adapter() -> None:
    manifest = json.loads(
        patch.ANFIS_20260613_MANIFEST_PATH.read_text(encoding="utf-8")
    )
...
def test_real_anfis_20260613_manifest_is_consumable_under_mj_authority() -> None:
    manifest = json.loads(
        patch.ANFIS_20260613_MANIFEST_PATH.read_text(encoding="utf-8")
    )
    assert sequence_builder.validate_state_slot_manifest(
        ...
        state_path=patch.ANFIS_20260613_STATE_PATH,
        consumer_authority=_effective_summary(),
    ) == (True, "", False)


def test_anfis_20260613_bundle_rejects_seed_drift(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manifest = json.loads(
        patch.ANFIS_20260613_MANIFEST_PATH.read_text(encoding="utf-8")
    ...
    with pytest.raises(
        patch.P1SequenceSeed20260613PatchError,
        match="ANFIS 20260613 manifest drifted",
    ):
        patch._anfis_20260613_state_bundle(
            execution_head=patch._git("rev-parse", "HEAD")
        )


def test_target_namespace_is_exactly_28_absent_paths() -> None:
    paths = patch.p1_seed_20260613_namespace_paths()
    contract = patch.p1_seed_20260613_namespace_absence()
    assert len(paths) == len({path.as_posix() for path in paths}) == 28
    assert contract["model_id"] == "P1"
    assert contract["base_seed"] == 20_260_613
    assert contract["paths"] == [path.as_posix() for path in paths]
    assert contract["all_absent_at_lock"] is True


def test_real_progression_has_exactly_two_closed_p1_bundles() -> None:
    ...
    assert progression["completed_seeds"] == [1729, 20_260_612]
    assert progression["completed_seed_present_count"] == 12
    assert progression["remaining_absent_count"] == 128
    assert progression["prior_seed_residual_absent_count"] == 44
    assert progression["target_seed_absent_count"] == 28
    assert progression["later_seed_absent_count"] == 56
    assert progression["next_authorized_seed"] == 20_260_613
...
def test_gate_rejects_nonexclusive_identity_before_loading(
    ...
    with pytest.raises(
        patch.P1SequenceSeed20260613PatchError,
        match="only the one-shot P1 sequence build for seed 20260613",
    ):
        patch.require_p1_sequence_seed_20260613_authorized(model_id, base_seed)
...
def test_effective_gate_preserves_builder_only_false_seals(
    ...
    monkeypatch.setattr(patch, "p1_seed_20260613_namespace_absence", lambda: {})
    monkeypatch.setattr(patch, "closure_progression_prelock", lambda: {})
    observed = patch.require_p1_sequence_seed_20260613_authorized(
        "P1", 20_260_613
    )
...
def test_effective_gate_rejects_broadened_authority(
    ...
    with pytest.raises(
        ...
        patch.require_p1_sequence_seed_20260613_authorized("P1", 20_260_613)


def test_effective_loader_has_no_unpublished_bypass_arguments() -> None:
    assert inspect.signature(
        patch.load_and_validate_p1_sequence_seed_20260613_patch_lock
    ).parameters == {}
...
def test_schema_accepts_real_historical_authority_shapes() -> None:
    ...
    prelock = {
        ...
        "base_authorities": {
            "e0_mh": patch._historical_e0_mh_authority(
                execution_head=execution_head
            ),
            "e0_mi": patch._historical_e0_mi_authority(
                execution_head=execution_head
            ),
        },
        ...
        "anfis_20260613_state_bundle": patch._anfis_20260613_state_bundle(
            execution_head=execution_head
        ),
    ...
    payload = patch.build_p1_sequence_seed_20260613_patch_lock_payload(
        prelock,
...
def test_companion_separates_physical_and_historical_inputs() -> None:
    ...
    mh_superseded = [
        _record(path, e0_mh.PATCH_COMPONENT_ROLES[path], token)
        for path, token in zip(patch.E0_MH_SUPERSEDED_PATHS, ("c", "d"))
    ]
    publication_records = [
    ...
    state_records = [
        _record(path.as_posix(), f"anfis_20260613_{index}", "f")
        for index, path in enumerate(
            (
                patch.ANFIS_20260613_STATE_PATH,
                patch.ANFIS_20260613_POINTER_PATH,
                patch.ANFIS_20260613_MANIFEST_PATH,
            ),
            start=1,
        )
    ]
    payload = {
        ...
        "base_authorities": {
            "e0_mh": {
                ...
                "superseded_components": {"records": mh_superseded},
            },
            "e0_mi": {
                "lock": _record(
        ...
        "anfis_20260613_state_bundle": {"records": state_records},
    }
    lock_record = _record(
        patch.DEFAULT_PATCH_LOCK_PATH.as_posix(),
        "external_p1_sequence_seed_20260613_patch_lock",
    ...
    assert len(companion["inputs"]) == 22
    assert len(companion["historical_inputs"]) == 2
    assert {
        record["path"] for record in companion["historical_inputs"]
    } == set(patch.E0_MH_SUPERSEDED_PATHS)
    assert all(
        record["commit"] == patch.E0_MH_H_COMMIT
...
def test_verification_contract_is_exact_and_rejects_dvc_evidence(
    ...
    patch.validate_p1_sequence_seed_20260613_patch_verification(evidence)
...
def test_verification_preflights_schema_before_any_command(
    ...
    events: list[str] = []
    schema_preflight = patch.preflight_p1_sequence_seed_20260613_patch_schema()
    ...
    observed = locker.run_p1_sequence_seed_20260613_patch_verification(
...
def test_check_only_runs_no_verification_or_write(
    ...
    prelock = {
        ...
        "progression_prelock": {"remaining_absent_count": 128},
    }
    monkeypatch.setattr(
        locker,
        "_preflight_schema",
    ...
    monkeypatch.setattr(
        locker,
        "collect_p1_sequence_seed_20260613_patch_prelock_state",
        lambda **_kwargs: events.append("prelock") or prelock,
    )
    monkeypatch.setattr(
        locker,
        "run_p1_sequence_seed_20260613_patch_verification",
...
def test_execute_lock_rolls_back_if_companion_publication_fails(
    ...
    guard_directory = project_root / "tmp" / "closure_v1_e0_mj_locker"
    ...
    monkeypatch.setattr(
        locker,
        "build_p1_sequence_seed_20260613_patch_lock_payload",
    ...
    monkeypatch.setattr(
        locker,
        "_load_unpublished_p1_sequence_seed_20260613_patch_lock",
        lambda: ({}, {}),
    )
