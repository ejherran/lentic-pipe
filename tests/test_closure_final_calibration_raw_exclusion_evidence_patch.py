...
from src.experiments import (
    closure_final_calibration_raw_exclusion_evidence_patch as patch,
)
from src.experiments import (
    lock_closure_final_calibration_raw_exclusion_evidence_patch as locker,
)
from src.experiments import run_closure_anfis_learning_curve as e7_runner


ROOT = Path(__file__).resolve().parents[1]
BASE_P_MCALE = "79e799343e06d718797b61e8eee44d4af42bb1ca"
H_MCALE = "068dd7622db30953a2b1c6bd1c21ebc60a22efa5"
EXPECTED_SUPERSEDED = {
...
def _forbid_real_scientific_payload_reads(
    ...
    def forbidden(*args: Any, **kwargs: Any) -> Any:
        del args, kwargs
        raise AssertionError("MCALF governance opened a scientific payload")
...
def _valid_filter_evidence() -> list[dict[str, Any]]:
    return [
        dict(patch.mcale.TARGET_FILTER_EVIDENCE_CONTRACT),
        *(dict(record) for record in patch.RAW_EXCLUSION_EVIDENCE_CONTRACT),
    ]
    ...
    def require_snapshot(
        ...
        *,
        ...
        if expected != observed:
            raise patch.FinalCalibrationRawExclusionEvidencePatchError(
...
def test_h_p_r_topology_and_published_bases_are_exact() -> None:
    assert patch.PATCH_GATE == "E0-MCALF"
    assert patch.BASE_P_MCALE_COMMIT == BASE_P_MCALE
    assert patch.H_MCALE_COMMIT == H_MCALE
...
def test_h_mcale_partition_and_prior_p_inputs_are_exact() -> None:
    assert set(patch.H_MCALE_SUPERSEDED_PATHS) == EXPECTED_SUPERSEDED
    assert len(patch.H_MCALE_PRESERVED_PATHS) == 5
    assert not set(patch.H_MCALE_PRESERVED_PATHS) & EXPECTED_SUPERSEDED
    assert set(patch.P_MCALE_PATHS) == {
        "reports/closure_v1/00_protocol/"
        "final_calibration_target_filter_evidence_patch_lock.json",
        "reports/closure_v1/00_protocol/"
    ...
    authority_source = inspect.getsource(patch._p_mcale_authority)
    for token in (
        "mcale._state_for_h(",
        "mcale._p_mcald_authority(repo_root=repo_root)",
...
def test_raw_exclusion_evidence_contract_is_exact_and_arithmetically_closed() -> None:
    expected = (
        {
            "model_id": "B0",
            "source_path": (
            ...
            "excluded_target_keys_sha256": RAW_EXCLUSION_DIGEST,
        },
        {
            "model_id": "B1",
            "source_path": (
    ...
    assert patch.RAW_EXCLUSION_EVIDENCE_CONTRACT == expected
    ...
    for record in expected:
        assert set(record) == exact_keys
        candidate = record["candidate_row_count"]
        matched = record["matched_target_row_count"]
        excluded = record["excluded_incomplete_target_row_count"]
        assert type(candidate) is int
        assert type(matched) is int
        assert type(excluded) is int
        assert candidate == matched + excluded
        assert record["excluded_target_keys_sha256"] == RAW_EXCLUSION_DIGEST
    schema_contract = _schema()["$defs"]["rawExclusionEvidenceContract"]
    assert schema_contract == {
        "type": "array",
        "minItems": 4,
        "maxItems": 4,
        "items": {"$ref": "#/$defs/rawExclusionEvidenceRecord"},
    }
    schema_record = _schema()["$defs"]["rawExclusionEvidenceRecord"]
    assert schema_record["additionalProperties"] is False
    assert set(schema_record["required"]) == exact_keys
    for key in (
        "candidate_row_count",
        "matched_target_row_count",
        "excluded_incomplete_target_row_count",
    ):
        assert schema_record["properties"][key] == {"type": "integer"}


def test_producer_and_both_validators_share_one_exact_raw_contract() -> None:
    ...
    producer = inspect.getsource(calibration_runner._baseline_predictions)
    for token in (
        '"model_selection", "calibration_threshold"',
        "membership = candidate_keys.merge(",
        "excluded = frame.loc[~membership",
        '"excluded_incomplete_target_row_count": int((~membership).sum())',
        '_canonical_json_bytes({"keys": excluded_rows})',
    ):
        assert token in producer
    assert "closure_final_calibration_target_filter_evidence_patch" not in (
        inspect.getsource(calibration_runner._validate_input_filter_evidence)
    )


def test_old_counts_foreign_digest_types_order_keys_and_equations_fail_closed() -> None:
    mutations: list[tuple[int, str, Any]] = [
        (1, "candidate_row_count", 2931),
        (1, "excluded_incomplete_target_row_count", 285),
        (2, "candidate_row_count", 14655),
        (2, "excluded_incomplete_target_row_count", 1425),
        (1, "candidate_row_count", True),
        (1, "matched_target_row_count", 2645),
        (1, "excluded_incomplete_target_row_count", False),
        (1, "excluded_target_keys_sha256", "f" * 64),
        (1, "excluded_target_keys_sha256", int("1" * 64)),
        (1, "source_path", "foreign.parquet"),
        (1, "model_id", "B9"),
    ]

    def reject(evidence: list[dict[str, Any]]) -> None:
        with pytest.raises(patch.FinalCalibrationError):
            calibration_runner._validate_input_filter_evidence(evidence)
        with pytest.raises(patch.FinalCalibrationRawExclusionEvidencePatchError):
    ...
    for index, key, value in mutations:
        ...
        reject(evidence)
    missing = _valid_filter_evidence()
    missing[1].pop("source_path")
    reject(missing)
    extra = _valid_filter_evidence()
    extra[1]["foreign"] = False
    reject(extra)
    reordered = _valid_filter_evidence()
    reordered[1], reordered[2] = reordered[2], reordered[1]
    reject(reordered)
    target_drift = _valid_filter_evidence()
    target_drift[0]["materialized_row_count"] = 2646
    reject(target_drift)
...
def test_schema_preflight_and_semantic_payload_chain_are_closed(
    ...
    schema = _schema()
    assert schema["properties"]["gate"]["const"] == "E0-MCALF"
    result = patch.preflight_final_calibration_raw_exclusion_evidence_patch_schema()
    assert result["gate"] == "E0-MCALF"
    assert result["supported_subset_verified"] is True
    assert result["schema_count"] == 1

    prior_lock_path = patch.mcale.DEFAULT_PATCH_LOCK_PATH
    prior_manifest_path = patch.mcale.DEFAULT_PATCH_LOCK_MANIFEST_PATH
    ...
    assert prior_lock_bytes == patch.mcal._git_blob_bytes(
        ROOT, BASE_P_MCALE, prior_lock_path
    )
    assert prior_manifest_bytes == patch.mcal._git_blob_bytes(
        ROOT, BASE_P_MCALE, prior_manifest_path
    )
    ...
    prelock = {
        "repository": {
            "base_p_mcale_commit": BASE_P_MCALE,
            "h_patch_head": "a" * 40,
            "branch": "main",
            "remote_head": "a" * 40,
            "scope": {
    ...
    payload = patch.build_final_calibration_raw_exclusion_evidence_patch_lock_payload(
    ...
    def validate_synthetic_schema(value: Any, observed_schema: Any) -> None:
        ...
        assert value["gate"] == "E0-MCALF"
        assert value["raw_exclusion_evidence_contract"] == [
            dict(record) for record in patch.RAW_EXCLUSION_EVIDENCE_CONTRACT
        ]
    ...
    monkeypatch.setattr(
        patch,
        "collect_final_calibration_raw_exclusion_evidence_patch_prelock_state",
        lambda *, verify_remote, repo_root: json.loads(
            patch._canonical_json_bytes(prelock)
        ),
    )
    assert (
        patch.validate_final_calibration_raw_exclusion_evidence_patch_lock_payload(
...
def test_companion_is_exact_16_physical_4_historical_1_output_and_canonical(
    ...
    with monkeypatch.context() as loader_patch:
        ...
        for lock_ns, manifest_ns in (
            ...
            authority = (
                patch.load_effective_final_calibration_raw_exclusion_evidence_patch_authority(
...
def test_exact_four_superseded_paths_adopt_mcale_and_gate_before_io() -> None:
    assert calibration_runner.calibration is patch
    assert e7_runner.calibration is patch
    for path in EXPECTED_SUPERSEDED:
        text = (ROOT / path).read_text()
        assert "closure_final_calibration_raw_exclusion_evidence_patch" in text
...
def test_check_only_runs_schema_before_remote_prelock_and_is_nonwriting(
    ...
    monkeypatch.setattr(
        patch,
        "preflight_final_calibration_raw_exclusion_evidence_patch_schema",
        lambda: events.append("schema") or {"gate": "E0-MCALF"},
    )
    monkeypatch.setattr(
        patch,
        "collect_final_calibration_raw_exclusion_evidence_patch_prelock_state",
...
def test_effective_loader_and_run_namespace_close_zero_six_eight_and_races(
    ...
    with monkeypatch.context() as completed_patch:
        ...
        for runner in ("calibration", "e7"):
            with pytest.raises(
                patch.FinalCalibrationRawExclusionEvidencePatchError
            ):
                patch.require_final_calibration_run_namespace(
...
def test_full_publisher_is_canonical_manifest_last_frozen_and_no_clobber(
    ...
    with monkeypatch.context() as bundle_patch:
        ...
        observed, companion = (
            patch.publish_final_calibration_raw_exclusion_evidence_patch_lock_bundle(
