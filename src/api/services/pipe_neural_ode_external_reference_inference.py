"""Run calibrated reference-profile PIPE Neural ODE inference for API datasets."""
...
from src.experiments.evaluate_pipe_neural_ode_rollouts import (
    HISTORY_ROLLOUT_VERSION,
    _load_model,
    rollout_history_batch,
)
...
class NeuralOdeRolloutBloomCalibratorRecord:
...
@dataclass(frozen=True)
class PipeNeuralOdeExternalReferenceInferenceResult:
    """Artifacts and manifest payload emitted by calibrated Neural ODE inference."""
...
def neural_ode_reference_inference_artifacts_available() -> tuple[bool, list[str]]:
    """Return availability for reviewed artifacts required by Neural ODE reference inference."""
    ...
    for name, path in {
        "pipe_neural_ode_history_model": _REFERENCE_MODEL,
        "pipe_neural_ode_history_model_manifest": _REFERENCE_MODEL_MANIFEST,
        "pipe_neural_ode_rollout_calibrators": _ROLLOUT_CALIBRATOR_DIR,
        "pipe_neural_ode_rollout_calibration_manifest": _CALIBRATION_MANIFEST,
        "pipe_neural_ode_alert_policy_manifest": _POLICY_MANIFEST,
        "pipe_neural_ode_alert_policy_thresholds": _POLICY_THRESHOLDS,
    }.items():
        if not path.exists():
            missing.append(name)
    for horizon in (1, 2, 3):
        ...
        if not expected.exists():
            missing.append(f"pipe_neural_ode_rollout_bloom_calibrator_h{horizon}")
    return len(missing) == 0, missing


def run_external_pipe_neural_ode_reference_profile_inference(
    *,
    dataset_id: str,
...
) -> PipeNeuralOdeExternalReferenceInferenceResult:
    """Build the adaptive surface, run Neural ODE rollouts, and apply reviewed alert policy."""
    ...
    if model_input_columns != INPUT_COLUMNS:
        raise ValueError(
            "The API Neural ODE adapter currently supports the reviewed v1 model without extra "
            f"context columns. Model requested columns: {model_input_columns}"
        )
    ...
    horizons = (
        sorted(int(value) for value in rollouts["rollout_horizon_months"].dropna().unique())
        if "rollout_horizon_months" in rollouts
        else []
    )
    threshold_coverage = _threshold_coverage(policy_thresholds, horizons=horizons)
    calibrator_coverage = _calibrator_coverage(calibrator_records, horizons=horizons)
    row_counts = {
    ...
    manifest: dict[str, object] = {
        ...
        "inference_version": EXTERNAL_PIPE_NEURAL_ODE_REFERENCE_INFERENCE_VERSION,
        "rollout_version": HISTORY_ROLLOUT_VERSION,
        "reference_profile": _REFERENCE_PROFILE,
        "surface_contract": EXTERNAL_PIPE_ADAPTIVE_SURFACE_VERSION,
        "policy_name": policy_name,
        ...
        "policy_manifest": _file_record(_POLICY_MANIFEST, workspace=Path(".")) if _POLICY_MANIFEST.exists() else None,
        ...
        "limitations": [
            "This mode applies the reviewed adaptive WQP-focused reference transform, frozen Neural ODE v1 history model, rollout bloom calibrators, and 2B policy thresholds to an uploaded dataset.",
    ...
    report_path = run_dir / "pipe_neural_ode_reference_inference_report.md"
    manifest_path = run_dir / "pipe_neural_ode_reference_inference_manifest.json"
    ...
    _write_json(manifest_path, manifest)
    return PipeNeuralOdeExternalReferenceInferenceResult(
        manifest=manifest,
        row_counts=row_counts,
...
def _run_reference_rollouts(
    *,
    ...
    rollouts.insert(0, "rollout_version", EXTERNAL_PIPE_NEURAL_ODE_REFERENCE_INFERENCE_VERSION)
    rollouts["scientific_rollout_version"] = HISTORY_ROLLOUT_VERSION
...
def _load_rollout_bloom_calibrators(
    *,
    calibrator_dir: Path,
    rollout_horizon: int,
) -> dict[int, NeuralOdeRolloutBloomCalibratorRecord]:
    records: dict[int, NeuralOdeRolloutBloomCalibratorRecord] = {}
    for horizon in range(1, rollout_horizon + 1):
        ...
        records[horizon] = NeuralOdeRolloutBloomCalibratorRecord(
            horizon=horizon,
...
def _apply_rollout_bloom_calibrators(
    ...
    calibrators: Mapping[int, NeuralOdeRolloutBloomCalibratorRecord],
) -> pd.DataFrame:
    out = rollouts.copy()
...
def _build_reference_alerts(rollouts: pd.DataFrame, *, policy_name: str) -> pd.DataFrame:
    ...
    for row in rollouts.to_dict(orient="records"):
        base = {
            ...
            "policy_name": policy_name,
            "policy_version": "pipe_neural_ode_history_rollout_alert_policy_2b_v1",
        }
        rows.append(
            {
                **base,
                ...
                "interpretation": "2B policy threshold applied to Neural ODE IRC alert probability.",
            }
        )
        rows.append(
            {
                **base,
                ...
                "interpretation": "2B policy threshold applied to Neural ODE rollout-calibrated bloom probability.",
            }
        )
...
def _write_reference_outputs(
    *,
    ...
    rollouts_csv = run_dir / "pipe_neural_ode_reference_rollouts.csv"
    rollouts_parquet = run_dir / "pipe_neural_ode_reference_rollouts.parquet"
    summary_path = run_dir / "pipe_neural_ode_reference_rollout_summary.csv"
    policy_summary_path = run_dir / "pipe_neural_ode_reference_policy_summary.csv"
    alerts_csv = run_dir / "pipe_neural_ode_reference_alerts.csv"
    alerts_parquet = run_dir / "pipe_neural_ode_reference_alerts.parquet"
    top_alerts_path = run_dir / "pipe_neural_ode_reference_top_alerts.csv"
    recent_top_alerts_path = run_dir / "pipe_neural_ode_reference_recent_top_alerts.csv"
...
def _empty_reference_rollouts() -> pd.DataFrame:
    ...
    out["scientific_rollout_version"] = pd.Series(dtype="object")
    ...
    if not bool(calibrator_coverage.get("complete")):
        blockers.append(
            _issue(
                ...
                "One or more rollout horizons lack reviewed Neural ODE bloom calibrators.",
                calibrator_coverage,
            )
        )
    if not bool(threshold_coverage.get("complete")):
        blockers.append(
            _issue(
                ...
                "One or more rollout horizons lack reviewed Neural ODE 2B policy thresholds.",
                threshold_coverage,
            )
        )
    return blockers
...
def _reference_warnings(adaptive_surface: Mapping[str, object]) -> list[dict[str, object]]:
    warnings = [
        _issue(
            ...
            "The reviewed Neural ODE reference profile is applied mechanically; skill on this external water body is not guaranteed.",
            {"reference_profile": _REFERENCE_PROFILE},
        )
    ]
    surface_warnings = adaptive_surface.get("warnings", [])
    if isinstance(surface_warnings, list):
        warnings.extend(cast(dict[str, object], item) for item in surface_warnings if isinstance(item, dict))
    return warnings
...
def _reference_report(manifest: Mapping[str, object]) -> str:
    ...
    return "\n".join(
        [
            "# External PIPE Neural ODE Reference-Profile Inference",
            "",
            ...
            *(warnings or ["- none"]),
            ...
            "## Interpretation",
            "",
            "- The uploaded dataset was transformed through the adaptive ANFIS reference surface before Neural ODE rollout.",
            "- Bloom probabilities use the reviewed Neural ODE rollout calibrators.",
            "- Alert flags use the selected Neural ODE 2B policy thresholds.",
