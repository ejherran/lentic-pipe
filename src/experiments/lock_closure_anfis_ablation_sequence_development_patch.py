#!/usr/bin/env python
"""Lock the scientific-egress-closed E0-MS ANFIS-ablation sequence authority."""
...
from src.experiments import (  # noqa: E402
    closure_anfis_ablation_sequence_development_patch as patch,
)


PROJECT_ROOT = Path(__file__).resolve().parents[2]
OUTPUT_GUARD_DIRECTORY = (
    PROJECT_ROOT / "tmp" / "closure_v1_anfis_ablation_sequence_development_patch"
)
OUTPUT_GUARD_PATH = OUTPUT_GUARD_DIRECTORY / "lock_bundle.guard"
...
def _error(message: str) -> RuntimeError:
    return patch.AnfisAblationSequenceDevelopmentPatchError(message)
...
def _parse_focused_summary(stdout: str, stderr: str) -> dict[str, Any]:
    if patch.FOCUSED_TEST_COUNT <= 0:
        raise _error("E0-MS focused-test count must be frozen before execute-lock")
...
def run_anfis_ablation_sequence_development_patch_verification(
    *,
    expected_schema_preflight: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    """Run repository checks only; never build sequences, use DVC, or outcomes."""
    schema_preflight = (
        patch.preflight_anfis_ablation_sequence_development_patch_schema()
    )
    if (
        expected_schema_preflight is not None
        and dict(expected_schema_preflight) != schema_preflight
    ):
        raise _error("E0-MS schema changed before verification")
...
def _open_repo_directory(relative: Path, *, context: str) -> int:
    ...
    descriptor = _open_real_directory(
        Path(os.path.abspath(PROJECT_ROOT)), context="E0-MS repository root"
    )
    try:
...
def _owner_state(owner: OwnedFile | OutputGuard) -> str:
    try:
        anchored_directory = os.fstat(owner.directory_file_descriptor)
        opened_file = os.fstat(owner.file_descriptor)
        current_descriptor = _open_repo_directory(
            owner.relative_parent, context="E0-MS owned-file parent"
        )
    except OSError as exc:
        raise _error("E0-MS owned-file descriptors could not be inspected") from exc
    except patch.AnfisAblationSequenceDevelopmentPatchError:
        return "foreign"
    try:
        current_directory = os.fstat(current_descriptor)
        if (
        ...
        raise _error("E0-MS owned-file identity could not be inspected") from exc
    finally:
...
def _close_owner(owner: OwnedFile | OutputGuard) -> None:
    ...
    for descriptor in (owner.file_descriptor, owner.directory_file_descriptor):
        try:
            ...
            errors.append(exc)
    if errors:
        raise _error("E0-MS owned-file descriptors could not be closed") from errors[0]
...
def _acquire_guard() -> OutputGuard:
    ...
    if Path(os.path.abspath(OUTPUT_GUARD_DIRECTORY)) != expected:
        raise _error("E0-MS guard directory escaped ignored tmp/")
    relative_parent = _repo_relative(
        OUTPUT_GUARD_DIRECTORY, context="E0-MS guard directory"
    )
    directory_descriptor = _ensure_repo_directory(
        relative_parent, context="E0-MS guard directory"
    )
    flags = (
        os.O_RDWR
        | os.O_CREAT
    ...
    try:
        ...
        raise _error("E0-MS locker guard already exists") from exc
    except BaseException:
        os.close(directory_descriptor)
        raise
    ...
    try:
        ...
        if not stat.S_ISREG(metadata.st_mode):
            raise _error("E0-MS guard is not regular")
        guard = OutputGuard(
        ...
        if not _guard_is_owned(guard):
            raise _error("E0-MS guard identity drifted")
        _write_all(
            descriptor,
            f"pid={os.getpid()}\n".encode("ascii"),
            context="E0-MS guard",
        ...
        if guard is not None:
            try:
                _rollback_if_owned(guard, context="E0-MS guard")
        ...
        for owned_descriptor in (descriptor, directory_descriptor):
            try:
                ...
                cleanup_errors.append(cleanup_exc)
        if cleanup_errors:
            cleanup_error = _error("E0-MS guard creation cleanup failed closed")
...
def _release_guard(guard: OutputGuard) -> None:
    ...
    try:
        if _owner_state(guard) != "owned":
            raise _error("E0-MS guard ownership was lost; foreign state was preserved")
        ...
        if _owner_state(guard) != "absent":
            raise _error("E0-MS guard release did not establish absence")
    except BaseException as exc:
        error = exc
    try:
        _close_owner(guard)
...
def _write_temp(path: Path, expected: Path, payload: bytes) -> OwnedFile:
    lexical, relative_parent = _closed_output(
        path, expected, context="E0-MS temporary"
    )
    directory_descriptor = _open_repo_directory(
        relative_parent, context="E0-MS temporary parent"
    )
    flags = (
        os.O_RDWR
        | os.O_CREAT
    ...
    try:
        ...
        os.close(directory_descriptor)
        raise _error(
            f"Refusing to clobber E0-MS temporary: {expected.as_posix()}"
        ) from exc
    except BaseException:
        os.close(directory_descriptor)
        raise
    ...
    try:
        ...
        if not stat.S_ISREG(metadata.st_mode):
            raise _error("E0-MS temporary is not regular")
        _write_all(descriptor, payload, context="E0-MS temporary")
        if _owner_state(owner) != "owned":
            raise _error("E0-MS temporary identity drifted")
        return owner
    except BaseException as exc:
        cleanup_errors: list[BaseException] = []
        if owner is not None:
            try:
                _rollback_if_owned(owner, context="E0-MS temporary")
            except BaseException as cleanup_exc:
                cleanup_errors.append(cleanup_exc)
        for owned_descriptor in (descriptor, directory_descriptor):
            try:
                ...
                cleanup_errors.append(cleanup_exc)
        if cleanup_errors:
            cleanup_error = _error("E0-MS temporary creation cleanup failed closed")
            cleanup_error.add_note(
                "; ".join(
                    f"{type(item).__name__}: {item}" for item in cleanup_errors
                )
...
def _publish_temp(
    ...
    final, relative_parent = _closed_output(
        final_path, expected, context="E0-MS output"
    )
    if not _guard_is_owned(guard):
        raise _error("E0-MS guard changed before publication")
    if _owner_state(temp) != "owned":
        raise _error("E0-MS temporary ownership drifted")
    destination_descriptor = _open_repo_directory(
        relative_parent, context="E0-MS output parent"
    )
    ...
    try:
        ...
        if _owner_state(owner) != "owned":
            raise _error("E0-MS published output identity drifted")
        if not _guard_is_owned(guard):
            raise _error("E0-MS guard changed during publication")
        _rollback_if_owned(temp, context="E0-MS published temporary")
        _close_owner(temp)
        return owner
    except FileExistsError as exc:
        ...
        raise _error(
            f"Refusing to clobber E0-MS output: {expected.as_posix()}"
        ) from exc
    except BaseException as exc:
        cleanup_errors: list[BaseException] = []
        if owner is not None:
            try:
                _rollback_if_owned(owner, context="E0-MS output")
            except BaseException as cleanup_exc:
                cleanup_errors.append(cleanup_exc)
        ...
        if cleanup_errors:
            cleanup_error = _error(
                "E0-MS publication rollback could not be completed safely"
            )
            cleanup_error.add_note(
                "; ".join(
                    f"{type(item).__name__}: {item}" for item in cleanup_errors
...
def _read_owned_bytes(owner: OwnedFile) -> bytes:
    if _owner_state(owner) != "owned":
        raise _error("E0-MS output changed before verification")
    ...
    if _owner_state(owner) != "owned":
        raise _error("E0-MS output changed during verification")
    return payload
...
def _publish_lock_bundle(
    ...
    try:
        ...
        lock_record = _logical_record(
            lock_path, "anfis_ablation_sequence_development_patch_lock", lock_bytes
        )
        ...
        if _read_owned_bytes(published_lock) != lock_bytes:
            raise _error("Published E0-MS lock bytes drifted")
        if _read_owned_bytes(published_manifest) != companion_bytes:
            raise _error("Published E0-MS companion bytes drifted")
    ...
    if active_error is not None:
        for owner in reversed(published):
            try:
                _rollback_if_owned(owner, context="E0-MS published output")
            except BaseException as exc:
    ...
    if active_error is None and not cleanup_errors:
        for owner in published:
            try:
                if _owner_state(owner) != "owned":
                    raise _error("E0-MS published output changed after guard release")
            except BaseException as exc:
                cleanup_errors.append(exc)
        if cleanup_errors:
            for owner in reversed(published):
    ...
    if cleanup_errors:
        cleanup_error = _error("E0-MS lock resource cleanup failed closed")
        cleanup_error.add_note(
    ...
    if active_error is not None:
        raise active_error
    if result is None:
        raise _error("E0-MS publication produced no result")
    return result
...
def check_only() -> dict[str, Any]:
    ...
    prelock = patch.collect_anfis_ablation_sequence_development_patch_prelock_state()
    return {
...
def execute_lock() -> dict[str, Any]:
    ...
    prelock = patch.collect_anfis_ablation_sequence_development_patch_prelock_state()
    verification = run_anfis_ablation_sequence_development_patch_verification(
        expected_schema_preflight=schema_preflight
    )
    if (
        patch.collect_anfis_ablation_sequence_development_patch_prelock_state()
        != prelock
    ):
        raise _error("E0-MS prelock state changed during verification")
    payload = patch.build_anfis_ablation_sequence_development_patch_lock_payload(
        prelock,
        verification,
        created_at_utc=datetime.now(timezone.utc).isoformat(),
    )
    patch.validate_anfis_ablation_sequence_development_patch_lock_payload(payload)
    ...
    return {
        ...
        "anfis_ablation_sequence_builder_authorized": False,
...
def check_effective() -> dict[str, Any]:
    return patch.load_effective_anfis_ablation_sequence_development_authority()
