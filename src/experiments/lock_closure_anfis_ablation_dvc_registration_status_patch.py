#!/usr/bin/env python
"""Check and publish the outcome-closed E0-MZD porcelain-status authority."""
...
def _count(value: Any, name: str) -> int:
    ...
    raise patch.AnfisAblationDvcRegistrationStatusPatchError(
        f"E0-MZD check-only summary is missing exact integer: {name}"
    )
...
def _missing_pointer_validation(value: Any) -> dict[str, Any]:
    ...
    if not isinstance(value, Mapping) or not patch._exact_equal(dict(value), expected):
        raise patch.AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD missing-pointer validation contract drifted"
        )
    return expected


def _in_progress_namespace_validation(value: Any) -> dict[str, Any]:
    """Require the guarded exact-prefix policy preserved from E0-MZC."""
    ...
    if not isinstance(value, Mapping) or not patch._exact_equal(dict(value), expected):
        raise patch.AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD in-progress namespace validation contract drifted"
        )
    return expected


def _status_order_correction(value: Any) -> dict[str, Any]:
    """Require the exact R-MZC tracked-first porcelain-order incident."""

    expected = patch._status_order_correction(PROJECT_ROOT)
    if not isinstance(value, Mapping) or not patch._exact_equal(dict(value), expected):
        raise patch.AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD porcelain status-order incident/correction contract drifted"
        )
    return expected
...
def _porcelain_scope_validation(value: Any) -> dict[str, Any]:
    ...
    if not isinstance(value, Mapping) or not patch._exact_equal(dict(value), expected):
        raise patch.AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD porcelain scope-validation contract drifted"
        )
    return expected


def check_only() -> dict[str, Any]:
    """Run the schema-first, non-writing H-E0-MZD preflight."""

    schema = patch.preflight_anfis_ablation_dvc_registration_status_patch_schema()
    prelock = patch.collect_anfis_ablation_dvc_registration_status_patch_prelock_state(
        verify_remote=True
    )
    ...
    correction = _status_order_correction(prelock.get("status_order_correction"))
    ...
    if counts != expected_counts:
        raise patch.AnfisAblationDvcRegistrationStatusPatchError(
            f"E0-MZD check-only counts drifted: {counts}"
        )
    return {
        ...
        "base_p_mzc_commit": patch.P_MZC_COMMIT,
...
def check_effective() -> dict[str, Any]:
    """Inspect the exact published porcelain-status patch authority."""

    return patch.load_effective_anfis_ablation_dvc_registration_status_patch_authority(
        verify_remote=True
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except patch.AnfisAblationDvcRegistrationStatusPatchError as exc:
