#!/usr/bin/env python
"""Create and verify the additive Closure V1 E0-MF lock bundle."""
...
from src.experiments.closure_p1_temporal_consumer_pytest_summary_patch import (  # noqa: E402
...
OUTPUT_GUARD_DIRECTORY = PROJECT_ROOT / "tmp" / "closure_v1_e0_mf_locker"
OUTPUT_GUARD_NAMES = (
    "p1_temporal_consumer_pytest_summary_patch_lock.guard",
    "p1_temporal_consumer_pytest_summary_patch_lock_manifest.guard",
)
...
def _require_success_marker(stdout: str, stderr: str, marker: str) -> None:
    if marker not in stdout + "\n" + stderr:
        raise P1TemporalConsumerPytestSummaryPatchError(
            f"E0-MF verification marker is absent: {marker}"
        )
...
def _require_empty_output(stdout: str, stderr: str, *, context: str) -> None:
    if stdout.strip() or stderr.strip():
        raise P1TemporalConsumerPytestSummaryPatchError(
            f"E0-MF expected empty output from {context}"
        )
...
def _parse_focused_summary(stdout: str, stderr: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise P1TemporalConsumerPytestSummaryPatchError(
            "E0-MF focused pytest duration is malformed"
        ) from exc
    try:
        expected_clock = str(timedelta(seconds=int(duration)))
    except OverflowError as exc:
        raise P1TemporalConsumerPytestSummaryPatchError(
            "E0-MF focused pytest duration is out of range"
        ) from exc
    if duration < Decimal(60):
        if clock is not None:
            raise P1TemporalConsumerPytestSummaryPatchError(
                "E0-MF sub-minute pytest summary must not include a clock"
            )
        summary_format = "pytest_short"
    else:
        if clock != expected_clock:
            raise P1TemporalConsumerPytestSummaryPatchError(
                "E0-MF long pytest summary clock drifted"
            )
...
def _dvc_terminal_status(
    ...
    *,
    ...
    if len(terminals) != 1 or (
        ...
        raise P1TemporalConsumerPytestSummaryPatchError(
            "E0-MF targeted DVC push is not one closed idempotent terminal"
        )
    if any(
        ...
        raise P1TemporalConsumerPytestSummaryPatchError(
            "E0-MF targeted DVC push exceeded one object"
        )
    return terminals[0]


def run_p1_temporal_consumer_pytest_summary_patch_verification() -> dict[str, Any]:
    """Run the closed E0-MF verification set and in-process P1 audit."""
    if FOCUSED_TEST_COUNT <= 0:
        raise P1TemporalConsumerPytestSummaryPatchError(
            "E0-MF focused-test count has not been finalized"
        )
    # Keep the historical regular-file executable policy unchanged.  The P1
    # auditor is deliberately absent: it is a fixed in-process callable.
    try:
...
def _guard_paths(*, create_directory: bool) -> tuple[Path, Path]:
    ...
    if OUTPUT_GUARD_DIRECTORY != tmp_root / "closure_v1_e0_mf_locker":
        raise P1TemporalConsumerPytestSummaryPatchError(
            "E0-MF coordination directory escaped ignored tmp/"
        )
    if create_directory:
        try:
            hardened._ensure_real_directory(
                tmp_root,
                parent=PROJECT_ROOT,
                context="E0-MF tmp root",
            )
            hardened._ensure_real_directory(
                OUTPUT_GUARD_DIRECTORY,
                parent=tmp_root,
                context="E0-MF guard directory",
            )
        except hardened.DevelopmentRuntimeTemporalConsumerPatchError as exc:
            raise _translate(exc) from exc
    return (
...
def _refuse_existing_outputs(output: Path, companion: Path) -> None:
    ...
    if existing:
        raise P1TemporalConsumerPytestSummaryPatchError(
            f"Refusing to overwrite an existing E0-MF lock bundle: {existing}"
        )
...
def _acquire_guards(
    ...
    if len(guards) != 2:
        raise P1TemporalConsumerPytestSummaryPatchError(
            "E0-MF requires exactly two guards"
        )
...
def _execute_lock(output: Path, companion: Path) -> dict[str, Any]:
    ...
    try:
        ...
        prelock = collect_p1_temporal_consumer_pytest_summary_patch_prelock_state(
            verify_remote=True
        )
        verification = run_p1_temporal_consumer_pytest_summary_patch_verification()
        p1_consumer_namespace_absence()
        repeated = collect_p1_temporal_consumer_pytest_summary_patch_prelock_state(
            verify_remote=True
        )
        if repeated != prelock:
            raise P1TemporalConsumerPytestSummaryPatchError(
                "E0-MF authority changed during verification"
            )
        timestamp = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
        payload = build_p1_temporal_consumer_pytest_summary_patch_lock_payload(
            prelock,
            verification,
            created_at_utc=timestamp,
        )
        schema = _load_regular_json(DEFAULT_PATCH_LOCK_SCHEMA, context="E0-MF schema")
        validate_p1_temporal_consumer_pytest_summary_patch_lock_payload(
            payload,
            schema,
    ...
    try:
        ...
        raise P1TemporalConsumerPytestSummaryPatchError(
            "E0-MF lock transaction cleanup failed closed"
        ) from exc
    if active_error is not None:
        if isinstance(active_error, P1TemporalConsumerPytestSummaryPatchError):
            raise active_error
        raise P1TemporalConsumerPytestSummaryPatchError(
            "E0-MF lock transaction failed"
        ) from active_error
    if result is None:
        raise P1TemporalConsumerPytestSummaryPatchError(
            "E0-MF lock transaction produced no result"
        )
    return result
...
def _check_only(output: Path, companion: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "historical_e0_me": prelock["base_authority"]["e0_me"],
...
def _check_effective(output: Path, companion: Path) -> dict[str, Any]:
    ...
    summary = require_p1_temporal_consumer_pytest_summary_patch_authorized(
        model_id="P1",
        base_seed=1729,
        device="cpu",
    )
    ...
    if before != after:
        raise P1TemporalConsumerPytestSummaryPatchError(
            "E0-MF lock bundle changed during preflight"
        )
    return {
