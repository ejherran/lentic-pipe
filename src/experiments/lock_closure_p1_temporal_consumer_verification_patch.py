#!/usr/bin/env python
"""Create and verify the additive Closure V1 E0-ME lock bundle."""
...
from src.experiments.closure_p1_temporal_consumer_verification_patch import (  # noqa: E402
...
OUTPUT_GUARD_DIRECTORY = PROJECT_ROOT / "tmp" / "closure_v1_e0_me_locker"
OUTPUT_GUARD_NAMES = (
    "p1_temporal_consumer_verification_patch_lock.guard",
    "p1_temporal_consumer_verification_patch_lock_manifest.guard",
)
...
def _require_success_marker(stdout: str, stderr: str, marker: str) -> None:
    if marker not in stdout + "\n" + stderr:
        raise P1TemporalConsumerVerificationPatchError(
            f"E0-ME verification marker is absent: {marker}"
        )
...
def _require_empty_output(stdout: str, stderr: str, *, context: str) -> None:
    if stdout.strip() or stderr.strip():
        raise P1TemporalConsumerVerificationPatchError(
            f"E0-ME expected empty output from {context}"
        )
...
def _parse_focused_summary(stdout: str, stderr: str) -> int:
    ...
    if (
        ...
        raise P1TemporalConsumerVerificationPatchError(
            "E0-ME focused pytest summary is not one exact clean result"
        )
...
def _dvc_terminal_status(
    ...
    *,
    ...
    if len(terminals) != 1 or (
        ...
        raise P1TemporalConsumerVerificationPatchError(
            "E0-ME targeted DVC push is not one closed idempotent terminal"
        )
    if any(
        ...
        raise P1TemporalConsumerVerificationPatchError(
            "E0-ME targeted DVC push exceeded one object"
        )
    return terminals[0]


def run_p1_temporal_consumer_verification_patch_verification() -> dict[str, Any]:
    """Run the closed E0-ME verification set and in-process P1 audit."""
    if FOCUSED_TEST_COUNT <= 0:
        raise P1TemporalConsumerVerificationPatchError(
            "E0-ME focused-test count has not been finalized"
        )
...
def _guard_paths(*, create_directory: bool) -> tuple[Path, Path]:
    ...
    if OUTPUT_GUARD_DIRECTORY != tmp_root / "closure_v1_e0_me_locker":
        raise P1TemporalConsumerVerificationPatchError(
            "E0-ME coordination directory escaped ignored tmp/"
        )
    if create_directory:
        try:
            hardened._ensure_real_directory(
                tmp_root,
                parent=PROJECT_ROOT,
                context="E0-ME tmp root",
            )
            hardened._ensure_real_directory(
                OUTPUT_GUARD_DIRECTORY,
                parent=tmp_root,
                context="E0-ME guard directory",
            )
        except hardened.DevelopmentRuntimeTemporalConsumerPatchError as exc:
            raise _translate(exc) from exc
    return (
...
def _refuse_existing_outputs(output: Path, companion: Path) -> None:
    ...
    if existing:
        raise P1TemporalConsumerVerificationPatchError(
            f"Refusing to overwrite an existing E0-ME lock bundle: {existing}"
        )
...
def _acquire_guards(
    ...
    if len(guards) != 2:
        raise P1TemporalConsumerVerificationPatchError(
            "E0-ME requires exactly two guards"
        )
...
def _execute_lock(output: Path, companion: Path) -> dict[str, Any]:
    ...
    try:
        ...
        prelock = collect_p1_temporal_consumer_verification_patch_prelock_state(
            verify_remote=True
        )
        verification = run_p1_temporal_consumer_verification_patch_verification()
        p1_consumer_namespace_absence()
        repeated = collect_p1_temporal_consumer_verification_patch_prelock_state(
            verify_remote=True
        )
        if repeated != prelock:
            raise P1TemporalConsumerVerificationPatchError(
                "E0-ME authority changed during verification"
            )
        timestamp = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
        payload = build_p1_temporal_consumer_verification_patch_lock_payload(
            prelock,
            verification,
            created_at_utc=timestamp,
        )
        schema = _load_regular_json(DEFAULT_PATCH_LOCK_SCHEMA, context="E0-ME schema")
        validate_p1_temporal_consumer_verification_patch_lock_payload(
            payload,
            schema,
    ...
    try:
        ...
        raise P1TemporalConsumerVerificationPatchError(
            "E0-ME lock transaction cleanup failed closed"
        ) from exc
    if active_error is not None:
        if isinstance(active_error, P1TemporalConsumerVerificationPatchError):
            raise active_error
        raise P1TemporalConsumerVerificationPatchError(
            "E0-ME lock transaction failed"
        ) from active_error
    if result is None:
        raise P1TemporalConsumerVerificationPatchError(
            "E0-ME lock transaction produced no result"
        )
    return result
...
def _check_only(output: Path, companion: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "historical_e0_md": prelock["base_authority"]["e0_md"],
...
def _check_effective(output: Path, companion: Path) -> dict[str, Any]:
    ...
    summary = require_p1_temporal_consumer_verification_authorized(
        model_id="P1",
        base_seed=1729,
        device="cpu",
    )
    ...
    if before != after:
        raise P1TemporalConsumerVerificationPatchError(
            "E0-ME lock bundle changed during preflight"
        )
    return {
