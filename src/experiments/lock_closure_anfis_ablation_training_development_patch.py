#!/usr/bin/env python
"""Check and publish the outcome-closed E0-MT training authority."""
...
from src.experiments import (  # noqa: E402
    closure_anfis_ablation_training_development_patch as patch,
)
...
def _error(message: str) -> RuntimeError:
    return patch.AnfisAblationTrainingDevelopmentPatchError(message)
...
def _parse_focused_summary(stdout: str, stderr: str) -> dict[str, int]:
    ...
    if type(count) is not int or count <= 0:
        raise _error("E0-MT focused-test count is not frozen")
...
def run_anfis_ablation_training_development_patch_verification(
    *, expected_schema_preflight: Mapping[str, Any] | None = None
) -> dict[str, Any]:
    """Run repository checks without fitting, DVC, or scientific egress."""
    schema_preflight = patch.preflight_anfis_ablation_training_development_patch_schema()
    if expected_schema_preflight is not None and dict(expected_schema_preflight) != schema_preflight:
        raise _error("E0-MT schema changed before verification")
...
def check_only() -> dict[str, Any]:
    schema = patch.preflight_anfis_ablation_training_development_patch_schema()
    prelock = patch.collect_anfis_ablation_training_development_patch_prelock_state(
        verify_remote=True
    )
    return {
        ...
        "physical_input_count": prelock["runtime_contract"]["physical_input_count"],
        "writes_performed": False,
        "verification_commands_run": False,
        "fit_or_trainer_run": False,
        "auditor_run": False,
...
def execute_lock() -> dict[str, Any]:
    schema = patch.preflight_anfis_ablation_training_development_patch_schema()
    before = patch.collect_anfis_ablation_training_development_patch_prelock_state(
        verify_remote=True
    )
    verification = run_anfis_ablation_training_development_patch_verification(
        expected_schema_preflight=schema
    )
    after = patch.collect_anfis_ablation_training_development_patch_prelock_state(
        verify_remote=True
    )
    if before != after:
        raise _error("E0-MT prelock state changed during verification")
    payload = patch.build_anfis_ablation_training_development_patch_lock_payload(
        prelock=before, verification=verification
    )
    patch.validate_anfis_ablation_training_development_patch_lock_payload(payload)
    lock, companion = patch.publish_anfis_ablation_training_lock_bundle(payload)
    return {
...
def check_effective() -> dict[str, Any]:
    return patch.load_effective_anfis_ablation_training_authority(
        verify_remote=True, mode="audit"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except patch.AnfisAblationTrainingDevelopmentPatchError as exc:
