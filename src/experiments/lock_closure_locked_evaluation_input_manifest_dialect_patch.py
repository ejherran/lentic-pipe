#!/usr/bin/env python
"""Check, publish, and audit the E0-MID manifest-dialect authority."""
...
def _error(
    message: str,
) -> patch.ClosureLockedEvaluationInputManifestDialectPatchError:
    return patch.ClosureLockedEvaluationInputManifestDialectPatchError(message)
...
def _parse_focused_summary(stdout: str, stderr: str) -> dict[str, int]:
    ...
    if type(count) is not int or count != 48:
        raise _error("E0-MID focused-test count is not frozen at exact48")
...
def run_closure_locked_evaluation_input_manifest_dialect_patch_verification(
    *, expected_schema_preflight: Mapping[str, Any] | None = None
) -> dict[str, Any]:
    """Run the closed H checks without R restoration, science, or DVC."""

    schema_preflight = (
        patch.preflight_closure_locked_evaluation_input_manifest_dialect_patch_schema(
    ...
    if (
        ...
        raise _error("E0-MID schema changed before verification")
...
def _capture_metadata_only_state() -> tuple[Any, Any, dict[str, Any]]:
    """Capture H and contained R metadata without opening an R payload."""

    archive = patch._r_archive_metadata_snapshot(PROJECT_ROOT)
    physical = patch._physical_snapshot(PROJECT_ROOT)
    prelock = (
        patch.collect_closure_locked_evaluation_input_manifest_dialect_patch_prelock_state(
            verify_remote=True,
            repo_root=PROJECT_ROOT,
        )
    )
    return archive, physical, prelock


def check_only() -> dict[str, Any]:
    """Revalidate H and the contained R10 twice without writes or byte opens."""
    ...
    archive_before, physical_before, prelock_before = _capture_metadata_only_state()
    archive_after, physical_after, prelock_after = _capture_metadata_only_state()
    if archive_before != archive_after:
        raise _error("E0-MID archived R10 metadata changed during check-only")
    if physical_before != physical_after:
        raise _error("E0-MID physical authority inputs changed during check-only")
    if prelock_before != prelock_after:
        raise _error("E0-MID topology, refs, namespace, or prelock state changed")
    return {
        ...
        "archived_r_output_count": len(archive_before),
        "companion_input_count": patch.EXPECTED_COMPANION_INPUT_COUNT,
        "historical_input_count": patch.EXPECTED_HISTORICAL_INPUT_COUNT,
        "companion_output_count": patch.EXPECTED_COMPANION_OUTPUT_COUNT,
        "live_r_output_count": 0,
        "archive_metadata_revalidated": True,
...
def execute_lock() -> dict[str, Any]:
    """Verify H and publish only the lock/companion P pair."""
    ...
    archive_before, physical_before, prelock_before = _capture_metadata_only_state()
    verification = (
        run_closure_locked_evaluation_input_manifest_dialect_patch_verification(
            expected_schema_preflight=schema
        )
    )
    archive_after, physical_after, prelock_after = _capture_metadata_only_state()
    if archive_before != archive_after:
        raise _error("E0-MID archived R10 metadata changed during verification")
    if physical_before != physical_after:
        raise _error("E0-MID physical authority inputs changed during verification")
    if prelock_before != prelock_after:
        raise _error("E0-MID topology, refs, namespace, or prelock state changed")
    payload = (
        patch.build_closure_locked_evaluation_input_manifest_dialect_patch_lock_payload(
            prelock=prelock_before,
            verification=verification,
            repo_root=PROJECT_ROOT,
        )
    )
    patch.validate_closure_locked_evaluation_input_manifest_dialect_patch_lock_payload(
        payload,
        verify_remote=True,
        repo_root=PROJECT_ROOT,
    )
    lock, companion = (
        patch.publish_closure_locked_evaluation_input_manifest_dialect_patch_lock_bundle(
            payload,
            repo_root=PROJECT_ROOT,
        )
    )
    archive_published = patch._r_archive_metadata_snapshot(PROJECT_ROOT)
    physical_published = patch._physical_snapshot(PROJECT_ROOT)
    if archive_published != archive_before:
        raise _error("E0-MID publisher changed archived R10 metadata")
    if physical_published != physical_before:
        raise _error("E0-MID publisher changed immutable physical inputs")
    return {
...
def check_effective() -> dict[str, Any]:
    """Load published authority without depending on ignored local containment."""

    return (
        patch.load_effective_closure_locked_evaluation_input_manifest_dialect_patch_authority(
            verify_remote=True,
            repo_root=PROJECT_ROOT,
        )
    )
