"""Seal the exact raw-score exclusion evidence used by final calibration.

E0-MCALF is an additive authority over published P-E0-MCALE.  It changes no
scientific data, model, target, denominator, or result path.  It binds each
raw-score candidate surface to the exact common-origin target projection and
closes the row-count equations and excluded-key digest.
"""
...
from src.experiments import (
    closure_final_calibration_target_filter_evidence_patch as mcale,
)
...
BASE_P_MCALE_COMMIT = "79e799343e06d718797b61e8eee44d4af42bb1ca"
H_MCALE_COMMIT = "068dd7622db30953a2b1c6bd1c21ebc60a22efa5"
H_MCALE_PARENT = mcale.BASE_P_MCALD_COMMIT
PATCH_GATE = "E0-MCALF"
FINAL_CALIBRATION_GATE = PATCH_GATE
EXPERIMENT_ID = "closure_v1"
LOCK_SCHEMA_VERSION = "closure_final_calibration_raw_exclusion_evidence_patch_lock_v1"
COMPANION_SCHEMA_VERSION = (
    "closure_final_calibration_raw_exclusion_evidence_patch_lock_manifest_v1"
)
...
LOCKER_GUARD_PATH = Path(
    "tmp/closure_v1_e0_mcalf/final_calibration_raw_exclusion_evidence_patch_lock.guard"
)

H_MCALE_SUPERSEDED_PATHS = tuple(
    sorted(
        {
...
H_MCALE_PRESERVED_PATHS = tuple(
    path for path in mcale.PATCH_PATHS if path not in H_MCALE_SUPERSEDED_PATHS
)
P_MCALE_PATHS = (
    mcale.DEFAULT_PATCH_LOCK_PATH.as_posix(),
    mcale.DEFAULT_PATCH_LOCK_MANIFEST_PATH.as_posix(),
)
PATCH_PATHS = tuple(
    sorted(
        {
...
FINAL_CALIBRATION_H_STAGED_SCOPE = {
    path: ("M" if path in H_MCALE_SUPERSEDED_PATHS else "A")
...
FINAL_CALIBRATION_R_STAGED_SCOPE = dict(mcale.FINAL_CALIBRATION_R_STAGED_SCOPE)
R_OUTPUT_PATHS = mcale.R_OUTPUT_PATHS
...
RAW_EXCLUSION_EVIDENCE_CONTRACT = (
    {
        "model_id": "B0",
        "source_path": "data/closure_v1/development/baselines/B0/raw_scores.parquet",
        "candidate_row_count": 4140,
        "matched_target_row_count": 2646,
        "excluded_incomplete_target_row_count": 1494,
        "excluded_target_keys_sha256": EXCLUDED_TARGET_KEYS_SHA256,
    },
    {
        "model_id": "B1",
        "source_path": "data/closure_v1/development/baselines/B1/raw_scores.parquet",
        "candidate_row_count": 20700,
        "matched_target_row_count": 13230,
        "excluded_incomplete_target_row_count": 7470,
        "excluded_target_keys_sha256": EXCLUDED_TARGET_KEYS_SHA256,
    },
    {
        "model_id": "B2",
        "source_path": "data/closure_v1/development/baselines/B2/raw_scores.parquet",
        "candidate_row_count": 20700,
        "matched_target_row_count": 13230,
        "excluded_incomplete_target_row_count": 7470,
        "excluded_target_keys_sha256": EXCLUDED_TARGET_KEYS_SHA256,
    ...
    {
        "model_id": "M0",
        "source_path": "data/closure_v1/development/mifal/M0/raw_scores.parquet",
        "candidate_row_count": 4140,
        "matched_target_row_count": 2646,
        "excluded_incomplete_target_row_count": 1494,
        "excluded_target_keys_sha256": EXCLUDED_TARGET_KEYS_SHA256,
...
TYPE_CHECK_COMMAND = mcale.TYPE_CHECK_COMMAND
...
POETRY_CHECK_COMMAND = mcale.POETRY_CHECK_COMMAND
PUBLICATION_GUARD_COMMAND = mcale.PUBLICATION_GUARD_COMMAND
DIFF_CHECK_COMMAND = mcale.DIFF_CHECK_COMMAND
UNPUBLISHED_AUTHORIZATIONS = dict(mcale.UNPUBLISHED_AUTHORIZATIONS)
EMPTY_SHA256 = hashlib.sha256(b"").hexdigest()


class FinalCalibrationRawExclusionEvidencePatchError(mcale.FinalCalibrationError):
    """Raised when the E0-MCALF correction authority is not exact."""


FinalCalibrationError = mcale.FinalCalibrationError
P = ParamSpec("P")
R = TypeVar("R")


def _error(message: str) -> FinalCalibrationRawExclusionEvidencePatchError:
    return FinalCalibrationRawExclusionEvidencePatchError(message)
...
def _error_boundary(function: Callable[P, R]) -> Callable[P, R]:
    ...
    def wrapped(*args: P.args, **kwargs: P.kwargs) -> R:
        ...
        except FinalCalibrationRawExclusionEvidencePatchError:
            raise
        except mcale.FinalCalibrationError as exc:
            raise _error(str(exc).replace("E0-MCALE", "E0-MCALF")) from exc
...
def _git_record_at_commit(
    ...
    if mode != "100644":
        raise _error(f"E0-MCALF historical Git mode drifted: {path}")
    return {
        "role": role,
        "path": path,
        "bytes": len(payload),
...
def _p_mcale_authority(*, repo_root: Path) -> dict[str, Any]:
    """Reconstruct the published MCALE H/P authority and its transitive base."""

    if (
        mcal._single_parent(repo_root, H_MCALE_COMMIT, context="H-E0-MCALE")
        != H_MCALE_PARENT
    ):
        raise _error("E0-MCALF historical H-E0-MCALE parent drifted")
    h_scope = mcal._git_scope(repo_root, H_MCALE_PARENT, H_MCALE_COMMIT)
    if h_scope != {
        "added": 5,
        "modified": 4,
        "deleted": 0,
        "path_count": 9,
        "paths": list(mcale.PATCH_PATHS),
    }:
        raise _error("E0-MCALF historical H-E0-MCALE scope drifted")
    if (
        mcal._single_parent(repo_root, BASE_P_MCALE_COMMIT, context="P-E0-MCALE")
        != H_MCALE_COMMIT
    ):
        raise _error("E0-MCALF historical P-E0-MCALE parent drifted")
    p_scope = mcal._git_scope(repo_root, H_MCALE_COMMIT, BASE_P_MCALE_COMMIT)
    if p_scope != {
        "added": 2,
        "modified": 0,
        "deleted": 0,
        "path_count": 2,
        "paths": sorted(P_MCALE_PATHS),
    }:
        raise _error("E0-MCALF historical P-E0-MCALE scope drifted")

    h_components = [
        _git_record_at_commit(
            path,
            role="final_calibration_target_filter_evidence_patch_component",
            commit=H_MCALE_COMMIT,
            repo_root=repo_root,
        )
        for path in mcale.PATCH_PATHS
    ]
    preserved: list[dict[str, Any]] = []
    for path in H_MCALE_PRESERVED_PATHS:
        record = _git_record_at_commit(
            path,
            role="preserved_h_mcale_component",
            commit=H_MCALE_COMMIT,
            repo_root=repo_root,
        )
        ...
        if physical != record:
            raise _error(f"E0-MCALF preserved H-MCALE component drifted: {path}")
        preserved.append(record)
    historical = [
        {
            **_git_record_at_commit(
                path,
                role="superseded_h_mcale_component",
                commit=H_MCALE_COMMIT,
                repo_root=repo_root,
            ),
            "commit": H_MCALE_COMMIT,
        }
        for path in H_MCALE_SUPERSEDED_PATHS
    ]
    p_components: list[dict[str, Any]] = []
    for path in P_MCALE_PATHS:
        role = (
            "published_p_mcale_lock"
            if path == mcale.DEFAULT_PATCH_LOCK_PATH.as_posix()
            else "published_p_mcale_lock_manifest"
        )
        record = _git_record_at_commit(
            path,
            role=role,
            commit=BASE_P_MCALE_COMMIT,
            repo_root=repo_root,
        )
        physical = mcal._git_artifact_record(
            Path(path),
        ...
        if physical != record:
            raise _error(f"E0-MCALF published P-MCALE component drifted: {path}")
        p_components.append(record)

    lock_bytes = mcal._git_blob_bytes(
        repo_root, BASE_P_MCALE_COMMIT, mcale.DEFAULT_PATCH_LOCK_PATH
    )
    companion_bytes = mcal._git_blob_bytes(
        repo_root,
        BASE_P_MCALE_COMMIT,
        mcale.DEFAULT_PATCH_LOCK_MANIFEST_PATH,
    )
    try:
        lock_payload = mcal._parse_json_bytes(
            lock_bytes, context="published P-E0-MCALE lock"
        )
        companion = mcal._parse_json_bytes(
            companion_bytes, context="published P-E0-MCALE companion"
        )
    except mcale.FinalCalibrationError as exc:
        raise _error("E0-MCALF historical P-E0-MCALE JSON drifted") from exc
    if (
        not isinstance(lock_payload, Mapping)
        or not isinstance(companion, Mapping)
        or lock_bytes != mcale._canonical_json_bytes(lock_payload)
        or companion_bytes != mcale._canonical_json_bytes(companion)
        or lock_payload.get("gate") != mcale.PATCH_GATE
        or cast(Mapping[str, Any], lock_payload.get("repository", {})).get(
            "h_patch_head"
        )
        != H_MCALE_COMMIT
    ):
        raise _error("E0-MCALF historical P-E0-MCALE payload drifted")
    try:
        prior_schema = mcal._load_json_object(
            mcale.DEFAULT_PATCH_LOCK_SCHEMA, repo_root=repo_root
        )
        mcal.validate_json_schema(lock_payload, prior_schema)
        mcale._validate_timestamp(lock_payload.get("generated_at_utc"))
        mcale._validate_verification(
            lock_payload.get("verification"), repo_root=repo_root
        )
        prior_state = mcale._state_for_h(
            repository={
                "base_p_mcald_commit": H_MCALE_PARENT,
                "h_patch_head": H_MCALE_COMMIT,
                "branch": "main",
                "remote_head": H_MCALE_COMMIT,
                "scope": h_scope,
            },
            h_patch={
                "gate": "H-E0-MCALE",
        ...
        expected_prior = mcale.build_final_calibration_target_filter_evidence_patch_lock_payload(
            prior_state,
            cast(Mapping[str, Any], lock_payload["verification"]),
            generated_at_utc=cast(str, lock_payload["generated_at_utc"]),
        )
    except (KeyError, mcale.FinalCalibrationError) as exc:
        raise _error("E0-MCALF historical P-E0-MCALE authority drifted") from exc
    if _canonical_json_bytes(lock_payload) != _canonical_json_bytes(expected_prior):
        raise _error("E0-MCALF historical P-E0-MCALE reconstruction drifted")
    lock_record = {
        "role": "final_calibration_target_filter_evidence_patch_lock",
        "path": mcale.DEFAULT_PATCH_LOCK_PATH.as_posix(),
        "bytes": len(lock_bytes),
        "sha256": _sha256_bytes(lock_bytes),
    }
    if companion_bytes != mcale._canonical_json_bytes(
        mcale._expected_companion(lock_payload, lock_record)
    ):
        raise _error("E0-MCALF historical P-E0-MCALE companion drifted")
    transitive = mcale._p_mcald_authority(repo_root=repo_root)
    if _canonical_json_bytes(lock_payload.get("p_mcald_authority")) != (
        _canonical_json_bytes(transitive)
    ):
        raise _error("E0-MCALF transitive P-E0-MCALD authority drifted")
    sealed_runtime = lock_payload.get("runtime")
    if not isinstance(sealed_runtime, Mapping):
        raise _error("E0-MCALF historical P-E0-MCALE runtime binding is absent")
    return {
        "gate": "P-E0-MCALE",
        "commit": BASE_P_MCALE_COMMIT,
        "parent_h_mcale": H_MCALE_COMMIT,
        "h_mcale_parent": H_MCALE_PARENT,
        "h_scope": h_scope,
        "p_scope": p_scope,
        "h_component_count": 9,
...
def _candidate_status_is_exact(repo_root: Path) -> bool:
    ...
    return all(
        ...
        in (
            {" M", "M ", "MM"}
            if path in H_MCALE_SUPERSEDED_PATHS
            else {"??", "A "}
        )
        for path in PATCH_PATHS
    )
...
def _h_patch_authority(
    *, repo_root: Path, verify_remote: bool
    ...
    if type(verify_remote) is not bool:
        raise _error("E0-MCALF remote policy must be an exact boolean")
    head = mcal._git_head(repo_root)
    branch = cast(str, mcal._git(repo_root, "branch", "--show-current")).strip()
    if branch != "main":
        raise _error("E0-MCALF requires branch main")
    expected_scope = {
        "added": 5,
        "modified": 4,
        "deleted": 0,
    ...
    candidate = head == BASE_P_MCALE_COMMIT
    if candidate:
        if not _candidate_status_is_exact(repo_root):
            raise _error("E0-MCALF candidate workspace is not exact 4M+5A")
        component_commit: str | None = None
        h_head = BASE_P_MCALE_COMMIT
        scope = expected_scope
    else:
        if (
            mcal._single_parent(repo_root, head, context="H-E0-MCALF")
            != BASE_P_MCALE_COMMIT
        ):
            raise _error("E0-MCALF published H parent drifted")
        scope = mcal._git_scope(repo_root, BASE_P_MCALE_COMMIT, head)
        if scope != expected_scope or mcal._workspace_status_records(repo_root):
            raise _error("E0-MCALF published H scope/worktree drifted")
        component_commit = head
        h_head = head
    components = [
        mcal._git_artifact_record(
            Path(path),
            role="final_calibration_raw_exclusion_evidence_patch_component",
            repo_root=repo_root,
    ...
    expected_ref = BASE_P_MCALE_COMMIT if candidate else head
    if tracking != expected_ref:
        raise _error("E0-MCALF H tracking ref drifted")
    remote = mcal._live_remote_main_head(repo_root) if verify_remote else tracking
    if remote != expected_ref:
        raise _error("E0-MCALF H remote ref drifted")
    return (
        {
            "base_p_mcale_commit": BASE_P_MCALE_COMMIT,
            "h_patch_head": h_head,
            "branch": branch,
            "remote_head": remote,
            "scope": scope,
...
def _namespace_paths() -> tuple[Path, ...]:
    ...
    temporary_bases = (
        *forbidden_finals,
        mcale.mcalp.DEFAULT_PATCH_LOCK_PATH,
        mcale.mcalp.DEFAULT_PATCH_LOCK_MANIFEST_PATH,
        mcale.mcalc.DEFAULT_PATCH_LOCK_PATH,
        mcale.mcalc.DEFAULT_PATCH_LOCK_MANIFEST_PATH,
        mcale.mcald.DEFAULT_PATCH_LOCK_PATH,
        mcale.mcald.DEFAULT_PATCH_LOCK_MANIFEST_PATH,
        mcale.DEFAULT_PATCH_LOCK_PATH,
        mcale.DEFAULT_PATCH_LOCK_MANIFEST_PATH,
    )
    return (
        *forbidden_finals,
        *(mcal._temporary_path(path) for path in temporary_bases),
        ...
        mcale.mcalp.LOCKER_GUARD_PATH,
        mcale.mcalc.LOCKER_GUARD_PATH,
        mcale.mcald.LOCKER_GUARD_PATH,
        mcale.LOCKER_GUARD_PATH,
        LOCKER_GUARD_PATH,
...
def _require_prelock_namespace(*, repo_root: Path) -> None:
    ...
    if missing_base:
        raise _error(f"E0-MCALF base P-MCALE authority is absent: {missing_base}")
    occupied = [
        path.as_posix()
    ...
    if occupied:
        raise _error(f"E0-MCALF prelock namespace is occupied: {occupied}")
    if mcal._entry_exists(Path(mcal.mze.OUTCOME_ACCESS_LOG), repo_root=repo_root):
        raise _error("E0-MCALF outcome access log must remain absent")
    if any(
        mcal._entry_exists(Path(path), repo_root=repo_root)
        for path in mcal.mze.E0_M_PATHS
    ):
        raise _error("E0-MCALF final E0-M namespace must remain absent")


@_error_boundary
def preflight_final_calibration_raw_exclusion_evidence_patch_schema(
    *, repo_root: Path | None = None
) -> dict[str, Any]:
    root = _root(repo_root)
    schema = mcale.mcalp.mcal._load_json_object(
        DEFAULT_PATCH_LOCK_SCHEMA, repo_root=root
    )
    validator = getattr(
        mcale.mcalp.mcal.closure_contract, "_assert_supported_json_schema", None
    )
    if validator is None:
        raise _error("E0-MCALF closed schema preflight is unavailable")
    try:
        validator(schema)
    except mcale.mcalp.mcal.ClosureContractError as exc:
    ...
    return {
        ...
        "schema_count": 1,
        "schemas": [
            mcale.mcalp.mcal._file_record(
                DEFAULT_PATCH_LOCK_SCHEMA,
                role="final_calibration_raw_exclusion_evidence_patch_lock_schema",
                repo_root=root,
            )
        ],
...
def _state_for_h(
    *,
    ...
    schema_preflight = preflight_final_calibration_raw_exclusion_evidence_patch_schema(
        repo_root=repo_root
    )
    runtime = mcal.load_and_validate_final_calibration_runtime(repo_root=repo_root)
    p_mcale = _p_mcale_authority(repo_root=repo_root)
    ...
    sealed_runtime = cast(Mapping[str, Any], p_mcale["sealed_runtime"])
    ...
    runtime_contract = {
        ...
        "raw_exclusion_evidence_contract": _deep_copy(RAW_EXCLUSION_EVIDENCE_CONTRACT),
    }
    boundary = {
    ...
    prelock = {
        ...
        "base_p_mcale_output_present_count": 2,
...
@_error_boundary
def collect_final_calibration_raw_exclusion_evidence_patch_prelock_state(
...
@_error_boundary
def build_final_calibration_raw_exclusion_evidence_patch_lock_payload(
    ...
    *,
    ...
    if set(prelock) != required:
        raise _error("E0-MCALF prelock dialect drifted")
    ...
    return {
        ...
        "p_mcale_authority": _deep_copy(prelock["p_mcale_authority"]),
        "h_patch": _deep_copy(prelock["h_patch"]),
        "runtime": _deep_copy(prelock["runtime"]),
        "raw_exclusion_evidence_contract": _deep_copy(prelock["raw_exclusion_evidence_contract"]),
...
def _validate_timestamp(value: Any) -> None:
    if not isinstance(value, str):
        raise _error("E0-MCALF generated timestamp is absent")
    try:
        parsed = datetime.fromisoformat(value.replace("Z", "+00:00"))
    except ValueError as exc:
        raise _error("E0-MCALF generated timestamp is malformed") from exc
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise _error("E0-MCALF generated timestamp must be timezone-aware")
...
def _validate_verification(value: Any, *, repo_root: Path) -> None:
    ...
    if not isinstance(value, Mapping) or set(value) != {
        ...
        raise _error("E0-MCALF verification evidence dialect drifted")
    if _canonical_json_bytes(value["schema_preflight"]) != _canonical_json_bytes(
        preflight_final_calibration_raw_exclusion_evidence_patch_schema(repo_root=repo_root)
    ):
        raise _error("E0-MCALF schema verification evidence drifted")
    for key, command in (
        ...
        try:
            ...
            raise _error(str(exc).replace("E0-MCAL", "E0-MCALF")) from exc
    ...
    if (
        ...
        raise _error("E0-MCALF focused verification evidence drifted")
    try:
        mcal._validate_command_evidence(
...
@_error_boundary
def validate_final_calibration_raw_exclusion_evidence_patch_lock_payload(
    ...
    *,
    ...
    if not isinstance(payload, Mapping):
        raise _error("E0-MCALF lock payload must be an object")
    ...
    if payload.get("authorizations") != UNPUBLISHED_AUTHORIZATIONS:
        raise _error("E0-MCALF unpublished authorizations drifted")
    state = collect_final_calibration_raw_exclusion_evidence_patch_prelock_state(
        verify_remote=verify_remote, repo_root=root
    )
    expected = build_final_calibration_raw_exclusion_evidence_patch_lock_payload(
        state,
    ...
    if _canonical_json_bytes(payload) != _canonical_json_bytes(expected):
        raise _error("E0-MCALF lock semantic reconstruction drifted")
    return dict(payload)


_OwnedOutput = mcale._OwnedOutput
...
def _publish_bytes_no_clobber(
    ...
    try:
        return mcale._publish_bytes_no_clobber(
...
def _rollback_owned_output(output: _OwnedOutput) -> None:
    try:
        mcale._rollback_owned_output(output)
    except mcale.FinalCalibrationError as exc:
        raise _error(str(exc).replace("E0-MCALE", "E0-MCALF")) from exc
...
def _close_owned_output(output: _OwnedOutput) -> None:
    try:
        mcale._close_owned_output(output)
    except mcale.FinalCalibrationError as exc:
        raise _error(str(exc).replace("E0-MCALE", "E0-MCALF")) from exc
...
def _expected_companion(
    ...
    base = cast(Mapping[str, Any], payload["p_mcale_authority"])
    ...
    if (
        ...
        raise _error("E0-MCALF companion physical input set drifted")
...
def _require_publication_verification(
    ...
    if verification == _default_unrun_verification():
        raise _error(
            "E0-MCALF publication requires the exact frozen verification evidence"
        )
...
def _physical_snapshot(
    ...
    records = [
        dict(record)
        for record in mcale._physical_snapshot(
            repo_root, scientific_inventory=scientific_inventory
        )
    ]
    known = {cast(str, record["path"]) for record in records}
    current_paths = [*H_MCALE_PRESERVED_PATHS, *P_MCALE_PATHS, *PATCH_PATHS]
...
def _require_physical_snapshot(
    ...
    *,
    ...
    if _canonical_json_bytes(expected) != _canonical_json_bytes(
        ...
        raise _error(f"E0-MCALF physical authority changed {context}")
...
def _validate_owned_output_bytes(
    ...
    *,
    ...
    try:
        mcale._validate_owned_output(output)
        payload, metadata = mcal._read_regular_bytes_and_metadata(
            output.path, repo_root=repo_root
        )
    except Exception as exc:
        raise _error(f"E0-MCALF owned output validation failed {context}") from exc
    if (
        payload != expected
        ...
        raise _error(
            f"E0-MCALF owned output drifted {context}: {output.path.as_posix()}"
        )
    return metadata
...
def _rollback_outputs_best_effort(
...
) -> FinalCalibrationRawExclusionEvidencePatchError | None:
    ...
    if not errors:
        return None
    error = _error("E0-MCALF owned-output rollback was incomplete")
...
def _require_owned_guard_identity(guard: Any) -> None:
    try:
        ...
        raise _error("E0-MCALF publication guard identity check failed") from exc
    if (
        ...
        raise _error("E0-MCALF publication guard identity drifted")
...
def _require_publication_boundary(
    *, repo_root: Path, owned_guard: Any | None, outputs_present: bool
    ...
    if occupied:
        raise _error(f"E0-MCALF publication namespace is occupied: {occupied}")
    if mcal._entry_exists(Path(mcal.mze.OUTCOME_ACCESS_LOG), repo_root=repo_root):
        raise _error("E0-MCALF outcome access log must remain absent")
    if any(
        mcal._entry_exists(Path(path), repo_root=repo_root)
...
def _require_repository_checkpoint(
    *,
    ...
    if (
        ...
        raise _error(f"E0-MCALF repository changed {context}")
    if verify_remote and mcal._live_remote_main_head(repo_root) != expected_head:
        raise _error(f"E0-MCALF live remote changed {context}")
...
@_error_boundary
def publish_final_calibration_raw_exclusion_evidence_patch_lock_bundle(
    ...
    try:
        ...
        raise _error("E0-MCALF publication payload is not canonical JSON") from exc
    if (
        not isinstance(frozen_payload, dict)
        or _canonical_json_bytes(frozen_payload) != lock_bytes
    ):
        raise _error("E0-MCALF publication payload must be a canonical object")
    payload = frozen_payload
    repository = payload.get("repository")
    if (
        not isinstance(repository, Mapping)
        or repository.get("h_patch_head") == BASE_P_MCALE_COMMIT
    ):
        raise _error("E0-MCALF H must be published before P publication")
    _require_publication_verification(payload, repo_root=root)
    validate_final_calibration_raw_exclusion_evidence_patch_lock_payload(
        payload, repo_root=root, verify_remote=True
    )
    inventory = payload.get("scientific_input_inventory")
    if not isinstance(inventory, Mapping):
        raise _error("E0-MCALF scientific inventory is absent")
    ...
    if initial_head != repository.get("h_patch_head"):
        raise _error("E0-MCALF H refs drifted before publication")
    validate_final_calibration_raw_exclusion_evidence_patch_lock_payload(
        payload, repo_root=root, verify_remote=True
    )
    ...
    try:
        try:
            guard = mt._acquire_publication_guard(
                LOCKER_GUARD_PATH,
                b"E0-MCALF final calibration raw exclusion evidence lock publication\n",
                repo_root=root,
            )
        except Exception as exc:
            raise _error("E0-MCALF publication guard acquisition failed") from exc
        _require_physical_snapshot(
            snapshot,
            scientific_inventory=inventory,
            repo_root=root,
        ...
        lock_record = {
            "role": "final_calibration_raw_exclusion_evidence_patch_lock",
        ...
        try:
            ...
            raise _error("E0-MCALF publication guard release failed") from exc
        guard = None
        ...
        for pass_index in (1, 2):
            ...
            for output, expected in publication_set:
                ...
                if _metadata_identity(observed) != _metadata_identity(
                    ...
                    raise _error(
                        "E0-MCALF owned output changed before ownership transfer"
                    )
            mcale._require_owned_identity_set(
        ...
        for output, expected in publication_set:
            ...
            if _metadata_identity(observed) != _metadata_identity(
                ...
                raise _error("E0-MCALF owned output changed at ownership transfer")
        mcale._require_owned_identity_set(
            [output for output, _ in publication_set],
        ...
        if isinstance(exc, (KeyboardInterrupt, SystemExit)):
            raise
        if isinstance(exc, FinalCalibrationRawExclusionEvidencePatchError):
            raise
        raise _error("E0-MCALF lock bundle publication failed") from exc
    finally:
...
def _validate_calibration_filter_evidence(value: Any) -> None:
    ...
    if not isinstance(value, list) or len(value) != 5:
        raise _error("E0-MCALF calibration filter evidence cardinality drifted")
    target = value[0]
    integer_keys = {
    ...
    if (
        ...
        or set(target) != set(mcale.TARGET_FILTER_EVIDENCE_CONTRACT)
        or any(type(target.get(key)) is not int for key in integer_keys)
        or any(
            type(target.get(key)) is not str
            for key in set(mcale.TARGET_FILTER_EVIDENCE_CONTRACT) - integer_keys
        )
        or dict(target) != mcale.TARGET_FILTER_EVIDENCE_CONTRACT
        ...
        raise _error("E0-MCALF target filter evidence drifted")
    ...
    for raw, expected in zip(
        ...
        if (
            ...
            raise _error(f"E0-MCALF raw exclusion evidence drifted: {model_id}")
...
def _validate_calibrator_specs(
    ...
    if not isinstance(value, Mapping) or value.get("gate") != PATCH_GATE:
        raise _error("E0-MCALF calibrator spec gate drifted")
    adapted = _deep_copy(value)
    adapted["gate"] = mcale.PATCH_GATE
    try:
        return mcale._validate_calibrator_specs(adapted)
    except mcale.FinalCalibrationError as exc:
        raise _error(str(exc).replace("E0-MCALE", "E0-MCALF")) from exc
...
def _require_exact_output_group(
    ...
    *,
    ...
    if present and len(present) != len(paths):
        raise _error(f"E0-MCALF {context} output bundle is partial")
    if not present:
        return 0
    ...
    for path in paths:
        ...
        if path.suffix == ".json":
            ...
            if payload != _canonical_json_bytes(value):
                raise _error(
                    f"E0-MCALF {context} JSON output is not canonical: "
                    f"{path.as_posix()}"
                )
            json_values[path] = value
    manifest = json_values.get(manifest_path)
    if not isinstance(manifest, Mapping):
        raise _error(f"E0-MCALF {context} manifest is absent")
    output_records = [
        {
            "path": path.as_posix(),
    ...
    if manifest.get("outputs") != output_records:
        raise _error(f"E0-MCALF {context} manifest output bindings drifted")
    ...
    if (
        ...
        raise _error(f"E0-MCALF {context} scientific boundary drifted")
    if context == "calibration":
        ...
        if (
            ...
            raise _error("E0-MCALF calibration manifest scientific dialect drifted")
        evidence = manifest.get("input_filter_evidence")
        if not isinstance(evidence, list):
            raise _error("E0-MCALF calibration input-filter evidence is absent")
        ...
        if (
            ...
            or not mcale._torch_policy_has_exact_integer_fields(
                execution.get("torch_cpu_execution_policy")
            )
        ):
            raise _error("E0-MCALF calibration execution policy drifted")
        try:
        ...
        if (
            ...
            raise _error("E0-MCALF E7 manifest scientific dialect drifted")
        ...
        if (
            ...
            raise _error("E0-MCALF E7 execution policy drifted")
        try:
            mcale._require_exact_e7_sample_evidence_types(
        ...
        raise _error("E0-MCALF output group context drifted")
    for path in paths:
        payload, observed = mcal._read_regular_bytes_and_metadata(
            path, repo_root=repo_root
        )
        if payload != payloads[path] or _metadata_identity(observed) != (
            ...
            raise _error(f"E0-MCALF {context} output changed during validation")
    return len(paths)
...
def _parse_canonical_json_with_metadata(
    ...
    try:
        ...
        raise _error(f"E0-MCALF published JSON is absent: {path.as_posix()}") from exc
    if not isinstance(value, dict) or payload != _canonical_json_bytes(value):
        raise _error(f"E0-MCALF published JSON is not canonical: {path.as_posix()}")
    return value, payload, metadata


def _published_h_state(h_head: str, *, repo_root: Path) -> dict[str, Any]:
    if h_head == BASE_P_MCALE_COMMIT or re.fullmatch(r"[0-9a-f]{40}", h_head) is None:
        raise _error("E0-MCALF published H commit is absent")
    if (
        mcal._single_parent(repo_root, h_head, context="H-E0-MCALF")
        != BASE_P_MCALE_COMMIT
    ):
        raise _error("E0-MCALF published H parent drifted")
    scope = mcal._git_scope(repo_root, BASE_P_MCALE_COMMIT, h_head)
    expected_scope = {
        "added": 5,
        "modified": 4,
        "deleted": 0,
    ...
    if scope != expected_scope:
        raise _error("E0-MCALF published H scope drifted")
    components = [
        mcal._git_artifact_record(
            Path(path),
            role="final_calibration_raw_exclusion_evidence_patch_component",
...
def _validate_published_lock_payload(
    ...
    if payload.get("authorizations") != UNPUBLISHED_AUTHORIZATIONS:
        raise _error("E0-MCALF published authorizations drifted")
    repository = payload.get("repository")
    if not isinstance(repository, Mapping) or not isinstance(
        repository.get("h_patch_head"), str
    ):
        raise _error("E0-MCALF published H binding is absent")
    ...
    if _canonical_json_bytes(payload) != _canonical_json_bytes(expected):
        raise _error("E0-MCALF published lock reconstruction drifted")
...
def _validate_p_publication(
    ...
    if any(code != "??" or path not in allowed_r_paths for code, path in workspace):
        raise _error("E0-MCALF published P worktree drifted")
    if (
        cast(str, mcal._git(repo_root, "branch", "--show-current")).strip()
        != "main"
        or mcal._single_parent(repo_root, head, context="P-E0-MCALF") != h_head
        ...
        raise _error("E0-MCALF published P topology drifted")
    tracking = mcal._git_head(repo_root, "origin/main")
    remote = mcal._live_remote_main_head(repo_root) if verify_remote else tracking
    if tracking != head or remote != head:
        raise _error("E0-MCALF published P refs drifted")
    for path in (DEFAULT_PATCH_LOCK_PATH, DEFAULT_PATCH_LOCK_MANIFEST_PATH):
        ...
        if mode != "100644" or physical != mcal._git_blob_bytes(
            ...
            raise _error(
                f"E0-MCALF published P physical/Git binding drifted: {path.as_posix()}"
            )
...
def _require_static_effective_boundary(*, repo_root: Path) -> None:
    ...
    if missing:
        raise _error(f"E0-MCALF effective authority is absent: {missing}")
    forbidden = (
        mcal.DEFAULT_PATCH_LOCK_PATH,
        mcal.DEFAULT_PATCH_LOCK_MANIFEST_PATH,
        mcal._temporary_path(mcal.DEFAULT_PATCH_LOCK_PATH),
        mcal._temporary_path(mcal.DEFAULT_PATCH_LOCK_MANIFEST_PATH),
        mcal._temporary_path(mcale.mcalp.DEFAULT_PATCH_LOCK_PATH),
        mcal._temporary_path(mcale.mcalp.DEFAULT_PATCH_LOCK_MANIFEST_PATH),
        mcal._temporary_path(mcale.mcalc.DEFAULT_PATCH_LOCK_PATH),
        mcal._temporary_path(mcale.mcalc.DEFAULT_PATCH_LOCK_MANIFEST_PATH),
        mcal._temporary_path(mcale.mcald.DEFAULT_PATCH_LOCK_PATH),
        mcal._temporary_path(mcale.mcald.DEFAULT_PATCH_LOCK_MANIFEST_PATH),
        mcal._temporary_path(mcale.DEFAULT_PATCH_LOCK_PATH),
        mcal._temporary_path(mcale.DEFAULT_PATCH_LOCK_MANIFEST_PATH),
    ...
    if occupied:
        raise _error(f"E0-MCALF effective P namespace drifted: {occupied}")
    if mcal._entry_exists(Path(mcal.mze.OUTCOME_ACCESS_LOG), repo_root=repo_root):
        raise _error("E0-MCALF outcome access log appeared")
    if any(
        mcal._entry_exists(Path(path), repo_root=repo_root)
        for path in mcal.mze.E0_M_PATHS
    ):
        raise _error("E0-MCALF final E0-M outputs appeared")
...
def _validate_effective_namespace(*, repo_root: Path) -> dict[str, Any]:
    ...
    if occupied:
        raise _error(
            "E0-MCALF effective coordination/temporary namespace is occupied: "
            f"{occupied}"
        )
    ...
    if (calibration_count, e7_count) not in {(0, 0), (6, 0), (6, 2)}:
        raise _error("E0-MCALF R bundle order drifted")
    lifecycle = {
...
def _require_effective_loading_checkpoint(
    *,
    ...
    if _canonical_json_bytes(observed) != _canonical_json_bytes(expected_namespace):
        raise _error(f"E0-MCALF R namespace changed {context}")


@_error_boundary
def load_effective_final_calibration_raw_exclusion_evidence_patch_authority(
    *, verify_remote: bool = True, repo_root: Path | None = None
    ...
    if _canonical_json_bytes(companion) != _canonical_json_bytes(
        ...
        raise _error("E0-MCALF published companion drifted")
    ...
    if initial_head != publication["p_patch_head"]:
        raise _error("E0-MCALF P refs do not match the captured baseline")
    _validate_published_lock_payload(payload, repo_root=root)
    recaptured = _validate_p_publication(
        payload, verify_remote=verify_remote, repo_root=root
    )
    if recaptured != publication:
        raise _error("E0-MCALF P publication changed during baseline recapture")
    ...
    result = {
        ...
        **publication,
        ...
        "companion": mcal._file_record(
            DEFAULT_PATCH_LOCK_MANIFEST_PATH,
            role="final_calibration_raw_exclusion_evidence_patch_lock_manifest",
            repo_root=root,
        ),
        ...
        "target_filter_evidence_correction": _deep_copy(
            mcale.TARGET_FILTER_EVIDENCE_CONTRACT
        ),
        "inference_role_correction": _deep_copy(mcale.INFERENCE_ROLE_CONTRACT),
        "candidate_semantics_correction": {
            "data_rewrite_performed": False,
            "failed_mcalp_retry_authorized": False,
            "raw_score_candidate_values": _deep_copy(
                mcale.RAW_SCORE_CANDIDATE_VALUES
            ),
        },
        "failed_attempt": _deep_copy(FAILED_ATTEMPT),
    ...
    if (
        ...
        raise _error("E0-MCALF P authority changed during effective loading")
    _require_effective_loading_checkpoint(
        repo_root=root,
        expected_head=initial_head,
        verify_remote=verify_remote,
    ...
    if (
        ...
        raise _error("E0-MCALF P authority changed at effective linearization")
    return result
...
def require_final_calibration_authority(
    *, verify_remote: bool = True, repo_root: Path | None = None
    ...
    return load_effective_final_calibration_raw_exclusion_evidence_patch_authority(
...
def require_final_calibration_run_namespace(
    *, runner: str, repo_root: Path | None = None
    ...
    if type(runner) is not str or runner not in {"calibration", "e7"}:
        raise _error("E0-MCALF run namespace requires calibration or e7")
    namespace = _validate_effective_namespace(repo_root=root)
    required_state = (
        "ready_for_calibration_bundle"
    ...
    if namespace["r_lifecycle_state"] != required_state:
        raise _error(f"E0-MCALF {runner} one-shot namespace is not ready")
    return {
        "gate": PATCH_GATE,
        "runner": runner,
...
def __getattr__(name: str) -> Any:
    """Delegate unchanged scientific/runtime surfaces to E0-MCALE."""

    return getattr(mcale, name)
