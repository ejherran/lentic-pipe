#!/usr/bin/env python
"""Check, publish, and audit the E0-MCALD inference-role authority."""
...
def _error(message: str) -> patch.FinalCalibrationInferenceRolePatchError:
    return patch.FinalCalibrationInferenceRolePatchError(message)
...
def _parse_focused_summary(stdout: str, stderr: str) -> dict[str, int]:
    ...
    if type(count) is not int or count <= 0:
        raise _error("E0-MCALD focused-test count is not frozen")
...
def check_only() -> dict[str, Any]:
    schema = patch.preflight_final_calibration_inference_role_patch_schema()
...
def execute_lock() -> dict[str, Any]:
    schema = patch.preflight_final_calibration_inference_role_patch_schema()
    before = patch.collect_final_calibration_inference_role_patch_prelock_state(
        verify_remote=True
    )
    verification = run_final_calibration_inference_role_patch_verification(
        expected_schema_preflight=schema
    )
    after = patch.collect_final_calibration_inference_role_patch_prelock_state(
        verify_remote=True
    )
    if before != after:
        raise _error("E0-MCALD prelock state changed during verification")
    ...
    lock, companion = (
        patch.publish_final_calibration_inference_role_patch_lock_bundle(payload)
    )
    return {
...
def check_effective() -> dict[str, Any]:
    return patch.load_effective_final_calibration_inference_role_patch_authority(
        verify_remote=True
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except patch.FinalCalibrationInferenceRolePatchError as exc:
