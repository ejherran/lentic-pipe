#!/usr/bin/env python
"""Validate the additive Closure V1 P1/314159 temporal-consumer authority.

E0-MO is a fail-closed successor to the published P1/314159 sequence
bundle.  It reconstructs E0-MM and E0-MN from immutable Git history, binds
the new sequence auditor without running it during H-E0-MO, and can authorize
only the P1/314159/CPU consumer after a future two-file P-E0-MO publication.
...
from src.experiments import audit_closure_p1_seed_314159_sequence_bundle as p1_audit
from src.experiments import closure_p1_sequence_seed_314159_patch as e0_mn
...
LOCK_VERSION = "closure_p1_temporal_consumer_seed_314159_patch_lock_v1"
PATCH_GATE = "E0-MO"
PATCH_ID = "p1_temporal_consumer_seed_314159_authority_patch_1"
PATCH_STATUS = "locked"
...
P1_BUNDLE_COMMIT = "2d69cc82f2611aaebef245bbffd38b4fed0c82a9"
...
PATCH_COMPONENT_ROLES = {
    DEFAULT_PATCH_LOCK_SCHEMA.as_posix(): (
        "p1_temporal_consumer_seed_314159_patch_lock_schema"
    ),
    "docs/closure_v1/E0_M_P1_TEMPORAL_CONSUMER_SEED_314159_PATCH_1.md": (
        "p1_temporal_consumer_seed_314159_patch_protocol"
    ),
    "src/experiments/closure_p1_temporal_consumer_seed_314159_patch.py": (
        "p1_temporal_consumer_seed_314159_patch_validator"
    ),
    "src/experiments/lock_closure_p1_temporal_consumer_seed_314159_patch.py": (
        "p1_temporal_consumer_seed_314159_patch_locker"
    ),
    "src/experiments/train_closure_pipe.py": "p1_temporal_consumer_mo_gate_routing",
    "tests/test_closure_p1_temporal_consumer_seed_314159_patch.py": (
        "p1_temporal_consumer_seed_314159_patch_tests"
    ),
    "tests/test_train_closure_pipe.py": "p1_temporal_consumer_mo_gate_tests",
}
...
MM_SUPERSEDED_PATHS = PATCH_MODIFIED_PATHS
MM_PRESERVED_PATHS = tuple(
    path for path in e0_mm.PATCH_PATHS if path not in MM_SUPERSEDED_PATHS
)
...
PATCH_CORRECTION = {
    ...
    "historical_e0_mn_effective_loader_called": False,
    "p1_1729_consumer_preserved": True,
    "p1_20260612_sequence_and_consumer_preserved": True,
    "p1_20260613_sequence_and_consumer_preserved": True,
    "p1_20260614_sequence_and_consumer_preserved": True,
    "p1_314159_sequence_bundle_preserved": True,
...
PATCH_SEALS = {
    ...
    "e0_mn_preserved_as_historical_authority": True,
    "mm_preserved_component_count": 5,
    "mm_superseded_component_count": 2,
    "mn_preserved_component_count": 7,
...
FOCUSED_TEST_COMMAND = (
    ...
    "tests/test_closure_p1_temporal_consumer_seed_314159_patch.py",
    "tests/test_train_closure_pipe.py",
    "tests/test_audit_closure_p1_seed_314159_sequence_bundle.py",
    "-q",
)
# Exact closed collection for H/P-E0-MO plus the live P1/314159 auditor.
FOCUSED_TEST_COUNT = 137
...
AUDITOR_MODULE = "src.experiments.audit_closure_p1_seed_314159_sequence_bundle"
AUDITOR_NAME = "audit_p1_seed_314159_sequence_bundle"
AUDITOR_QUALNAME = "audit_p1_seed_314159_sequence_bundle"
AUDITOR_SOURCE_PATH = p1_audit.AUDITOR_PATH.as_posix()


class P1TemporalConsumerSeed314159PatchError(RuntimeError):
    """Raised when E0-MO cannot prove its closed authority."""


def _translate(error: BaseException) -> P1TemporalConsumerSeed314159PatchError:
    return P1TemporalConsumerSeed314159PatchError(str(error))
...
def _git(*args: str) -> str:
    try:
        return e0_mn._git(*args)
    except e0_mn.P1SequenceSeed314159PatchError as exc:
        raise _translate(exc) from exc


def _require_commit(value: str, *, context: str) -> str:
    try:
        return e0_mn._require_commit(value, context=context)
    except e0_mn.P1SequenceSeed314159PatchError as exc:
        raise _translate(exc) from exc
...
def _require_ancestor(ancestor: str, descendant: str) -> None:
    try:
        e0_mn._require_ancestor(ancestor, descendant)
    except e0_mn.P1SequenceSeed314159PatchError as exc:
        raise _translate(exc) from exc
...
def _git_blob(commit: str, path: str) -> bytes:
    try:
        return e0_mn._git_blob(commit, path)
    except e0_mn.P1SequenceSeed314159PatchError as exc:
        raise _translate(exc) from exc
...
def _file_record(path: Path, *, role: str) -> dict[str, Any]:
    try:
        return e0_mn._file_record(path, role=role)
    except e0_mn.P1SequenceSeed314159PatchError as exc:
        raise _translate(exc) from exc
...
def _load_regular_json(path: Path, *, context: str) -> dict[str, Any]:
    try:
        return e0_mn._load_regular_json(path, context=context)
    except e0_mn.P1SequenceSeed314159PatchError as exc:
        raise _translate(exc) from exc
...
def _assert_schema_definition_supported(schema: Mapping[str, Any]) -> None:
    ...
    if not callable(validator):
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO closure-contract schema-definition validator is unavailable"
        )
    try:
        validator(schema)
...
def preflight_p1_temporal_consumer_seed_314159_patch_schema(
    schema_path: Path = DEFAULT_PATCH_LOCK_SCHEMA,
) -> dict[str, Any]:
    """Validate the physical MO schema before guards, commands, or egress."""
    if schema_path != DEFAULT_PATCH_LOCK_SCHEMA:
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO schema preflight requires the closed default path"
        )
    schema = _load_regular_json(schema_path, context="E0-MO schema preflight")
    ...
    if minimum_count or format_count:
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO schema contains keywords outside the closed contract subset: "
            f"minimum={minimum_count}, format={format_count}"
        )
    _assert_schema_definition_supported(schema)
    record = _file_record(
        schema_path,
        role="p1_temporal_consumer_seed_314159_patch_lock_schema",
    )
    if type(record["bytes"]) is not int or int(record["bytes"]) <= 0:
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO schema must be a non-empty regular JSON file"
        )
    return {
        "gate": PATCH_GATE,
...
def _observed_diff_entries(base: str, head: str) -> list[dict[str, str]]:
    try:
        return e0_mn._observed_diff_entries(base, head)
    except e0_mn.P1SequenceSeed314159PatchError as exc:
        raise _translate(exc) from exc
...
def _assert_paths_untouched(
    ...
    *,
    ...
    try:
        e0_mn._assert_paths_untouched(base, descendant, paths, context=context)
    except e0_mn.P1SequenceSeed314159PatchError as exc:
        raise _translate(exc) from exc
...
def _introduced_commit(path: str) -> str:
    try:
        return e0_mn._introduced_commit(path)
    except e0_mn.P1SequenceSeed314159PatchError as exc:
        raise _translate(exc) from exc
...
def patch_git_diff_payload(patch_head: str) -> dict[str, Any]:
    patch_head = _require_commit(patch_head, context="H-E0-MO")
    ancestry = _git("rev-list", "--parents", "-n", "1", patch_head).split()
    if ancestry != [patch_head, PATCH_BASE_COMMIT]:
        raise P1TemporalConsumerSeed314159PatchError(
            "H-E0-MO must be the direct non-merge child of P1/314159"
        )
    expected = [
        {
    ...
    if observed != expected:
        raise P1TemporalConsumerSeed314159PatchError(
            f"H-E0-MO diff differs from its closed 2M+5A allowlist: {observed}"
        )
    return {
...
def _historical_e0_mm_authority(*, execution_head: str) -> dict[str, Any]:
    """Reconstruct E0-MM with trainer and trainer tests explicitly superseded."""
    ...
    try:
        validate_json_schema(payload, schema, instance_path="$.historical_e0_mm_lock")
    except ClosureContractError as exc:
        raise _translate(exc) from exc
    fixed = {
    ...
    if repository.get("head") != E0_MM_H_COMMIT:
        raise P1TemporalConsumerSeed314159PatchError(
            "Historical E0-MM H commit drifted"
        )
    git_diff = e0_mm.patch_git_diff_payload(E0_MM_H_COMMIT)
    ...
    if payload.get("git_diff") != git_diff or payload.get("patch_components") != components:
        raise P1TemporalConsumerSeed314159PatchError(
            "Historical E0-MM Git authority drifted"
        )
    lock_record, companion_record = _published_lock_records(
        h_commit=E0_MM_H_COMMIT,
        p_commit=E0_MM_P_COMMIT,
        lock_path=e0_mm.DEFAULT_PATCH_LOCK_PATH,
        companion_path=e0_mm.DEFAULT_PATCH_MANIFEST_PATH,
        lock_role="external_p1_temporal_consumer_seed_20260614_patch_lock",
        companion_role="p1_temporal_consumer_seed_20260614_patch_companion",
    ...
    if companion != e0_mm._expected_companion(payload, lock_record=lock_record):
        raise P1TemporalConsumerSeed314159PatchError(
            "Historical E0-MM companion drifted"
        )
    records = _component_records(cast(Mapping[str, Any], components))
    by_path = {str(record["path"]): record for record in records}
    if set(by_path) != set(e0_mm.PATCH_PATHS):
        raise P1TemporalConsumerSeed314159PatchError(
            "Historical E0-MM component paths drifted"
        )
    superseded = [by_path[path] for path in MM_SUPERSEDED_PATHS]
    preserved = [by_path[path] for path in MM_PRESERVED_PATHS]
    for record in records:
        ...
        if record != expected:
            raise P1TemporalConsumerSeed314159PatchError(
                f"Historical E0-MM Git record drifted: {record['path']}"
            )
...
def _historical_e0_mn_authority(*, execution_head: str) -> dict[str, Any]:
    """Reconstruct the published builder authority without an effective loader."""
    payload = _load_regular_json(e0_mn.DEFAULT_PATCH_LOCK_PATH, context="P-E0-MN lock")
    schema = _load_regular_json(e0_mn.DEFAULT_PATCH_LOCK_SCHEMA, context="H-E0-MN schema")
    try:
        validate_json_schema(payload, schema, instance_path="$.historical_e0_mn_lock")
    except ClosureContractError as exc:
        raise _translate(exc) from exc
    fixed = {
        "lock_version": e0_mn.LOCK_VERSION,
        "status": e0_mn.PATCH_STATUS,
        "experiment_id": e0_mn.EXPERIMENT_ID,
        "surface_id": e0_mn.SURFACE_ID,
        "gate": e0_mn.PATCH_GATE,
        "patch_id": e0_mn.PATCH_ID,
        "authorizations": e0_mn.PATCH_AUTHORIZATIONS,
        "seals": e0_mn.PATCH_SEALS,
    }
    if any(payload.get(field) != value for field, value in fixed.items()):
        raise P1TemporalConsumerSeed314159PatchError(
            "Historical E0-MN fixed contract drifted"
        )
    repository = cast(Mapping[str, Any], payload.get("patch_repository", {}))
    if repository.get("head") != E0_MN_H_COMMIT:
        raise P1TemporalConsumerSeed314159PatchError(
            "Historical E0-MN H commit drifted"
        )
    git_diff = e0_mn.patch_git_diff_payload(E0_MN_H_COMMIT)
    components = e0_mn.patch_component_bundle(E0_MN_H_COMMIT)
    if payload.get("git_diff") != git_diff or payload.get("patch_components") != components:
        raise P1TemporalConsumerSeed314159PatchError(
            "Historical E0-MN Git authority drifted"
        )
    lock_record, companion_record = _published_lock_records(
        h_commit=E0_MN_H_COMMIT,
        p_commit=E0_MN_P_COMMIT,
        lock_path=e0_mn.DEFAULT_PATCH_LOCK_PATH,
        companion_path=e0_mn.DEFAULT_PATCH_MANIFEST_PATH,
        lock_role="external_p1_sequence_seed_314159_patch_lock",
        companion_role="p1_sequence_seed_314159_patch_companion",
        execution_head=execution_head,
        context="P-E0-MN",
    )
    companion = _load_regular_json(
        e0_mn.DEFAULT_PATCH_MANIFEST_PATH,
        context="P-E0-MN companion",
    )
    if companion != e0_mn._expected_companion(payload, lock_record=lock_record):
        raise P1TemporalConsumerSeed314159PatchError(
            "Historical E0-MN companion drifted"
        )
    records = _component_records(cast(Mapping[str, Any], components))
    if {str(record["path"]) for record in records} != set(e0_mn.PATCH_PATHS):
        raise P1TemporalConsumerSeed314159PatchError(
            "Historical E0-MN component paths drifted"
        )
    for record in records:
        if record != _git_record(
            ...
            raise P1TemporalConsumerSeed314159PatchError(
                f"Historical E0-MN Git record drifted: {record['path']}"
            )
        if _file_record(Path(str(record["path"])), role=str(record["role"])) != record:
            raise P1TemporalConsumerSeed314159PatchError(
                f"Preserved E0-MN component drifted: {record['path']}"
            )
    _assert_paths_untouched(
        E0_MN_H_COMMIT,
        execution_head,
        e0_mn.PATCH_PATHS,
        context="preserved H-E0-MN components",
    )
    ...
    if not isinstance(base_authorities, Mapping) or set(base_authorities) != {
        ...
        raise P1TemporalConsumerSeed314159PatchError(
            "Historical E0-MN base-authority dialect drifted"
        )
    sealed_ml = cast(Mapping[str, Any], base_authorities["e0_ml"])
    sealed_mm = cast(Mapping[str, Any], base_authorities["e0_mm"])
    if (
        sealed_ml.get("gate") != "E0-ML"
        or sealed_ml.get("patch_head") != e0_mn.E0_ML_H_COMMIT
        or sealed_ml.get("lock_commit") != e0_mn.E0_ML_P_COMMIT
        or sealed_ml.get("effective_loader_called") is not False
        or sealed_ml.get("future_outcomes_accessed") is not False
        or sealed_mm.get("gate") != "E0-MM"
        or sealed_mm.get("patch_head") != E0_MM_H_COMMIT
        or sealed_mm.get("lock_commit") != E0_MM_P_COMMIT
        or sealed_mm.get("effective_loader_called") is not False
        or sealed_mm.get("future_outcomes_accessed") is not False
    ):
        raise P1TemporalConsumerSeed314159PatchError(
            "Historical E0-MN nested E0-ML/E0-MM authorities drifted"
        )
    if payload.get("p1_1729_publication") != e0_mn._published_p1_1729_bundle(
        execution_head=execution_head
    ):
        raise P1TemporalConsumerSeed314159PatchError(
            "Historical E0-MN sealed P1/1729 publication drifted"
        )
    if payload.get(
        "p1_20260612_publication"
    ) != e0_mn._published_p1_20260612_bundle(execution_head=execution_head):
        raise P1TemporalConsumerSeed314159PatchError(
            "Historical E0-MN sealed P1/20260612 publication drifted"
        )
    if payload.get(
        "p1_20260613_publication"
    ) != e0_mn._published_p1_20260613_bundle(execution_head=execution_head):
        raise P1TemporalConsumerSeed314159PatchError(
            "Historical E0-MN sealed P1/20260613 publication drifted"
        )
    if payload.get(
        "p1_20260614_publication"
    ) != e0_mn._published_p1_20260614_bundle(execution_head=execution_head):
        raise P1TemporalConsumerSeed314159PatchError(
            "Historical E0-MN sealed P1/20260614 publication drifted"
        )
    builder = payload.get("current_runtime_builder_record")
    if builder != {**P1_ARTIFACT_BUILDER_RECORD, "role": "current_runtime_builder"}:
        raise P1TemporalConsumerSeed314159PatchError(
            "Historical E0-MN runtime builder drifted"
        )
    return {
        "gate": "E0-MN",
        "patch_head": E0_MN_H_COMMIT,
        "lock_commit": E0_MN_P_COMMIT,
        "git_diff": git_diff,
        "patch_components": components,
        "lock": lock_record,
        "companion_manifest": companion_record,
        ...
        "publication_topology_verified": True,
        "historical_e0_ml_and_e0_mm_seals_verified": True,
        "historical_authority_verified": True,
        "effective_loader_called": False,
        "future_outcomes_accessed": False,
    }
...
def _published_p1_314159_bundle(*, execution_head: str) -> dict[str, Any]:
    ancestry = _git("rev-list", "--parents", "-n", "1", P1_BUNDLE_COMMIT).split()
    if ancestry != [P1_BUNDLE_COMMIT, E0_MN_P_COMMIT]:
        raise P1TemporalConsumerSeed314159PatchError(
            "P1/314159 publication topology drifted"
        )
    versioned_roles = {
        "data/closure_v1/development/sequences/P1/seed_314159.parquet.dvc": (
            "p1_seed_314159_sequence_dvc_pointer"
        ),
        "reports/closure_v1/01_surface/sequences/P1/seed_314159_manifest.json": (
            "p1_seed_314159_sequence_manifest"
        ),
        "reports/closure_v1/01_surface/sequences/P1/seed_314159_summary.csv": (
            "p1_seed_314159_sequence_summary"
        ),
        "src/experiments/audit_closure_p1_seed_314159_sequence_bundle.py": (
            "p1_seed_314159_sequence_auditor"
        ),
        "tests/test_audit_closure_p1_seed_314159_sequence_bundle.py": (
            "p1_seed_314159_sequence_auditor_tests"
        ),
    }
    expected_diff = [
        {"status": "A", "path": path} for path in sorted(versioned_roles)
    ]
    if _observed_diff_entries(E0_MN_P_COMMIT, P1_BUNDLE_COMMIT) != expected_diff:
        raise P1TemporalConsumerSeed314159PatchError(
            "P1/314159 publication scope drifted"
        )
    ...
    _assert_paths_untouched(
        ...
        context="P1/314159 published bundle",
    )
    expected = {
        "data/closure_v1/development/sequences/P1/seed_314159.parquet.dvc": (
            102,
    ...
    for path in sorted(versioned_roles):
        ...
        if (
            ...
            raise P1TemporalConsumerSeed314159PatchError(
                f"P1/314159 published artifact drifted: {path}"
            )
        records.append(record)
    parquet = _file_record(
        Path("data/closure_v1/development/sequences/P1/seed_314159.parquet"),
        role="p1_seed_314159_sequence_payload",
    )
    if (
        parquet["bytes"] != 1_380_649
        or parquet["sha256"]
        != "c156afcd96c27a20619948fd90962f1d8f688a18d20aadb1d5ec313bbc3afbba"
    ):
        raise P1TemporalConsumerSeed314159PatchError(
            "P1/314159 Parquet payload drifted"
        )
    return {
        "commit": P1_BUNDLE_COMMIT,
        "parent": E0_MN_P_COMMIT,
...
def _auditor_identity_record() -> dict[str, Any]:
    callable_object = p1_audit.audit_p1_seed_314159_sequence_bundle
...
def run_p1_bundle_audit_in_process() -> dict[str, Any]:
    """Invoke exactly the published P1/314159 auditor in this process."""
    _auditor_identity_record()
    try:
        result = p1_audit.audit_p1_seed_314159_sequence_bundle()
    except p1_audit.ClosureP1Seed314159SequenceAuditError as exc:
        raise _translate(exc) from exc
...
def p1_consumer_namespace_absence() -> dict[str, Any]:
    ...
    if existing:
        raise P1TemporalConsumerSeed314159PatchError(
            f"P1/314159 consumer namespace is not empty: {existing}"
        )
    return {
...
def closure_progression_prelock() -> dict[str, Any]:
    ...
    if missing_required or unexpected_registered or forbidden_external:
        raise P1TemporalConsumerSeed314159PatchError(
            "P1/314159 progression prelock drifted: "
...
def collect_p1_temporal_consumer_seed_314159_patch_prelock_state(
    *,
    verify_remote: bool,
) -> dict[str, Any]:
    ...
    if status:
        raise P1TemporalConsumerSeed314159PatchError(
            f"H-E0-MO lock requires a clean worktree: {status}"
        )
    head = _require_commit(_git("rev-parse", "HEAD"), context="H-E0-MO HEAD")
    if _git("branch", "--show-current") != "main":
        raise P1TemporalConsumerSeed314159PatchError(
            "H-E0-MO lock requires branch main"
        )
    published = _require_commit(_git("rev-parse", PUBLISHED_REF), context=PUBLISHED_REF)
    if published != head:
        raise P1TemporalConsumerSeed314159PatchError(
            "H-E0-MO HEAD differs from origin/main"
        )
    remote = _remote_main_oid() if verify_remote else published
    if remote != head:
        raise P1TemporalConsumerSeed314159PatchError(
            "H-E0-MO HEAD differs from live origin/main"
        )
    ...
    for record in cast(Sequence[Mapping[str, Any]], components["records"]):
        if _file_record(Path(str(record["path"])), role=str(record["role"])) != record:
            raise P1TemporalConsumerSeed314159PatchError(
                f"Physical H-E0-MO component drifted: {record['path']}"
            )
    historical_mm = _historical_e0_mm_authority(execution_head=head)
    historical_mn = _historical_e0_mn_authority(execution_head=head)
    published_bundle = _published_p1_314159_bundle(execution_head=head)
    return {
        "patch_repository": {
            "head": head,
        ...
        "p1_314159_publication": published_bundle,
...
def build_p1_temporal_consumer_seed_314159_patch_lock_payload(
    ...
    *,
    ...
    return {
        ...
        "p1_314159_publication": dict(
            cast(Mapping[str, Any], prelock["p1_314159_publication"])
        ),
        ...
        "lock_artifact": {
            ...
            "role": "external_p1_temporal_consumer_seed_314159_patch_lock",
...
def _validate_command_evidence(
    ...
    *,
    ...
    if set(evidence).difference(required | optional):
        raise P1TemporalConsumerSeed314159PatchError(
            f"E0-MO {context} evidence has unexpected fields"
        )
    if not required.issubset(evidence) or evidence.get("command") != list(command):
        raise P1TemporalConsumerSeed314159PatchError(
            f"E0-MO {context} evidence drifted"
        )
    if evidence.get("returncode") != 0:
        raise P1TemporalConsumerSeed314159PatchError(
            f"E0-MO {context} did not pass"
        )
    for field in ("stdout_line_count", "stderr_line_count"):
        value = evidence.get(field)
        if type(value) is not int or value < 0:
            raise P1TemporalConsumerSeed314159PatchError(
                f"E0-MO {context} {field} must be a non-negative integer"
            )
...
def _validate_focused_duration_evidence(evidence: Mapping[str, Any]) -> None:
    ...
    if not isinstance(duration_text, str):
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO focused pytest duration evidence is malformed"
        )
    try:
    ...
    if duration < Decimal(60):
        ...
        try:
            ...
            raise P1TemporalConsumerSeed314159PatchError(
                "E0-MO focused pytest duration evidence is out of range"
            ) from exc
        valid = summary_format == "pytest_timedelta" and clock == expected_clock
    if not valid:
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO focused pytest duration/clock evidence drifted"
        )
...
def _validate_audit_evidence_shape(evidence: Mapping[str, Any]) -> None:
    ...
    if drifted:
        raise P1TemporalConsumerSeed314159PatchError(
            f"E0-MO in-process audit evidence drifted: {drifted}"
        )
    identity = _auditor_identity_record()
    ...
    if (
        ...
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO canonical audit hash evidence is malformed"
        )


def validate_p1_temporal_consumer_seed_314159_patch_verification(
    verification: Mapping[str, Any],
) -> None:
    expected_fields = {
    ...
    if set(verification) != expected_fields:
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO verification fields drifted"
        )
    preflight = verification.get("schema_subset_preflight")
    if (
        not isinstance(preflight, Mapping)
        or dict(preflight)
        != preflight_p1_temporal_consumer_seed_314159_patch_schema()
    ):
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO schema-subset preflight evidence drifted"
        )
    commands = {
    ...
    for field, command in commands.items():
        ...
        if not isinstance(value, Mapping):
            raise P1TemporalConsumerSeed314159PatchError(
                f"E0-MO {field} evidence is malformed"
            )
    ...
    if (
        ...
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO focused-test evidence drifted"
        )
    ...
    if first.get("terminal_status") not in {
        ...
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO first targeted DVC push evidence drifted"
        )
    if second.get("terminal_status") != "Everything is up to date.":
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO second targeted DVC push is not idempotent"
        )
    audit = verification.get("p1_bundle_audit")
    if not isinstance(audit, Mapping):
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO in-process audit evidence is malformed"
        )
    _validate_audit_evidence_shape(audit)


def validate_p1_temporal_consumer_seed_314159_patch_lock_payload(
    ...
    *,
    ...
    try:
        validate_json_schema(
            payload,
            schema,
            instance_path="$.p1_temporal_consumer_seed_314159_patch_lock",
        )
    except ClosureContractError as exc:
        raise _translate(exc) from exc
    fixed = {
    ...
    for field, expected in fixed.items():
        if payload.get(field) != expected:
            raise P1TemporalConsumerSeed314159PatchError(
                f"E0-MO fixed field drifted: {field}"
            )
    created = payload.get("created_at_utc")
    if not isinstance(created, str):
        raise P1TemporalConsumerSeed314159PatchError("E0-MO timestamp is invalid")
    try:
        timestamp = datetime.fromisoformat(created.replace("Z", "+00:00"))
    except ValueError as exc:
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO timestamp is invalid"
        ) from exc
    if timestamp.utcoffset() is None:
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO timestamp requires a timezone"
        )
    repository = cast(Mapping[str, Any], payload["patch_repository"])
    patch_head = _require_commit(str(repository.get("head", "")), context="H-E0-MO")
    if repository != {
        "head": patch_head,
        "parent": PATCH_BASE_COMMIT,
        "branch": "main",
    ...
    _assert_paths_untouched(
        ...
        execution_head,
        PATCH_PATHS,
        context="H-E0-MO components",
    )
    expected_authorities = {
        "e0_mm": _historical_e0_mm_authority(execution_head=execution_head),
        "e0_mn": _historical_e0_mn_authority(execution_head=execution_head),
    }
    if payload.get("base_authorities") != expected_authorities:
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO historical authorities drifted"
        )
    if payload.get("p1_314159_publication") != _published_p1_314159_bundle(
        execution_head=execution_head
    ):
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO P1/314159 publication drifted"
        )
...
def _expected_companion(
    ...
    *,
    ...
    mm_authority = cast(Mapping[str, Any], authorities["e0_mm"])
    mn_authority = cast(Mapping[str, Any], authorities["e0_mn"])
    publication = cast(Mapping[str, Any], payload["p1_314159_publication"])
    ...
    inputs = [
        ...
        *[
            ...
            for record in cast(
                ...
                cast(Mapping[str, Any], mm_authority["preserved_components"])["records"],
            )
        ],
        dict(cast(Mapping[str, Any], mm_authority["lock"])),
        dict(cast(Mapping[str, Any], mm_authority["companion_manifest"])),
        *[
            dict(record)
            for record in cast(
                Sequence[Mapping[str, Any]],
                cast(Mapping[str, Any], mn_authority["preserved_components"])["records"],
            )
        ],
        dict(cast(Mapping[str, Any], mn_authority["lock"])),
        dict(cast(Mapping[str, Any], mn_authority["companion_manifest"])),
        *[
            dict(record)
    ...
    historical_inputs = [
        {
            **dict(record),
            "commit": E0_MM_H_COMMIT,
            "hash_source": "git_blob_at_commit",
        }
        for record in cast(
            Sequence[Mapping[str, Any]],
            cast(Mapping[str, Any], mm_authority["superseded_components"])["records"],
        )
    ]
    historical_inputs.sort(key=lambda record: str(record["path"]))
    return {
        "manifest_version": (
            "closure_p1_temporal_consumer_seed_314159_patch_manifest_v1"
        ),
        "status": "completed",
        "experiment_id": EXPERIMENT_ID,
        "surface_id": SURFACE_ID,
...
def _validate_publication_bundle(
    ...
    *,
    ...
    if _observed_diff_entries(patch_head, lock_commit) != expected:
        raise P1TemporalConsumerSeed314159PatchError(
            "P-E0-MO must add exactly lock plus companion"
        )
    if _git("branch", "--show-current") != "main":
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO effective authority requires branch main"
        )
    if execution_head != lock_commit:
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO execution HEAD must equal the exact P-E0-MO lock commit"
        )
    ...
    if set(refs.values()) != {lock_commit}:
        raise P1TemporalConsumerSeed314159PatchError(
            f"E0-MO local/tracking refs diverged: {refs}"
        )
    if verify_remote and _remote_main_oid() != lock_commit:
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO execution HEAD differs from live origin/main"
        )
    return (
        lock_commit,
        _git_record(
...
def load_and_validate_p1_temporal_consumer_seed_314159_patch_lock(
    ...
    *,
    ...
    if (
        ...
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO requires closed default paths"
        )
    payload = _load_regular_json(lock_path, context="E0-MO lock")
    schema = _load_regular_json(lock_schema, context="E0-MO schema")
    # Static/Git validation deliberately precedes any physical P1 audit.
    validate_p1_temporal_consumer_seed_314159_patch_lock_payload(
        payload,
        schema,
    ...
    companion = _load_regular_json(companion_path, context="E0-MO companion")
    if companion != _expected_companion(payload, lock_record=lock_record):
        raise P1TemporalConsumerSeed314159PatchError("E0-MO companion drifted")
    ...
    if require_published:
        ...
        if status:
            raise P1TemporalConsumerSeed314159PatchError(
                f"E0-MO execution requires a clean worktree: {status}"
            )
        ...
        if execution_head != patch_head:
            raise P1TemporalConsumerSeed314159PatchError(
                "Unpublished E0-MO validation must run at H-E0-MO"
            )
        validate_p1_temporal_consumer_seed_314159_patch_lock_payload(
            payload,
            schema,
    ...
    mn_lock_record = dict(cast(Mapping[str, Any], mn_authority["lock"]))
    mn_companion_record = dict(
        cast(Mapping[str, Any], mn_authority["companion_manifest"])
    )
    ...
    summary = {
        "status": (
            "published_p1_temporal_consumer_seed_314159_patch_valid"
            if effective
...
def require_p1_temporal_consumer_seed_314159_patch_authorized(
    *,
    model_id: str,
    base_seed: int,
    device: str,
) -> dict[str, Any]:
    """Require published E0-MO for exactly the P1/314159/CPU consumer."""
    if (
        model_id != AUTHORIZED_MODEL_ID
        ...
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO authorizes only the P1 seed 314159 CPU temporal consumer"
        )
    _, summary = load_and_validate_p1_temporal_consumer_seed_314159_patch_lock(
    ...
    required_true = (
        ...
        "historical_e0_mn_verified",
        "p1_1729_slot_preserved",
        "p1_20260612_slot_preserved",
        "p1_20260613_slot_preserved",
        "p1_20260614_slot_preserved",
        "p1_314159_sequence_bundle_verified",
    ...
    if failed:
        raise P1TemporalConsumerSeed314159PatchError(
            f"E0-MO authorization predicates failed: {failed}"
        )
    required_false = (
        ...
        "historical_mm_effective_loader_called",
        "historical_mn_effective_loader_called",
    ...
    if drifted:
        raise P1TemporalConsumerSeed314159PatchError(
            f"E0-MO fail-closed seals drifted: {drifted}"
        )
    if summary.get("fit_availability") != FIT_AVAILABILITY:
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO fit availability drifted"
        )
    if summary.get("schema_subset_preflight_evidence") != (
        preflight_p1_temporal_consumer_seed_314159_patch_schema()
    ):
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO schema-subset preflight summary drifted"
        )
    if summary.get("state_consumer_authority") is not None:
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO must not apply the historical seed-1729 ANFIS exception"
        )
...
__all__ = [
    ...
    "DIFF_CHECK_COMMAND",
    "P1TemporalConsumerSeed314159PatchError",
    "build_p1_temporal_consumer_seed_314159_patch_lock_payload",
    "collect_p1_temporal_consumer_seed_314159_patch_prelock_state",
    "load_and_validate_p1_temporal_consumer_seed_314159_patch_lock",
    ...
    "validate_p1_temporal_consumer_seed_314159_patch_lock_payload",
    "validate_p1_temporal_consumer_seed_314159_patch_verification",
]
