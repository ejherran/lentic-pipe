#!/usr/bin/env python
"""Check, publish, and audit the E0-MCALM post-R authority."""
...
def _error(
    ...
    return patch.FinalCalibrationR8PostPublicationAuthorityPatchError(message)
...
def _parse_focused_summary(stdout: str, stderr: str) -> dict[str, int]:
    ...
    if type(count) is not int or count <= 0:
        raise _error("E0-MCALM focused-test count is not frozen")
...
def run_final_calibration_r8_post_publication_authority_patch_verification(
    *, expected_schema_preflight: Mapping[str, Any] | None = None
) -> dict[str, Any]:
    """Run the closed repository checks without R science, DVC, or outcomes."""

    schema_preflight = (
        patch.preflight_final_calibration_r8_post_publication_authority_patch_schema()
    )
    if (
        expected_schema_preflight is not None
        and dict(expected_schema_preflight) != schema_preflight
    ):
        raise _error("E0-MCALM schema changed before verification")
...
def check_only() -> dict[str, Any]:
    ...
    prelock_before = (
        patch.collect_final_calibration_r8_post_publication_authority_patch_prelock_state(
            verify_remote=True,
    ...
    if prelock_before != prelock_after:
        raise _error(
            "E0-MCALM terminal publication or namespace changed during check-only"
        )
    if physical_before != physical_after:
        raise _error(
            "E0-MCALM physical input or immutable R8 identity changed during check-only"
        )
    return {
        ...
        "historical_published_final_count": 20,
        "never_published_final_count": 4,
        "current_lock_count": 2,
        "r8_output_count": len(patch.R_OUTPUT_PATHS),
        "coordination_forbidden_count": len(patch.COORDINATION_NAMESPACE_PATHS),
        "terminal_r_publication_revalidated": True,
        "physical_snapshot_revalidated": True,
        "prelock_revalidated": True,
        "coordination_namespace_revalidated": True,
...
def execute_lock() -> dict[str, Any]:
    ...
    physical_before = patch._physical_snapshot(PROJECT_ROOT)
    verification = run_final_calibration_r8_post_publication_authority_patch_verification(
        expected_schema_preflight=schema
    )
    physical_after = patch._physical_snapshot(PROJECT_ROOT)
    if physical_before != physical_after:
        raise _error(
            "E0-MCALM physical input or immutable R8 identity changed during verification"
        )
    after = (
        patch.collect_final_calibration_r8_post_publication_authority_patch_prelock_state(
            verify_remote=True,
    ...
    if before != after:
        raise _error(
            "E0-MCALM terminal publication, namespace, or R8 state changed during verification"
        )
    payload = patch.build_final_calibration_r8_post_publication_authority_patch_lock_payload(
        prelock=before,
        verification=verification,
    )
    patch.validate_final_calibration_r8_post_publication_authority_patch_lock_payload(
        payload,
        verify_remote=True,
        repo_root=PROJECT_ROOT,
    )
    ...
    return {
        ...
        "coordination_forbidden_count": len(patch.COORDINATION_NAMESPACE_PATHS),
        "terminal_r_publication_revalidated": True,
        "r8_physical_identity_preserved": True,
        "coordination_namespace_revalidated": True,
        "publication_assistant_run": False,
        "r8_files_touched": False,
        "r8_files_staged": False,
...
def check_effective() -> dict[str, Any]:
    return patch.load_effective_final_calibration_r8_post_publication_authority_patch_authority(
        verify_remote=True,
        repo_root=PROJECT_ROOT,
    )
