"""Adopt DVC's exact ``/models`` ignore entry under E0-MZC.

E0-MZC is an additive governance overlay over published P-E0-MZB.  It records
that all eleven R-E0-MZB ``dvc add --no-relink`` commands completed, after
which DVC's automatic ``/models`` append made the exact Git scope fail.  The
transaction restored pointers, ``models.dvc``, the index, and coordination,
but deliberately could not claim the unowned ``.gitignore`` mutation.  H-MZC
adopts that exact one-line suffix before any retry.  H/P never fit, register,
push, or access scientific outcomes; only published P-E0-MZC can authorize the
unchanged exact ten-pointer plus ``models.dvc`` R scope.
"""
...
PROJECT_ROOT = mzb.PROJECT_ROOT
H_MZB_COMMIT = "233e9d4b89f8bcc58742c0d95c51ea2ec3e5049d"
P_MZB_COMMIT = "d202dead382fb42f9388e2a280240eaf59030ced"
BASE_COMMIT = P_MZB_COMMIT
PATCH_GATE = "E0-MZC"
SCHEMA_VERSION = "closure_anfis_ablation_dvc_registration_gitignore_patch_lock_v1"
COMPANION_VERSION = (
    "closure_anfis_ablation_dvc_registration_gitignore_patch_lock_manifest_v1"
)
...
LOCKER_GUARD_PATH = Path(
    "tmp/closure_v1_e0_mzc_locker/registration_gitignore_patch.lock"
)
...
PATCH_COMPONENT_ROLES = {
    ".gitignore": "dvc_models_ignore_contract",
    "configs/closure_v1/anfis_ablation_dvc_registration_gitignore_patch.schema.json": (
        "anfis_ablation_dvc_registration_gitignore_patch_schema"
    ),
    "docs/closure_v1/ANFIS_ABLATION_DVC_REGISTRATION_GITIGNORE_PATCH.md": (
        "anfis_ablation_dvc_registration_gitignore_patch_protocol"
    ),
    ...
    "src/experiments/closure_anfis_ablation_dvc_registration_gitignore_patch.py": (
        "anfis_ablation_dvc_registration_gitignore_patch_validator"
    ),
    "src/experiments/lock_closure_anfis_ablation_dvc_registration_gitignore_patch.py": (
        "anfis_ablation_dvc_registration_gitignore_patch_locker"
    ),
    "tests/test_closure_anfis_ablation_dvc_registration_gitignore_patch.py": (
        "anfis_ablation_dvc_registration_gitignore_patch_test"
    ),
    "tests/test_closure_anfis_ablation_dvc_registration_namespace_patch.py": (
        "anfis_ablation_dvc_registration_namespace_patch_gitignore_regression_test"
    ),
...
SUPERSEDED_MZB_PATHS = tuple(
    path for path in PATCH_MODIFIED_PATHS if path in set(mzb.PATCH_PATHS)
)
ADOPTED_BASE_PATHS = (".gitignore",)
PRESERVED_MZB_PATHS = tuple(
    path for path in mzb.PATCH_PATHS if path not in set(SUPERSEDED_MZB_PATHS)
)
PATCH_COMPONENT_GIT_MODES = {
    path: "100755" if path == "src/data/prepare_commit_artifacts.py" else "100644"
    for path in PATCH_PATHS
}
ANFIS_ABLATION_H_MZC_STAGED_SCOPE = {
    path: ("A" if path in set(PATCH_ADDED_PATHS) else "M") for path in PATCH_PATHS
}
ANFIS_ABLATION_P_MZC_STAGED_SCOPE = {
    DEFAULT_PATCH_LOCK_PATH.as_posix(): "A",
    DEFAULT_PATCH_LOCK_MANIFEST_PATH.as_posix(): "A",
}
BASE_MZB_LOCK_PATH = mzb.DEFAULT_PATCH_LOCK_PATH
BASE_MZB_COMPANION_PATH = mzb.DEFAULT_PATCH_LOCK_MANIFEST_PATH
BASE_MZB_LOCK_BYTES = 49_479
BASE_MZB_LOCK_SHA256 = "47f6eeb6e09d99145b1c6ff3881df6ec6cd2667c987a333e1775a0e3392bc7d9"
BASE_MZB_COMPANION_BYTES = 12_088
BASE_MZB_COMPANION_SHA256 = "392d28d203de3db09594d53994ba170d10175128f29d08b6fd1a6e74f196283e"
GITIGNORE_PATH = Path(".gitignore")
GITIGNORE_ENTRY = "/models"
GITIGNORE_SUFFIX = b"/models\n"
P_MZB_GITIGNORE_BYTES = 6_622
P_MZB_GITIGNORE_SHA256 = (
    "6deac139b03970cb324989e4f5879a051f557d976acc5e21387177043e31faaf"
)
P_MZB_GITIGNORE_GIT_OID = "04f1ec817111eb3bc617c5392380fcaa7f449d3d"
...
OUTCOME_ACCESS_LOG = mzb.OUTCOME_ACCESS_LOG
E0_M_PATHS = mzb.E0_M_PATHS
ORDERED_SLOTS = mzb.ORDERED_SLOTS
SLOT_ROLE_ORDER = (
    "model",
    "checkpoint",
    "preprocessor",
...
EXPECTED_COMPANION_INPUT_COUNT = 19
EXPECTED_HISTORICAL_INPUT_COUNT = 26
...
LOCK_SEALS = {
    ...
    "registration_requires_published_p_mzb": True,
    "published_p_mzb_reconstructed": True,
    "historical_h_mzb_reconstructed": True,
    "blocked_r_mzb_attempt_rolled_back": True,
    "blocked_r_mzb_gitignore_residue_adopted": True,
...
class AnfisAblationDvcRegistrationGitignorePatchError(RuntimeError):
    """Raised when E0-MZC authority, artifacts, or topology drift."""


def _translate(exc: BaseException) -> AnfisAblationDvcRegistrationGitignorePatchError:
    return AnfisAblationDvcRegistrationGitignorePatchError(str(exc))
...
def _mzc_error_boundary(function: Callable[_P, _R]) -> Callable[_P, _R]:
    @wraps(function)
    def wrapped(*args: _P.args, **kwargs: _P.kwargs) -> _R:
        try:
            return function(*args, **kwargs)
        except AnfisAblationDvcRegistrationGitignorePatchError:
            raise
...
def _permissions_from_git_mode(value: str) -> int:
    if re.fullmatch(r"100[0-7]{3}", value) is None:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            f"E0-MZC unsupported regular-file Git mode: {value!r}"
        )
...
def _open_anchored_parent(path: Path, *, repo_root: Path) -> tuple[int, str]:
    if path.is_absolute() or not path.parts or any(
        ...
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            f"E0-MZC path must remain repository-relative: {path.as_posix()}"
        )
    ...
    *,
    ...
    try:
        ...
        if (
            ...
            raise AnfisAblationDvcRegistrationGitignorePatchError(
                f"E0-MZC path is not one stable regular {expected_mode:04o} file: "
                f"{path.as_posix()}"
            )
        chunks: list[bytes] = []
        while True:
        ...
        if identity_before != identity_after or identity_after != named_identity:
            raise AnfisAblationDvcRegistrationGitignorePatchError(
                f"E0-MZC path changed while read: {path.as_posix()}"
            )
        ...
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            f"Required stable E0-MZC input is unavailable: {path.as_posix()}"
        ) from exc
    finally:
...
def _git_blob_bytes(repo_root: Path, commit: str, path: Path) -> bytes:
    ...
    if result.returncode != 0 or result.stderr:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            f"E0-MZC cannot reconstruct Git blob {commit}:{path.as_posix()}"
        )
...
def _load_json(path: Path, *, repo_root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            f"E0-MZC JSON cannot be decoded: {path.as_posix()}"
        ) from exc
    if not isinstance(value, dict):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            f"E0-MZC JSON must be an object: {path.as_posix()}"
        )
    return value
...
def _validate_timestamp(value: Any) -> None:
    if not isinstance(value, str):
        raise AnfisAblationDvcRegistrationGitignorePatchError("E0-MZC timestamp is absent")
    try:
        parsed = datetime.fromisoformat(value.replace("Z", "+00:00"))
    except ValueError as exc:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC timestamp is malformed"
        ) from exc
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC timestamp must be timezone-aware"
        )
...
def _slot_paths(model_id: str, base_seed: int) -> dict[str, Path]:
    ...
    if set(paths) != set(SLOT_ROLE_ORDER):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            f"E0-MZC slot path roles drifted: {model_id}/{base_seed}"
        )
    return paths
...
def _family_physical_snapshot(repo_root: Path) -> tuple[dict[str, Any], ...]:
    ...
    if len(snapshot) != FAMILY_FINAL_COUNT:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC physical family snapshot must contain exact80 identities"
        )
...
def _require_family_physical_snapshot(
    ...
    if list(expected) != list(_family_physical_snapshot(repo_root)):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            f"E0-MZC exact80 physical family changed {context}"
        )
        ...
        sorted(
            {
                ...
                *(Path(path) for path in PRESERVED_MZB_PATHS),
                BASE_MZB_LOCK_PATH,
                BASE_MZB_COMPANION_PATH,
                DVC_CONFIG_PATH,
...
def _require_publication_state(
    ...
    *,
    ...
    if list(physical_snapshot) != list(_publication_input_snapshot(repo_root)):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            f"E0-MZC physical governance input changed {context}"
        )
    if not _exact_equal(git_snapshot, _publication_git_snapshot(repo_root)):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            f"E0-MZC Git publication state changed {context}"
        )
    _validate_family_namespace(
        registered=False,
        repo_root=repo_root,
...
def _dvc_configuration_contract(repo_root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC DVC repository/local configuration drifted"
        )
...
def _forbidden_family_namespace_paths() -> tuple[Path, ...]:
    return tuple(
        sorted(
            {
                *mzb._forbidden_family_namespace_paths(),
    ...
    *,
    ...
    for path in expected_paths:
        try:
            ...
            raise AnfisAblationDvcRegistrationGitignorePatchError(
                f"E0-MZC {context} expected path escaped its root"
            ) from exc
        if relative == Path("."):
            raise AnfisAblationDvcRegistrationGitignorePatchError(
                f"E0-MZC {context} expected file path is malformed"
            )
    ...
    def walk(descriptor: int, relative_parent: Path) -> None:
        ...
        if not stat.S_ISDIR(before.st_mode):
            raise AnfisAblationDvcRegistrationGitignorePatchError(
                f"E0-MZC {context} walk escaped its directory root"
            )
        try:
            entries = sorted(os.listdir(descriptor))
        except OSError as exc:
            raise AnfisAblationDvcRegistrationGitignorePatchError(
                f"E0-MZC cannot walk exact {context} root"
            ) from exc
        for name in entries:
            relative = (
                Path(name)
            ...
            if stat.S_ISDIR(metadata.st_mode):
                if relative not in expected_directories:
                    raise AnfisAblationDvcRegistrationGitignorePatchError(
                        f"E0-MZC {context} contains an unexpected directory: {relative}"
                    )
                ...
                try:
                    ...
                    if identity(opened) != identity(metadata):
                        raise AnfisAblationDvcRegistrationGitignorePatchError(
                            f"E0-MZC {context} directory changed while opened: {relative}"
                        )
                    ...
                    if identity(os.fstat(child)) != identity(named_after):
                        raise AnfisAblationDvcRegistrationGitignorePatchError(
                            f"E0-MZC {context} directory changed while walked: {relative}"
                        )
                finally:
                    os.close(child)
                ...
                if (
                    ...
                    raise AnfisAblationDvcRegistrationGitignorePatchError(
                        f"E0-MZC {context} contains a foreign/non-0644 file: {relative}"
                    )
                file_descriptor = os.open(name, file_flags, dir_fd=descriptor)
                try:
                    ...
                    if (
                        ...
                        raise AnfisAblationDvcRegistrationGitignorePatchError(
                            f"E0-MZC {context} file changed while inspected: {relative}"
                        )
                ...
                raise AnfisAblationDvcRegistrationGitignorePatchError(
                    f"E0-MZC {context} contains a symlink/nonregular entry: {relative}"
                )
        if identity(before) != identity(os.fstat(descriptor)):
            raise AnfisAblationDvcRegistrationGitignorePatchError(
                f"E0-MZC {context} directory changed while walked: {relative_parent}"
            )
    ...
    try:
        ...
        if identity(root_before) != identity(named_before):
            raise AnfisAblationDvcRegistrationGitignorePatchError(
                f"E0-MZC {context} root changed while opened"
            )
        ...
        if (
            ...
            raise AnfisAblationDvcRegistrationGitignorePatchError(
                f"E0-MZC {context} root changed while walked"
            )
    except AnfisAblationDvcRegistrationGitignorePatchError:
        raise
    except OSError as exc:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            f"E0-MZC cannot open exact {context} root without following links"
        ) from exc
    finally:
    ...
    if observed_files != expected_files or observed_directories != expected_directories:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            f"E0-MZC {context} tree does not contain its exact closed namespace"
        )
...
def _validate_family_namespace(
    *,
    ...
    if type(registered) is not bool or type(allow_locker_guard) is not bool:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC namespace policy must use exact booleans"
        )
    if registration_transaction is not None and not registered:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC transaction coordination is post-registration only"
        )
    ...
    if occupied:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            f"E0-MZC ignored/coordination namespace is occupied: {occupied}"
        )
...
def _base_models_dvc_bytes() -> bytes:
    ...
    if len(payload) != BASE_MODELS_DVC_BYTES or _sha256_bytes(payload) != BASE_MODELS_DVC_SHA256:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC sealed base models.dvc constants drifted"
        )
    return payload
...
def _coordination_identity_record(
    ...
    *,
    ...
    if payload != expected_payload:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            f"E0-MZC transaction coordination bytes drifted: {path.as_posix()}"
        )
    return {
        "path": path.as_posix(),
        "device": int(metadata.st_dev),
...
def _coordination_directory_record(
    ...
    try:
        ...
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            f"E0-MZC coordination directory is absent/unreadable: {path.as_posix()}"
        ) from exc
    finally:
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            f"E0-MZC coordination directory identity drifted: {path.as_posix()}"
        )
    return {
        "path": path.as_posix(),
...
def _validate_registration_transaction_record(
    ...
    if not isinstance(value, Mapping) or set(value) != {
        ...
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC private transaction record dialect drifted"
        )
    mode = value.get("mode")
    if mode not in {"in_place", "atomic_replace"}:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC private transaction mode drifted"
        )
    expected_guard = _coordination_identity_record(
        REGISTRATION_GUARD_PATH,
        expected_mode=0o600,
        expected_payload=b"E0-MZC exact ANFIS-ablation DVC registration\n",
        repo_root=repo_root,
    )
    ...
    expected_gitignore = _coordination_identity_record(
        ...
        expected_mode=0o644,
        expected_payload=(
            _git_blob_bytes(repo_root, P_MZB_COMMIT, GITIGNORE_PATH)
            + GITIGNORE_SUFFIX
        ),
        repo_root=repo_root,
    )
    if not _exact_equal(value.get("guard"), expected_guard) or not _exact_equal(
        ...
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC private transaction identity record drifted"
        )
    ...
    if not _exact_equal(
        ...
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC private DVC config-isolation record drifted"
        )
    allowed = {
        REGISTRATION_GUARD_PATH,
        REGISTRATION_MODELS_BYTES_BACKUP_PATH,
    ...
    if mode == "in_place":
        if value.get("anchor") is not None or _lexists(
            ...
            raise AnfisAblationDvcRegistrationGitignorePatchError(
                "E0-MZC in-place transaction retained a hardlink anchor"
            )
    else:
        ...
        if not _exact_equal(value.get("anchor"), expected_anchor):
            raise AnfisAblationDvcRegistrationGitignorePatchError(
                "E0-MZC atomic transaction anchor record drifted"
            )
...
def _expected_registration_records(repo_root: Path) -> dict[str, Any]:
    ...
    if len(models) != EXPECTED_MODELS_DVC_BYTES or _sha256_bytes(models) != EXPECTED_MODELS_DVC_SHA256:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC expected models.dvc dialect drifted"
        )
    return {
...
def _registration_inventory(repo_root: Path) -> dict[str, Any]:
    try:
        return mzb._registration_inventory(repo_root)
    except Exception as exc:
        raise _translate(exc) from exc
...
def _base_models_owner(repo_root: Path) -> dict[str, Any]:
    try:
        return mzb._base_models_owner(repo_root)
    except Exception as exc:
        raise _translate(exc) from exc
...
def _h_components(head: str, repo_root: Path) -> list[dict[str, Any]]:
    ...
    for record in records:
        ...
        if (
            ...
            raise AnfisAblationDvcRegistrationGitignorePatchError(
                f"E0-MZC H component differs from Git: {record['path']}"
            )
    return records


def _historical_inputs(repo_root: Path) -> list[dict[str, Any]]:
    base_mzb = _base_mzb_authority(repo_root)
    inherited = [
        dict(record)
        for record in cast(
            Sequence[Mapping[str, Any]], base_mzb["historical_inputs"]
        )
    ]
    superseded = [
        _historical_git_blob_record(
            path,
            role=f"superseded_h_mzb_{mzb.PATCH_COMPONENT_ROLES[path]}",
            commit=H_MZB_COMMIT,
            repo_root=repo_root,
        )
        for path in SUPERSEDED_MZB_PATHS
    ]
    ...
    if len(records) != EXPECTED_HISTORICAL_INPUT_COUNT or len(identities) != len(records):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC historical companion inputs must be exact26"
        )
    return records


def _historical_h_mzb_authority(repo_root: Path) -> dict[str, Any]:
    expected_scope = {
        "added": 5,
        "modified": 6,
        "deleted": 0,
        "paths": list(mzb.PATCH_PATHS),
    }
    if (
        _single_parent(repo_root, H_MZB_COMMIT, context="historical H-E0-MZB")
        != mzb.BASE_COMMIT
        or _git_scope(repo_root, mzb.BASE_COMMIT, H_MZB_COMMIT) != expected_scope
    ):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC historical H-E0-MZB topology drifted"
        )
    _require_exact_git_modes(
        repo_root,
        H_MZB_COMMIT,
        mzb.PATCH_COMPONENT_GIT_MODES,
        context="historical H-E0-MZB",
    )
    preserved: list[dict[str, Any]] = []
    for path in PRESERVED_MZB_PATHS:
        role = mzb.PATCH_COMPONENT_ROLES[path]
        current = _file_record(Path(path), role=role, repo_root=repo_root)
        historical = _historical_git_blob_record(
            path, role=role, commit=H_MZB_COMMIT, repo_root=repo_root
        )
        if current["bytes"] != historical["bytes"] or current["sha256"] != historical["sha256"]:
            raise AnfisAblationDvcRegistrationGitignorePatchError(
                f"E0-MZC preserved H-E0-MZB component drifted: {path}"
            )
    ...
    if len(preserved) != 4 or len(superseded) != 7:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC H-E0-MZB preserved/superseded partition drifted"
        )
    return {
        "gate": "E0-MZB",
        "head": H_MZB_COMMIT,
        "parent": mzb.BASE_COMMIT,
...
def _base_mzb_authority(repo_root: Path) -> dict[str, Any]:
    if (
        _single_parent(repo_root, P_MZB_COMMIT, context="P-E0-MZB") != H_MZB_COMMIT
        or _git_scope(repo_root, H_MZB_COMMIT, P_MZB_COMMIT)
        != {
            "added": 2,
            "modified": 0,
            "deleted": 0,
            "paths": sorted(
                (BASE_MZB_LOCK_PATH.as_posix(), BASE_MZB_COMPANION_PATH.as_posix())
            ),
        }
    ):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC base P-E0-MZB topology drifted"
        )
    lock = _file_record(
        BASE_MZB_LOCK_PATH,
    ...
    _require_exact_git_modes(
        ...
        P_MZB_COMMIT,
        {
            BASE_MZB_LOCK_PATH.as_posix(): "100644",
            BASE_MZB_COMPANION_PATH.as_posix(): "100644",
        },
        context="P-E0-MZB authority",
    )
    for path, record in (
        (BASE_MZB_LOCK_PATH, lock),
        (BASE_MZB_COMPANION_PATH, companion),
    ):
        git_payload = _git_blob_bytes(repo_root, P_MZB_COMMIT, path)
        if record["bytes"] != len(git_payload) or record["sha256"] != _sha256_bytes(git_payload):
            raise AnfisAblationDvcRegistrationGitignorePatchError(
                f"E0-MZC P-E0-MZB physical input differs from Git: {path.as_posix()}"
            )
    if (
        lock["bytes"] != BASE_MZB_LOCK_BYTES
        or lock["sha256"] != BASE_MZB_LOCK_SHA256
        or companion["bytes"] != BASE_MZB_COMPANION_BYTES
        or companion["sha256"] != BASE_MZB_COMPANION_SHA256
    ):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC sealed P-E0-MZB output hashes drifted"
        )
    lock_payload = _load_json(BASE_MZB_LOCK_PATH, repo_root=repo_root)
    companion_payload = _load_json(BASE_MZB_COMPANION_PATH, repo_root=repo_root)
    historical_inputs = companion_payload.get("historical_inputs")
    if (
        lock_payload.get("gate") != "E0-MZB"
        ...
        or len(historical_inputs) != mzb.EXPECTED_HISTORICAL_INPUT_COUNT
        or not _exact_equal(historical_inputs, mzb._historical_inputs(repo_root))
    ):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC P-E0-MZB semantic/history binding drifted"
        )
    return {
        "gate": "E0-MZB",
        "p_head": P_MZB_COMMIT,
...
def _gitignore_correction(repo_root: Path) -> dict[str, Any]:
    targets = _selection_payload_paths()
    pointers = _selection_pointer_paths()
    if len(targets) != 10 or len(pointers) != 10:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC sealed gitignore incident target set drifted"
        )
    base_payload = _git_blob_bytes(repo_root, P_MZB_COMMIT, GITIGNORE_PATH)
    base_oid = _git(repo_root, "rev-parse", f"{P_MZB_COMMIT}:.gitignore").strip()
    adopted_payload, adopted_metadata = _read_regular_bytes_and_metadata(
        GITIGNORE_PATH,
        repo_root=repo_root,
        require_nlink_one=True,
        expected_mode=0o644,
    ...
    adopted_git_payload = _git_blob_bytes(repo_root, current_head, GITIGNORE_PATH)
    adopted_oid = _git(repo_root, "rev-parse", f"{current_head}:.gitignore").strip()
    if (
        len(base_payload) != P_MZB_GITIGNORE_BYTES
        or _sha256_bytes(base_payload) != P_MZB_GITIGNORE_SHA256
        or base_oid != P_MZB_GITIGNORE_GIT_OID
        or len(adopted_payload) != ADOPTED_GITIGNORE_BYTES
        or _sha256_bytes(adopted_payload) != ADOPTED_GITIGNORE_SHA256
        or adopted_payload != base_payload + GITIGNORE_SUFFIX
        or adopted_payload.count(GITIGNORE_SUFFIX) != 1
        or adopted_git_payload != adopted_payload
        or adopted_oid != ADOPTED_GITIGNORE_GIT_OID
        or stat.S_IMODE(adopted_metadata.st_mode) != 0o644
        or adopted_metadata.st_nlink != 1
    ):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC exact .gitignore adoption/Git binding drifted"
        )
    base_record = _historical_git_blob_record(
        GITIGNORE_PATH.as_posix(),
        role="p_mzb_gitignore_before_dvc_models_append",
        commit=P_MZB_COMMIT,
        repo_root=repo_root,
    )
    # Bind the portable record to the same anchored FD read whose metadata was
    # validated above.  Reopening the pathname here would admit an incoherent
    # bytes/identity pair under an atomic replacement race.
    adopted_record = {
        "role": "adopted_dvc_models_gitignore_contract",
        "path": GITIGNORE_PATH.as_posix(),
        "bytes": len(adopted_payload),
        "sha256": _sha256_bytes(adopted_payload),
    }
    return {
        ...
        "rollback_error": "E0-MZB registration rollback could not be completed safely",
        "residual_git_status": [" M .gitignore"],
        "base_gitignore": base_record,
        "adopted_gitignore": adopted_record,
        "base_git_oid": P_MZB_GITIGNORE_GIT_OID,
        "adopted_git_oid": ADOPTED_GITIGNORE_GIT_OID,
        "dvc_ignore_entry": GITIGNORE_ENTRY,
        "dvc_ignore_suffix": GITIGNORE_SUFFIX.decode("ascii"),
...
def _companion_physical_inputs(
    *,
    ...
    base_mzb: Mapping[str, Any],
    historical_h_mzb: Mapping[str, Any],
) -> list[dict[str, Any]]:
    records = [
        dict(cast(Mapping[str, Any], base_mzb["lock"])),
        dict(cast(Mapping[str, Any], base_mzb["companion"])),
        *(
            dict(record)
            for record in cast(
                Sequence[Mapping[str, Any]], historical_h_mzb["preserved_components"]
            )
        ),
    ...
    if len(records) != EXPECTED_COMPANION_INPUT_COUNT or len(identities) != len(records):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC companion physical inputs must be exact19"
        )
    return records
...
def _expected_untracked_light_status(repo_root: Path) -> list[str]:
    ...
    if tracked != expected_tracked or len(tracked) != FAMILY_TRACKED_LIGHT_COUNT:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC all fifty light outputs must remain tracked"
        )
    return []
...
@_mzc_error_boundary
def preflight_anfis_ablation_dvc_registration_gitignore_patch_schema(
    *, repo_root: Path | None = None
    ...
    if unsupported:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            f"E0-MZC schema exceeds the supported subset: {unsupported}"
        )
    ...
    try:
        ...
        raise AnfisAblationDvcRegistrationGitignorePatchError(str(exc)) from exc
...
@_mzc_error_boundary
def collect_anfis_ablation_dvc_registration_gitignore_patch_prelock_state(
    *,
    ...
    if type(verify_remote) is not bool or type(_allow_locker_guard) is not bool:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC prelock policies must be exact booleans"
        )
    root = _root(repo_root)
    preflight_anfis_ablation_dvc_registration_gitignore_patch_schema(repo_root=root)
    head = _git_head(root)
    expected_scope = {
        "added": 5,
        "modified": 8,
    ...
    if (
        _single_parent(root, head, context="H-E0-MZC") != BASE_COMMIT
        or _git_scope(root, BASE_COMMIT, head) != expected_scope
    ):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "H-E0-MZC must be the exact 8M+5A child of published P-E0-MZB"
        )
    _require_exact_git_modes(
        root, head, PATCH_COMPONENT_GIT_MODES, context="H-E0-MZC"
    )
    ...
    if branch != "main" or tracking != head or remote != head:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "H-E0-MZC refs are not aligned with main"
        )
    status = [
        line
    ...
    if status != expected_status:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            f"E0-MZC prelock worktree must be clean with all fifty lights tracked: {status}"
        )
    ...
    if occupied_outputs:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            f"E0-MZC prelock output namespace is occupied: {occupied_outputs}"
        )
    ...
    return {
        ...
        "base_mzb_authority": base_mzb,
        "historical_h_mzb": historical_h_mzb,
        "gitignore_correction": gitignore,
...
@_mzc_error_boundary
def build_anfis_ablation_dvc_registration_gitignore_patch_lock_payload(
    ...
    *,
    ...
    if set(prelock) != required:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC prelock bundle dialect drifted"
        )
    return {
...
def _validate_command_evidence(
    ...
    *,
    ...
    if not isinstance(value, Mapping) or set(value) != keys:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            f"E0-MZC {context} evidence dialect drifted"
        )
    if (
        ...
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            f"E0-MZC {context} command/result drifted"
        )
    if (
        ...
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            f"E0-MZC {context} digest/line evidence drifted"
        )
    if exact_stdout is not None and (
        ...
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            f"E0-MZC {context} stdout evidence drifted"
        )
...
def _validate_verification(value: Any, *, repo_root: Path) -> None:
    if not isinstance(value, Mapping):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC verification evidence is absent"
        )
    required = {
        "schema_preflight",
        "type_check",
    ...
    if set(value) != required:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC verification evidence key set drifted"
        )
    expected_schema = preflight_anfis_ablation_dvc_registration_gitignore_patch_schema(
        repo_root=repo_root
    )
    if not _exact_equal(value.get("schema_preflight"), expected_schema):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC schema-preflight evidence drifted"
        )
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC focused pytest evidence drifted"
        )
    ...
    if not isinstance(stdout_text, str):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC focused pytest stdout text is absent"
        )
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC focused pytest stdout binding drifted"
        )
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC family semantic audit evidence drifted"
        )
    ...
    if not _exact_equal(boundaries, expected_boundaries):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC verification execution boundaries drifted"
        )


def _validate_anfis_ablation_dvc_registration_gitignore_patch_lock_payload(
    ...
    *,
    ...
    if type(allow_registered_state) is not bool:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC registered-state policy must be an exact boolean"
        )
    if _registration_transaction is not None and not allow_registered_state:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC private transaction record requires registered-state audit"
        )
    root = _root(repo_root)
    schema = _load_json(DEFAULT_PATCH_LOCK_SCHEMA, repo_root=root)
    try:
    ...
    if not _exact_equal(h_patch, expected_h):
        raise AnfisAblationDvcRegistrationGitignorePatchError("E0-MZC H binding drifted")
    base_mzb = _base_mzb_authority(root)
    if not _exact_equal(payload.get("base_mzb_authority"), base_mzb):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC published P-E0-MZB reconstruction drifted"
        )
    historical_h_mzb = _historical_h_mzb_authority(root)
    if not _exact_equal(payload.get("historical_h_mzb"), historical_h_mzb):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC historical H-E0-MZB reconstruction drifted"
        )
    gitignore = _gitignore_correction(root)
    if not _exact_equal(payload.get("gitignore_correction"), gitignore):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC blocked R-E0-MZB gitignore incident binding drifted"
        )
    inventory = _registration_inventory(root)
    if not _exact_equal(payload.get("artifact_inventory"), inventory):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC registration inventory binding drifted"
        )
    family = _family_records(
        root,
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC completed family binding drifted"
        )
    models_before = (
    ...
    if not _exact_equal(payload.get("models_owner_transition"), expected_transition):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC models.dvc transition drifted"
        )
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC exact registration plan drifted"
        )
    ...
    if not _exact_equal(payload.get("companion_contract"), expected_contract):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC companion contract drifted"
        )
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC prelock evidence drifted"
        )
    _validate_verification(payload.get("verification"), repo_root=root)


@_mzc_error_boundary
def validate_anfis_ablation_dvc_registration_gitignore_patch_lock_payload(
    payload: Mapping[str, Any],
    *,
    allow_registered_state: bool = False,
    repo_root: Path | None = None,
...
def _expected_companion(
    ...
    *,
    ...
    root = _root(repo_root)
    h_patch = payload.get("h_patch")
    base_mzb = payload.get("base_mzb_authority")
    historical_h_mzb = payload.get("historical_h_mzb")
    if (
        not isinstance(h_patch, Mapping)
        or not isinstance(base_mzb, Mapping)
        or not isinstance(historical_h_mzb, Mapping)
    ):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "Cannot construct E0-MZC companion"
        )
    components = h_patch.get("components")
    if not isinstance(components, list):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "Cannot construct E0-MZC companion component list"
        )
    ...
    if script is None:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC companion script is absent"
        )
    output = dict(lock_record)
    if (
        output.get("path") != DEFAULT_PATCH_LOCK_PATH.as_posix()
        or output.get("role") != "anfis_ablation_dvc_registration_gitignore_patch_lock"
    ):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC companion output record drifted"
        )
    return {
...
def _run_command(
    ...
    *,
    ...
    if result.returncode != 0:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            f"E0-MZC verification command failed: {list(command)}"
        )
...
def _parse_focused_summary(stdout: str, stderr: str) -> dict[str, int]:
    if stderr or FORBIDDEN_FOCUSED_SUMMARY_RE.search(stdout):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC focused pytest result is not one clean pass"
        )
    ...
    if len(matched) != 1 or lines[-1] != matched[0].group(0):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC focused pytest summary is not unique and terminal"
        )
    return {
...
@_mzc_error_boundary
def run_anfis_ablation_dvc_registration_gitignore_patch_verification(
    *,
    ...
    schema = preflight_anfis_ablation_dvc_registration_gitignore_patch_schema(repo_root=root)
    _require_family_physical_snapshot(
...
) -> AnfisAblationDvcRegistrationGitignorePatchError | None:
    first_error: AnfisAblationDvcRegistrationGitignorePatchError | None = None
    for output in reversed(published):
        try:
            mt._rollback_owned_output(output)
        except Exception as exc:
            if first_error is None:
                first_error = AnfisAblationDvcRegistrationGitignorePatchError(
                    "E0-MZC owned-output rollback failed for "
...
def _close_published_outputs_best_effort(
    ...
    for output in published:
        try:
            ...
            if first_error is None:
                first_error = AnfisAblationDvcRegistrationGitignorePatchError(
                    "E0-MZC owned-output descriptor close failed for "
                    f"{output.path.as_posix()}: {exc}"
                )
    ...
    *,
    ...
    try:
        ...
        if (
            ...
            raise AnfisAblationDvcRegistrationGitignorePatchError(
                f"E0-MZC owned output identity/metadata drifted {context}: "
        ...
        if (
            ...
            raise AnfisAblationDvcRegistrationGitignorePatchError(
                f"E0-MZC owned output bytes drifted {context}: "
                f"{output.path.as_posix()}"
            )
        mt._validate_owned_output(output)
    except AnfisAblationDvcRegistrationGitignorePatchError:
...
@_mzc_error_boundary
def execute_and_publish_anfis_ablation_dvc_registration_gitignore_patch_lock_bundle(
    *, repo_root: Path | None = None
    ...
    before = collect_anfis_ablation_dvc_registration_gitignore_patch_prelock_state(
        verify_remote=True, repo_root=root
    )
    verification = run_anfis_ablation_dvc_registration_gitignore_patch_verification(
        family_snapshot=family_snapshot,
        publication_input_snapshot=input_snapshot,
        publication_git_snapshot=git_snapshot,
        repo_root=root,
    )
    after = collect_anfis_ablation_dvc_registration_gitignore_patch_prelock_state(
        verify_remote=True, repo_root=root
    )
    if not _exact_equal(before, after):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC prelock state changed during verification"
        )
    _require_family_physical_snapshot(
    ...
    payload = build_anfis_ablation_dvc_registration_gitignore_patch_lock_payload(
        before, verification, created_at_utc=datetime.now(timezone.utc).isoformat()
    )
    validate_anfis_ablation_dvc_registration_gitignore_patch_lock_payload(payload, repo_root=root)
    controlled = (
    ...
    if occupied:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            f"E0-MZC lock namespace is occupied: {occupied}"
        )
    ...
    try:
        guard = mt._acquire_publication_guard(
            LOCKER_GUARD_PATH,
            b"E0-MZC lock bundle publication in progress\n",
            repo_root=root,
        )
        _require_family_physical_snapshot(
        ...
        if not _exact_equal(
            ...
            raise AnfisAblationDvcRegistrationGitignorePatchError(
                "E0-MZC guarded prelock state drifted"
            )
        ...
        if isinstance(exc, AnfisAblationDvcRegistrationGitignorePatchError):
            translated = exc
...
@_mzc_error_boundary
def publish_anfis_ablation_dvc_registration_gitignore_patch_lock_bundle(
    *, repo_root: Path | None = None
) -> tuple[dict[str, Any], dict[str, Any]]:
    return execute_and_publish_anfis_ablation_dvc_registration_gitignore_patch_lock_bundle(
        repo_root=repo_root
    )
...
def _validate_p_publication(
    ...
    if not isinstance(repository, Mapping) or not isinstance(repository.get("head"), str):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC H repository binding is absent"
        )
    h_head = str(repository["head"])
    head = _git_head(repo_root)
    tracking = _git_head(repo_root, "origin/main")
    ...
    if (
        ...
        or _single_parent(repo_root, head, context="P-E0-MZC") != h_head
        or tracking != head
        or remote != head
        ...
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "Published P-E0-MZC topology/refs drifted"
        )
    _require_exact_git_modes(
        repo_root,
        head,
    ...
    for raw_path in expected_paths:
        ...
        if physical != _git_blob_bytes(repo_root, head, path):
            raise AnfisAblationDvcRegistrationGitignorePatchError(
                f"Published P-E0-MZC physical bytes differ from Git: {raw_path}"
            )
...
def _validate_registered_state(
    ...
    for record in pointers:
        ...
        if len(payload) != record["bytes"] or _sha256_bytes(payload) != record["sha256"]:
            raise AnfisAblationDvcRegistrationGitignorePatchError(
                f"E0-MZC registered pointer drifted: {path.as_posix()}"
            )
    ...
    if len(payload) != models["bytes"] or _sha256_bytes(payload) != models["sha256"]:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC registered models.dvc drifted"
        )
    return {
...
def _load_effective_anfis_ablation_dvc_registration_gitignore_patch_authority(
    *,
    ...
    if type(audit_current_unpublished) is not bool or verify_remote is not True:
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC effective authority requires exact audit mode and live remote"
        )
    root = _root(repo_root)
    ...
    try:
        ...
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "Published E0-MZC lock is not valid UTF-8 JSON"
        ) from exc
    if not isinstance(payload, dict) or payload_bytes != _canonical_json(payload):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "Published E0-MZC lock is not canonical JSON"
        )
    ...
    try:
        ...
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "Published E0-MZC companion is not valid UTF-8 JSON"
        ) from exc
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "Published E0-MZC companion drifted"
        )
    ...
    return {
        ...
        **publication,
        ...
        "gitignore_correction": dict(
            cast(Mapping[str, Any], payload["gitignore_correction"])
        ),
...
@_mzc_error_boundary
def load_effective_anfis_ablation_dvc_registration_gitignore_patch_authority(
    *,
    audit_current_unpublished: bool = False,
    verify_remote: bool = True,
    repo_root: Path | None = None,
    ...
    return _load_effective_anfis_ablation_dvc_registration_gitignore_patch_authority(
        audit_current_unpublished=audit_current_unpublished,
...
@_mzc_error_boundary
def _load_effective_anfis_ablation_dvc_registration_gitignore_patch_during_registration(
    *,
    transaction_record: Mapping[str, Any],
    verify_remote: bool = True,
    repo_root: Path | None = None,
) -> dict[str, Any]:
    """Internal R-E0-MZC audit while durable rollback coordination is owned."""
    if not isinstance(transaction_record, Mapping):
        raise AnfisAblationDvcRegistrationGitignorePatchError(
            "E0-MZC internal transaction record is absent"
        )
    return _load_effective_anfis_ablation_dvc_registration_gitignore_patch_authority(
...
@_mzc_error_boundary
def require_anfis_ablation_dvc_registration_gitignore_patch_authority(
    *,
    audit_current_unpublished: bool = False,
    verify_remote: bool = True,
    repo_root: Path | None = None,
) -> dict[str, Any]:
    return load_effective_anfis_ablation_dvc_registration_gitignore_patch_authority(
        audit_current_unpublished=audit_current_unpublished,
        verify_remote=verify_remote,
        repo_root=repo_root,
    )
...
def require_anfis_ablation_dvc_registration_authority(
    *, verify_remote: bool = True, repo_root: Path | None = None
    ...
    return require_anfis_ablation_dvc_registration_gitignore_patch_authority(
...
@_mzc_error_boundary
def load_effective_anfis_ablation_dvc_registration_gitignore_patch(
    *,
    audit_current_unpublished: bool = False,
    verify_remote: bool = True,
    repo_root: Path | None = None,
...
__all__ = [
    "AnfisAblationDvcRegistrationGitignorePatchError",
    "ANFIS_ABLATION_H_MZC_STAGED_SCOPE",
    "ANFIS_ABLATION_P_MZC_STAGED_SCOPE",
    "ANFIS_ABLATION_R_MZC_STAGED_SCOPE",
    "BASE_COMMIT",
    ...
    "GITIGNORE_ENTRY",
    "GITIGNORE_SUFFIX",
    "P_MZB_GITIGNORE_BYTES",
    "P_MZB_GITIGNORE_SHA256",
    "P_MZB_GITIGNORE_GIT_OID",
    ...
    "collect_anfis_ablation_dvc_registration_gitignore_patch_prelock_state",
    "execute_and_publish_anfis_ablation_dvc_registration_gitignore_patch_lock_bundle",
    "load_effective_anfis_ablation_dvc_registration_gitignore_patch_authority",
    "load_effective_anfis_ablation_dvc_registration_gitignore_patch",
    ...
    "require_anfis_ablation_dvc_registration_gitignore_patch_authority",
