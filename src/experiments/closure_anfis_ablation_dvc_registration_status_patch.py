"""Adopt Git porcelain ordering without weakening the exact R scope.

E0-MZD is an additive governance overlay over published P-E0-MZC.  All eleven
R-E0-MZC ``dvc add --no-relink`` commands completed, but the final progress
check compared raw porcelain lines in lexical-path order while Git emitted the
tracked ``models.dvc`` modification before the ten untracked pointer paths.
The exact path-to-status map matched and the transaction rolled back cleanly.
MZD makes only raw line order non-authoritative; path set, status codes,
canonical command order, ownership, identities, and namespace remain exact.
"""
...
from src.experiments import (
    closure_anfis_ablation_dvc_registration_gitignore_patch as mzc,
)
...
PROJECT_ROOT = mzc.PROJECT_ROOT
H_MZC_COMMIT = "15478da7ecc8ae1baf064744b57d984cbaad6a28"
P_MZC_COMMIT = "3e1d319cb909c89eabdc1a1429294cef12ac481c"
BASE_COMMIT = P_MZC_COMMIT
PATCH_GATE = "E0-MZD"
SCHEMA_VERSION = "closure_anfis_ablation_dvc_registration_status_patch_lock_v1"
COMPANION_VERSION = (
    "closure_anfis_ablation_dvc_registration_status_patch_lock_manifest_v1"
)
...
LOCKER_GUARD_PATH = Path(
    "tmp/closure_v1_e0_mzd_locker/registration_status_patch.lock"
)
...
PATCH_COMPONENT_ROLES = {
    "configs/closure_v1/anfis_ablation_dvc_registration_status_patch.schema.json": (
        "anfis_ablation_dvc_registration_status_patch_schema"
    ),
    "docs/closure_v1/ANFIS_ABLATION_DVC_REGISTRATION_STATUS_PATCH.md": (
        "anfis_ablation_dvc_registration_status_patch_protocol"
    ),
    ...
    "src/experiments/closure_anfis_ablation_dvc_registration_status_patch.py": (
        "anfis_ablation_dvc_registration_status_patch_validator"
    ),
    "src/experiments/lock_closure_anfis_ablation_dvc_registration_status_patch.py": (
        "anfis_ablation_dvc_registration_status_patch_locker"
    ),
    "tests/test_closure_anfis_ablation_dvc_registration_status_patch.py": (
        "anfis_ablation_dvc_registration_status_patch_test"
    ),
    "tests/test_closure_anfis_ablation_dvc_registration_gitignore_patch.py": (
        "anfis_ablation_dvc_registration_gitignore_patch_status_regression_test"
    ),
...
SUPERSEDED_MZC_PATHS = tuple(
    path for path in PATCH_MODIFIED_PATHS if path in set(mzc.PATCH_PATHS)
)
PRESERVED_MZC_PATHS = tuple(
    path for path in mzc.PATCH_PATHS if path not in set(SUPERSEDED_MZC_PATHS)
)
PATCH_COMPONENT_GIT_MODES = {
    path: "100755" if path == "src/data/prepare_commit_artifacts.py" else "100644"
    for path in PATCH_PATHS
}
ANFIS_ABLATION_H_MZD_STAGED_SCOPE = {
    path: ("A" if path in set(PATCH_ADDED_PATHS) else "M") for path in PATCH_PATHS
}
ANFIS_ABLATION_P_MZD_STAGED_SCOPE = {
    DEFAULT_PATCH_LOCK_PATH.as_posix(): "A",
    DEFAULT_PATCH_LOCK_MANIFEST_PATH.as_posix(): "A",
}
BASE_MZC_LOCK_PATH = mzc.DEFAULT_PATCH_LOCK_PATH
BASE_MZC_COMPANION_PATH = mzc.DEFAULT_PATCH_LOCK_MANIFEST_PATH
BASE_MZC_LOCK_BYTES = 54_547
BASE_MZC_LOCK_SHA256 = "57b1dd042e23179aff6700d2eba863acf2bbc0f001459e85782b36f8055a46c6"
BASE_MZC_COMPANION_BYTES = 15_190
BASE_MZC_COMPANION_SHA256 = "d82c67d85082009c462a1883b87ed742a7978fbf3c8a62de52e88346e98ad277"
GITIGNORE_PATH = Path(".gitignore")
GITIGNORE_ENTRY = "/models"
GITIGNORE_SUFFIX = b"/models\n"
P_MZC_GITIGNORE_BYTES = 6_630
P_MZC_GITIGNORE_SHA256 = (
    "406c174a073b9b41d610e1c434e94f4ab37b601dedd02b61cb8542bcc0eb7f52"
)
P_MZC_GITIGNORE_GIT_OID = "8a9ff4adac268b770f93ab7333beaf3029745429"
...
OUTCOME_ACCESS_LOG = mzc.OUTCOME_ACCESS_LOG
E0_M_PATHS = mzc.E0_M_PATHS
ORDERED_SLOTS = mzc.ORDERED_SLOTS
SLOT_ROLE_ORDER = (
    "model",
    "checkpoint",
    "preprocessor",
...
EXPECTED_HISTORICAL_INPUT_COUNT = 34
...
LOCK_SEALS = {
    ...
    "registration_requires_published_p_mzc": True,
    "published_p_mzc_reconstructed": True,
    "historical_h_mzc_reconstructed": True,
    "blocked_r_mzc_attempt_rolled_back": True,
    "blocked_r_mzc_porcelain_order_failure_adopted": True,
...
class AnfisAblationDvcRegistrationStatusPatchError(RuntimeError):
    """Raised when E0-MZD authority, artifacts, or topology drift."""


def _translate(exc: BaseException) -> AnfisAblationDvcRegistrationStatusPatchError:
    return AnfisAblationDvcRegistrationStatusPatchError(str(exc))
...
def _mzd_error_boundary(function: Callable[_P, _R]) -> Callable[_P, _R]:
    @wraps(function)
    def wrapped(*args: _P.args, **kwargs: _P.kwargs) -> _R:
        try:
            return function(*args, **kwargs)
        except AnfisAblationDvcRegistrationStatusPatchError:
            raise
...
def _permissions_from_git_mode(value: str) -> int:
    if re.fullmatch(r"100[0-7]{3}", value) is None:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            f"E0-MZD unsupported regular-file Git mode: {value!r}"
        )
...
def _open_anchored_parent(path: Path, *, repo_root: Path) -> tuple[int, str]:
    if path.is_absolute() or not path.parts or any(
        ...
        raise AnfisAblationDvcRegistrationStatusPatchError(
            f"E0-MZD path must remain repository-relative: {path.as_posix()}"
        )
    ...
    *,
    ...
    try:
        ...
        if (
            ...
            raise AnfisAblationDvcRegistrationStatusPatchError(
                f"E0-MZD path is not one stable regular {expected_mode:04o} file: "
                f"{path.as_posix()}"
            )
        chunks: list[bytes] = []
        while True:
        ...
        if identity_before != identity_after or identity_after != named_identity:
            raise AnfisAblationDvcRegistrationStatusPatchError(
                f"E0-MZD path changed while read: {path.as_posix()}"
            )
        ...
        raise AnfisAblationDvcRegistrationStatusPatchError(
            f"Required stable E0-MZD input is unavailable: {path.as_posix()}"
        ) from exc
    finally:
...
def _git_blob_bytes(repo_root: Path, commit: str, path: Path) -> bytes:
    ...
    if result.returncode != 0 or result.stderr:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            f"E0-MZD cannot reconstruct Git blob {commit}:{path.as_posix()}"
        )
...
def _load_json(path: Path, *, repo_root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise AnfisAblationDvcRegistrationStatusPatchError(
            f"E0-MZD JSON cannot be decoded: {path.as_posix()}"
        ) from exc
    if not isinstance(value, dict):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            f"E0-MZD JSON must be an object: {path.as_posix()}"
        )
    return value
...
def _validate_timestamp(value: Any) -> None:
    if not isinstance(value, str):
        raise AnfisAblationDvcRegistrationStatusPatchError("E0-MZD timestamp is absent")
    try:
        parsed = datetime.fromisoformat(value.replace("Z", "+00:00"))
    except ValueError as exc:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD timestamp is malformed"
        ) from exc
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD timestamp must be timezone-aware"
        )
...
def _slot_paths(model_id: str, base_seed: int) -> dict[str, Path]:
    ...
    if set(paths) != set(SLOT_ROLE_ORDER):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            f"E0-MZD slot path roles drifted: {model_id}/{base_seed}"
        )
    return paths
...
def _family_physical_snapshot(repo_root: Path) -> tuple[dict[str, Any], ...]:
    ...
    if len(snapshot) != FAMILY_FINAL_COUNT:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD physical family snapshot must contain exact80 identities"
        )
...
def _require_family_physical_snapshot(
    ...
    if list(expected) != list(_family_physical_snapshot(repo_root)):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            f"E0-MZD exact80 physical family changed {context}"
        )
        ...
        sorted(
            {
                ...
                *(Path(path) for path in PRESERVED_MZC_PATHS),
                BASE_MZC_LOCK_PATH,
                BASE_MZC_COMPANION_PATH,
                DVC_CONFIG_PATH,
...
def _require_publication_state(
    ...
    *,
    ...
    if list(physical_snapshot) != list(_publication_input_snapshot(repo_root)):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            f"E0-MZD physical governance input changed {context}"
        )
    if not _exact_equal(git_snapshot, _publication_git_snapshot(repo_root)):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            f"E0-MZD Git publication state changed {context}"
        )
    _validate_family_namespace(
        registered=False,
        repo_root=repo_root,
...
def _dvc_configuration_contract(repo_root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD DVC repository/local configuration drifted"
        )
...
def _forbidden_family_namespace_paths() -> tuple[Path, ...]:
    return tuple(
        sorted(
            {
                *mzc._forbidden_family_namespace_paths(),
    ...
    *,
    ...
    for path in expected_paths:
        try:
            ...
            raise AnfisAblationDvcRegistrationStatusPatchError(
                f"E0-MZD {context} expected path escaped its root"
            ) from exc
        if relative == Path("."):
            raise AnfisAblationDvcRegistrationStatusPatchError(
                f"E0-MZD {context} expected file path is malformed"
            )
    ...
    def walk(descriptor: int, relative_parent: Path) -> None:
        ...
        if not stat.S_ISDIR(before.st_mode):
            raise AnfisAblationDvcRegistrationStatusPatchError(
                f"E0-MZD {context} walk escaped its directory root"
            )
        try:
            entries = sorted(os.listdir(descriptor))
        except OSError as exc:
            raise AnfisAblationDvcRegistrationStatusPatchError(
                f"E0-MZD cannot walk exact {context} root"
            ) from exc
        for name in entries:
            relative = (
                Path(name)
            ...
            if stat.S_ISDIR(metadata.st_mode):
                if relative not in expected_directories:
                    raise AnfisAblationDvcRegistrationStatusPatchError(
                        f"E0-MZD {context} contains an unexpected directory: {relative}"
                    )
                ...
                try:
                    ...
                    if identity(opened) != identity(metadata):
                        raise AnfisAblationDvcRegistrationStatusPatchError(
                            f"E0-MZD {context} directory changed while opened: {relative}"
                        )
                    ...
                    if identity(os.fstat(child)) != identity(named_after):
                        raise AnfisAblationDvcRegistrationStatusPatchError(
                            f"E0-MZD {context} directory changed while walked: {relative}"
                        )
                finally:
                    os.close(child)
                ...
                if (
                    ...
                    raise AnfisAblationDvcRegistrationStatusPatchError(
                        f"E0-MZD {context} contains a foreign/non-0644 file: {relative}"
                    )
                file_descriptor = os.open(name, file_flags, dir_fd=descriptor)
                try:
                    ...
                    if (
                        ...
                        raise AnfisAblationDvcRegistrationStatusPatchError(
                            f"E0-MZD {context} file changed while inspected: {relative}"
                        )
                ...
                raise AnfisAblationDvcRegistrationStatusPatchError(
                    f"E0-MZD {context} contains a symlink/nonregular entry: {relative}"
                )
        if identity(before) != identity(os.fstat(descriptor)):
            raise AnfisAblationDvcRegistrationStatusPatchError(
                f"E0-MZD {context} directory changed while walked: {relative_parent}"
            )
    ...
    try:
        ...
        if identity(root_before) != identity(named_before):
            raise AnfisAblationDvcRegistrationStatusPatchError(
                f"E0-MZD {context} root changed while opened"
            )
        ...
        if (
            ...
            raise AnfisAblationDvcRegistrationStatusPatchError(
                f"E0-MZD {context} root changed while walked"
            )
    except AnfisAblationDvcRegistrationStatusPatchError:
        raise
    except OSError as exc:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            f"E0-MZD cannot open exact {context} root without following links"
        ) from exc
    finally:
    ...
    if observed_files != expected_files or observed_directories != expected_directories:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            f"E0-MZD {context} tree does not contain its exact closed namespace"
        )
...
def _validate_family_namespace(
    *,
    ...
    if type(registered) is not bool or type(allow_locker_guard) is not bool:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD namespace policy must use exact booleans"
        )
    if registration_transaction is not None and not registered:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD transaction coordination is post-registration only"
        )
    ...
    if occupied:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            f"E0-MZD ignored/coordination namespace is occupied: {occupied}"
        )
...
def _base_models_dvc_bytes() -> bytes:
    ...
    if len(payload) != BASE_MODELS_DVC_BYTES or _sha256_bytes(payload) != BASE_MODELS_DVC_SHA256:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD sealed base models.dvc constants drifted"
        )
    return payload
...
def _coordination_identity_record(
    ...
    *,
    ...
    if payload != expected_payload:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            f"E0-MZD transaction coordination bytes drifted: {path.as_posix()}"
        )
    return {
        "path": path.as_posix(),
        "device": int(metadata.st_dev),
...
def _coordination_directory_record(
    ...
    try:
        ...
        raise AnfisAblationDvcRegistrationStatusPatchError(
            f"E0-MZD coordination directory is absent/unreadable: {path.as_posix()}"
        ) from exc
    finally:
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationStatusPatchError(
            f"E0-MZD coordination directory identity drifted: {path.as_posix()}"
        )
    return {
        "path": path.as_posix(),
...
def _validate_registration_transaction_record(
    ...
    if not isinstance(value, Mapping) or set(value) != {
        ...
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD private transaction record dialect drifted"
        )
    mode = value.get("mode")
    if mode not in {"in_place", "atomic_replace"}:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD private transaction mode drifted"
        )
    expected_guard = _coordination_identity_record(
        REGISTRATION_GUARD_PATH,
        expected_mode=0o600,
        expected_payload=b"E0-MZD exact ANFIS-ablation DVC registration\n",
        repo_root=repo_root,
    )
    ...
    expected_gitignore = _coordination_identity_record(
        GITIGNORE_PATH,
        expected_mode=0o644,
        expected_payload=_git_blob_bytes(repo_root, P_MZC_COMMIT, GITIGNORE_PATH),
        repo_root=repo_root,
    )
    if not _exact_equal(value.get("guard"), expected_guard) or not _exact_equal(
        ...
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD private transaction identity record drifted"
        )
    ...
    if not _exact_equal(
        ...
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD private DVC config-isolation record drifted"
        )
    allowed = {
        REGISTRATION_GUARD_PATH,
        REGISTRATION_MODELS_BYTES_BACKUP_PATH,
    ...
    if mode == "in_place":
        if value.get("anchor") is not None or _lexists(
            ...
            raise AnfisAblationDvcRegistrationStatusPatchError(
                "E0-MZD in-place transaction retained a hardlink anchor"
            )
    else:
        ...
        if not _exact_equal(value.get("anchor"), expected_anchor):
            raise AnfisAblationDvcRegistrationStatusPatchError(
                "E0-MZD atomic transaction anchor record drifted"
            )
...
def _expected_registration_records(repo_root: Path) -> dict[str, Any]:
    ...
    if len(models) != EXPECTED_MODELS_DVC_BYTES or _sha256_bytes(models) != EXPECTED_MODELS_DVC_SHA256:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD expected models.dvc dialect drifted"
        )
    return {
...
def _registration_inventory(repo_root: Path) -> dict[str, Any]:
    try:
        return mzc._registration_inventory(repo_root)
    except Exception as exc:
        raise _translate(exc) from exc
...
def _base_models_owner(repo_root: Path) -> dict[str, Any]:
    try:
        return mzc._base_models_owner(repo_root)
    except Exception as exc:
        raise _translate(exc) from exc
...
def _h_components(head: str, repo_root: Path) -> list[dict[str, Any]]:
    ...
    for record in records:
        ...
        if (
            ...
            raise AnfisAblationDvcRegistrationStatusPatchError(
                f"E0-MZD H component differs from Git: {record['path']}"
            )
    return records


def _historical_inputs(repo_root: Path) -> list[dict[str, Any]]:
    base_mzc = _base_mzc_authority(repo_root)
    inherited = [
        dict(record)
        for record in cast(
            Sequence[Mapping[str, Any]], base_mzc["historical_inputs"]
        )
    ]
    superseded = [
        _historical_git_blob_record(
            path,
            role=f"superseded_h_mzc_{mzc.PATCH_COMPONENT_ROLES[path]}",
            commit=H_MZC_COMMIT,
            repo_root=repo_root,
        )
        for path in SUPERSEDED_MZC_PATHS
    ]
    ...
    if len(records) != EXPECTED_HISTORICAL_INPUT_COUNT or len(identities) != len(records):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD historical companion inputs must be exact34"
        )
    return records


def _historical_h_mzc_authority(repo_root: Path) -> dict[str, Any]:
    expected_scope = {
        "added": 5,
        "modified": 8,
        "deleted": 0,
        "paths": list(mzc.PATCH_PATHS),
    }
    if (
        _single_parent(repo_root, H_MZC_COMMIT, context="historical H-E0-MZC")
        != mzc.BASE_COMMIT
        or _git_scope(repo_root, mzc.BASE_COMMIT, H_MZC_COMMIT) != expected_scope
    ):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD historical H-E0-MZC topology drifted"
        )
    _require_exact_git_modes(
        repo_root,
        H_MZC_COMMIT,
        mzc.PATCH_COMPONENT_GIT_MODES,
        context="historical H-E0-MZC",
    )
    preserved: list[dict[str, Any]] = []
    for path in PRESERVED_MZC_PATHS:
        role = mzc.PATCH_COMPONENT_ROLES[path]
        current = _file_record(Path(path), role=role, repo_root=repo_root)
        historical = _historical_git_blob_record(
            path, role=role, commit=H_MZC_COMMIT, repo_root=repo_root
        )
        if current["bytes"] != historical["bytes"] or current["sha256"] != historical["sha256"]:
            raise AnfisAblationDvcRegistrationStatusPatchError(
                f"E0-MZD preserved H-E0-MZC component drifted: {path}"
            )
    ...
    if len(preserved) != 5 or len(superseded) != 8:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD H-E0-MZC preserved/superseded partition drifted"
        )
    return {
        "gate": "E0-MZC",
        "head": H_MZC_COMMIT,
        "parent": mzc.BASE_COMMIT,
...
def _base_mzc_authority(repo_root: Path) -> dict[str, Any]:
    if (
        _single_parent(repo_root, P_MZC_COMMIT, context="P-E0-MZC") != H_MZC_COMMIT
        or _git_scope(repo_root, H_MZC_COMMIT, P_MZC_COMMIT)
        != {
            "added": 2,
            "modified": 0,
            "deleted": 0,
            "paths": sorted(
                (BASE_MZC_LOCK_PATH.as_posix(), BASE_MZC_COMPANION_PATH.as_posix())
            ),
        }
    ):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD base P-E0-MZC topology drifted"
        )
    lock = _file_record(
        BASE_MZC_LOCK_PATH,
        role="anfis_ablation_dvc_registration_gitignore_patch_lock",
        repo_root=repo_root,
    )
    companion = _file_record(
        BASE_MZC_COMPANION_PATH,
        role="anfis_ablation_dvc_registration_gitignore_patch_lock_manifest",
        repo_root=repo_root,
    )
    _require_exact_git_modes(
        repo_root,
        P_MZC_COMMIT,
        {
            BASE_MZC_LOCK_PATH.as_posix(): "100644",
            BASE_MZC_COMPANION_PATH.as_posix(): "100644",
        },
        context="P-E0-MZC authority",
    )
    for path, record in (
        (BASE_MZC_LOCK_PATH, lock),
        (BASE_MZC_COMPANION_PATH, companion),
    ):
        git_payload = _git_blob_bytes(repo_root, P_MZC_COMMIT, path)
        if record["bytes"] != len(git_payload) or record["sha256"] != _sha256_bytes(git_payload):
            raise AnfisAblationDvcRegistrationStatusPatchError(
                f"E0-MZD P-E0-MZC physical input differs from Git: {path.as_posix()}"
            )
    if (
        lock["bytes"] != BASE_MZC_LOCK_BYTES
        or lock["sha256"] != BASE_MZC_LOCK_SHA256
        or companion["bytes"] != BASE_MZC_COMPANION_BYTES
        or companion["sha256"] != BASE_MZC_COMPANION_SHA256
    ):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD sealed P-E0-MZC output hashes drifted"
        )
    lock_payload = _load_json(BASE_MZC_LOCK_PATH, repo_root=repo_root)
    companion_payload = _load_json(BASE_MZC_COMPANION_PATH, repo_root=repo_root)
    historical_inputs = companion_payload.get("historical_inputs")
    if (
        lock_payload.get("gate") != "E0-MZC"
        ...
        or len(historical_inputs) != mzc.EXPECTED_HISTORICAL_INPUT_COUNT
        or not _exact_equal(historical_inputs, mzc._historical_inputs(repo_root))
    ):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD P-E0-MZC semantic/history binding drifted"
        )
    return {
        "gate": "E0-MZC",
        "p_head": P_MZC_COMMIT,
...
def _status_order_correction(repo_root: Path) -> dict[str, Any]:
    targets = _selection_payload_paths()
    pointers = _selection_pointer_paths()
    if len(targets) != 10 or len(pointers) != 10:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD sealed porcelain-order incident target set drifted"
        )
    base_payload = _git_blob_bytes(repo_root, P_MZC_COMMIT, GITIGNORE_PATH)
    base_oid = _git(repo_root, "rev-parse", f"{P_MZC_COMMIT}:.gitignore").strip()
    current_payload, current_metadata = _read_regular_bytes_and_metadata(
        GITIGNORE_PATH,
        repo_root=repo_root,
        require_nlink_one=True,
        expected_mode=0o644,
    ...
    current_git_payload = _git_blob_bytes(repo_root, current_head, GITIGNORE_PATH)
    current_oid = _git(repo_root, "rev-parse", f"{current_head}:.gitignore").strip()
    if (
        len(base_payload) != P_MZC_GITIGNORE_BYTES
        or _sha256_bytes(base_payload) != P_MZC_GITIGNORE_SHA256
        or base_oid != P_MZC_GITIGNORE_GIT_OID
        or current_payload != base_payload
        or current_git_payload != current_payload
        or current_oid != P_MZC_GITIGNORE_GIT_OID
        or current_payload.count(GITIGNORE_SUFFIX) != 1
        or stat.S_IMODE(current_metadata.st_mode) != 0o644
        or current_metadata.st_nlink != 1
    ):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD exact unchanged .gitignore/Git binding drifted"
        )
    final_map = _final_unstaged_registration_status_map()
    legacy_expected = [
        f"{status} {path}" for path, status in sorted(final_map.items())
    ]
    tracked_first_observed = [
        " M models.dvc",
        *(f"?? {path.as_posix()}" for path in sorted(pointers)),
    ]
    if (
        len(final_map) != EXPECTED_REGISTRATION_GIT_PATH_COUNT
        or legacy_expected[-1] != " M models.dvc"
        or tracked_first_observed[0] != " M models.dvc"
        or set(legacy_expected) != set(tracked_first_observed)
    ):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD sealed porcelain order dialect drifted"
        )
    gitignore_record = {
    ...
    return {
        "blocked_gate": "R-E0-MZC",
        "published_p_mzc_head": P_MZC_COMMIT,
        "status": "adopted_exact_porcelain_order_only_failure",
        ...
        "failure_phase": "post_models_add_progress_scope_validation",
        "failed_validator": "transaction.verify_progress_scope",
        "failure_error": (
            "E0-MZC registration progress scope drifted; possible .gitignore "
            "or concurrent Git mutation"
        ),
        "root_cause": "raw_git_status_line_order_was_treated_as_semantic_scope",
        "legacy_expected_order": "lexical_path_across_status_classes",
        "observed_git_order": "tracked_modification_before_untracked_additions",
        "legacy_expected_lines": legacy_expected,
        "observed_lines": tracked_first_observed,
        "expected_final_status_map": final_map,
        "observed_final_status_map": dict(final_map),
        "exact_path_status_map_matched": True,
        "pre_stage_validator_reached": False,
        "pre_stage_validator_has_same_order_bug": True,
        ...
        "gitignore": gitignore_record,
        "gitignore_unchanged": True,
        "gitignore_git_oid": P_MZC_GITIGNORE_GIT_OID,
...
def _companion_physical_inputs(
    *,
    ...
    base_mzc: Mapping[str, Any],
    historical_h_mzc: Mapping[str, Any],
) -> list[dict[str, Any]]:
    records = [
        dict(cast(Mapping[str, Any], base_mzc["lock"])),
        dict(cast(Mapping[str, Any], base_mzc["companion"])),
        *(
            dict(record)
            for record in cast(
                Sequence[Mapping[str, Any]], historical_h_mzc["preserved_components"]
            )
        ),
    ...
    if len(records) != EXPECTED_COMPANION_INPUT_COUNT or len(identities) != len(records):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD companion physical inputs must be exact20"
        )
    return records
...
def _expected_untracked_light_status(repo_root: Path) -> list[str]:
    ...
    if tracked != expected_tracked or len(tracked) != FAMILY_TRACKED_LIGHT_COUNT:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD all fifty light outputs must remain tracked"
        )
    return []
...
@_mzd_error_boundary
def preflight_anfis_ablation_dvc_registration_status_patch_schema(
    *, repo_root: Path | None = None
    ...
    if unsupported:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            f"E0-MZD schema exceeds the supported subset: {unsupported}"
        )
    ...
    try:
        ...
        raise AnfisAblationDvcRegistrationStatusPatchError(str(exc)) from exc
...
@_mzd_error_boundary
def collect_anfis_ablation_dvc_registration_status_patch_prelock_state(
    *,
    ...
    if type(verify_remote) is not bool or type(_allow_locker_guard) is not bool:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD prelock policies must be exact booleans"
        )
    root = _root(repo_root)
    preflight_anfis_ablation_dvc_registration_status_patch_schema(repo_root=root)
    head = _git_head(root)
    expected_scope = {
        "added": 5,
        "modified": 8,
    ...
    if (
        _single_parent(root, head, context="H-E0-MZD") != BASE_COMMIT
        or _git_scope(root, BASE_COMMIT, head) != expected_scope
    ):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "H-E0-MZD must be the exact 8M+5A child of published P-E0-MZC"
        )
    _require_exact_git_modes(
        root, head, PATCH_COMPONENT_GIT_MODES, context="H-E0-MZD"
    )
    ...
    if branch != "main" or tracking != head or remote != head:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "H-E0-MZD refs are not aligned with main"
        )
    status = [
        line
    ...
    if status != expected_status:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            f"E0-MZD prelock worktree must be clean with all fifty lights tracked: {status}"
        )
    ...
    if occupied_outputs:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            f"E0-MZD prelock output namespace is occupied: {occupied_outputs}"
        )
    ...
    base_mzc = _base_mzc_authority(root)
    historical_h_mzc = _historical_h_mzc_authority(root)
    status_order = _status_order_correction(root)
    physical_inputs = _companion_physical_inputs(
        h_components=h_components,
        base_mzc=base_mzc,
        historical_h_mzc=historical_h_mzc,
    )
    historical_inputs = _historical_inputs(root)
    if len(historical_inputs) != EXPECTED_HISTORICAL_INPUT_COUNT:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD historical inputs must be exact34"
        )
    boundaries = _scientific_boundaries(root)
    if not all(boundaries.values()):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD scientific boundary drifted"
        )
    return {
        "repository": {
            "branch": branch,
        ...
        "base_mzc_authority": base_mzc,
        "historical_h_mzc": historical_h_mzc,
        "status_order_correction": status_order,
...
@_mzd_error_boundary
def build_anfis_ablation_dvc_registration_status_patch_lock_payload(
    ...
    *,
    ...
    if set(prelock) != required:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD prelock bundle dialect drifted"
        )
    return {
...
def _validate_command_evidence(
    ...
    *,
    ...
    if not isinstance(value, Mapping) or set(value) != keys:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            f"E0-MZD {context} evidence dialect drifted"
        )
    if (
        ...
        raise AnfisAblationDvcRegistrationStatusPatchError(
            f"E0-MZD {context} command/result drifted"
        )
    if (
        ...
        raise AnfisAblationDvcRegistrationStatusPatchError(
            f"E0-MZD {context} digest/line evidence drifted"
        )
    if exact_stdout is not None and (
        ...
        raise AnfisAblationDvcRegistrationStatusPatchError(
            f"E0-MZD {context} stdout evidence drifted"
        )
...
def _validate_verification(value: Any, *, repo_root: Path) -> None:
    if not isinstance(value, Mapping):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD verification evidence is absent"
        )
    required = {
        "schema_preflight",
        "type_check",
    ...
    if set(value) != required:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD verification evidence key set drifted"
        )
    expected_schema = preflight_anfis_ablation_dvc_registration_status_patch_schema(
        repo_root=repo_root
    )
    if not _exact_equal(value.get("schema_preflight"), expected_schema):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD schema-preflight evidence drifted"
        )
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD focused pytest evidence drifted"
        )
    ...
    if not isinstance(stdout_text, str):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD focused pytest stdout text is absent"
        )
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD focused pytest stdout binding drifted"
        )
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD family semantic audit evidence drifted"
        )
    ...
    if not _exact_equal(boundaries, expected_boundaries):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD verification execution boundaries drifted"
        )


def _validate_anfis_ablation_dvc_registration_status_patch_lock_payload(
    ...
    *,
    ...
    if type(allow_registered_state) is not bool:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD registered-state policy must be an exact boolean"
        )
    if _registration_transaction is not None and not allow_registered_state:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD private transaction record requires registered-state audit"
        )
    root = _root(repo_root)
    schema = _load_json(DEFAULT_PATCH_LOCK_SCHEMA, repo_root=root)
    try:
    ...
    if payload.get("schema_version") != SCHEMA_VERSION or payload.get("gate") != PATCH_GATE:
        raise AnfisAblationDvcRegistrationStatusPatchError("E0-MZD lock identity drifted")
    if not _exact_equal(payload.get("authorizations"), UNPUBLISHED_AUTHORIZATIONS):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD unpublished authorizations drifted"
        )
    if not _exact_equal(payload.get("seals"), LOCK_SEALS):
        raise AnfisAblationDvcRegistrationStatusPatchError("E0-MZD seals drifted")
    repository = payload.get("repository")
    if not isinstance(repository, Mapping) or not isinstance(repository.get("head"), str):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD repository binding is absent"
        )
    h_head = str(repository["head"])
    if (
        SHA1_RE.fullmatch(h_head) is None
        or repository.get("parent") != BASE_COMMIT
        or _single_parent(root, h_head, context="H-E0-MZD") != BASE_COMMIT
        or _git_scope(root, BASE_COMMIT, h_head)
        != {"added": 5, "modified": 8, "deleted": 0, "paths": list(PATCH_PATHS)}
    ):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD repository/H topology drifted"
        )
    ...
    if not _exact_equal(h_patch, expected_h):
        raise AnfisAblationDvcRegistrationStatusPatchError("E0-MZD H binding drifted")
    base_mzc = _base_mzc_authority(root)
    if not _exact_equal(payload.get("base_mzc_authority"), base_mzc):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD published P-E0-MZC reconstruction drifted"
        )
    historical_h_mzc = _historical_h_mzc_authority(root)
    if not _exact_equal(payload.get("historical_h_mzc"), historical_h_mzc):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD historical H-E0-MZC reconstruction drifted"
        )
    status_order = _status_order_correction(root)
    if not _exact_equal(payload.get("status_order_correction"), status_order):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD blocked R-E0-MZC porcelain-order incident binding drifted"
        )
    inventory = _registration_inventory(root)
    if not _exact_equal(payload.get("artifact_inventory"), inventory):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD registration inventory binding drifted"
        )
    family = _family_records(
        root,
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD completed family binding drifted"
        )
    models_before = (
    ...
    if not _exact_equal(payload.get("models_owner_transition"), expected_transition):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD models.dvc transition drifted"
        )
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD exact registration plan drifted"
        )
    ...
    if not _exact_equal(payload.get("companion_contract"), expected_contract):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD companion contract drifted"
        )
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD prelock evidence drifted"
        )
    _validate_verification(payload.get("verification"), repo_root=root)


@_mzd_error_boundary
def validate_anfis_ablation_dvc_registration_status_patch_lock_payload(
    payload: Mapping[str, Any],
    *,
    allow_registered_state: bool = False,
    repo_root: Path | None = None,
) -> None:
    _validate_anfis_ablation_dvc_registration_status_patch_lock_payload(
        payload,
...
def _expected_companion(
    ...
    *,
    ...
    root = _root(repo_root)
    h_patch = payload.get("h_patch")
    base_mzc = payload.get("base_mzc_authority")
    historical_h_mzc = payload.get("historical_h_mzc")
    if (
        not isinstance(h_patch, Mapping)
        or not isinstance(base_mzc, Mapping)
        or not isinstance(historical_h_mzc, Mapping)
    ):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "Cannot construct E0-MZD companion"
        )
    components = h_patch.get("components")
    if not isinstance(components, list):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "Cannot construct E0-MZD companion component list"
        )
    ...
    if script is None:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD companion script is absent"
        )
    output = dict(lock_record)
    if (
        output.get("path") != DEFAULT_PATCH_LOCK_PATH.as_posix()
        or output.get("role") != "anfis_ablation_dvc_registration_status_patch_lock"
    ):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD companion output record drifted"
        )
    return {
...
def _run_command(
    ...
    *,
    ...
    if result.returncode != 0:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            f"E0-MZD verification command failed: {list(command)}"
        )
...
def _parse_focused_summary(stdout: str, stderr: str) -> dict[str, int]:
    if stderr or FORBIDDEN_FOCUSED_SUMMARY_RE.search(stdout):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD focused pytest result is not one clean pass"
        )
    ...
    if len(matched) != 1 or lines[-1] != matched[0].group(0):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD focused pytest summary is not unique and terminal"
        )
    return {
...
@_mzd_error_boundary
def run_anfis_ablation_dvc_registration_status_patch_verification(
    *,
    ...
    schema = preflight_anfis_ablation_dvc_registration_status_patch_schema(repo_root=root)
    _require_family_physical_snapshot(
...
) -> AnfisAblationDvcRegistrationStatusPatchError | None:
    first_error: AnfisAblationDvcRegistrationStatusPatchError | None = None
    for output in reversed(published):
        try:
            mt._rollback_owned_output(output)
        except Exception as exc:
            if first_error is None:
                first_error = AnfisAblationDvcRegistrationStatusPatchError(
                    "E0-MZD owned-output rollback failed for "
...
def _close_published_outputs_best_effort(
    ...
    for output in published:
        try:
            ...
            if first_error is None:
                first_error = AnfisAblationDvcRegistrationStatusPatchError(
                    "E0-MZD owned-output descriptor close failed for "
                    f"{output.path.as_posix()}: {exc}"
                )
    ...
    *,
    ...
    try:
        ...
        if (
            ...
            raise AnfisAblationDvcRegistrationStatusPatchError(
                f"E0-MZD owned output identity/metadata drifted {context}: "
        ...
        if (
            ...
            raise AnfisAblationDvcRegistrationStatusPatchError(
                f"E0-MZD owned output bytes drifted {context}: "
                f"{output.path.as_posix()}"
            )
        mt._validate_owned_output(output)
    except AnfisAblationDvcRegistrationStatusPatchError:
...
@_mzd_error_boundary
def execute_and_publish_anfis_ablation_dvc_registration_status_patch_lock_bundle(
    *, repo_root: Path | None = None
    ...
    before = collect_anfis_ablation_dvc_registration_status_patch_prelock_state(
        verify_remote=True, repo_root=root
    )
    verification = run_anfis_ablation_dvc_registration_status_patch_verification(
        family_snapshot=family_snapshot,
        publication_input_snapshot=input_snapshot,
        publication_git_snapshot=git_snapshot,
        repo_root=root,
    )
    after = collect_anfis_ablation_dvc_registration_status_patch_prelock_state(
        verify_remote=True, repo_root=root
    )
    if not _exact_equal(before, after):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD prelock state changed during verification"
        )
    _require_family_physical_snapshot(
    ...
    payload = build_anfis_ablation_dvc_registration_status_patch_lock_payload(
        before, verification, created_at_utc=datetime.now(timezone.utc).isoformat()
    )
    validate_anfis_ablation_dvc_registration_status_patch_lock_payload(payload, repo_root=root)
    controlled = (
    ...
    if occupied:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            f"E0-MZD lock namespace is occupied: {occupied}"
        )
    ...
    try:
        guard = mt._acquire_publication_guard(
            LOCKER_GUARD_PATH,
            b"E0-MZD lock bundle publication in progress\n",
            repo_root=root,
        )
        _require_family_physical_snapshot(
        ...
        if not _exact_equal(
            before,
            collect_anfis_ablation_dvc_registration_status_patch_prelock_state(
                verify_remote=True,
                repo_root=root,
            ...
            raise AnfisAblationDvcRegistrationStatusPatchError(
                "E0-MZD guarded prelock state drifted"
            )
        ...
        if isinstance(exc, AnfisAblationDvcRegistrationStatusPatchError):
            translated = exc
...
@_mzd_error_boundary
def publish_anfis_ablation_dvc_registration_status_patch_lock_bundle(
    *, repo_root: Path | None = None
) -> tuple[dict[str, Any], dict[str, Any]]:
    return execute_and_publish_anfis_ablation_dvc_registration_status_patch_lock_bundle(
        repo_root=repo_root
    )
...
def _validate_p_publication(
    ...
    if not isinstance(repository, Mapping) or not isinstance(repository.get("head"), str):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD H repository binding is absent"
        )
    h_head = str(repository["head"])
    head = _git_head(repo_root)
    tracking = _git_head(repo_root, "origin/main")
    ...
    if (
        ...
        or _single_parent(repo_root, head, context="P-E0-MZD") != h_head
        or tracking != head
        or remote != head
        ...
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "Published P-E0-MZD topology/refs drifted"
        )
    _require_exact_git_modes(
        repo_root,
        head,
    ...
    for raw_path in expected_paths:
        ...
        if physical != _git_blob_bytes(repo_root, head, path):
            raise AnfisAblationDvcRegistrationStatusPatchError(
                f"Published P-E0-MZD physical bytes differ from Git: {raw_path}"
            )
...
def _validate_registered_state(
    ...
    for record in pointers:
        ...
        if len(payload) != record["bytes"] or _sha256_bytes(payload) != record["sha256"]:
            raise AnfisAblationDvcRegistrationStatusPatchError(
                f"E0-MZD registered pointer drifted: {path.as_posix()}"
            )
    ...
    if len(payload) != models["bytes"] or _sha256_bytes(payload) != models["sha256"]:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD registered models.dvc drifted"
        )
    return {
...
def _load_effective_anfis_ablation_dvc_registration_status_patch_authority(
    *,
    ...
    if type(audit_current_unpublished) is not bool or verify_remote is not True:
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD effective authority requires exact audit mode and live remote"
        )
    root = _root(repo_root)
    ...
    try:
        ...
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "Published E0-MZD lock is not valid UTF-8 JSON"
        ) from exc
    if not isinstance(payload, dict) or payload_bytes != _canonical_json(payload):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "Published E0-MZD lock is not canonical JSON"
        )
    ...
    try:
        ...
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "Published E0-MZD companion is not valid UTF-8 JSON"
        ) from exc
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "Published E0-MZD companion drifted"
        )
    ...
    return {
        ...
        **publication,
        ...
        "status_order_correction": dict(
            cast(Mapping[str, Any], payload["status_order_correction"])
        ),
...
@_mzd_error_boundary
def load_effective_anfis_ablation_dvc_registration_status_patch_authority(
    *,
    audit_current_unpublished: bool = False,
    verify_remote: bool = True,
    repo_root: Path | None = None,
    ...
    return _load_effective_anfis_ablation_dvc_registration_status_patch_authority(
        audit_current_unpublished=audit_current_unpublished,
...
@_mzd_error_boundary
def _load_effective_anfis_ablation_dvc_registration_status_patch_during_registration(
    *,
    transaction_record: Mapping[str, Any],
    verify_remote: bool = True,
    repo_root: Path | None = None,
) -> dict[str, Any]:
    """Internal R-E0-MZD audit while durable rollback coordination is owned."""
    if not isinstance(transaction_record, Mapping):
        raise AnfisAblationDvcRegistrationStatusPatchError(
            "E0-MZD internal transaction record is absent"
        )
    return _load_effective_anfis_ablation_dvc_registration_status_patch_authority(
...
@_mzd_error_boundary
def require_anfis_ablation_dvc_registration_status_patch_authority(
    *,
    audit_current_unpublished: bool = False,
    verify_remote: bool = True,
    repo_root: Path | None = None,
) -> dict[str, Any]:
    return load_effective_anfis_ablation_dvc_registration_status_patch_authority(
        audit_current_unpublished=audit_current_unpublished,
        verify_remote=verify_remote,
        repo_root=repo_root,
    )
...
def require_anfis_ablation_dvc_registration_authority(
    *, verify_remote: bool = True, repo_root: Path | None = None
    ...
    return require_anfis_ablation_dvc_registration_status_patch_authority(
...
@_mzd_error_boundary
def load_effective_anfis_ablation_dvc_registration_status_patch(
    *,
    audit_current_unpublished: bool = False,
    verify_remote: bool = True,
    repo_root: Path | None = None,
...
__all__ = [
    "AnfisAblationDvcRegistrationStatusPatchError",
    "ANFIS_ABLATION_H_MZD_STAGED_SCOPE",
    "ANFIS_ABLATION_P_MZD_STAGED_SCOPE",
    "ANFIS_ABLATION_R_MZD_STAGED_SCOPE",
    "BASE_COMMIT",
    ...
    "GITIGNORE_ENTRY",
    "GITIGNORE_SUFFIX",
    "P_MZC_GITIGNORE_BYTES",
    "P_MZC_GITIGNORE_SHA256",
    "P_MZC_GITIGNORE_GIT_OID",
    ...
    "PATCH_GATE",
    "PATCH_PATHS",
    "build_anfis_ablation_dvc_registration_status_patch_lock_payload",
    "collect_anfis_ablation_dvc_registration_status_patch_prelock_state",
    "execute_and_publish_anfis_ablation_dvc_registration_status_patch_lock_bundle",
    "load_effective_anfis_ablation_dvc_registration_status_patch_authority",
    "load_effective_anfis_ablation_dvc_registration_status_patch",
    "preflight_anfis_ablation_dvc_registration_status_patch_schema",
    "publish_anfis_ablation_dvc_registration_status_patch_lock_bundle",
    "require_anfis_ablation_dvc_registration_authority",
    "require_anfis_ablation_dvc_registration_adoption_authority",
    "require_anfis_ablation_dvc_registration_status_patch_authority",
    "run_anfis_ablation_dvc_registration_status_patch_verification",
    "validate_anfis_ablation_dvc_registration_status_patch_lock_payload",
]
