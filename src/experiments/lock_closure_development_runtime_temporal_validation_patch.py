#!/usr/bin/env python
"""Create the one-time additive Closure V1 E0-DLTV lock bundle."""
...
OUTPUT_GUARD_DIRECTORY = PROJECT_ROOT / "tmp" / "closure_v1_e0_dltv_locker"
OUTPUT_GUARD_NAMES = (
    "development_runtime_temporal_validation_patch_lock.guard",
    "development_runtime_temporal_validation_patch_lock_manifest.guard",
)
...
def _parse_focused_summary(stdout: str, stderr: str) -> int:
    ...
    if (
        ...
        raise DevelopmentRuntimeTemporalValidationPatchError(
            "E0-DLTV focused pytest summary is not one exact clean result"
        )
...
def _require_success_marker(stdout: str, stderr: str, marker: str) -> None:
    if marker not in stdout + "\n" + stderr:
        raise DevelopmentRuntimeTemporalValidationPatchError(
            f"E0-DLTV verification marker is absent: {marker}"
        )
...
def _require_empty_output(stdout: str, stderr: str, *, context: str) -> None:
    if stdout.strip() or stderr.strip():
        raise DevelopmentRuntimeTemporalValidationPatchError(
            f"E0-DLTV expected empty output from {context}"
        )
...
def _terminal_status(stdout: str, stderr: str) -> str:
    ...
    if lines.count(marker) != 1 or any(
        ...
        raise DevelopmentRuntimeTemporalValidationPatchError(
            "E0-DLTV targeted DVC push is not exactly idempotent"
        )
    return marker


def run_temporal_validation_patch_verification() -> dict[str, Any]:
    """Run the closed H-DLTV verification set; this is intentionally heavy."""
...
def _guard_paths(*, create_directory: bool) -> tuple[Path, Path]:
    ...
    if OUTPUT_GUARD_DIRECTORY != tmp_root / "closure_v1_e0_dltv_locker":
        raise DevelopmentRuntimeTemporalValidationPatchError(
            "E0-DLTV coordination directory escaped ignored tmp/"
        )
    for path, context in (
        (tmp_root, "E0-DLTV tmp root"),
        (OUTPUT_GUARD_DIRECTORY, "E0-DLTV guard directory"),
    ):
    ...
    if create_directory:
        hardened._ensure_real_directory(
            tmp_root,
            parent=PROJECT_ROOT,
            context="E0-DLTV tmp root",
        )
        hardened._ensure_real_directory(
            OUTPUT_GUARD_DIRECTORY,
            parent=tmp_root,
            context="E0-DLTV guard directory",
        )
    return (
...
def _refuse_existing_outputs(output: Path, companion: Path) -> None:
    ...
    if existing:
        raise DevelopmentRuntimeTemporalValidationPatchError(
            f"Refusing to overwrite an existing E0-DLTV lock bundle: {existing}"
        )
...
def _execute_lock(output: Path, companion: Path) -> dict[str, Any]:
    ...
    try:
        ...
        prelock = collect_temporal_validation_patch_prelock_state(
            require_physical_artifacts=True,
            verify_remote=True,
        )
        verification = run_temporal_validation_patch_verification()
        consumer_namespace_absence()
        repeated = collect_temporal_validation_patch_prelock_state(
            require_physical_artifacts=True,
            verify_remote=True,
        )
        if repeated != prelock:
            raise DevelopmentRuntimeTemporalValidationPatchError(
                "E0-DLTV authority changed during verification"
            )
        hardened._assert_lock_namespace(lock_path, companion_path, guards, owners)

        timestamp = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
        payload = build_temporal_validation_patch_lock_payload(
            prelock,
            verification,
            created_at_utc=timestamp,
        )
        schema = _load_regular_json(DEFAULT_PATCH_LOCK_SCHEMA, context="E0-DLTV schema")
        validate_development_runtime_temporal_validation_patch_lock_payload(
            payload,
            schema,
            require_physical_artifacts=True,
        ...
        companion_record = hardened._owned_file_record(
            companion_owner,
            role="development_runtime_temporal_validation_patch_companion",
        )
        load_and_validate_development_runtime_temporal_validation_patch_lock(
