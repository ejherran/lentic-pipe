"""Adopt the sealed raw-score candidate semantics for final calibration.

E0-MCALC is an additive authority over the published E0-MCALP gate.  It
changes no data, target, denominator, model, or output path.  Its scientific
correction is limited to recognising the already-sealed empty ``candidate``
sentinel for B0/B1 while retaining exact non-empty candidate identities for
B2 and M0.
"""
...
from src.experiments import (
    closure_final_calibration_publication_guard_patch as mcalp,
)


PROJECT_ROOT = Path(__file__).resolve().parents[2]
BASE_P_MCALP_COMMIT = "6b74440e31d67a6b1a26609347639ae2ba33ec01"
H_MCALP_COMMIT = "59225001a8b1c006213b6a3d963126a6b3f73ccf"
H_MCALP_PARENT = mcalp.BASE_H_MCAL_COMMIT
PATCH_GATE = "E0-MCALC"
FINAL_CALIBRATION_GATE = PATCH_GATE
EXPERIMENT_ID = "closure_v1"
LOCK_SCHEMA_VERSION = "closure_final_calibration_candidate_semantics_patch_lock_v1"
COMPANION_SCHEMA_VERSION = (
    "closure_final_calibration_candidate_semantics_patch_lock_manifest_v1"
)
...
LOCKER_GUARD_PATH = Path(
    "tmp/closure_v1_e0_mcalc/final_calibration_candidate_semantics_patch_lock.guard"
)

H_MCALP_SUPERSEDED_PATHS = tuple(
    sorted(
        {
...
H_MCALP_PRESERVED_PATHS = tuple(
    path for path in mcalp.PATCH_PATHS if path not in H_MCALP_SUPERSEDED_PATHS
)
P_MCALP_PATHS = (
    mcalp.DEFAULT_PATCH_LOCK_PATH.as_posix(),
    mcalp.DEFAULT_PATCH_LOCK_MANIFEST_PATH.as_posix(),
)
PATCH_PATHS = tuple(
    sorted(
        {
...
FINAL_CALIBRATION_H_STAGED_SCOPE = {
    path: ("M" if path in H_MCALP_SUPERSEDED_PATHS else "A")
...
FINAL_CALIBRATION_R_STAGED_SCOPE = dict(mcalp.FINAL_CALIBRATION_R_STAGED_SCOPE)
R_OUTPUT_PATHS = mcalp.R_OUTPUT_PATHS
...
TYPE_CHECK_COMMAND = mcalp.TYPE_CHECK_COMMAND
...
POETRY_CHECK_COMMAND = mcalp.POETRY_CHECK_COMMAND
PUBLICATION_GUARD_COMMAND = mcalp.PUBLICATION_GUARD_COMMAND
DIFF_CHECK_COMMAND = mcalp.DIFF_CHECK_COMMAND
UNPUBLISHED_AUTHORIZATIONS = dict(mcalp.UNPUBLISHED_AUTHORIZATIONS)
EMPTY_SHA256 = hashlib.sha256(b"").hexdigest()
SHA1_RE = re.compile(r"^[0-9a-f]{40}$")


class FinalCalibrationCandidateSemanticsPatchError(mcalp.FinalCalibrationError):
    """Raised when the E0-MCALC correction authority is not exact."""


FinalCalibrationError = mcalp.FinalCalibrationError
P = ParamSpec("P")
R = TypeVar("R")


def _error(message: str) -> FinalCalibrationCandidateSemanticsPatchError:
    return FinalCalibrationCandidateSemanticsPatchError(message)
...
def _error_boundary(function: Callable[P, R]) -> Callable[P, R]:
    ...
    def wrapped(*args: P.args, **kwargs: P.kwargs) -> R:
        ...
        except FinalCalibrationCandidateSemanticsPatchError:
            raise
        except mcalp.FinalCalibrationError as exc:
            raise _error(str(exc).replace("E0-MCALP", "E0-MCALC")) from exc
...
def validate_raw_score_candidate_semantics(
    model_id: str, candidates: Sequence[Any]
) -> None:
    """Validate the producer-sealed candidate identity without coercion."""

    if type(model_id) is not str or model_id not in RAW_SCORE_CANDIDATE_VALUES:
        raise _error("E0-MCALC raw-score model candidate authority is unknown")
    values = list(candidates)
    if not values or any(type(value) is not str for value in values):
        raise _error(f"E0-MCALC {model_id} raw-score candidate type drifted")
    expected = set(RAW_SCORE_CANDIDATE_VALUES[model_id])
    if set(values) != expected:
        raise _error(f"E0-MCALC {model_id} raw-score candidate identity drifted")
    if model_id in {"B0", "B1"} and any(values):
        raise _error(f"E0-MCALC {model_id} candidate sentinel must remain empty")
    if model_id in {"B2", "M0"} and any(not value for value in values):
        raise _error(f"E0-MCALC {model_id} candidate identity must remain non-empty")
...
def _git_record_at_commit(
    ...
    relative = Path(path)
    mode, oid = mcalp.mcal._git_mode_oid(repo_root, commit, relative)
    payload = mcalp.mcal._git_blob_bytes(repo_root, commit, relative)
    if mode != "100644":
        raise _error(f"E0-MCALC historical Git mode drifted: {path}")
    return {
        "role": role,
        "path": path,
        "bytes": len(payload),
...
def _p_mcalp_authority(*, repo_root: Path) -> dict[str, Any]:
    """Reconstruct the published MCALP H/P authority without live globals."""

    if (
        mcalp.mcal._single_parent(
            repo_root, H_MCALP_COMMIT, context="H-E0-MCALP"
        )
        != H_MCALP_PARENT
    ):
        raise _error("E0-MCALC historical H-E0-MCALP parent drifted")
    h_scope = mcalp.mcal._git_scope(
        repo_root, H_MCALP_PARENT, H_MCALP_COMMIT
    )
    if h_scope != {
        "added": 5,
        "modified": 4,
        "deleted": 0,
        "path_count": 9,
        "paths": list(mcalp.PATCH_PATHS),
    }:
        raise _error("E0-MCALC historical H-E0-MCALP scope drifted")
    if (
        mcalp.mcal._single_parent(
            repo_root, BASE_P_MCALP_COMMIT, context="P-E0-MCALP"
        )
        != H_MCALP_COMMIT
    ):
        raise _error("E0-MCALC historical P-E0-MCALP parent drifted")
    p_scope = mcalp.mcal._git_scope(
        repo_root, H_MCALP_COMMIT, BASE_P_MCALP_COMMIT
    )
    if p_scope != {
        "added": 2,
        "modified": 0,
        "deleted": 0,
        "path_count": 2,
        "paths": sorted(P_MCALP_PATHS),
    }:
        raise _error("E0-MCALC historical P-E0-MCALP scope drifted")

    h_components = [
        _git_record_at_commit(
            path,
            role="final_calibration_publication_guard_patch_component",
            commit=H_MCALP_COMMIT,
            repo_root=repo_root,
        )
        for path in mcalp.PATCH_PATHS
    ]
    preserved: list[dict[str, Any]] = []
    for path in H_MCALP_PRESERVED_PATHS:
        record = _git_record_at_commit(
            path,
            role="preserved_h_mcalp_component",
            commit=H_MCALP_COMMIT,
            repo_root=repo_root,
        )
        physical = mcalp.mcal._git_artifact_record(
            Path(path),
            role="preserved_h_mcalp_component",
            repo_root=repo_root,
            commit=H_MCALP_COMMIT,
        ...
        if physical != record:
            raise _error(f"E0-MCALC preserved H-MCALP component drifted: {path}")
        preserved.append(record)
    historical = [
        {
            **_git_record_at_commit(
                path,
                role="superseded_h_mcalp_component",
                commit=H_MCALP_COMMIT,
                repo_root=repo_root,
            ),
            "commit": H_MCALP_COMMIT,
        }
        for path in H_MCALP_SUPERSEDED_PATHS
    ]
    p_components: list[dict[str, Any]] = []
    for path in P_MCALP_PATHS:
        role = (
            "published_p_mcalp_lock"
            if path == mcalp.DEFAULT_PATCH_LOCK_PATH.as_posix()
            else "published_p_mcalp_lock_manifest"
        )
        record = _git_record_at_commit(
            path,
            role=role,
            commit=BASE_P_MCALP_COMMIT,
            repo_root=repo_root,
        )
        physical = mcalp.mcal._git_artifact_record(
            Path(path),
        ...
        if physical != record:
            raise _error(f"E0-MCALC published P-MCALP component drifted: {path}")
        p_components.append(record)

    lock_bytes = mcalp.mcal._git_blob_bytes(
        repo_root, BASE_P_MCALP_COMMIT, mcalp.DEFAULT_PATCH_LOCK_PATH
    )
    companion_bytes = mcalp.mcal._git_blob_bytes(
        repo_root,
        BASE_P_MCALP_COMMIT,
        mcalp.DEFAULT_PATCH_LOCK_MANIFEST_PATH,
    )
    try:
        lock_payload = mcalp.mcal._parse_json_bytes(
            lock_bytes, context="published P-E0-MCALP lock"
        )
        companion = mcalp.mcal._parse_json_bytes(
            companion_bytes, context="published P-E0-MCALP companion"
        )
    except mcalp.FinalCalibrationError as exc:
        raise _error("E0-MCALC historical P-E0-MCALP JSON drifted") from exc
    if (
        not isinstance(lock_payload, Mapping)
        or not isinstance(companion, Mapping)
        or lock_bytes != mcalp._canonical_json_bytes(lock_payload)
        or companion_bytes != mcalp._canonical_json_bytes(companion)
        or lock_payload.get("gate") != mcalp.PATCH_GATE
        or cast(Mapping[str, Any], lock_payload.get("repository", {})).get(
            "h_patch_head"
        )
        != H_MCALP_COMMIT
    ):
        raise _error("E0-MCALC historical P-E0-MCALP payload drifted")
    lock_record = {
        "role": "final_calibration_publication_guard_patch_lock",
        "path": mcalp.DEFAULT_PATCH_LOCK_PATH.as_posix(),
        "bytes": len(lock_bytes),
        "sha256": _sha256_bytes(lock_bytes),
    }
    expected_companion = mcalp._expected_companion(lock_payload, lock_record)
    if companion_bytes != mcalp._canonical_json_bytes(expected_companion):
        raise _error("E0-MCALC historical P-E0-MCALP companion drifted")
    h_mcal_authority = lock_payload.get("h_mcal_authority")
    if not isinstance(h_mcal_authority, Mapping):
        raise _error("E0-MCALC transitive H-E0-MCAL authority is absent")
    transitive_preserved: list[dict[str, Any]] = []
    for path in mcalp.H_MCAL_PRESERVED_PATHS:
        record = _git_record_at_commit(
            path,
            role="preserved_h_mcal_component",
            commit=H_MCALP_PARENT,
            repo_root=repo_root,
        )
        physical = mcalp.mcal._git_artifact_record(
            Path(path),
        ...
        if physical != record:
            raise _error(
                f"E0-MCALC transitive preserved H-MCAL component drifted: {path}"
            )
        transitive_preserved.append(record)
    if (
        h_mcal_authority.get("commit") != H_MCALP_PARENT
        or h_mcal_authority.get("preserved_count")
        != len(mcalp.H_MCAL_PRESERVED_PATHS)
        or _canonical_json_bytes(h_mcal_authority.get("preserved_components"))
        != _canonical_json_bytes(transitive_preserved)
        or h_mcal_authority.get("preserved_components_sha256")
        != mcalp.mcal._digest_records(transitive_preserved)
    ):
        raise _error("E0-MCALC transitive preserved H-E0-MCAL authority drifted")
    sealed_runtime = lock_payload.get("runtime")
    if not isinstance(sealed_runtime, Mapping):
        raise _error("E0-MCALC historical P-E0-MCALP runtime binding is absent")
    for key, expected_path in (
        ("runtime", mcalp.mcal.DEFAULT_RUNTIME_PATH.as_posix()),
        ("runtime_schema", mcalp.mcal.DEFAULT_RUNTIME_SCHEMA.as_posix()),
    ):
        record = sealed_runtime.get(key)
        if (
            not isinstance(record, Mapping)
            or record.get("path") != expected_path
            or record.get("sha256")
            != _sha256_bytes(
                mcalp.mcal._git_blob_bytes(
                    repo_root, H_MCALP_PARENT, Path(expected_path)
                )
            )
        ):
            raise _error(
                f"E0-MCALC historical P-E0-MCALP {key} binding drifted"
            )
    return {
        "gate": "P-E0-MCALP",
        "commit": BASE_P_MCALP_COMMIT,
        "parent_h_mcalp": H_MCALP_COMMIT,
        "h_mcalp_parent": H_MCALP_PARENT,
        "h_scope": h_scope,
        "p_scope": p_scope,
        "h_component_count": 9,
        "h_components": h_components,
        "h_components_sha256": mcalp.mcal._digest_records(h_components),
        "preserved_count": 5,
        "preserved_components": preserved,
        "preserved_components_sha256": mcalp.mcal._digest_records(preserved),
        "superseded_count": 4,
        "historical_inputs": historical,
        "historical_inputs_sha256": mcalp.mcal._digest_records(historical),
        "p_component_count": 2,
        "p_components": p_components,
        "p_components_sha256": mcalp.mcal._digest_records(p_components),
...
def _candidate_status_is_exact(repo_root: Path) -> bool:
    records = mcalp.mcal._workspace_status_records(repo_root)
    ...
    return all(
        ...
        in (
            {" M", "M ", "MM"}
            if path in H_MCALP_SUPERSEDED_PATHS
            else {"??", "A "}
        )
        for path in PATCH_PATHS
    )
...
def _h_patch_authority(
    *, repo_root: Path, verify_remote: bool
    ...
    if type(verify_remote) is not bool:
        raise _error("E0-MCALC remote policy must be an exact boolean")
    head = mcalp.mcal._git_head(repo_root)
    branch = cast(
        str, mcalp.mcal._git(repo_root, "branch", "--show-current")
    ).strip()
    if branch != "main":
        raise _error("E0-MCALC H authority requires branch main")
    expected_scope = {
        "added": 5,
        "modified": 4,
        "deleted": 0,
    ...
    candidate = head == BASE_P_MCALP_COMMIT
    if candidate:
        if not _candidate_status_is_exact(repo_root):
            raise _error("E0-MCALC candidate workspace is not exact 4M+5A")
        commit_for_components: str | None = None
        h_head = BASE_P_MCALP_COMMIT
        scope = expected_scope
    else:
        if (
            mcalp.mcal._single_parent(repo_root, head, context="H-E0-MCALC")
            != BASE_P_MCALP_COMMIT
        ):
            raise _error("E0-MCALC published H parent drifted")
        scope = mcalp.mcal._git_scope(repo_root, BASE_P_MCALP_COMMIT, head)
        if scope != expected_scope or mcalp.mcal._workspace_status_records(repo_root):
            raise _error("E0-MCALC published H scope/worktree drifted")
        commit_for_components = head
        h_head = head
    components = [
        mcalp.mcal._git_artifact_record(
            Path(path),
            role="final_calibration_candidate_semantics_patch_component",
            repo_root=repo_root,
            commit=commit_for_components,
        )
        for path in PATCH_PATHS
    ]
    tracking = mcalp.mcal._git_head(repo_root, "origin/main")
    expected_ref = BASE_P_MCALP_COMMIT if candidate else head
    if tracking != expected_ref:
        raise _error("E0-MCALC H tracking ref drifted")
    remote = (
        mcalp.mcal._live_remote_main_head(repo_root)
        if verify_remote
        else tracking
    )
    if remote != expected_ref:
        raise _error("E0-MCALC H remote ref drifted")
    repository = {
        "base_p_mcalp_commit": BASE_P_MCALP_COMMIT,
        "h_patch_head": h_head,
        "branch": branch,
        "remote_head": remote,
        "scope": scope,
    ...
    return repository, {
        ...
        "components_sha256": mcalp.mcal._digest_records(components),
    }


def _namespace_paths() -> tuple[Path, ...]:
    forbidden_finals = (
        mcalp.mcal.DEFAULT_PATCH_LOCK_PATH,
        mcalp.mcal.DEFAULT_PATCH_LOCK_MANIFEST_PATH,
    ...
    temporary_bases = (
        *forbidden_finals,
        mcalp.DEFAULT_PATCH_LOCK_PATH,
        mcalp.DEFAULT_PATCH_LOCK_MANIFEST_PATH,
    )
    return (
        *forbidden_finals,
        *(mcalp.mcal._temporary_path(path) for path in temporary_bases),
        mcalp.mcal.LOCKER_GUARD_PATH,
        mcalp.LOCKER_GUARD_PATH,
        LOCKER_GUARD_PATH,
        mcalp.mcal.CALIBRATION_GUARD_PATH,
        mcalp.mcal.E7_GUARD_PATH,
    )
...
def _require_owned_guard_identity(guard: Any) -> None:
    try:
        ...
        named = mcalp.mt._named_identity(guard.parent_descriptor, guard.name)
    except (AttributeError, OSError) as exc:
        raise _error("E0-MCALC publication guard identity check failed") from exc
    if (
        ...
        raise _error("E0-MCALC publication guard identity drifted")
...
def _require_publication_boundary(
    *,
    ...
    missing_base = [
        ...
        if not mcalp.mcal._entry_exists(path, repo_root=repo_root)
    ]
    if missing_base:
        raise _error(f"E0-MCALC base P-MCALP authority is absent: {missing_base}")
    allowed = (
    ...
    occupied = [
        ...
        if mcalp.mcal._entry_exists(path, repo_root=repo_root)
    ]
    if occupied:
        raise _error(f"E0-MCALC publication namespace is occupied: {occupied}")
    if mcalp.mcal._entry_exists(
        Path(mcalp.mcal.mze.OUTCOME_ACCESS_LOG), repo_root=repo_root
    ):
        raise _error("E0-MCALC outcome access log must remain absent")
    if any(
        mcalp.mcal._entry_exists(Path(path), repo_root=repo_root)
        for path in mcalp.mcal.mze.E0_M_PATHS
    ):
        raise _error("E0-MCALC final E0-M namespace must remain absent")
...
def _require_repository_checkpoint(
    *,
    ...
    status = sorted(mcalp.mcal._workspace_status_records(repo_root))
    if (
        branch != "main"
        ...
        raise _error(f"E0-MCALC repository changed {context}")
    if (
        verify_remote
        and mcalp.mcal._live_remote_main_head(repo_root) != expected_head
    ):
        raise _error(f"E0-MCALC live remote changed {context}")


@_error_boundary
def preflight_final_calibration_candidate_semantics_patch_schema(
    *, repo_root: Path | None = None
) -> dict[str, Any]:
    root = _root(repo_root)
    schema = mcalp.mcal._load_json_object(DEFAULT_PATCH_LOCK_SCHEMA, repo_root=root)
    validator = getattr(
        mcalp.mcal.closure_contract, "_assert_supported_json_schema", None
    )
    if not callable(validator):
        raise _error("E0-MCALC supported JSON-schema validator is unavailable")
    try:
        validator(schema)
    except mcalp.mcal.ClosureContractError as exc:
    ...
    return {
        ...
        "schema_count": 1,
        "schemas": [
            mcalp.mcal._file_record(
                DEFAULT_PATCH_LOCK_SCHEMA,
                role="final_calibration_candidate_semantics_patch_lock_schema",
                repo_root=root,
            )
        ],
...
def _state_for_h(
    *,
    ...
    schema_preflight = preflight_final_calibration_candidate_semantics_patch_schema(
        repo_root=repo_root
    )
    runtime = mcalp.mcal.load_and_validate_final_calibration_runtime(
        repo_root=repo_root
    )
    p_mcalp = _p_mcalp_authority(repo_root=repo_root)
    scientific_inputs = mcalp.mcal._scientific_input_inventory(repo_root=repo_root)
    if require_empty_namespace:
        _require_prelock_namespace(repo_root=repo_root)
    sealed_runtime = cast(Mapping[str, Any], p_mcalp["sealed_runtime"])
    ...
    runtime_contract = {
        ...
        "raw_score_candidate_values": _deep_copy(RAW_SCORE_CANDIDATE_VALUES),
    ...
    prelock = {
        ...
        "base_p_mcalp_output_present_count": 2,
    ...
    return {
        ...
        "output_contract": mcalp.mcal._output_contract(runtime),
        "prelock": prelock,
    }


@_error_boundary
def collect_final_calibration_candidate_semantics_patch_prelock_state(
...
@_error_boundary
def build_final_calibration_candidate_semantics_patch_lock_payload(
    ...
    *,
    ...
    if set(prelock) != required:
        raise _error("E0-MCALC prelock dialect drifted")
    ...
    return {
        ...
        "p_mcalp_authority": _deep_copy(prelock["p_mcalp_authority"]),
...
def _validate_timestamp(value: Any) -> None:
    if not isinstance(value, str):
        raise _error("E0-MCALC generated timestamp is absent")
    try:
        parsed = datetime.fromisoformat(value.replace("Z", "+00:00"))
    except ValueError as exc:
        raise _error("E0-MCALC generated timestamp is malformed") from exc
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise _error("E0-MCALC generated timestamp must be timezone-aware")
...
def _validate_verification(value: Any, *, repo_root: Path) -> None:
    ...
    if not isinstance(value, Mapping) or set(value) != {
        ...
        raise _error("E0-MCALC verification evidence dialect drifted")
    expected_preflight = preflight_final_calibration_candidate_semantics_patch_schema(
        repo_root=repo_root
    )
    if _canonical_json_bytes(value["schema_preflight"]) != _canonical_json_bytes(
        expected_preflight
    ):
        raise _error("E0-MCALC schema verification evidence drifted")
    for key, command in (
        ...
        try:
            mcalp.mcal._validate_command_evidence(
    ...
    if (
        ...
        raise _error("E0-MCALC focused verification evidence drifted")
    try:
        mcalp.mcal._validate_command_evidence(
...
@_error_boundary
def validate_final_calibration_candidate_semantics_patch_lock_payload(
    ...
    *,
    ...
    if not isinstance(payload, Mapping):
        raise _error("E0-MCALC lock payload must be an object")
    schema = mcalp.mcal._load_json_object(DEFAULT_PATCH_LOCK_SCHEMA, repo_root=root)
    try:
        mcalp.mcal.validate_json_schema(payload, schema)
    except mcalp.mcal.ClosureContractError as exc:
        raise _error(str(exc)) from exc
    ...
    if payload.get("authorizations") != UNPUBLISHED_AUTHORIZATIONS:
        raise _error("E0-MCALC unpublished authorizations drifted")
    state = collect_final_calibration_candidate_semantics_patch_prelock_state(
        verify_remote=verify_remote, repo_root=root
    )
    expected = build_final_calibration_candidate_semantics_patch_lock_payload(
        state,
    ...
    if _canonical_json_bytes(payload) != _canonical_json_bytes(expected):
        raise _error("E0-MCALC lock semantic reconstruction drifted")
    return dict(payload)


_OwnedOutput = mcalp._OwnedOutput


def _publish_bytes_no_clobber(
    final_path: Path, payload: bytes, *, repo_root: Path
) -> _OwnedOutput:
    """Use MCALP's temp-identity primitive and translate only its domain."""

    try:
        return mcalp._publish_bytes_no_clobber(
...
def _rollback_owned_output(output: _OwnedOutput) -> None:
    try:
        mcalp._rollback_owned_output(output)
    except mcalp.FinalCalibrationError as exc:
        raise _error(str(exc).replace("E0-MCALP", "E0-MCALC")) from exc
...
def _close_owned_output(output: _OwnedOutput) -> None:
    try:
        mcalp._close_owned_output(output)
    except mcalp.FinalCalibrationError as exc:
        raise _error(str(exc).replace("E0-MCALP", "E0-MCALC")) from exc
...
def _expected_companion(
    ...
    base = cast(Mapping[str, Any], payload["p_mcalp_authority"])
    ...
    if (
        ...
        raise _error("E0-MCALC companion physical input set drifted")
...
def _require_publication_verification(
    ...
    if verification == _default_unrun_verification():
        raise _error(
            "E0-MCALC publication requires the exact frozen verification evidence"
        )
...
def _physical_snapshot(
    ...
    for path_text in current_paths:
        if path_text in known:
            continue
        path = Path(path_text)
        payload, metadata = mcalp.mcal._read_regular_bytes_and_metadata(
...
def _require_physical_snapshot(
    ...
    *,
    ...
    if _canonical_json_bytes(expected) != _canonical_json_bytes(
        ...
        raise _error(f"E0-MCALC physical authority changed {context}")
...
def _validate_owned_output_bytes(
    ...
    *,
    ...
    try:
        mcalp._validate_owned_output(output)
        payload, metadata = mcalp.mcal._read_regular_bytes_and_metadata(
            output.path, repo_root=repo_root
        )
    except Exception as exc:
        raise _error(f"E0-MCALC owned output validation failed {context}") from exc
    if (
        payload != expected
        ...
        raise _error(
            f"E0-MCALC owned output drifted {context}: {output.path.as_posix()}"
        )
    return metadata
...
def _rollback_outputs_best_effort(
...
) -> FinalCalibrationCandidateSemanticsPatchError | None:
    ...
    if not errors:
        return None
    error = _error("E0-MCALC owned-output rollback was incomplete")
...
@_error_boundary
def publish_final_calibration_candidate_semantics_patch_lock_bundle(
    ...
    try:
        ...
        raise _error("E0-MCALC publication payload is not canonical JSON") from exc
    if (
        not isinstance(frozen_payload, dict)
        or _canonical_json_bytes(frozen_payload) != lock_bytes
    ):
        raise _error("E0-MCALC publication payload must be a canonical object")
    payload = frozen_payload
    repository = payload.get("repository")
    if (
        not isinstance(repository, Mapping)
        or repository.get("h_patch_head") == BASE_P_MCALP_COMMIT
    ):
        raise _error("E0-MCALC H must be published before P publication")
    ...
    if not isinstance(scientific_inventory, Mapping):
        raise _error("E0-MCALC scientific inventory is absent")
    snapshot = _physical_snapshot(root, scientific_inventory=scientific_inventory)
    initial_head = mcalp.mcal._git_head(root)
    if initial_head != repository.get("h_patch_head"):
        raise _error("E0-MCALC H refs drifted before publication")
    validate_final_calibration_candidate_semantics_patch_lock_payload(
        payload, repo_root=root, verify_remote=True
    )
    ...
    try:
        try:
            guard = mcalp.mt._acquire_publication_guard(
                LOCKER_GUARD_PATH,
                b"E0-MCALC final calibration candidate semantics lock publication\n",
                repo_root=root,
            )
        except Exception as exc:
            raise _error("E0-MCALC publication guard acquisition failed") from exc
        _require_physical_snapshot(
            snapshot,
            scientific_inventory=scientific_inventory,
            repo_root=root,
        ...
        try:
            mcalp.mt._release_publication_guard(guard)
        except Exception as exc:
            raise _error("E0-MCALC publication guard release failed") from exc
        guard = None
        ...
        for pass_index in (1, 2):
            ...
            for output, expected in publication_set:
                ...
                if (
                    ...
                    raise _error(
                        "E0-MCALC owned output changed before ownership transfer"
                    )
            mcalp._require_owned_identity_set(
        ...
        for output, expected in publication_set:
            ...
            if (
                ...
                raise _error(
                    "E0-MCALC owned output changed at ownership transfer"
                )
        mcalp._require_owned_identity_set(
            [output for output, _ in publication_set],
        ...
        if isinstance(exc, (KeyboardInterrupt, SystemExit)):
            raise
        if isinstance(exc, FinalCalibrationCandidateSemanticsPatchError):
            raise
        raise _error("E0-MCALC lock bundle publication failed") from exc
    finally:
        if guard is not None:
            try:
                mcalp.mt._release_publication_guard(guard, tolerate_foreign=True)
            except Exception:
                pass
        if committed:
...
def _scientific_input_inventory(*, repo_root: Path) -> dict[str, Any]:
    return mcalp.mcal._scientific_input_inventory(repo_root=repo_root)


def _validate_calibrator_specs(
    value: Any,
) -> dict[tuple[str, int, int], Mapping[str, Any]]:
    """Adapt only MCALC's gate token before the strict MCALP parser."""

    if not isinstance(value, Mapping) or value.get("gate") != PATCH_GATE:
        raise _error("E0-MCALC calibrator spec gate drifted")
    adapted = _deep_copy(value)
    adapted["gate"] = mcalp.PATCH_GATE
    try:
        return mcalp._validate_calibrator_specs(adapted)
    except mcalp.FinalCalibrationError as exc:
        raise _error(str(exc).replace("E0-MCALP", "E0-MCALC")) from exc
...
def _effective_authority_binding_sha256(
    *,
    ...
    effective_head = (
        mcalp.mcal._git_head(repo_root) if p_patch_head is None else p_patch_head
    )
    effective_lock = (
        mcalp.mcal._file_record(
            DEFAULT_PATCH_LOCK_PATH,
...
def _torch_policy_has_exact_integer_fields(value: Any) -> bool:
    ...
    return all(
        ...
        for key in (
            ...
            "torch_num_threads_observed",
            "torch_num_interop_threads_observed",
        )
    )
...
def _require_exact_calibration_filter_evidence_types(value: Any) -> None:
    if not isinstance(value, list) or len(value) != 5:
        raise _error("E0-MCALC calibration filter evidence type drifted")
    target = value[0]
    if (
        not isinstance(target, Mapping)
        or any(
            type(target.get(key)) is not int
            for key in (
        ...
        or not _is_exact_sha256(target.get("development_site_ids_sha256"))
    ):
        raise _error("E0-MCALC target filter evidence type drifted")
    for record in value[1:]:
        if (
            ...
            or not _is_exact_sha256(record.get("excluded_target_keys_sha256"))
        ):
            raise _error("E0-MCALC raw exclusion evidence type drifted")


def _require_exact_e7_sample_evidence_types(value: Any) -> None:
    if not isinstance(value, list) or len(value) != 45:
        raise _error("E0-MCALC E7 sample evidence type drifted")
    integer_fields = {
        "training_size",
        "base_seed",
        "eligible_rows",
    ...
    for record in value:
        if not isinstance(record, Mapping):
            raise _error("E0-MCALC E7 sample evidence type drifted")
        if any(
            key in record and type(record.get(key)) is not int
            for key in integer_fields
        ) or any(
            key in record and not _is_exact_sha256(record.get(key))
            for key in digest_fields
        ):
            raise _error("E0-MCALC E7 sample evidence scalar type drifted")
        for key in ("replacement", "replacement_used"):
            if key in record and type(record.get(key)) is not bool:
                raise _error("E0-MCALC E7 replacement flag type drifted")
        strata = record.get("strata")
        if strata is not None:
            if not isinstance(strata, list) or any(
                not isinstance(stratum, Mapping)
                or type(stratum.get("eligible_rows")) is not int
                or type(stratum.get("selected_rows")) is not int
                for stratum in strata
            ):
                raise _error("E0-MCALC E7 stratum count type drifted")
        derivation = record.get("strata_derivation")
        if derivation is not None:
            if not isinstance(derivation, Mapping):
                raise _error("E0-MCALC E7 strata derivation type drifted")
            cuts = derivation.get("expert_anchor_band_cuts")
            if (
                not isinstance(cuts, list)
                or len(cuts) != 4
                or any(type(cut) is not float for cut in cuts)
                or not _is_exact_sha256(
                    derivation.get("month_period_map_sha256")
                )
            ):
                raise _error("E0-MCALC E7 strata derivation scalar type drifted")
...
def _require_exact_output_group(
    ...
    *,
    ...
    """Validate an R group using only MCALC-local authority bindings."""
    ...
    if present and len(present) != len(paths):
        raise _error(f"E0-MCALC {context} output bundle is partial")
    if not present:
        return 0
    ...
    for path in paths:
        payload, observed = mcalp.mcal._read_regular_bytes_and_metadata(
            path, repo_root=repo_root
        )
        payloads[path] = payload
        metadata[path] = observed
        if path.suffix == ".json":
            value = mcalp.mcal._parse_json_bytes(
                payload, context=path.as_posix()
            )
            if payload != _canonical_json_bytes(value):
                raise _error(
                    f"E0-MCALC {context} JSON output is not canonical: "
                    f"{path.as_posix()}"
                )
            json_values[path] = value
    manifest = json_values.get(manifest_path)
    if not isinstance(manifest, Mapping):
        raise _error(f"E0-MCALC {context} manifest is absent")
    output_records = [
        {
            "path": path.as_posix(),
    ...
    if manifest.get("outputs") != output_records:
        raise _error(f"E0-MCALC {context} manifest output bindings drifted")
    ...
    if (
        ...
        raise _error(f"E0-MCALC {context} scientific boundary drifted")
    if context == "calibration":
        ...
        if (
            ...
            or manifest.get("scientific_boundary") != boundary
            or not isinstance(manifest.get("execution_policy"), Mapping)
        ):
            raise _error("E0-MCALC calibration manifest scientific dialect drifted")
        evidence = manifest.get("input_filter_evidence")
        if not isinstance(evidence, list):
            raise _error("E0-MCALC calibration input-filter evidence is absent")
        _require_exact_calibration_filter_evidence_types(evidence)
        try:
            mcalp.mcal._validate_calibration_filter_evidence(evidence)
        except mcalp.FinalCalibrationError as exc:
            raise _error(str(exc).replace("E0-MCAL", "E0-MCALC")) from exc
        ...
        if (
            ...
            != mcalp.mcal.EXPECTED_DEVELOPMENT_RUNTIME_SCHEMA_VERSION
            or execution.get("development_runtime_audit_sha256")
            != mcalp.mcal.EXPECTED_DEVELOPMENT_RUNTIME_AUDIT_SHA256
            or not _torch_policy_has_exact_integer_fields(
                execution.get("torch_cpu_execution_policy")
            )
        ):
            raise _error("E0-MCALC calibration execution policy drifted")
        try:
            mcalp.mcal._validate_torch_cpu_execution_policy(
                execution.get("torch_cpu_execution_policy"),
                context="calibration",
            )
        except mcalp.FinalCalibrationError as exc:
        ...
        calibrators = _validate_calibrator_specs(
            json_values.get(mcalp.mcal.CALIBRATOR_SPECS_PATH)
        )
        try:
            mcalp.mcal._validate_calibration_csv_outputs(
        ...
        slot_order = [
            ...
            for seed in mcalp.mcal.REGISTERED_SEEDS
        ]
        ...
        terminal_flag_keys = set(terminal) - terminal_count_keys
        if (
            ...
            or any(
                type(manifest.get(key)) is not int for key in terminal_count_keys
            )
            or any(
                type(manifest.get(key)) is not bool for key in terminal_flag_keys
            )
            ...
            or any(
                type(evidence.get(key)) is not int for key in terminal_count_keys
            )
            or any(
                type(evidence.get(key)) is not bool for key in terminal_flag_keys
            )
            ...
            raise _error("E0-MCALC E7 manifest scientific dialect drifted")
        ...
        if (
            ...
            raise _error("E0-MCALC E7 execution policy drifted")
        _require_exact_e7_sample_evidence_types(evidence.get("sample_evidence"))
        try:
            mcalp.mcal._validate_torch_cpu_execution_policy(
                execution.get("torch_cpu_execution_policy"), context="E7"
            )
            mcalp.mcal._validate_e7_csv_output(
                payloads[mcalp.mcal.ANFIS_LEARNING_CURVE_PATH],
                terminal_evidence=evidence,
            )
        except mcalp.FinalCalibrationError as exc:
            raise _error(str(exc).replace("E0-MCAL", "E0-MCALC")) from exc
    else:
        raise _error("E0-MCALC output group context drifted")
    for path in paths:
        payload, observed = mcalp.mcal._read_regular_bytes_and_metadata(
            path, repo_root=repo_root
        )
        ...
        if payload != payloads[path] or (
            ...
            raise _error(f"E0-MCALC {context} output changed during validation")
    return len(paths)
...
def _parse_canonical_json_with_metadata(
    ...
    try:
        ...
        value = mcalp.mcal._parse_json_bytes(payload, context=path.as_posix())
    except Exception as exc:
        raise _error(f"E0-MCALC published JSON is absent: {path.as_posix()}") from exc
    if not isinstance(value, dict) or payload != _canonical_json_bytes(value):
        raise _error(f"E0-MCALC published JSON is not canonical: {path.as_posix()}")
    return value, payload, metadata


def _published_h_state(h_head: str, *, repo_root: Path) -> dict[str, Any]:
    if h_head == BASE_P_MCALP_COMMIT or SHA1_RE.fullmatch(h_head) is None:
        raise _error("E0-MCALC published H commit is absent")
    if (
        mcalp.mcal._single_parent(repo_root, h_head, context="H-E0-MCALC")
        != BASE_P_MCALP_COMMIT
    ):
        raise _error("E0-MCALC published H parent drifted")
    scope = mcalp.mcal._git_scope(repo_root, BASE_P_MCALP_COMMIT, h_head)
    expected_scope = {
        "added": 5,
        "modified": 4,
        "deleted": 0,
    ...
    if scope != expected_scope:
        raise _error("E0-MCALC published H scope drifted")
    components = [
        mcalp.mcal._git_artifact_record(
            Path(path),
            role="final_calibration_candidate_semantics_patch_component",
...
def _validate_published_lock_payload(
    ...
    schema = mcalp.mcal._load_json_object(DEFAULT_PATCH_LOCK_SCHEMA, repo_root=repo_root)
    try:
        mcalp.mcal.validate_json_schema(payload, schema)
    except mcalp.mcal.ClosureContractError as exc:
        raise _error(str(exc)) from exc
    ...
    if payload.get("authorizations") != UNPUBLISHED_AUTHORIZATIONS:
        raise _error("E0-MCALC published authorizations drifted")
    repository = payload.get("repository")
    if not isinstance(repository, Mapping) or not isinstance(
        repository.get("h_patch_head"), str
    ):
        raise _error("E0-MCALC published H binding is absent")
    ...
    if _canonical_json_bytes(payload) != _canonical_json_bytes(expected):
        raise _error("E0-MCALC published lock reconstruction drifted")
...
def _validate_p_publication(
    ...
    head = mcalp.mcal._git_head(repo_root)
    workspace = mcalp.mcal._workspace_status_records(repo_root)
    ...
    if any(code != "??" or path not in allowed_r_paths for code, path in workspace):
        raise _error("E0-MCALC published P worktree drifted")
    if (
        cast(
            str, mcalp.mcal._git(repo_root, "branch", "--show-current")
        ).strip()
        != "main"
        or mcalp.mcal._single_parent(repo_root, head, context="P-E0-MCALC")
        != h_head
        or mcalp.mcal._git_scope(repo_root, h_head, head)
        != {
            "added": 2,
            "modified": 0,
            "deleted": 0,
        ...
        raise _error("E0-MCALC published P topology drifted")
    tracking = mcalp.mcal._git_head(repo_root, "origin/main")
    remote = (
        mcalp.mcal._live_remote_main_head(repo_root)
        if verify_remote
    ...
    if tracking != head or remote != head:
        raise _error("E0-MCALC published P refs drifted")
    for path in (DEFAULT_PATCH_LOCK_PATH, DEFAULT_PATCH_LOCK_MANIFEST_PATH):
        physical = mcalp.mcal._read_regular_bytes(path, repo_root=repo_root)
        mode, _ = mcalp.mcal._git_mode_oid(repo_root, head, path)
        if (
            mode != "100644"
            or physical != mcalp.mcal._git_blob_bytes(repo_root, head, path)
        ):
            raise _error(
                f"E0-MCALC published P physical/Git binding drifted: "
                f"{path.as_posix()}"
            )
...
def _require_static_effective_boundary(*, repo_root: Path) -> None:
    ...
    if missing:
        raise _error(f"E0-MCALC effective authority is absent: {missing}")
    forbidden = (
        mcalp.mcal.DEFAULT_PATCH_LOCK_PATH,
        mcalp.mcal.DEFAULT_PATCH_LOCK_MANIFEST_PATH,
        mcalp.mcal._temporary_path(mcalp.mcal.DEFAULT_PATCH_LOCK_PATH),
        mcalp.mcal._temporary_path(mcalp.mcal.DEFAULT_PATCH_LOCK_MANIFEST_PATH),
        mcalp.mcal._temporary_path(mcalp.DEFAULT_PATCH_LOCK_PATH),
        mcalp.mcal._temporary_path(mcalp.DEFAULT_PATCH_LOCK_MANIFEST_PATH),
        mcalp.mcal._temporary_path(DEFAULT_PATCH_LOCK_PATH),
        mcalp.mcal._temporary_path(DEFAULT_PATCH_LOCK_MANIFEST_PATH),
        mcalp.mcal.LOCKER_GUARD_PATH,
        mcalp.LOCKER_GUARD_PATH,
        LOCKER_GUARD_PATH,
    )
    ...
    if occupied:
        raise _error(f"E0-MCALC effective P namespace drifted: {occupied}")
    if mcalp.mcal._entry_exists(
        Path(mcalp.mcal.mze.OUTCOME_ACCESS_LOG), repo_root=repo_root
    ):
        raise _error("E0-MCALC outcome access log appeared")
    if any(
        mcalp.mcal._entry_exists(Path(path), repo_root=repo_root)
        for path in mcalp.mcal.mze.E0_M_PATHS
    ):
        raise _error("E0-MCALC final E0-M outputs appeared")
...
def _validate_effective_namespace(*, repo_root: Path) -> dict[str, Any]:
    forbidden = (
        mcalp.mcal._temporary_path(DEFAULT_PATCH_LOCK_PATH),
        mcalp.mcal._temporary_path(DEFAULT_PATCH_LOCK_MANIFEST_PATH),
        *(mcalp.mcal._temporary_path(path) for path in R_OUTPUT_PATHS),
        LOCKER_GUARD_PATH,
        mcalp.mcal.CALIBRATION_GUARD_PATH,
        mcalp.mcal.E7_GUARD_PATH,
    )
    ...
    if occupied:
        raise _error(
            "E0-MCALC effective coordination/temporary namespace is occupied: "
            f"{occupied}"
        )
    calibration_count = _require_exact_output_group(
        mcalp.mcal.CALIBRATION_OUTPUT_PATHS,
        manifest_path=mcalp.mcal.FINAL_CALIBRATION_MANIFEST_PATH,
        repo_root=repo_root,
        context="calibration",
    )
    e7_count = _require_exact_output_group(
        mcalp.mcal.E7_OUTPUT_PATHS,
        manifest_path=mcalp.mcal.ANFIS_LEARNING_CURVE_MANIFEST_PATH,
        repo_root=repo_root,
        context="E7",
    )
    if (calibration_count, e7_count) not in {(0, 0), (6, 0), (6, 2)}:
        raise _error("E0-MCALC R bundle order drifted")
    lifecycle = {
...
def _require_effective_loading_checkpoint(
    *,
    ...
    if expected_namespace.get("calibration_output_present_count") == 6:
        expected_r_paths.extend(mcalp.mcal.CALIBRATION_OUTPUT_PATHS)
    if expected_namespace.get("e7_output_present_count") == 2:
        expected_r_paths.extend(mcalp.mcal.E7_OUTPUT_PATHS)
    _require_repository_checkpoint(
        repo_root=repo_root,
    ...
    observed_namespace = _validate_effective_namespace(repo_root=repo_root)
    if _canonical_json_bytes(observed_namespace) != _canonical_json_bytes(
        expected_namespace
    ):
        raise _error(f"E0-MCALC R namespace changed {context}")


@_error_boundary
def load_effective_final_calibration_candidate_semantics_patch_authority(
    *, verify_remote: bool = True, repo_root: Path | None = None
    ...
    lock_record = mcalp.mcal._file_record(
        DEFAULT_PATCH_LOCK_PATH,
        role="final_calibration_candidate_semantics_patch_lock",
        repo_root=root,
    )
    ...
    if _canonical_json_bytes(companion) != _canonical_json_bytes(
        ...
        raise _error("E0-MCALC published companion drifted")
    ...
    authority_snapshot = _physical_snapshot(
        root, scientific_inventory=inventory
    )
    initial_head = mcalp.mcal._git_head(root)
    if initial_head != publication["p_patch_head"]:
        raise _error("E0-MCALC P refs do not match the captured baseline")
    _validate_published_lock_payload(payload, repo_root=root)
    recaptured_publication = _validate_p_publication(
        payload, verify_remote=verify_remote, repo_root=root
    )
    if recaptured_publication != publication:
        raise _error("E0-MCALC P publication changed during baseline recapture")
    ...
    base = mcalp.mcal._base_r_mze_authority(repo_root=root)
    ...
    result = {
        ...
        **publication,
        "lock": lock_record,
        "companion": mcalp.mcal._file_record(
            DEFAULT_PATCH_LOCK_MANIFEST_PATH,
            role="final_calibration_candidate_semantics_patch_lock_manifest",
            repo_root=root,
        ),
        ...
        "model_ids": list(mcalp.mcal.MODEL_IDS),
        "bloom_group_count": mcalp.mcal.BLOOM_GROUP_COUNT,
        "ordinal_group_count": mcalp.mcal.ORDINAL_GROUP_COUNT,
        "uncertainty_group_count": mcalp.mcal.UNCERTAINTY_GROUP_COUNT,
        "q_c_levels": list(mcalp.mcal.Q_C_LEVELS),
        ...
        "historical_e7_blockers": mcalp.mcal._historical_e7_blockers(
            repo_root=root
        ),
        "e7_authority_correction": {
            ...
            "final_runtime_path": mcalp.mcal.DEFAULT_RUNTIME_PATH.as_posix(),
        },
        ...
        **namespace,
        ...
        != "ready_for_calibration_bundle",
        "e7_one_shot_consumed": lifecycle
    ...
    if (
        ...
        raise _error("E0-MCALC P authority changed during effective loading")
    _require_effective_loading_checkpoint(
        repo_root=root,
        expected_head=initial_head,
        verify_remote=verify_remote,
    ...
    if (
        ...
        or (
            terminal_lock_metadata.st_dev,
            terminal_lock_metadata.st_ino,
            terminal_lock_metadata.st_mode,
            terminal_lock_metadata.st_nlink,
            terminal_lock_metadata.st_size,
            terminal_lock_metadata.st_mtime_ns,
            terminal_lock_metadata.st_ctime_ns,
        )
        != (
            lock_metadata.st_dev,
            lock_metadata.st_ino,
        ...
        or (
            terminal_companion_metadata.st_dev,
            terminal_companion_metadata.st_ino,
            terminal_companion_metadata.st_mode,
            terminal_companion_metadata.st_nlink,
            terminal_companion_metadata.st_size,
            terminal_companion_metadata.st_mtime_ns,
            terminal_companion_metadata.st_ctime_ns,
        )
        != (
            companion_metadata.st_dev,
            companion_metadata.st_ino,
        ...
        raise _error("E0-MCALC P authority changed at effective linearization")
    return result
...
def require_final_calibration_authority(
    *, verify_remote: bool = True, repo_root: Path | None = None
    ...
    return load_effective_final_calibration_candidate_semantics_patch_authority(
...
def require_final_calibration_run_namespace(
    *, runner: str, repo_root: Path | None = None
    ...
    if type(runner) is not str or runner not in {"calibration", "e7"}:
        raise _error("E0-MCALC run namespace requires calibration or e7")
    namespace = _validate_effective_namespace(repo_root=root)
    required_state = (
        "ready_for_calibration_bundle"
    ...
    if namespace["r_lifecycle_state"] != required_state:
        raise _error(f"E0-MCALC {runner} one-shot namespace is not ready")
    return {
        "gate": PATCH_GATE,
        "runner": runner,
...
def __getattr__(name: str) -> Any:
    """Delegate unchanged scientific/runtime surfaces to E0-MCALP."""

    return getattr(mcalp, name)
