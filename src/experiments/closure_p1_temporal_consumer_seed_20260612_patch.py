#!/usr/bin/env python
"""Validate the additive Closure V1 P1/20260612 temporal-consumer authority.

E0-MI is a fail-closed successor to the published P1/20260612 sequence
bundle.  It reconstructs E0-MG and E0-MH from immutable Git history, binds
the new sequence auditor without running it during H-E0-MI, and can authorize
only the P1/20260612/CPU consumer after a future two-file P-E0-MI publication.
...
from src.experiments import audit_closure_p1_seed_20260612_sequence_bundle as p1_audit
...
LOCK_VERSION = "closure_p1_temporal_consumer_seed_20260612_patch_lock_v1"
PATCH_GATE = "E0-MI"
PATCH_ID = "p1_temporal_consumer_seed_20260612_authority_patch_1"
PATCH_STATUS = "locked"
...
P1_BUNDLE_COMMIT = "b448e1fb0ee75b6135da11f0ea9a8877d89e0ee1"
...
PATCH_COMPONENT_ROLES = {
    DEFAULT_PATCH_LOCK_SCHEMA.as_posix(): (
        "p1_temporal_consumer_seed_20260612_patch_lock_schema"
    ),
    "docs/closure_v1/E0_M_P1_TEMPORAL_CONSUMER_SEED_20260612_PATCH_1.md": (
        "p1_temporal_consumer_seed_20260612_patch_protocol"
    ),
    "src/experiments/closure_p1_temporal_consumer_seed_20260612_patch.py": (
        "p1_temporal_consumer_seed_20260612_patch_validator"
    ),
    "src/experiments/lock_closure_p1_temporal_consumer_seed_20260612_patch.py": (
        "p1_temporal_consumer_seed_20260612_patch_locker"
    ),
    "src/experiments/train_closure_pipe.py": "p1_temporal_consumer_mi_gate_routing",
    "tests/test_closure_p1_temporal_consumer_seed_20260612_patch.py": (
        "p1_temporal_consumer_seed_20260612_patch_tests"
    ),
    "tests/test_train_closure_pipe.py": "p1_temporal_consumer_mi_gate_tests",
}
...
MG_SUPERSEDED_PATHS = PATCH_MODIFIED_PATHS
MG_PRESERVED_PATHS = tuple(
    path for path in e0_mg.PATCH_PATHS if path not in MG_SUPERSEDED_PATHS
)
...
PATCH_SEALS = {
    ...
    "mg_preserved_component_count": 5,
    "mg_superseded_component_count": 2,
    "mh_preserved_component_count": 7,
...
# Exact collection reserved for the future P-E0-MI execute-lock.
FOCUSED_TEST_COUNT = 118
...
AUDITOR_MODULE = "src.experiments.audit_closure_p1_seed_20260612_sequence_bundle"
AUDITOR_NAME = "audit_p1_seed_20260612_sequence_bundle"
AUDITOR_QUALNAME = "audit_p1_seed_20260612_sequence_bundle"
AUDITOR_SOURCE_PATH = p1_audit.AUDITOR_PATH.as_posix()


class P1TemporalConsumerSeed20260612PatchError(RuntimeError):
    """Raised when E0-MI cannot prove its closed authority."""


def _translate(error: BaseException) -> P1TemporalConsumerSeed20260612PatchError:
    return P1TemporalConsumerSeed20260612PatchError(str(error))
...
def _assert_schema_definition_supported(schema: Mapping[str, Any]) -> None:
    ...
    if not callable(validator):
        raise P1TemporalConsumerSeed20260612PatchError(
            "E0-MI closure-contract schema-definition validator is unavailable"
        )
    try:
        validator(schema)
...
def preflight_p1_temporal_consumer_seed_20260612_patch_schema(
    schema_path: Path = DEFAULT_PATCH_LOCK_SCHEMA,
) -> dict[str, Any]:
    """Validate the physical MI schema before guards, commands, or egress."""
    if schema_path != DEFAULT_PATCH_LOCK_SCHEMA:
        raise P1TemporalConsumerSeed20260612PatchError(
            "E0-MI schema preflight requires the closed default path"
        )
    schema = _load_regular_json(schema_path, context="E0-MI schema preflight")
    ...
    if minimum_count or format_count:
        raise P1TemporalConsumerSeed20260612PatchError(
            "E0-MI schema contains keywords outside the closed contract subset: "
            f"minimum={minimum_count}, format={format_count}"
        )
    _assert_schema_definition_supported(schema)
    record = _file_record(
        schema_path,
        role="p1_temporal_consumer_seed_20260612_patch_lock_schema",
    )
    if type(record["bytes"]) is not int or int(record["bytes"]) <= 0:
        raise P1TemporalConsumerSeed20260612PatchError(
            "E0-MI schema must be a non-empty regular JSON file"
        )
    return {
        "gate": PATCH_GATE,
...
def patch_git_diff_payload(patch_head: str) -> dict[str, Any]:
    patch_head = _require_commit(patch_head, context="H-E0-MI")
    ancestry = _git("rev-list", "--parents", "-n", "1", patch_head).split()
    if ancestry != [patch_head, PATCH_BASE_COMMIT]:
        raise P1TemporalConsumerSeed20260612PatchError(
            "H-E0-MI must be the direct non-merge child of P1/20260612"
        )
    expected = [
        {
    ...
    if observed != expected:
        raise P1TemporalConsumerSeed20260612PatchError(
            f"H-E0-MI diff differs from its closed 2M+5A allowlist: {observed}"
        )
    return {
...
def _historical_e0_mg_authority(*, execution_head: str) -> dict[str, Any]:
    """Reconstruct E0-MG with trainer and trainer tests explicitly superseded."""
    ...
    try:
        validate_json_schema(payload, schema, instance_path="$.historical_e0_mg_lock")
    except ClosureContractError as exc:
        raise _translate(exc) from exc
    fixed = {
    ...
    if repository.get("head") != E0_MG_H_COMMIT:
        raise P1TemporalConsumerSeed20260612PatchError(
            "Historical E0-MG H commit drifted"
        )
    git_diff = e0_mg.patch_git_diff_payload(E0_MG_H_COMMIT)
    ...
    if payload.get("git_diff") != git_diff or payload.get("patch_components") != components:
        raise P1TemporalConsumerSeed20260612PatchError(
            "Historical E0-MG Git authority drifted"
        )
    lock_record, companion_record = _published_lock_records(
        h_commit=E0_MG_H_COMMIT,
        p_commit=E0_MG_P_COMMIT,
        lock_path=e0_mg.DEFAULT_PATCH_LOCK_PATH,
        companion_path=e0_mg.DEFAULT_PATCH_MANIFEST_PATH,
        lock_role="external_p1_temporal_consumer_schema_subset_patch_lock",
        companion_role="p1_temporal_consumer_schema_subset_patch_companion",
    ...
    if companion != e0_mg._expected_companion(payload, lock_record=lock_record):
        raise P1TemporalConsumerSeed20260612PatchError(
            "Historical E0-MG companion drifted"
        )
    records = _component_records(cast(Mapping[str, Any], components))
    by_path = {str(record["path"]): record for record in records}
    if set(by_path) != set(e0_mg.PATCH_PATHS):
        raise P1TemporalConsumerSeed20260612PatchError(
            "Historical E0-MG component paths drifted"
        )
    superseded = [by_path[path] for path in MG_SUPERSEDED_PATHS]
    preserved = [by_path[path] for path in MG_PRESERVED_PATHS]
    for record in records:
        ...
        if record != expected:
            raise P1TemporalConsumerSeed20260612PatchError(
                f"Historical E0-MG Git record drifted: {record['path']}"
            )
...
def _historical_e0_mh_authority(*, execution_head: str) -> dict[str, Any]:
    ...
    payload = _load_regular_json(e0_mh.DEFAULT_PATCH_LOCK_PATH, context="P-E0-MH lock")
    schema = _load_regular_json(e0_mh.DEFAULT_PATCH_LOCK_SCHEMA, context="H-E0-MH schema")
    try:
        validate_json_schema(payload, schema, instance_path="$.historical_e0_mh_lock")
    except ClosureContractError as exc:
        raise _translate(exc) from exc
    fixed = {
    ...
    if repository.get("head") != E0_MH_H_COMMIT:
        raise P1TemporalConsumerSeed20260612PatchError(
            "Historical E0-MH H commit drifted"
        )
    git_diff = e0_mh.patch_git_diff_payload(E0_MH_H_COMMIT)
    components = e0_mh.patch_component_bundle(E0_MH_H_COMMIT)
    if payload.get("git_diff") != git_diff or payload.get("patch_components") != components:
        raise P1TemporalConsumerSeed20260612PatchError(
            "Historical E0-MH Git authority drifted"
        )
    lock_record, companion_record = _published_lock_records(
        h_commit=E0_MH_H_COMMIT,
        p_commit=E0_MH_P_COMMIT,
        lock_path=e0_mh.DEFAULT_PATCH_LOCK_PATH,
        companion_path=e0_mh.DEFAULT_PATCH_MANIFEST_PATH,
        lock_role="external_p1_sequence_seed_20260612_patch_lock",
        companion_role="p1_sequence_seed_20260612_patch_companion",
        execution_head=execution_head,
        context="P-E0-MH",
    )
    companion = _load_regular_json(
    ...
    if companion != e0_mh._expected_companion(payload, lock_record=lock_record):
        raise P1TemporalConsumerSeed20260612PatchError(
            "Historical E0-MH companion drifted"
        )
    records = _component_records(cast(Mapping[str, Any], components))
    if {str(record["path"]) for record in records} != set(e0_mh.PATCH_PATHS):
        raise P1TemporalConsumerSeed20260612PatchError(
    ...
    if not isinstance(base_authorities, Mapping) or set(base_authorities) != {
        ...
        raise P1TemporalConsumerSeed20260612PatchError(
            "Historical E0-MH base-authority dialect drifted"
        )
    sealed_mg = cast(Mapping[str, Any], base_authorities["e0_mg"])
    if (
        sealed_mg.get("gate") != "E0-MG"
        or sealed_mg.get("patch_head") != E0_MG_H_COMMIT
        or sealed_mg.get("lock_commit") != E0_MG_P_COMMIT
        or sealed_mg.get("effective_loader_called") is not False
        or sealed_mg.get("future_outcomes_accessed") is not False
    ):
        raise P1TemporalConsumerSeed20260612PatchError(
            "Historical E0-MH sealed E0-MG authority drifted"
        )
    builder = payload.get("current_runtime_builder_record")
    if builder != {**P1_ARTIFACT_BUILDER_RECORD, "role": "current_runtime_builder"}:
        raise P1TemporalConsumerSeed20260612PatchError(
            "Historical E0-MH runtime builder drifted"
        )
    return {
        "gate": "E0-MH",
        ...
        "publication_topology_verified": True,
        "historical_e0_mg_seal_verified": True,
        "historical_authority_verified": True,
        "effective_loader_called": False,
        "future_outcomes_accessed": False,
    }
...
def _published_p1_20260612_bundle(*, execution_head: str) -> dict[str, Any]:
    ...
    if ancestry != [P1_BUNDLE_COMMIT, E0_MH_P_COMMIT]:
        raise P1TemporalConsumerSeed20260612PatchError(
            "P1/20260612 publication topology drifted"
        )
    versioned_roles = {
        "data/closure_v1/development/sequences/P1/seed_20260612.parquet.dvc": (
            "p1_seed_20260612_sequence_dvc_pointer"
        ),
        "reports/closure_v1/01_surface/sequences/P1/seed_20260612_manifest.json": (
            "p1_seed_20260612_sequence_manifest"
        ),
        "reports/closure_v1/01_surface/sequences/P1/seed_20260612_summary.csv": (
            "p1_seed_20260612_sequence_summary"
        ),
        "src/experiments/audit_closure_p1_seed_20260612_sequence_bundle.py": (
            "p1_seed_20260612_sequence_auditor"
        ),
        "tests/test_audit_closure_p1_seed_20260612_sequence_bundle.py": (
            "p1_seed_20260612_sequence_auditor_tests"
        ),
    }
    expected_diff = [
        {"status": "A", "path": path} for path in sorted(versioned_roles)
    ]
    if _observed_diff_entries(E0_MH_P_COMMIT, P1_BUNDLE_COMMIT) != expected_diff:
        raise P1TemporalConsumerSeed20260612PatchError(
            "P1/20260612 publication scope drifted"
        )
    ...
    _assert_paths_untouched(
        ...
        context="P1/20260612 published bundle",
    )
    expected = {
        "data/closure_v1/development/sequences/P1/seed_20260612.parquet.dvc": (
            104,
        ...
        "src/experiments/audit_closure_p1_seed_20260612_sequence_bundle.py": (
            63_336,
            "b2e2c32d622626ba0da24047bd852254519349491ab69a1ecebc0dd304a34275",
        ),
        "tests/test_audit_closure_p1_seed_20260612_sequence_bundle.py": (
            26_242,
            "459742182157cae779ae439e4ad0f6f84ac57131d003d92b930810350bfc4959",
        ),
    }
...
def _auditor_identity_record() -> dict[str, Any]:
    callable_object = p1_audit.audit_p1_seed_20260612_sequence_bundle
...
def run_p1_bundle_audit_in_process() -> dict[str, Any]:
    """Invoke exactly the published P1/20260612 auditor in this process."""
    _auditor_identity_record()
    try:
        result = p1_audit.audit_p1_seed_20260612_sequence_bundle()
    except p1_audit.ClosureP1Seed20260612SequenceAuditError as exc:
        raise _translate(exc) from exc
...
def p1_consumer_namespace_absence() -> dict[str, Any]:
    ...
    if existing:
        raise P1TemporalConsumerSeed20260612PatchError(
            f"P1/20260612 consumer namespace is not empty: {existing}"
        )
    return {
...
def closure_progression_prelock() -> dict[str, Any]:
    ...
    if missing_required or unexpected_registered or forbidden_external:
        raise P1TemporalConsumerSeed20260612PatchError(
            "P1/20260612 progression prelock drifted: "
    ...
    return {
        ...
        "prior_residual_absent_count": 28 - len(prior),
...
def collect_p1_temporal_consumer_seed_20260612_patch_prelock_state(
    *,
    verify_remote: bool,
) -> dict[str, Any]:
    ...
    if status:
        raise P1TemporalConsumerSeed20260612PatchError(
            f"H-E0-MI lock requires a clean worktree: {status}"
        )
    head = _require_commit(_git("rev-parse", "HEAD"), context="H-E0-MI HEAD")
    if _git("branch", "--show-current") != "main":
        raise P1TemporalConsumerSeed20260612PatchError(
            "H-E0-MI lock requires branch main"
        )
    published = _require_commit(_git("rev-parse", PUBLISHED_REF), context=PUBLISHED_REF)
    if published != head:
        raise P1TemporalConsumerSeed20260612PatchError(
            "H-E0-MI HEAD differs from origin/main"
        )
    remote = _remote_main_oid() if verify_remote else published
    if remote != head:
        raise P1TemporalConsumerSeed20260612PatchError(
            "H-E0-MI HEAD differs from live origin/main"
        )
    ...
    for record in cast(Sequence[Mapping[str, Any]], components["records"]):
        if _file_record(Path(str(record["path"])), role=str(record["role"])) != record:
            raise P1TemporalConsumerSeed20260612PatchError(
                f"Physical H-E0-MI component drifted: {record['path']}"
            )
    historical_mg = _historical_e0_mg_authority(execution_head=head)
    historical_mh = _historical_e0_mh_authority(execution_head=head)
    published_bundle = _published_p1_20260612_bundle(execution_head=head)
    return {
        "patch_repository": {
            "head": head,
        ...
        "p1_20260612_publication": published_bundle,
...
def build_p1_temporal_consumer_seed_20260612_patch_lock_payload(
...
def _validate_command_evidence(
    ...
    *,
    ...
    if set(evidence).difference(required | optional):
        raise P1TemporalConsumerSeed20260612PatchError(
            f"E0-MI {context} evidence has unexpected fields"
        )
    if not required.issubset(evidence) or evidence.get("command") != list(command):
        raise P1TemporalConsumerSeed20260612PatchError(
            f"E0-MI {context} evidence drifted"
        )
    if evidence.get("returncode") != 0:
        raise P1TemporalConsumerSeed20260612PatchError(
            f"E0-MI {context} did not pass"
        )
    for field in ("stdout_line_count", "stderr_line_count"):
        value = evidence.get(field)
        if type(value) is not int or value < 0:
            raise P1TemporalConsumerSeed20260612PatchError(
                f"E0-MI {context} {field} must be a non-negative integer"
            )
...
def _validate_focused_duration_evidence(evidence: Mapping[str, Any]) -> None:
    ...
    if not isinstance(duration_text, str):
        raise P1TemporalConsumerSeed20260612PatchError(
            "E0-MI focused pytest duration evidence is malformed"
        )
    try:
    ...
    if duration < Decimal(60):
        ...
        try:
            ...
            raise P1TemporalConsumerSeed20260612PatchError(
                "E0-MI focused pytest duration evidence is out of range"
            ) from exc
        valid = summary_format == "pytest_timedelta" and clock == expected_clock
    if not valid:
        raise P1TemporalConsumerSeed20260612PatchError(
            "E0-MI focused pytest duration/clock evidence drifted"
        )
...
def _validate_audit_evidence_shape(evidence: Mapping[str, Any]) -> None:
    ...
    if drifted:
        raise P1TemporalConsumerSeed20260612PatchError(
            f"E0-MI in-process audit evidence drifted: {drifted}"
        )
    identity = _auditor_identity_record()
    ...
    if (
        ...
        raise P1TemporalConsumerSeed20260612PatchError(
            "E0-MI canonical audit hash evidence is malformed"
        )


def validate_p1_temporal_consumer_seed_20260612_patch_verification(
    verification: Mapping[str, Any],
) -> None:
    expected_fields = {
    ...
    if set(verification) != expected_fields:
        raise P1TemporalConsumerSeed20260612PatchError(
            "E0-MI verification fields drifted"
        )
    preflight = verification.get("schema_subset_preflight")
    if (
        not isinstance(preflight, Mapping)
        or dict(preflight)
        != preflight_p1_temporal_consumer_seed_20260612_patch_schema()
    ):
        raise P1TemporalConsumerSeed20260612PatchError(
            "E0-MI schema-subset preflight evidence drifted"
        )
    commands = {
    ...
    for field, command in commands.items():
        ...
        if not isinstance(value, Mapping):
            raise P1TemporalConsumerSeed20260612PatchError(
                f"E0-MI {field} evidence is malformed"
            )
    ...
    if (
        ...
        raise P1TemporalConsumerSeed20260612PatchError(
            "E0-MI focused-test evidence drifted"
        )
    ...
    if first.get("terminal_status") not in {
        ...
        raise P1TemporalConsumerSeed20260612PatchError(
            "E0-MI first targeted DVC push evidence drifted"
        )
    if second.get("terminal_status") != "Everything is up to date.":
        raise P1TemporalConsumerSeed20260612PatchError(
            "E0-MI second targeted DVC push is not idempotent"
        )
    audit = verification.get("p1_bundle_audit")
    if not isinstance(audit, Mapping):
        raise P1TemporalConsumerSeed20260612PatchError(
            "E0-MI in-process audit evidence is malformed"
        )
    _validate_audit_evidence_shape(audit)


def validate_p1_temporal_consumer_seed_20260612_patch_lock_payload(
    ...
    *,
    ...
    try:
        validate_json_schema(
            payload,
            schema,
            instance_path="$.p1_temporal_consumer_seed_20260612_patch_lock",
        )
    except ClosureContractError as exc:
        raise _translate(exc) from exc
    fixed = {
    ...
    for field, expected in fixed.items():
        if payload.get(field) != expected:
            raise P1TemporalConsumerSeed20260612PatchError(
                f"E0-MI fixed field drifted: {field}"
            )
    created = payload.get("created_at_utc")
    if not isinstance(created, str):
        raise P1TemporalConsumerSeed20260612PatchError("E0-MI timestamp is invalid")
    try:
        timestamp = datetime.fromisoformat(created.replace("Z", "+00:00"))
    except ValueError as exc:
        raise P1TemporalConsumerSeed20260612PatchError(
            "E0-MI timestamp is invalid"
        ) from exc
    if timestamp.utcoffset() is None:
        raise P1TemporalConsumerSeed20260612PatchError(
            "E0-MI timestamp requires a timezone"
        )
    repository = cast(Mapping[str, Any], payload["patch_repository"])
    patch_head = _require_commit(str(repository.get("head", "")), context="H-E0-MI")
    if repository != {
        "head": patch_head,
        "parent": PATCH_BASE_COMMIT,
        "branch": "main",
    ...
    if payload.get("base_authorities") != expected_authorities:
        raise P1TemporalConsumerSeed20260612PatchError(
            "E0-MI historical authorities drifted"
        )
    if payload.get("p1_20260612_publication") != _published_p1_20260612_bundle(
        execution_head=execution_head
    ):
        raise P1TemporalConsumerSeed20260612PatchError(
            "E0-MI P1/20260612 publication drifted"
        )
...
def _expected_companion(
    ...
    *,
    ...
    mg_authority = cast(Mapping[str, Any], authorities["e0_mg"])
    mh_authority = cast(Mapping[str, Any], authorities["e0_mh"])
    publication = cast(Mapping[str, Any], payload["p1_20260612_publication"])
    ...
    inputs = [
        ...
        *[
            ...
            for record in cast(
                ...
                cast(Mapping[str, Any], mg_authority["preserved_components"])["records"],
            )
        ],
        dict(cast(Mapping[str, Any], mg_authority["lock"])),
        dict(cast(Mapping[str, Any], mg_authority["companion_manifest"])),
        *[
            dict(record)
            for record in cast(
                Sequence[Mapping[str, Any]],
                cast(Mapping[str, Any], mh_authority["preserved_components"])["records"],
            )
        ],
        dict(cast(Mapping[str, Any], mh_authority["lock"])),
        dict(cast(Mapping[str, Any], mh_authority["companion_manifest"])),
        *[
            dict(record)
    ...
    historical_inputs = [
        {
            **dict(record),
            "commit": E0_MG_H_COMMIT,
            "hash_source": "git_blob_at_commit",
        }
        for record in cast(
            Sequence[Mapping[str, Any]],
            cast(Mapping[str, Any], mg_authority["superseded_components"])["records"],
        )
    ]
    historical_inputs.sort(key=lambda record: str(record["path"]))
    return {
        "manifest_version": (
            "closure_p1_temporal_consumer_seed_20260612_patch_manifest_v1"
        ),
        "status": "completed",
        "experiment_id": EXPERIMENT_ID,
        "surface_id": SURFACE_ID,
...
def _validate_publication_bundle(
    ...
    *,
    ...
    if _observed_diff_entries(patch_head, lock_commit) != expected:
        raise P1TemporalConsumerSeed20260612PatchError(
            "P-E0-MI must add exactly lock plus companion"
        )
    if _git("branch", "--show-current") != "main":
        raise P1TemporalConsumerSeed20260612PatchError(
            "E0-MI effective authority requires branch main"
        )
    if execution_head != lock_commit:
        raise P1TemporalConsumerSeed20260612PatchError(
            "E0-MI execution HEAD must equal the exact P-E0-MI lock commit"
        )
    ...
    if set(refs.values()) != {lock_commit}:
        raise P1TemporalConsumerSeed20260612PatchError(
            f"E0-MI local/tracking refs diverged: {refs}"
        )
    if verify_remote and _remote_main_oid() != lock_commit:
        raise P1TemporalConsumerSeed20260612PatchError(
            "E0-MI execution HEAD differs from live origin/main"
        )
    return (
        lock_commit,
        _git_record(
...
def load_and_validate_p1_temporal_consumer_seed_20260612_patch_lock(
    ...
    *,
    ...
    if (
        ...
        raise P1TemporalConsumerSeed20260612PatchError(
            "E0-MI requires closed default paths"
        )
    payload = _load_regular_json(lock_path, context="E0-MI lock")
    schema = _load_regular_json(lock_schema, context="E0-MI schema")
    # Static/Git validation deliberately precedes any physical P1 audit.
    validate_p1_temporal_consumer_seed_20260612_patch_lock_payload(
        payload,
        schema,
    ...
    companion = _load_regular_json(companion_path, context="E0-MI companion")
    if companion != _expected_companion(payload, lock_record=lock_record):
        raise P1TemporalConsumerSeed20260612PatchError("E0-MI companion drifted")
    ...
    if require_published:
        ...
        if status:
            raise P1TemporalConsumerSeed20260612PatchError(
                f"E0-MI execution requires a clean worktree: {status}"
            )
        ...
        if execution_head != patch_head:
            raise P1TemporalConsumerSeed20260612PatchError(
                "Unpublished E0-MI validation must run at H-E0-MI"
            )
        validate_p1_temporal_consumer_seed_20260612_patch_lock_payload(
            payload,
            schema,
    ...
    mh_lock_record = dict(cast(Mapping[str, Any], mh_authority["lock"]))
    mh_companion_record = dict(
        cast(Mapping[str, Any], mh_authority["companion_manifest"])
    )
    ...
    summary = {
        "status": (
            "published_p1_temporal_consumer_seed_20260612_patch_valid"
            if effective
...
def require_p1_temporal_consumer_seed_20260612_patch_authorized(
    *,
    model_id: str,
    base_seed: int,
    device: str,
) -> dict[str, Any]:
    """Require published E0-MI for exactly the P1/20260612/CPU consumer."""
    if (
        model_id != AUTHORIZED_MODEL_ID
        ...
        raise P1TemporalConsumerSeed20260612PatchError(
            "E0-MI authorizes only the P1 seed 20260612 CPU temporal consumer"
        )
    _, summary = load_and_validate_p1_temporal_consumer_seed_20260612_patch_lock(
    ...
    if failed:
        raise P1TemporalConsumerSeed20260612PatchError(
            f"E0-MI authorization predicates failed: {failed}"
        )
    required_false = (
    ...
    if drifted:
        raise P1TemporalConsumerSeed20260612PatchError(
            f"E0-MI fail-closed seals drifted: {drifted}"
        )
    if summary.get("fit_availability") != FIT_AVAILABILITY:
        raise P1TemporalConsumerSeed20260612PatchError(
            "E0-MI fit availability drifted"
        )
    if summary.get("schema_subset_preflight_evidence") != (
        preflight_p1_temporal_consumer_seed_20260612_patch_schema()
    ):
        raise P1TemporalConsumerSeed20260612PatchError(
            "E0-MI schema-subset preflight summary drifted"
        )
    if summary.get("state_consumer_authority") is not None:
        raise P1TemporalConsumerSeed20260612PatchError(
            "E0-MI must not apply the historical seed-1729 ANFIS exception"
        )
...
__all__ = [
    ...
    "DIFF_CHECK_COMMAND",
    "P1TemporalConsumerSeed20260612PatchError",
    "build_p1_temporal_consumer_seed_20260612_patch_lock_payload",
    "collect_p1_temporal_consumer_seed_20260612_patch_prelock_state",
    "load_and_validate_p1_temporal_consumer_seed_20260612_patch_lock",
    ...
    "validate_p1_temporal_consumer_seed_20260612_patch_lock_payload",
    "validate_p1_temporal_consumer_seed_20260612_patch_verification",
]
