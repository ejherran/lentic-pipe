...
P1_SEQUENCE_20260613_POINTER_PATH = Path(
...
P1_SEQUENCE_20260613_AUDITOR_PATH = Path(
...
P1_20260613_AUTHORITY_SOURCE_PATHS = (
    *P1_20260612_AUTHORITY_SOURCE_PATHS,
    E0_MJ_GATE_PATH,
...
P1_FIT_FAILURE_REASON_COUNTS = {"missing_target_state": 488}
P1_CALIBRATION_FAILURE_COUNT = 17
...
def builder_records_from_p1_temporal_consumer_schema_subset_authority(
    authority: Mapping[str, Any],
) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    observed_runtime = _file_record(PROJECT_ROOT / SEQUENCE_BUILDER_PATH)
    if current_runtime != observed_runtime:
        raise ClosurePipeTrainingError("Current runtime builder differs from E0-MG authority")
...
def _p1_schema_subset_authority_input_records(
    authority: Mapping[str, Any],
) -> tuple[dict[str, Any], ...]:
    ...
    if (
        ...
        raise ClosurePipeTrainingError("E0-MG authority input records drifted")
    expected = (
        (
            E0_MG_LOCK_PATH.as_posix(),
            "external_p1_temporal_consumer_schema_subset_patch_lock",
        ),
        (
            E0_MG_MANIFEST_PATH.as_posix(),
            "p1_temporal_consumer_schema_subset_patch_companion",
        ),
    )
    ...
    for raw, (expected_path, expected_role) in zip(raw_records, expected, strict=True):
        if not isinstance(raw, Mapping) or set(raw) != {
            ...
            raise ClosurePipeTrainingError("E0-MG authority input record dialect drifted")
        record = cast(Mapping[str, Any], raw)
        if record.get("path") != expected_path or record.get("role") != expected_role:
            raise ClosurePipeTrainingError("E0-MG authority input path/role drifted")
        ...
        if any(
            ...
            raise ClosurePipeTrainingError(
                f"E0-MG authority input differs from physical bytes: {expected_path}"
            )
        observed.append(physical)
    return tuple(observed)


def validate_p1_temporal_consumer_schema_subset_authority(
    authority: Mapping[str, Any],
    *,
    model_id: str,
    base_seed: int,
    ...
    """Validate the effective E0-MG summary before any P1 dependency I/O."""
    expected = {
        "gate": "E0-MG",
    ...
    if drifted:
        raise ClosurePipeTrainingError(f"E0-MG authorization predicates drifted: {drifted}")
    if (model_id, base_seed, device) != ("P1", 1729, "cpu"):
        raise ClosurePipeTrainingError("E0-MG authorizes only P1 seed 1729 on cpu")
    schema_preflight = authority.get("schema_subset_preflight_evidence")
    expected_schema_preflight_fields = {
        "gate",
        "schema_path",
        "schema_bytes",
        "schema_sha256",
    ...
    if not isinstance(schema_preflight, Mapping) or set(schema_preflight) != (
        expected_schema_preflight_fields
    ):
        raise ClosurePipeTrainingError("E0-MG schema-subset preflight evidence drifted")
    schema_bytes = schema_preflight.get("schema_bytes")
    schema_sha256 = schema_preflight.get("schema_sha256")
    if (
        schema_preflight.get("gate") != "E0-MG"
        or schema_preflight.get("schema_path") != E0_MG_SCHEMA_PATH.as_posix()
    ...
    if fit_availability != expected_fit_availability:
        raise ClosurePipeTrainingError("E0-MG fit-availability contract drifted")
    in_process_audit = authority.get("in_process_audit_evidence")
    if not isinstance(in_process_audit, Mapping):
        raise ClosurePipeTrainingError("E0-MG lacks in-process audit evidence")
    expected_audit = {
    ...
    audit_drifted = [
        ...
        if in_process_audit.get(field) != value
    ]
    result_bytes = in_process_audit.get("result_bytes")
    result_sha256 = in_process_audit.get("result_sha256")
    source_git = in_process_audit.get("callable_source_git")
    source_physical = in_process_audit.get("callable_source_physical")
    if (
        audit_drifted
        ...
        raise ClosurePipeTrainingError(
            f"E0-MG in-process audit evidence drifted: {audit_drifted}"
        )
    e0_mf_context = authority.get("e0_mf_context_authorization")
    expected_e0_mf_context = {
        "gate": "E0-MF",
    ...
    if not isinstance(e0_mf_context, Mapping) or any(
        e0_mf_context.get(field) != value
        for field, value in expected_e0_mf_context.items()
    ):
        raise ClosurePipeTrainingError("E0-MG lacks its E0-MF context authorization")
    ...
    if not isinstance(e0_me_context, Mapping) or any(
        ...
        raise ClosurePipeTrainingError("E0-MG lacks its E0-ME context authorization")
    ...
    if (
        ...
        raise ClosurePipeTrainingError("E0-MG lacks its E0-MD context authorization")
    ...
    if not isinstance(e0_mc_context, Mapping) or e0_mc_context.get("gate") != "E0-MC":
        raise ClosurePipeTrainingError("E0-MG lacks its E0-MC context authorization")
    artifact, current_runtime = (
        builder_records_from_p1_temporal_consumer_schema_subset_authority(authority)
    )
    authority_inputs = _p1_schema_subset_authority_input_records(authority)
...
def builder_records_from_p1_seed_20260613_temporal_consumer_authority(
    authority: Mapping[str, Any],
) -> tuple[dict[str, Any], dict[str, Any]]:
    """Bind the P1/20260613 artifact builder to the unchanged live builder."""
    artifact = _authority_file_record(
        authority.get("p1_artifact_builder_record"),
        field="p1_artifact_builder_record",
    )
    ...
    observed_runtime = _file_record(PROJECT_ROOT / SEQUENCE_BUILDER_PATH)
    if current_runtime != observed_runtime:
        raise ClosurePipeTrainingError("Current runtime builder differs from E0-MK authority")
    if artifact != current_runtime:
        raise ClosurePipeTrainingError(
            "P1/20260613 artifact builder differs from the E0-MK runtime builder"
        )
    return artifact, current_runtime

def validate_p1_temporal_consumer_seed_20260613_authority(
    authority: Mapping[str, Any],
    *,
    model_id: str,
    base_seed: int,
    ...
    """Validate effective E0-MK before any sequence, model, or output I/O."""
    expected = {
        "gate": "E0-MK",
        "publication_verified": True,
        "remote_publication_verified": True,
    ...
    drifted = [field for field, value in expected.items() if authority.get(field) != value]
    if drifted:
        raise ClosurePipeTrainingError(f"E0-MK authorization predicates drifted: {drifted}")
    if (model_id, base_seed, device) != ("P1", 20_260_613, "cpu"):
        raise ClosurePipeTrainingError("E0-MK authorizes only P1 seed 20260613 on cpu")
    if authority.get("state_consumer_authority") is not None:
        raise ClosurePipeTrainingError("E0-MK state consumer authority must be null")
    ...
    if not isinstance(schema_preflight, Mapping) or set(schema_preflight) != expected_schema_fields:
        raise ClosurePipeTrainingError("E0-MK schema-subset preflight evidence drifted")
    schema_bytes = schema_preflight.get("schema_bytes")
    schema_sha256 = schema_preflight.get("schema_sha256")
    if (
        schema_preflight.get("gate") != "E0-MK"
        or schema_preflight.get("schema_path") != E0_MK_SCHEMA_PATH.as_posix()
        or type(schema_bytes) is not int
        or schema_bytes <= 0
        or not isinstance(schema_sha256, str)
        or len(schema_sha256) != 64
    ...
    if fit_availability != expected_fit_availability:
        raise ClosurePipeTrainingError("E0-MK fit-availability contract drifted")

    audit = authority.get("in_process_audit_evidence")
    if not isinstance(audit, Mapping):
        raise ClosurePipeTrainingError("E0-MK lacks in-process audit evidence")
    expected_audit = {
        "execution_mode": "in_process_callable",
        "callable_module": (
    ...
    if (
        ...
        or not isinstance(source_physical, Mapping)
        or source_git != source_physical
        or source_git.get("path") != P1_SEQUENCE_20260613_AUDITOR_PATH.as_posix()
        or type(result_bytes) is not int
        or result_bytes <= 0
        or not isinstance(result_sha256, str)
        or len(result_sha256) != 64
        ...
        raise ClosurePipeTrainingError(
            f"E0-MK in-process audit evidence drifted: {audit_drifted}"
        )

    artifact, current = builder_records_from_p1_seed_20260613_temporal_consumer_authority(
        authority
    )
    sequence_inputs = _closed_temporal_authority_input_records(
        authority,
        field="sequence_authority_input_records",
        expected=(
            (E0_MJ_LOCK_PATH.as_posix(), "external_p1_sequence_seed_20260613_patch_lock"),
            (E0_MJ_MANIFEST_PATH.as_posix(), "p1_sequence_seed_20260613_patch_companion"),
        ),
    )
    consumer_inputs = _closed_temporal_authority_input_records(
        authority,
...
def builder_records_from_p1_seed_20260614_temporal_consumer_authority(
    authority: Mapping[str, Any],
) -> tuple[dict[str, Any], dict[str, Any]]:
    """Bind the P1/20260614 artifact builder to the unchanged live builder."""
    artifact = _authority_file_record(
        authority.get("p1_artifact_builder_record"),
        field="p1_artifact_builder_record",
    )
    ...
    observed_runtime = _file_record(PROJECT_ROOT / SEQUENCE_BUILDER_PATH)
    if current_runtime != observed_runtime:
        raise ClosurePipeTrainingError("Current runtime builder differs from E0-MM authority")
    if artifact != current_runtime:
        raise ClosurePipeTrainingError(
            "P1/20260614 artifact builder differs from the E0-MM runtime builder"
        )
    return artifact, current_runtime


def validate_p1_temporal_consumer_seed_20260614_authority(
    authority: Mapping[str, Any],
    *,
    model_id: str,
    base_seed: int,
    ...
    """Validate effective E0-MM before any sequence, model, or output I/O."""
    expected = {
        "gate": "E0-MM",
        "publication_verified": True,
        "remote_publication_verified": True,
    ...
    drifted = [field for field, value in expected.items() if authority.get(field) != value]
    if drifted:
        raise ClosurePipeTrainingError(f"E0-MM authorization predicates drifted: {drifted}")
    if (model_id, base_seed, device) != ("P1", 20_260_614, "cpu"):
        raise ClosurePipeTrainingError("E0-MM authorizes only P1 seed 20260614 on cpu")
    if authority.get("state_consumer_authority") is not None:
        raise ClosurePipeTrainingError("E0-MM state consumer authority must be null")
    ...
    if not isinstance(schema_preflight, Mapping) or set(schema_preflight) != expected_schema_fields:
        raise ClosurePipeTrainingError("E0-MM schema-subset preflight evidence drifted")
    schema_bytes = schema_preflight.get("schema_bytes")
    schema_sha256 = schema_preflight.get("schema_sha256")
    if (
        schema_preflight.get("gate") != "E0-MM"
        or schema_preflight.get("schema_path") != E0_MM_SCHEMA_PATH.as_posix()
        or type(schema_bytes) is not int
        or schema_bytes <= 0
        or not isinstance(schema_sha256, str)
        or len(schema_sha256) != 64
    ...
    if fit_availability != expected_fit_availability:
        raise ClosurePipeTrainingError("E0-MM fit-availability contract drifted")

    audit = authority.get("in_process_audit_evidence")
    if not isinstance(audit, Mapping):
        raise ClosurePipeTrainingError("E0-MM lacks in-process audit evidence")
    expected_audit = {
        "execution_mode": "in_process_callable",
        "callable_module": (
    ...
    if (
        ...
        or not isinstance(source_physical, Mapping)
        or source_git != source_physical
        or source_git.get("path") != P1_SEQUENCE_20260614_AUDITOR_PATH.as_posix()
        or type(result_bytes) is not int
        or result_bytes <= 0
        or not isinstance(result_sha256, str)
        or len(result_sha256) != 64
        ...
        raise ClosurePipeTrainingError(
            f"E0-MM in-process audit evidence drifted: {audit_drifted}"
        )

    artifact, current = builder_records_from_p1_seed_20260614_temporal_consumer_authority(
        authority
    )
    sequence_inputs = _closed_temporal_authority_input_records(
        authority,
        field="sequence_authority_input_records",
        expected=(
            (E0_ML_LOCK_PATH.as_posix(), "external_p1_sequence_seed_20260614_patch_lock"),
            (E0_ML_MANIFEST_PATH.as_posix(), "p1_sequence_seed_20260614_patch_companion"),
        ),
    )
    consumer_inputs = _closed_temporal_authority_input_records(
        authority,
        ...
        expected=(
            (
                E0_MM_LOCK_PATH.as_posix(),
                "external_p1_temporal_consumer_seed_20260614_patch_lock",
            ),
            (
                E0_MM_MANIFEST_PATH.as_posix(),
                "p1_temporal_consumer_seed_20260614_patch_companion",
            ),
        ),
    )
    return artifact, current, None, sequence_inputs, consumer_inputs
...
def collect_sequence_input_contract(
    *,
    ...
    state_available, _, diagnostic_state_declared = validate_state_slot_manifest(
        ...
        consumer_authority=state_consumer_authority,
    )
...
def collect_temporal_model_input_contract(
    *,
    ...
    dependency_paths = (
        ...
        *(p1_dependency_paths if model_id == "P1" else ()),
        *source_paths,
    )
    records = {
    ...
    for raw_record in authority_input_records:
        if not isinstance(raw_record, Mapping) or set(raw_record) != {
            "path",
            "bytes",
            "sha256",
        }:
            raise ClosurePipeTrainingError("Temporal authority input record dialect drifted")
        path = PROJECT_ROOT / str(raw_record["path"])
        record = _file_record(path)
        if dict(raw_record) != record:
            raise ClosurePipeTrainingError(
                f"Temporal authority input differs from physical bytes: {record['path']}"
            )
        existing = records.get(str(record["path"]))
        if existing is not None and existing != record:
            raise ClosurePipeTrainingError(
    ...
    *,
    ...
    if args.model_id == "P1":
        (
            ...
            else validate_p1_temporal_consumer_seed_20260613_authority(
                p1_temporal_consumer_authority,
                model_id=args.model_id,
                base_seed=args.base_seed,
                device=args.device,
            ...
            else validate_p1_temporal_consumer_seed_20260612_authority(
                p1_temporal_consumer_authority,
                model_id=args.model_id,
                base_seed=args.base_seed,
                device=args.device,
        ...
        p1_authority_source_paths = (
            ...
            if args.base_seed == 20_260_613
            else P1_20260612_AUTHORITY_SOURCE_PATHS
        )
    else:
    ...
    if args.model_id == "P1":
        if availability.available:
            raise ClosurePipeTrainingError(
                "The sealed P1 authority forbids fitting because fit rows are unavailable"
            )
        if (
            ...
            raise ClosurePipeTrainingError(
                "P1 unavailable-fit evidence differs from its closed authority"
            )
    if not availability.available:
...
def main() -> None:
    ...
    with _temporal_slot_guard(args.model_id, args.base_seed):
        _run_temporal_slot(
            args=args,
            paths=paths,
            p1_temporal_consumer_authority=p1_temporal_consumer_authority,
        )
