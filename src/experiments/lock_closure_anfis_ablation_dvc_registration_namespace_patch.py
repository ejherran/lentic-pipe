#!/usr/bin/env python
"""Check and publish the outcome-closed E0-MZB namespace authority."""
...
def _count(value: Any, name: str) -> int:
    ...
    raise patch.AnfisAblationDvcRegistrationNamespacePatchError(
        f"E0-MZB check-only summary is missing exact integer: {name}"
    )
...
def _missing_pointer_validation(value: Any) -> dict[str, Any]:
    ...
    if not isinstance(value, Mapping) or not patch._exact_equal(dict(value), expected):
        raise patch.AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB missing-pointer validation contract drifted"
        )
    return expected


def _in_progress_namespace_validation(value: Any) -> dict[str, Any]:
    """Require exact public endpoints and the guarded transaction prefix policy."""
    ...
    if not isinstance(value, Mapping) or not patch._exact_equal(dict(value), expected):
        raise patch.AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB in-progress namespace validation contract drifted"
        )
    return expected


def _namespace_correction(value: Any) -> dict[str, Any]:
    """Require the exact failed-first-pointer incident and completed rollback."""

    first_target = patch._selection_payload_paths()[0].as_posix()
    first_pointer = patch._selection_pointer_paths()[0].as_posix()
    expected = {
        "blocked_gate": "E0-MZA",
        "published_p_mza_head": patch.P_MZA_COMMIT,
        ...
        "family_records_sha256": patch.FAMILY_RECORDS_SHA256,
        "dvc_push_run": False,
    ...
    if not isinstance(value, Mapping) or not patch._exact_equal(dict(value), expected):
        raise patch.AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB namespace incident/rollback contract drifted"
        )
    return expected


def check_only() -> dict[str, Any]:
    """Run the schema-first, non-writing H-E0-MZB preflight."""

    schema = patch.preflight_anfis_ablation_dvc_registration_namespace_patch_schema()
    prelock = patch.collect_anfis_ablation_dvc_registration_namespace_patch_prelock_state(
        verify_remote=True
    )
    ...
    namespace_correction = _namespace_correction(prelock.get("namespace_correction"))
    ...
    if counts != expected_counts:
        raise patch.AnfisAblationDvcRegistrationNamespacePatchError(
            f"E0-MZB check-only counts drifted: {counts}"
        )
    return {
...
def check_effective() -> dict[str, Any]:
    """Inspect the exact published registration-namespace authorization."""

    return patch.load_effective_anfis_ablation_dvc_registration_namespace_patch_authority(
        verify_remote=True
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except patch.AnfisAblationDvcRegistrationNamespacePatchError as exc:
