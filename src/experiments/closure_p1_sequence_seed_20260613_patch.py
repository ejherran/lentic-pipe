#!/usr/bin/env python
"""Validate the additive Closure V1 P1/20260613 sequence authority.

E0-MJ is a builder-only successor to the published P1/20260612 unavailable
consumer evidence.  It reconstructs E0-MH and E0-MI from immutable Git
history, preserves the P1/1729 bundle, binds the published P1/20260612
sequence+consumer bundle and ANFIS state for seed 20260613, and authorizes one
future P1 sequence build for seed 20260613.  It never authorizes training,
...
from src.experiments import closure_p1_sequence_seed_20260612_patch as e0_mh
...
E0_MH_H_COMMIT = "16662cdd5f258a1aa035c0f192145b3bd4d30fab"
E0_MH_P_COMMIT = "f8a1589b48865c5380584270d5ad83b23e5475a4"
E0_MI_H_COMMIT = "e8efbf800e1b171ba70bd3a863aec64138b6b0fb"
E0_MI_P_COMMIT = "16ee69f8fa923e4b7a9466024d4240b39a1a20d3"
P1_1729_CONSUMER_COMMIT = e0_mh.P1_1729_CONSUMER_COMMIT
P1_1729_SEQUENCE_COMMIT = e0_mh.P1_1729_SEQUENCE_COMMIT
...
PATCH_BASE_COMMIT = P1_20260612_CONSUMER_COMMIT
ANFIS_20260613_COMMIT = "f197780c3b5d215b45738ce425fd5ff639d76bab"
ANFIS_20260613_PARENT_COMMIT = "b16b27d5e38afae493202bba7576bb0c4f52283e"
...
DEFAULT_PATCH_LOCK_SCHEMA = Path(
    "configs/closure_v1/p1_sequence_seed_20260613_patch_lock.schema.json"
)

PATCH_COMPONENT_ROLES = {
    DEFAULT_PATCH_LOCK_SCHEMA.as_posix(): "p1_sequence_seed_20260613_patch_lock_schema",
    "docs/closure_v1/E0_M_P1_SEQUENCE_SEED_20260613_PATCH_1.md": (
        "p1_sequence_seed_20260613_patch_protocol"
    ),
    "src/experiments/build_closure_pipe_sequences.py": (
        "p1_sequence_seed_20260613_gate_routing"
    ),
    "src/experiments/closure_p1_sequence_seed_20260613_patch.py": (
        "p1_sequence_seed_20260613_patch_validator"
    ),
    "src/experiments/lock_closure_p1_sequence_seed_20260613_patch.py": (
        "p1_sequence_seed_20260613_patch_locker"
    ),
    "tests/test_build_closure_pipe_sequences.py": (
        "p1_sequence_seed_20260613_builder_gate_tests"
    ),
    "tests/test_closure_p1_sequence_seed_20260613_patch.py": (
        "p1_sequence_seed_20260613_patch_tests"
    ),
}
...
E0_MH_SUPERSEDED_PATHS = PATCH_MODIFIED_PATHS
E0_MH_PRESERVED_PATHS = tuple(
    path for path in e0_mh.PATCH_PATHS if path not in E0_MH_SUPERSEDED_PATHS
)
...
FOCUSED_TEST_COMMAND = (
    ...
    "tests/test_closure_p1_sequence_seed_20260613_patch.py",
    "tests/test_build_closure_pipe_sequences.py",
    "tests/test_closure_p1_temporal_consumer_seed_20260612_patch.py",
    "-q",
)
# Exact closed collection for the three H-E0-MJ focal files above.  Earlier
# one-shot suites encode pre-publication namespaces and remain historical
# evidence after the two published P1 sequence+consumer bundles.
FOCUSED_TEST_COUNT = 125
...
ANFIS_20260613_STATE_PATH = Path(
    "data/closure_v1/development/anfis/seed_20260613/"
    "adaptive_no_current_state.parquet"
)
ANFIS_20260613_POINTER_PATH = Path(
    f"{ANFIS_20260613_STATE_PATH.as_posix()}.dvc"
)
ANFIS_20260613_MANIFEST_PATH = Path(
...
EXPECTED_ARTIFACTS = {
    ...
    ANFIS_20260613_STATE_PATH.as_posix(): (1_215_359, "53f9a733e660d1968f8cf45916b1947ffa902346c16fabe2f07df9d27c4618a2"),
    ANFIS_20260613_POINTER_PATH.as_posix(): (116, "7afddc7b3b22d07e9630a90d851e3d7cc5972b0d94904678cf49eee0168a9e93"),
    ANFIS_20260613_MANIFEST_PATH.as_posix(): (20_881, "c40c9ef87729d135933ef37ffce7236523b6b8fe957d90e33dde4837e5a2bd00"),
}

PATCH_CORRECTION = {
    "issue_id": "p1_sequence_next_seed_authority_1",
    "classification": "ordered_seed_progression_authority_only",
    "historical_e0_mh_effective_loader_called": False,
...
class P1SequenceSeed20260613PatchError(RuntimeError):
    """Raised when E0-MJ cannot prove its closed next-seed authority."""


def _translate(error: BaseException) -> P1SequenceSeed20260613PatchError:
    return P1SequenceSeed20260613PatchError(str(error))
...
def _git(*args: str) -> str:
    try:
        return e0_mh._git(*args)
    except e0_mh.P1SequenceSeed20260612PatchError as exc:
        raise _translate(exc) from exc


def _require_commit(value: str, *, context: str) -> str:
    try:
        return e0_mh._require_commit(value, context=context)
    except e0_mh.P1SequenceSeed20260612PatchError as exc:
        raise _translate(exc) from exc
...
def _require_ancestor(ancestor: str, descendant: str) -> None:
    try:
        e0_mh._require_ancestor(ancestor, descendant)
    except e0_mh.P1SequenceSeed20260612PatchError as exc:
        raise _translate(exc) from exc
...
def _git_blob(commit: str, path: str) -> bytes:
    try:
        return e0_mh._git_blob(commit, path)
    except e0_mh.P1SequenceSeed20260612PatchError as exc:
        raise _translate(exc) from exc
...
def _file_record(path: Path, *, role: str) -> dict[str, Any]:
    try:
        return e0_mh._file_record(path, role=role)
    except e0_mh.P1SequenceSeed20260612PatchError as exc:
        raise _translate(exc) from exc
...
def _load_regular_json(path: Path, *, context: str) -> dict[str, Any]:
    try:
        return e0_mh._load_regular_json(path, context=context)
    except e0_mh.P1SequenceSeed20260612PatchError as exc:
        raise _translate(exc) from exc
...
def preflight_p1_sequence_seed_20260613_patch_schema(
    schema_path: Path = DEFAULT_PATCH_LOCK_SCHEMA,
) -> dict[str, Any]:
    """Prove the physical E0-MJ schema is supported before heavy checks."""
    if schema_path != DEFAULT_PATCH_LOCK_SCHEMA:
        raise P1SequenceSeed20260613PatchError(
            "E0-MJ schema preflight requires the closed default path"
        )
    schema = _load_regular_json(schema_path, context="E0-MJ schema preflight")
    ...
    if type(record["bytes"]) is not int or record["bytes"] <= 0:
        raise P1SequenceSeed20260613PatchError(
            "E0-MJ schema must be a non-empty regular JSON file"
        )
    return {
        "gate": PATCH_GATE,
...
def _canonical_json_record(
    ...
    *,
    ...
    try:
        return e0_mh._canonical_json_record(
            payload,
            path,
            role=role,
            context=context,
...
def _introduced_commit(path: str) -> str:
    try:
        return e0_mh._introduced_commit(path)
    except e0_mh.P1SequenceSeed20260612PatchError as exc:
        raise _translate(exc) from exc
...
def _observed_diff_entries(base: str, head: str) -> list[dict[str, str]]:
    try:
        return e0_mh._observed_diff_entries(base, head)
    except e0_mh.P1SequenceSeed20260612PatchError as exc:
        raise _translate(exc) from exc
...
def _assert_paths_untouched(
    ...
    *,
    ...
    try:
        e0_mh._assert_paths_untouched(base, descendant, paths, context=context)
    except e0_mh.P1SequenceSeed20260612PatchError as exc:
        raise _translate(exc) from exc
...
def patch_git_diff_payload(patch_head: str) -> dict[str, Any]:
    patch_head = _require_commit(patch_head, context="H-E0-MJ")
    ancestry = _git("rev-list", "--parents", "-n", "1", patch_head).split()
    if ancestry != [patch_head, PATCH_BASE_COMMIT]:
        raise P1SequenceSeed20260613PatchError(
            "H-E0-MJ must be the direct non-merge child of P1/20260612"
        )
    expected = [
    ...
    if observed != expected:
        raise P1SequenceSeed20260613PatchError(
            f"H-E0-MJ diff differs from its closed 2M+5A allowlist: {observed}"
        )
    return {
...
def _historical_e0_mh_authority(*, execution_head: str) -> dict[str, Any]:
    payload = _load_regular_json(
        e0_mh.DEFAULT_PATCH_LOCK_PATH,
        context="P-E0-MH lock",
    )
    schema = _load_regular_json(
        e0_mh.DEFAULT_PATCH_LOCK_SCHEMA,
        context="H-E0-MH schema",
    )
    try:
        ...
        raise _translate(exc) from exc
    fixed = {
        "lock_version": e0_mh.LOCK_VERSION,
        "status": e0_mh.PATCH_STATUS,
        "experiment_id": e0_mh.EXPERIMENT_ID,
        "surface_id": e0_mh.SURFACE_ID,
        "gate": e0_mh.PATCH_GATE,
        "patch_id": e0_mh.PATCH_ID,
        "authorizations": e0_mh.PATCH_AUTHORIZATIONS,
        "seals": e0_mh.PATCH_SEALS,
    }
    if any(payload.get(field) != expected for field, expected in fixed.items()):
        raise P1SequenceSeed20260613PatchError(
            "Historical E0-MH fixed contract drifted"
        )
    repository = cast(Mapping[str, Any], payload["patch_repository"])
    if repository.get("head") != E0_MH_H_COMMIT:
        raise P1SequenceSeed20260613PatchError("Historical E0-MH H commit drifted")
    if payload.get("git_diff") != e0_mh.patch_git_diff_payload(E0_MH_H_COMMIT):
        raise P1SequenceSeed20260613PatchError("Historical E0-MH diff drifted")
    expected_components = e0_mh.patch_component_bundle(E0_MH_H_COMMIT)
    if payload.get("patch_components") != expected_components:
        raise P1SequenceSeed20260613PatchError(
            "Historical E0-MH component bundle drifted"
        )
    ancestry = _git("rev-list", "--parents", "-n", "1", E0_MH_P_COMMIT).split()
    if ancestry != [E0_MH_P_COMMIT, E0_MH_H_COMMIT]:
        raise P1SequenceSeed20260613PatchError("P-E0-MH topology drifted")
    expected_publication = [
        {"status": "A", "path": e0_mh.DEFAULT_PATCH_LOCK_PATH.as_posix()},
        {"status": "A", "path": e0_mh.DEFAULT_PATCH_MANIFEST_PATH.as_posix()},
    ]
    if _observed_diff_entries(E0_MH_H_COMMIT, E0_MH_P_COMMIT) != expected_publication:
        raise P1SequenceSeed20260613PatchError("P-E0-MH scope drifted")
    _require_ancestor(E0_MH_P_COMMIT, execution_head)
    _assert_paths_untouched(
        E0_MH_P_COMMIT,
        execution_head,
        (
            e0_mh.DEFAULT_PATCH_LOCK_PATH.as_posix(),
            e0_mh.DEFAULT_PATCH_MANIFEST_PATH.as_posix(),
        ),
    ...
    if set(by_path) != set(e0_mh.PATCH_PATHS) or len(by_path) != len(records):
        raise P1SequenceSeed20260613PatchError(
            "Historical E0-MH component paths drifted"
        )
    superseded = [by_path[path] for path in E0_MH_SUPERSEDED_PATHS]
    preserved = [by_path[path] for path in E0_MH_PRESERVED_PATHS]
    for record in records:
        if record != _git_record(
            ...
            raise P1SequenceSeed20260613PatchError(
                f"Historical E0-MH Git record drifted: {record['path']}"
            )
    for record in preserved:
        if _file_record(
            ...
            raise P1SequenceSeed20260613PatchError(
                f"Preserved E0-MH component drifted: {record['path']}"
            )
    _assert_paths_untouched(
        E0_MH_H_COMMIT,
        execution_head,
        E0_MH_PRESERVED_PATHS,
        context="preserved H-E0-MH components",
    )
    ...
    companion_record = _file_record(
        e0_mh.DEFAULT_PATCH_MANIFEST_PATH,
    ...
    if lock_record != _git_record(
        ...
        raise P1SequenceSeed20260613PatchError(
            "Published E0-MH lock bundle drifted"
        )
    companion = _load_regular_json(
        e0_mh.DEFAULT_PATCH_MANIFEST_PATH,
        context="P-E0-MH companion",
    )
    try:
        expected_companion = e0_mh._expected_companion(
            payload,
    ...
    if companion != expected_companion:
        raise P1SequenceSeed20260613PatchError(
            "Published E0-MH companion drifted"
        )
    return {
        "gate": "E0-MH",
        "patch_head": E0_MH_H_COMMIT,
        "lock_commit": E0_MH_P_COMMIT,
        "lock": lock_record,
...
def _historical_e0_mi_authority(*, execution_head: str) -> dict[str, Any]:
    ...
    if cast(Mapping[str, Any], payload["patch_repository"]).get("head") != E0_MI_H_COMMIT:
        raise P1SequenceSeed20260613PatchError("Historical E0-MI H commit drifted")
    if payload.get("git_diff") != e0_mi.patch_git_diff_payload(E0_MI_H_COMMIT):
        raise P1SequenceSeed20260613PatchError("Historical E0-MI diff drifted")
    components = e0_mi.patch_component_bundle(E0_MI_H_COMMIT)
    if payload.get("patch_components") != components:
        raise P1SequenceSeed20260613PatchError(
            "Historical E0-MI component bundle drifted"
        )
    ancestry = _git("rev-list", "--parents", "-n", "1", E0_MI_P_COMMIT).split()
    if ancestry != [E0_MI_P_COMMIT, E0_MI_H_COMMIT]:
        raise P1SequenceSeed20260613PatchError("P-E0-MI topology drifted")
    expected_publication = [
        {"status": "A", "path": e0_mi.DEFAULT_PATCH_LOCK_PATH.as_posix()},
        {"status": "A", "path": e0_mi.DEFAULT_PATCH_MANIFEST_PATH.as_posix()},
    ]
    if _observed_diff_entries(E0_MI_H_COMMIT, E0_MI_P_COMMIT) != expected_publication:
        raise P1SequenceSeed20260613PatchError("P-E0-MI scope drifted")
    _require_ancestor(E0_MI_P_COMMIT, execution_head)
    _assert_paths_untouched(
        E0_MI_P_COMMIT,
        execution_head,
        (
            e0_mi.DEFAULT_PATCH_LOCK_PATH.as_posix(),
            e0_mi.DEFAULT_PATCH_MANIFEST_PATH.as_posix(),
        ),
        context="P-E0-MI publication",
    )
    lock_record = _file_record(
        e0_mi.DEFAULT_PATCH_LOCK_PATH,
    ...
    if lock_record != _git_record(
        ...
        raise P1SequenceSeed20260613PatchError(
            "Published E0-MI lock bundle drifted"
        )
    companion = _load_regular_json(
        e0_mi.DEFAULT_PATCH_MANIFEST_PATH,
    ...
    try:
        expected_companion = e0_mi._expected_companion(
            payload,
            lock_record=lock_record,
        )
    except e0_mi.P1TemporalConsumerSeed20260612PatchError as exc:
        raise _translate(exc) from exc
    if companion != expected_companion:
        raise P1SequenceSeed20260613PatchError("Historical E0-MI companion drifted")
    records = [
        dict(record)
        for record in cast(Sequence[Mapping[str, Any]], components["records"])
    ]
...
def _published_p1_1729_bundle(*, execution_head: str) -> dict[str, Any]:
    ...
    if sequence_ancestry != [P1_1729_SEQUENCE_COMMIT, e0_mh.E0_MC_P_COMMIT]:
        raise P1SequenceSeed20260613PatchError(
    ...
    if _observed_diff_entries(e0_mh.E0_MC_P_COMMIT, P1_1729_SEQUENCE_COMMIT) != (
        expected_sequence_diff
    ):
        raise P1SequenceSeed20260613PatchError(
    ...
    if ancestry != [P1_1729_CONSUMER_COMMIT, e0_mh.E0_MG_P_COMMIT]:
        raise P1SequenceSeed20260613PatchError(
    ...
    if _observed_diff_entries(e0_mh.E0_MG_P_COMMIT, P1_1729_CONSUMER_COMMIT) != expected_diff:
        raise P1SequenceSeed20260613PatchError(
...
def _anfis_20260613_state_bundle(*, execution_head: str) -> dict[str, Any]:
    ancestry = _git("rev-list", "--parents", "-n", "1", ANFIS_20260613_COMMIT).split()
    if ancestry != [ANFIS_20260613_COMMIT, ANFIS_20260613_PARENT_COMMIT]:
        raise P1SequenceSeed20260613PatchError(
            "ANFIS 20260613 publication topology drifted"
        )
    _require_ancestor(ANFIS_20260613_COMMIT, execution_head)
    tracked = (
        ANFIS_20260613_POINTER_PATH.as_posix(),
        ANFIS_20260613_MANIFEST_PATH.as_posix(),
    )
    _assert_paths_untouched(
        ANFIS_20260613_COMMIT,
        execution_head,
        tracked,
        context="ANFIS 20260613 publication",
    )
    records = [
        _expected_artifact_record(
            ANFIS_20260613_STATE_PATH,
            role="anfis_20260613_state_parquet",
        ),
        _expected_artifact_record(
            ANFIS_20260613_POINTER_PATH,
            role="anfis_20260613_state_pointer",
        ),
        _expected_artifact_record(
            ANFIS_20260613_MANIFEST_PATH,
    ...
    for record in records[1:]:
        if _git_record(
            ...
            raise P1SequenceSeed20260613PatchError(
                f"ANFIS 20260613 Git record drifted: {record['path']}"
            )
    manifest = _load_regular_json(
        ANFIS_20260613_MANIFEST_PATH,
        context="ANFIS 20260613 manifest",
    )
    expected_fields = {
    ...
    if drifted:
        raise P1SequenceSeed20260613PatchError(
            f"ANFIS 20260613 manifest drifted: {drifted}"
        )
    state_expected = EXPECTED_ARTIFACTS[ANFIS_20260613_STATE_PATH.as_posix()]
    outputs = manifest.get("outputs")
    if not isinstance(outputs, Sequence) or isinstance(outputs, (str, bytes)):
        raise P1SequenceSeed20260613PatchError("ANFIS 20260613 outputs drifted")
    state_matches = [
        record
        for record in outputs
        if isinstance(record, Mapping)
        and record.get("path") == ANFIS_20260613_STATE_PATH.as_posix()
        and record.get("bytes") == state_expected[0]
        and record.get("sha256") == state_expected[1]
    ]
    if len(state_matches) != 1:
        raise P1SequenceSeed20260613PatchError(
            "ANFIS 20260613 manifest does not bind its state exactly once"
        )
    return {
        "publication_commit": ANFIS_20260613_COMMIT,
        "records": records,
        "records_sha256": _record_digest(records),
        "slot_status": "available",
        "fit_status": "passed",
...
def p1_seed_20260613_namespace_absence() -> dict[str, Any]:
    contract = _sequence_prelock_contract()
    existing = [
        path.as_posix()
        for path in p1_seed_20260613_namespace_paths()
        if _path_entry_exists(path)
    ]
    if existing:
        raise P1SequenceSeed20260613PatchError(
            f"P1 seed 20260613 namespace is not pristine: {existing}"
        )
    return contract
...
def _progression_prelock_contract() -> dict[str, Any]:
    ...
    present_paths = (*P1_1729_PRESENT_PATHS, *P1_20260612_PRESENT_PATHS)
    ...
    prior_residuals = [
        path.as_posix()
        for seed in expected_seed_order[:2]
        for path in availability._p1_absence_paths(seed)
        if path.as_posix() not in present_set
    ]
    ...
    if (
        ...
        or len(present) != 12
        or len(absent) != 128
        or len(prior_residuals) != 44
        or len(target_paths) != 28
        or len(later_paths) != 56
    ):
        raise P1SequenceSeed20260613PatchError(
    ...
    return {
        ...
        "completed_seeds": [1729, 20_260_612],
...
def closure_progression_prelock() -> dict[str, Any]:
    ...
    expected = {
        path.as_posix()
        for path in (*P1_1729_PRESENT_PATHS, *P1_20260612_PRESENT_PATHS)
    }
    existing_e0_m = [
    ...
    if observed != expected or existing_e0_m or outcome:
        raise P1SequenceSeed20260613PatchError(
            "Closure progression is not pristine before P1 seed 20260613: "
...
# preserve the ordered progression (1729 and 20260612 closed; 20260613 absent).
def p1_sequence_namespace_absence() -> dict[str, Any]:
    return p1_seed_20260613_namespace_absence()
...
def collect_p1_sequence_seed_20260613_patch_prelock_state(
    *,
    verify_remote: bool,
) -> dict[str, Any]:
    ...
    if status:
        raise P1SequenceSeed20260613PatchError(
            f"H-E0-MJ lock requires a clean worktree: {status}"
        )
    head = _require_commit(_git("rev-parse", "HEAD"), context="H-E0-MJ HEAD")
    if _git("branch", "--show-current") != "main":
        raise P1SequenceSeed20260613PatchError("H-E0-MJ requires branch main")
    published = _require_commit(_git("rev-parse", PUBLISHED_REF), context=PUBLISHED_REF)
    if published != head:
        raise P1SequenceSeed20260613PatchError("H-E0-MJ HEAD differs from origin/main")
    remote = _remote_main_oid() if verify_remote else published
    if remote != head:
        raise P1SequenceSeed20260613PatchError(
            "H-E0-MJ HEAD differs from live origin/main"
        )
    ...
    for record in cast(Sequence[Mapping[str, Any]], components["records"]):
        if _file_record(Path(str(record["path"])), role=str(record["role"])) != record:
            raise P1SequenceSeed20260613PatchError(
                f"Physical H-E0-MJ component drifted: {record['path']}"
            )
    builder = next(
        dict(record)
        for record in cast(Sequence[Mapping[str, Any]], components["records"])
    ...
    return {
        ...
        "base_authorities": {
            "e0_mh": _historical_e0_mh_authority(execution_head=head),
            "e0_mi": _historical_e0_mi_authority(execution_head=head),
        },
        "p1_1729_publication": _published_p1_1729_bundle(execution_head=head),
        "p1_20260612_publication": _published_p1_20260612_bundle(
            execution_head=head
        ),
        "anfis_20260613_state_bundle": _anfis_20260613_state_bundle(
            execution_head=head
        ),
        "current_runtime_builder_record": builder,
        "sequence_prelock": p1_seed_20260613_namespace_absence(),
...
def _validate_command_evidence(
    ...
    *,
    ...
    if not isinstance(value, Mapping):
        raise P1SequenceSeed20260613PatchError(
            f"E0-MJ {context} evidence must be an object"
        )
    expected_keys = {
        "command",
        "returncode",
    ...
    if set(value) != expected_keys or tuple(value.get("command", ())) != tuple(command):
        raise P1SequenceSeed20260613PatchError(
            f"E0-MJ {context} command evidence drifted"
        )
    if value.get("returncode") != 0:
        raise P1SequenceSeed20260613PatchError(f"E0-MJ {context} did not pass")
    for field in ("stdout_sha256", "stderr_sha256"):
        digest = value.get(field)
        if not isinstance(digest, str) or len(digest) != 64:
            raise P1SequenceSeed20260613PatchError(
                f"E0-MJ {context} {field} drifted"
            )
...
def validate_p1_sequence_seed_20260613_patch_verification(
    payload: Mapping[str, Any],
) -> None:
    commands = {
    ...
    if set(payload) != {*commands, "schema_subset_preflight"}:
        raise P1SequenceSeed20260613PatchError(
            "E0-MJ verification fields drifted"
        )
    schema_preflight = payload.get("schema_subset_preflight")
    if (
        not isinstance(schema_preflight, Mapping)
        or dict(schema_preflight)
        != preflight_p1_sequence_seed_20260613_patch_schema()
    ):
        raise P1SequenceSeed20260613PatchError(
            "E0-MJ schema preflight evidence drifted"
        )
    ...
    if (
        ...
        raise P1SequenceSeed20260613PatchError(
            "E0-MJ focused-test evidence drifted"
        )


def build_p1_sequence_seed_20260613_patch_lock_payload(
    ...
    *,
    ...
    return {
        ...
        "anfis_20260613_state_bundle": dict(
            cast(Mapping[str, Any], prelock["anfis_20260613_state_bundle"])
        ),
...
def validate_p1_sequence_seed_20260613_patch_lock_payload(
    payload: Mapping[str, Any],
    ...
    *,
    ...
    try:
        validate_json_schema(
            payload,
            schema,
            instance_path="$.p1_sequence_seed_20260613_patch_lock",
        )
    except ClosureContractError as exc:
        raise _translate(exc) from exc
    fixed = {
    ...
    for field, expected in fixed.items():
        if payload.get(field) != expected:
            raise P1SequenceSeed20260613PatchError(
                f"E0-MJ fixed field drifted: {field}"
            )
    created = payload.get("created_at_utc")
    if not isinstance(created, str):
        raise P1SequenceSeed20260613PatchError("E0-MJ timestamp is invalid")
    try:
        timestamp = datetime.fromisoformat(created.replace("Z", "+00:00"))
    except ValueError as exc:
        raise P1SequenceSeed20260613PatchError("E0-MJ timestamp is invalid") from exc
    if timestamp.utcoffset() is None:
        raise P1SequenceSeed20260613PatchError(
            "E0-MJ timestamp requires a timezone"
        )
    repository = cast(Mapping[str, Any], payload["patch_repository"])
    patch_head = _require_commit(str(repository.get("head", "")), context="H-E0-MJ")
    if repository != {
        "head": patch_head,
        "parent": PATCH_BASE_COMMIT,
        "branch": "main",
        ...
        raise P1SequenceSeed20260613PatchError(
            "E0-MJ patch repository record drifted"
        )
    if payload.get("git_diff") != patch_git_diff_payload(patch_head):
        raise P1SequenceSeed20260613PatchError("E0-MJ Git diff drifted")
    components = patch_component_bundle(patch_head)
    if payload.get("patch_components") != components:
        raise P1SequenceSeed20260613PatchError("E0-MJ component bundle drifted")
    ...
    if require_physical_patch_components:
        ...
        _assert_paths_untouched(
            ...
            execution_head,
            PATCH_PATHS,
            context="H-E0-MJ components",
        )
    expected_authorities = {
        "e0_mh": _historical_e0_mh_authority(execution_head=execution_head),
        "e0_mi": _historical_e0_mi_authority(execution_head=execution_head),
    }
    if payload.get("base_authorities") != expected_authorities:
        raise P1SequenceSeed20260613PatchError(
            "E0-MJ historical authorities drifted"
        )
    if payload.get("p1_1729_publication") != _published_p1_1729_bundle(
        execution_head=execution_head
    ):
        raise P1SequenceSeed20260613PatchError("P1/1729 publication drifted")
    if payload.get("p1_20260612_publication") != _published_p1_20260612_bundle(
        execution_head=execution_head
    ):
        raise P1SequenceSeed20260613PatchError("P1/20260612 publication drifted")
    if payload.get("anfis_20260613_state_bundle") != _anfis_20260613_state_bundle(
        execution_head=execution_head
    ):
        raise P1SequenceSeed20260613PatchError("ANFIS 20260613 bundle drifted")
    builder_records = [
        record
        for record in cast(Sequence[Mapping[str, Any]], components["records"])
        if record["path"] == "src/experiments/build_closure_pipe_sequences.py"
    ...
    if len(builder_records) != 1:
        raise P1SequenceSeed20260613PatchError("E0-MJ builder record is not unique")
    ...
    if payload.get("current_runtime_builder_record") != expected_builder:
        raise P1SequenceSeed20260613PatchError("E0-MJ current builder drifted")
    if payload.get("sequence_prelock") != _sequence_prelock_contract():
        raise P1SequenceSeed20260613PatchError("E0-MJ sequence prelock drifted")
    if payload.get("progression_prelock") != _progression_prelock_contract():
        raise P1SequenceSeed20260613PatchError("E0-MJ progression prelock drifted")
    validate_p1_sequence_seed_20260613_patch_verification(
...
def _expected_companion(
    ...
    *,
    ...
    mh = cast(Mapping[str, Any], authorities["e0_mh"])
    mi = cast(Mapping[str, Any], authorities["e0_mi"])
    ...
    state = cast(Mapping[str, Any], payload["anfis_20260613_state_bundle"])
    inputs = [
        _generic_record(
        ...
        _generic_record(
            by_path["src/experiments/closure_p1_sequence_seed_20260613_patch.py"],
        ...
        dict(cast(Mapping[str, Any], mh["lock"])),
        dict(cast(Mapping[str, Any], mh["companion_manifest"])),
        dict(cast(Mapping[str, Any], mi["lock"])),
        dict(cast(Mapping[str, Any], mi["companion_manifest"])),
        *[
            dict(record)
            for record in cast(
    ...
    historical_inputs = [
        {
            **dict(record),
            "commit": E0_MH_H_COMMIT,
            "hash_source": "git_blob_at_commit",
        }
        for record in cast(
            Sequence[Mapping[str, Any]],
            cast(Mapping[str, Any], mh["superseded_components"])["records"],
        )
    ]
    ...
    return {
        ...
        "script": _generic_record(
            by_path["src/experiments/lock_closure_p1_sequence_seed_20260613_patch.py"],
...
def _validate_p_commit_topology(
    ...
    *,
    ...
    if lock_commit != _introduced_commit(companion_path):
        raise P1SequenceSeed20260613PatchError("E0-MJ lock commits differ")
    ancestry = _git("rev-list", "--parents", "-n", "1", lock_commit).split()
    if ancestry != [lock_commit, patch_head]:
        raise P1SequenceSeed20260613PatchError(
            "P-E0-MJ must be the direct child of H-E0-MJ"
        )
    expected = [
    ...
    if _observed_diff_entries(patch_head, lock_commit) != expected:
        raise P1SequenceSeed20260613PatchError(
            "P-E0-MJ must add exactly lock plus companion"
        )
    for path in (lock_path, companion_path):
        ...
        if (
            ...
            raise P1SequenceSeed20260613PatchError(
                f"P-E0-MJ publication mode drifted: {path}"
            )
...
def _load_unpublished_p1_sequence_seed_20260613_patch_lock(
) -> tuple[dict[str, Any], dict[str, Any]]:
    payload = _load_regular_json(DEFAULT_PATCH_LOCK_PATH, context="E0-MJ lock")
    schema = _load_regular_json(DEFAULT_PATCH_LOCK_SCHEMA, context="E0-MJ schema")
    validate_p1_sequence_seed_20260613_patch_lock_payload(payload, schema)
    ...
    companion = _load_regular_json(
        DEFAULT_PATCH_MANIFEST_PATH,
        context="E0-MJ companion",
    )
    if companion != _expected_companion(payload, lock_record=lock_record):
        raise P1SequenceSeed20260613PatchError("E0-MJ companion drifted")
    execution_head = _require_commit(_git("rev-parse", "HEAD"), context="execution HEAD")
    patch_head = str(cast(Mapping[str, Any], payload["patch_repository"])["head"])
    if execution_head != patch_head:
        raise P1SequenceSeed20260613PatchError(
            "Unpublished E0-MJ validation must run at H-E0-MJ"
        )
    return payload, {
...
def load_published_p1_sequence_seed_20260613_patch_historical_authority(
) -> tuple[dict[str, Any], dict[str, Any]]:
    payload = _load_regular_json(DEFAULT_PATCH_LOCK_PATH, context="E0-MJ lock")
    schema = _load_regular_json(DEFAULT_PATCH_LOCK_SCHEMA, context="E0-MJ schema")
    validate_p1_sequence_seed_20260613_patch_lock_payload(
        payload,
        schema,
    ...
    if lock_record != _canonical_json_record(
        ...
        raise P1SequenceSeed20260613PatchError("Published E0-MJ lock drifted")
    companion = _load_regular_json(
        DEFAULT_PATCH_MANIFEST_PATH,
        context="E0-MJ companion",
    )
    if companion != _expected_companion(payload, lock_record=lock_record):
        raise P1SequenceSeed20260613PatchError("Published E0-MJ companion drifted")
    if companion_record != _canonical_json_record(
        ...
        raise P1SequenceSeed20260613PatchError(
            "Published E0-MJ companion record drifted"
        )
    return payload, {
        "status": "published_p1_sequence_seed_20260613_historical_authority_valid",
        "gate": PATCH_GATE,
...
def load_and_validate_p1_sequence_seed_20260613_patch_lock(
) -> tuple[dict[str, Any], dict[str, Any]]:
    sequence_before = p1_seed_20260613_namespace_absence()
    ...
    if status:
        raise P1SequenceSeed20260613PatchError(
            f"E0-MJ execution requires a clean worktree: {status}"
        )
    lock_commit, lock_record, companion_record = _validate_p_commit_topology(
        payload,
        execution_head=execution_head,
    ...
    if execution_head != lock_commit:
        raise P1SequenceSeed20260613PatchError(
            "E0-MJ execution requires HEAD at the exact P commit"
        )
    if _git("branch", "--show-current") != "main":
        raise P1SequenceSeed20260613PatchError("E0-MJ requires branch main")
    refs = {
    ...
    if set(refs.values()) != {lock_commit}:
        raise P1SequenceSeed20260613PatchError(
            f"E0-MJ publication refs diverged: {refs}"
        )
    if _remote_main_oid() != lock_commit:
        raise P1SequenceSeed20260613PatchError(
            "E0-MJ differs from live origin/main"
        )
    if lock_record != _canonical_json_record(
        payload,
        DEFAULT_PATCH_LOCK_PATH,
    ...
    if p1_seed_20260613_namespace_absence() != sequence_before:
        raise P1SequenceSeed20260613PatchError(
            "E0-MJ sequence namespace changed during gate"
        )
    if closure_progression_prelock() != progression_before:
        raise P1SequenceSeed20260613PatchError(
            "E0-MJ progression namespace changed during gate"
        )
    return payload, {
        "status": "published_p1_sequence_seed_20260613_patch_valid",
        "gate": PATCH_GATE,
        "patch_head": payload["patch_repository"]["head"],
        "lock_commit": lock_commit,
        "execution_head": execution_head,
        ...
        "anfis_20260613_state_verified": True,
...
def require_p1_sequence_seed_20260613_authorized(
    model_id: str,
    base_seed: int | None,
) -> dict[str, Any]:
    if model_id != AUTHORIZED_MODEL_ID or base_seed != AUTHORIZED_BASE_SEED:
        raise P1SequenceSeed20260613PatchError(
            "E0-MJ authorizes only the one-shot P1 sequence build for seed 20260613"
        )
    _, summary = load_and_validate_p1_sequence_seed_20260613_patch_lock()
    required_true = (
    ...
    if failed:
        raise P1SequenceSeed20260613PatchError(
            f"E0-MJ authorization predicates failed: {failed}"
        )
    required_false = (
    ...
    if drifted:
        raise P1SequenceSeed20260613PatchError(
            f"E0-MJ fail-closed seals drifted: {drifted}"
        )
    p1_seed_20260613_namespace_absence()
