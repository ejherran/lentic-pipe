#!/usr/bin/env python
"""Check, publish, and audit the E0-MIB input-bundle lock authority."""
...
from src.experiments import closure_locked_evaluation_input_bundle as patch  # noqa: E402
...
def _error(message: str) -> patch.ClosureLockedEvaluationInputBundleError:
    return patch.ClosureLockedEvaluationInputBundleError(message)
...
def _parse_focused_summary(stdout: str, stderr: str) -> dict[str, int]:
    ...
    if type(count) is not int or count <= 0:
        raise _error("E0-MIB focused-test count is not frozen")
...
def run_closure_locked_evaluation_input_bundle_verification(
    *, expected_schema_preflight: Mapping[str, Any] | None = None
) -> dict[str, Any]:
    """Run the closed H checks without input-bundle science, DVC, or outcomes."""

    schema_preflight = patch.preflight_closure_locked_evaluation_input_bundle_schema()
    if (
        expected_schema_preflight is not None
        and dict(expected_schema_preflight) != schema_preflight
    ):
        raise _error("E0-MIB schema changed before verification")
    ...
    publication_guard, stdout, stderr = _run_command(patch.PUBLICATION_GUARD_COMMAND)
...
def check_only() -> dict[str, Any]:
    """Capture the H prelock state twice without scientific imports or writes."""

    schema = patch.preflight_closure_locked_evaluation_input_bundle_schema()
    physical_before = patch._physical_snapshot(PROJECT_ROOT)
    prelock_before = patch.collect_closure_locked_evaluation_input_bundle_prelock_state(
        verify_remote=True,
        repo_root=PROJECT_ROOT,
    )
    prelock_after = patch.collect_closure_locked_evaluation_input_bundle_prelock_state(
        verify_remote=True,
        repo_root=PROJECT_ROOT,
    )
    physical_after = patch._physical_snapshot(PROJECT_ROOT)
    if prelock_before != prelock_after:
        raise _error("E0-MIB topology or namespace changed during check-only")
    if physical_before != physical_after:
        raise _error("E0-MIB physical authority inputs changed during check-only")
    return {
...
def execute_lock() -> dict[str, Any]:
    ...
    physical_before = patch._physical_snapshot(PROJECT_ROOT)
    verification = run_closure_locked_evaluation_input_bundle_verification(
        expected_schema_preflight=schema
    )
    physical_after = patch._physical_snapshot(PROJECT_ROOT)
    if physical_before != physical_after:
        raise _error("E0-MIB physical authority inputs changed during verification")
    after = patch.collect_closure_locked_evaluation_input_bundle_prelock_state(
        verify_remote=True,
        repo_root=PROJECT_ROOT,
    )
    if before != after:
        raise _error("E0-MIB topology or namespace changed during verification")
    payload = patch.build_closure_locked_evaluation_input_bundle_lock_payload(
        prelock=before,
        verification=verification,
    )
    patch.validate_closure_locked_evaluation_input_bundle_lock_payload(
        payload,
        verify_remote=True,
        repo_root=PROJECT_ROOT,
    )
    lock, companion = patch.publish_closure_locked_evaluation_input_bundle_lock_bundle(
        payload,
        repo_root=PROJECT_ROOT,
    )
    return {
...
def check_effective() -> dict[str, Any]:
    return patch.load_effective_closure_locked_evaluation_input_bundle_authority(
        verify_remote=True,
        repo_root=PROJECT_ROOT,
    )
