...
E0-MP is an implementation lock for the development-only B0/B1/B2 batch.
...
authorization false; only a published, exact two-file P-E0-MP descendant can
...
LOCK_VERSION = "closure_baseline_development_patch_lock_v1"
PATCH_GATE = "E0-MP"
PATCH_ID = "baseline_development_authority_patch_1"
EXPERIMENT_ID = "closure_v1"
SURFACE_ID = "closure_v1_wqp_adaptive_no_current_chla"
PATCH_BASE_COMMIT = "a9aa51aaa1566d0b8e7154697fae69c458c5019f"
...
PATCH_COMPONENT_ROLES = {
    ...
    DEFAULT_PATCH_LOCK_SCHEMA.as_posix(): "baseline_development_patch_lock_schema",
...
EXPECTED_COMPANION_INPUT_COUNT = 48
...
PATCH_SEALS = {
    "h_scope_exact_eight_additions": True,
...
class BaselineDevelopmentPatchError(RuntimeError):
    """Raised when E0-MP cannot prove its exact closed authority."""
...
def _relative(path: Path, *, repo_root: Path | None = None) -> str:
    try:
        ...
        raise BaselineDevelopmentPatchError(f"Path escapes repository: {path}") from exc
...
def _load_regular_json(
    ...
    *,
    ...
    try:
        ...
        raise BaselineDevelopmentPatchError(f"{context} is not valid JSON") from exc
    if not isinstance(value, dict):
        raise BaselineDevelopmentPatchError(f"{context} must be a JSON object")
    return value
...
def preflight_baseline_development_patch_schema(
    ...
    *,
    ...
    if schema_path != DEFAULT_PATCH_LOCK_SCHEMA:
        raise BaselineDevelopmentPatchError("E0-MP requires the closed default schema path")
    schema = _load_regular_json(
        schema_path,
        context="E0-MP lock schema",
        repo_root=repo_root,
    )
    ...
    if minimum_count or format_count:
        raise BaselineDevelopmentPatchError(
            "E0-MP schema uses unsupported keywords: "
    ...
    try:
        ...
        raise BaselineDevelopmentPatchError(str(exc)) from exc
    record = _file_record(
        schema_path,
...
def load_and_validate_baseline_development_runtime(
    ...
    *,
    ...
    if runtime_path != DEFAULT_RUNTIME_PATH:
        raise BaselineDevelopmentPatchError("E0-MP requires the closed default runtime path")
    try:
        value = yaml.safe_load(_read_regular_bytes(runtime_path, repo_root=repo_root))
    except (yaml.YAMLError, UnicodeDecodeError) as exc:
        raise BaselineDevelopmentPatchError("E0-MP runtime is not valid YAML") from exc
    if not isinstance(value, dict):
        raise BaselineDevelopmentPatchError("E0-MP runtime must be a mapping")
    expected_identity = {
    ...
    for key, expected in expected_identity.items():
        if value.get(key) != expected:
            raise BaselineDevelopmentPatchError(f"Runtime identity drifted: {key}")
    ...
    if not isinstance(roles, Mapping) or (
        ...
        raise BaselineDevelopmentPatchError("Runtime target cutoff/role contract drifted")
    ...
    if not isinstance(denominators, Mapping) or (
        ...
        raise BaselineDevelopmentPatchError("Runtime denominators drifted")
    ...
    if not isinstance(seeds, Mapping) or (
        ...
        raise BaselineDevelopmentPatchError("Runtime seed contract drifted")
    models = value.get("models")
    if not isinstance(models, Mapping):
        raise BaselineDevelopmentPatchError("Runtime model contract is absent")
    b0 = models.get("B0")
    b1 = models.get("B1")
    b2 = models.get("B2")
    if not all(isinstance(item, Mapping) for item in (b0, b1, b2)):
        raise BaselineDevelopmentPatchError("Runtime B0/B1/B2 mappings are absent")
    ...
    if (
        ...
        raise BaselineDevelopmentPatchError("Runtime B0 contract drifted")
    if (
        ...
        raise BaselineDevelopmentPatchError("Runtime B1 contract drifted")
    ...
    if not isinstance(selection, Mapping) or (
        ...
        raise BaselineDevelopmentPatchError("Runtime B2 contract drifted")
    ...
    if not isinstance(outputs, Mapping) or (
        ...
        raise BaselineDevelopmentPatchError("Runtime output contract drifted")
    ...
    if reproducibility != {
        ...
        raise BaselineDevelopmentPatchError("Runtime reproducibility contract drifted")
    ...
    if not isinstance(publication, Mapping) or any(
        ...
        raise BaselineDevelopmentPatchError("Runtime atomic publication contract drifted")
    ...
    if not isinstance(authorizations, Mapping) or any(
        ...
        raise BaselineDevelopmentPatchError("Runtime authorizations are not all closed")
    dvc = value.get("dvc")
    if not isinstance(dvc, Mapping) or (
        ...
        raise BaselineDevelopmentPatchError("Runtime DVC contract drifted")
    ...
    if not isinstance(bundles, list) or [item.get("base_seed") for item in bundles] != list(MODEL_SEEDS):
        raise BaselineDevelopmentPatchError("Runtime ANFIS bundle order drifted")
...
def _verify_runtime_physical_pins(
    ...
    *,
    ...
    if not isinstance(authority, Mapping):
        raise BaselineDevelopmentPatchError("Runtime authority mapping is absent")
    for role, expected in authority.items():
        ...
        if record["sha256"] != expected.get("sha256") or (
            ...
            raise BaselineDevelopmentPatchError(f"Runtime authority pin drifted: {role}")
    ...
    for bundle in bundles:
        ...
        for role in ("state", "pointer", "manifest"):
            ...
            if not isinstance(expected, Mapping):
                raise BaselineDevelopmentPatchError(f"ANFIS {seed} {role} pin is absent")
            record = _file_record(
            ...
            if record["bytes"] != expected.get("bytes") or record["sha256"] != expected.get("sha256"):
                raise BaselineDevelopmentPatchError(f"ANFIS {seed} {role} pin drifted")
            records.append(record)
...
def baseline_final_paths(runtime: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    if not isinstance(outputs, Mapping):
        raise BaselineDevelopmentPatchError("Runtime outputs mapping is absent")
    ...
    if not isinstance(raw, Mapping) or not isinstance(light, Mapping):
        raise BaselineDevelopmentPatchError("Runtime output maps are absent")
    ...
    if len(result) != 69 or len(set(result)) != 69:
        raise BaselineDevelopmentPatchError("Baseline final namespace is not exactly 69 unique paths")
    return result
...
def baseline_output_namespace_absence(
    ...
    *,
    ...
    if not isinstance(dvc, Mapping):
        raise BaselineDevelopmentPatchError("Runtime DVC namespace is absent")
    ...
    if len(future_pointers) != 3 or len(set(future_pointers)) != 3:
        raise BaselineDevelopmentPatchError("Future raw-score DVC pointer namespace drifted")
...
def _live_remote_main_head(*, repo_root: Path | None = None) -> str:
    ...
    if len(lines) != 1:
        raise BaselineDevelopmentPatchError("Remote main observation is not unique")
    ...
    if (
        ...
        raise BaselineDevelopmentPatchError("Remote main observation is malformed")
    return fields[0]
    ...
    *,
    ...
    for line in output.splitlines():
        ...
        if len(fields) != 2 or fields[0] not in {"A", "M", "D"}:
            raise BaselineDevelopmentPatchError(f"Unsupported H diff entry: {line}")
...
def _validate_h_component_records(h_patch: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if paths != list(PATCH_PATHS) or not isinstance(components, list):
        raise BaselineDevelopmentPatchError("E0-MP H scope drifted")
    if len(components) != len(PATCH_PATHS):
        raise BaselineDevelopmentPatchError("E0-MP H component count drifted")
    normalized: list[dict[str, Any]] = []
    for index, path in enumerate(PATCH_PATHS):
        candidate = components[index]
        if not isinstance(candidate, Mapping):
            raise BaselineDevelopmentPatchError("E0-MP H component is not a mapping")
...
def _git_blob_record(
    ...
    *,
    ...
    if result.returncode != 0:
        raise BaselineDevelopmentPatchError(
            f"E0-MP cannot reconstruct H blob: {path}"
        )
    return {
        "path": path,
        "role": role,
...
def _reconstruct_h_components(
    ...
    *,
    ...
    if not isinstance(repository, Mapping) or not isinstance(h_patch, Mapping):
        raise BaselineDevelopmentPatchError("E0-MP H authority is absent")
    h_head = repository.get("head")
    if not isinstance(h_head, str) or len(h_head) != 40:
        raise BaselineDevelopmentPatchError("E0-MP H commit is malformed")
    if _git("rev-parse", f"{h_head}^", repo_root=repo_root) != PATCH_BASE_COMMIT:
        raise BaselineDevelopmentPatchError("E0-MP H parent drifted")
    ...
    if sorted(entries, key=lambda item: item["path"]) != expected_entries:
        raise BaselineDevelopmentPatchError("E0-MP H commit is not exactly eight additions")
    ...
    if reconstructed != locked:
        raise BaselineDevelopmentPatchError("E0-MP H Git blobs differ from the lock")
    current = [
        _file_record(
            Path(path),
            role=PATCH_COMPONENT_ROLES[path],
    ...
    if current != locked:
        raise BaselineDevelopmentPatchError("E0-MP H files drifted after publication")
    return reconstructed


def collect_baseline_development_patch_prelock_state(
    *,
    repo_root: Path | None = None,
) -> dict[str, Any]:
    ...
    parent = _git("rev-parse", "HEAD^", repo_root=root)
    ...
    if (
        ...
        raise BaselineDevelopmentPatchError("Published H topology/ref alignment drifted")
    if _git("status", "--porcelain=v1", "--untracked-files=all", repo_root=root):
        raise BaselineDevelopmentPatchError("E0-MP H repository must be clean")
    entries = _observed_diff_entries(PATCH_BASE_COMMIT, head, repo_root=root)
    expected_entries = [{"status": "A", "path": path} for path in PATCH_PATHS]
    if sorted(entries, key=lambda item: item["path"]) != expected_entries:
        raise BaselineDevelopmentPatchError("H-E0-MP is not exactly eight additions")
    components = [
    ...
    for path, expected in LEGACY_PRESERVED_RECORDS.items():
        ...
        if observed["bytes"] != expected["bytes"] or observed["sha256"] != expected["sha256"]:
            raise BaselineDevelopmentPatchError(f"Preserved historical path drifted: {path}")
    ...
    if e0_m_present or outcome_present:
        raise BaselineDevelopmentPatchError("E0-M or outcome-access log exists before E0-MP")
    runtime_record = next(record for record in components if record["path"] == DEFAULT_RUNTIME_PATH.as_posix())
    return {
        "repository": {
            "head": head,
            "parent": parent,
...
def build_baseline_development_patch_lock_payload(
...
def _validate_timestamp(value: Any) -> None:
    if not isinstance(value, str):
        raise BaselineDevelopmentPatchError("Lock timestamp must be a string")
    try:
        parsed = datetime.fromisoformat(value.replace("Z", "+00:00"))
    except ValueError as exc:
        raise BaselineDevelopmentPatchError("Lock timestamp is malformed") from exc
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise BaselineDevelopmentPatchError("Lock timestamp must include timezone")


def validate_baseline_development_patch_lock_payload(
    payload: Mapping[str, Any],
    *,
    repo_root: Path | None = None,
) -> dict[str, Any]:
    ...
    if payload.get("authorizations") != UNPUBLISHED_AUTHORIZATIONS:
        raise BaselineDevelopmentPatchError("Unpublished E0-MP authorizations drifted")
    if payload.get("seals") != PATCH_SEALS:
        raise BaselineDevelopmentPatchError("E0-MP seals drifted")
    h_patch = payload.get("h_patch")
    if not isinstance(h_patch, Mapping):
        raise BaselineDevelopmentPatchError("E0-MP H payload is absent")
    _validate_h_component_records(h_patch)
    runtime_contract = payload.get("runtime_contract")
    if not isinstance(runtime_contract, Mapping):
        raise BaselineDevelopmentPatchError("E0-MP runtime lock binding is absent")
    ...
    if (
        ...
        raise BaselineDevelopmentPatchError("E0-MP physical input binding drifted")
    namespace = payload.get("prelock", {}).get("output_namespace")
    if not isinstance(namespace, Mapping) or namespace.get("final_paths") != list(
        baseline_final_paths(load_and_validate_baseline_development_runtime(
            repo_root=repo_root,
            verify_physical_pins=False,
        ))
    ):
        raise BaselineDevelopmentPatchError("E0-MP output namespace drifted")
    return dict(payload)
...
def _expected_companion(
    ...
    if not isinstance(h_patch, Mapping) or not isinstance(runtime_contract, Mapping):
        raise BaselineDevelopmentPatchError("Cannot construct E0-MP companion inputs")
    h_components = _validate_h_component_records(h_patch)
    physical_inputs = runtime_contract.get("physical_inputs")
    if not isinstance(physical_inputs, list):
        raise BaselineDevelopmentPatchError("Cannot construct E0-MP physical inputs")
    ...
    if (
        ...
        raise BaselineDevelopmentPatchError(
            "E0-MP companion must bind exactly 48 unique physical inputs"
        )
    return {
        "manifest_version": "closure_baseline_development_patch_lock_manifest_v1",
        "gate": PATCH_GATE,
        "status": "completed",
        "inputs": inputs,
        "historical_inputs": [],
...
def load_and_validate_baseline_development_patch_lock(
    lock_path: Path = DEFAULT_PATCH_LOCK_PATH,
    *,
    repo_root: Path | None = None,
) -> dict[str, Any]:
    payload = _load_regular_json(lock_path, context="E0-MP lock", repo_root=repo_root)
    return validate_baseline_development_patch_lock_payload(payload, repo_root=repo_root)
...
def _validate_p_publication(
    ...
    *,
    ...
    if (
        ...
        raise BaselineDevelopmentPatchError("Published P topology/ref alignment drifted")
    if _git("branch", "--show-current", repo_root=root) != "main":
        raise BaselineDevelopmentPatchError("Published P branch drifted")
    if _git("status", "--porcelain=v1", "--untracked-files=all", repo_root=root):
        raise BaselineDevelopmentPatchError("Published P repository must be clean")
    ...
    if sorted(entries, key=lambda item: item["path"]) != expected:
        raise BaselineDevelopmentPatchError("P-E0-MP is not exactly lock+companion")
    lock_record = _file_record(DEFAULT_PATCH_LOCK_PATH, role="baseline_development_patch_lock", repo_root=root)
    companion = _load_regular_json(
        DEFAULT_PATCH_MANIFEST_PATH,
        context="E0-MP companion",
        repo_root=root,
    )
    if companion != _expected_companion(payload, lock_record):
        raise BaselineDevelopmentPatchError("Published E0-MP companion drifted")
    ...
    if (
        ...
        raise BaselineDevelopmentPatchError("Published E0-MP physical inputs drifted")
    baseline_output_namespace_absence(runtime, repo_root=root)
    return {"h_patch_head": parent, "p_patch_head": head, "remote_head": remote}


def load_effective_baseline_development_authority(
    repo_root: Path | None = None,
) -> dict[str, Any]:
    """Return the only effective E0-MP flags after exact P publication."""
    preflight_baseline_development_patch_schema(repo_root=repo_root)
    payload = load_and_validate_baseline_development_patch_lock(repo_root=repo_root)
    ...
    lock_record = _file_record(
        ...
        role="baseline_development_patch_lock",
        repo_root=repo_root,
    )
    companion_record = _file_record(
        DEFAULT_PATCH_MANIFEST_PATH,
        role="baseline_development_patch_lock_manifest",
        repo_root=repo_root,
    )
    return {
        "gate": PATCH_GATE,
...
def require_baseline_development_authority(
    *,
    ...
    if verify_remote is not True:
        raise BaselineDevelopmentPatchError("E0-MP requires tracking-ref verification")
    return load_effective_baseline_development_authority(repo_root=repo_root)
