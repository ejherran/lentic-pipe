#!/usr/bin/env python
"""Check and publish the outcome-closed E0-MZ registration authority."""
...
def _count(value: Any, name: str) -> int:
    ...
    raise patch.AnfisAblationDvcRegistrationAdoptionPatchError(
        f"E0-MZ check-only summary is missing exact integer: {name}"
    )


def check_only() -> dict[str, Any]:
    """Run the schema-first, non-writing H-E0-MZ preflight."""

    schema = (
        patch.preflight_anfis_ablation_dvc_registration_adoption_patch_schema()
    )
    prelock = (
        patch.collect_anfis_ablation_dvc_registration_adoption_patch_prelock_state(
            verify_remote=True
        )
    )
    ...
    if counts != expected_counts:
        raise patch.AnfisAblationDvcRegistrationAdoptionPatchError(
            f"E0-MZ check-only counts drifted: {counts}"
        )
    return {
        ...
        "adopted_light_commit": patch.ADOPTED_LIGHT_COMMIT,
        "adopted_light_count": patch.EXPECTED_ADOPTED_LIGHT_COUNT,
        **counts,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except patch.AnfisAblationDvcRegistrationAdoptionPatchError as exc:
