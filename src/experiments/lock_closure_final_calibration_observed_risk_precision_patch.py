#!/usr/bin/env python
"""Check, publish, and audit the E0-MCALH observed-risk-precision authority."""
...
def _error(message: str) -> patch.FinalCalibrationObservedRiskPrecisionPatchError:
    return patch.FinalCalibrationObservedRiskPrecisionPatchError(message)
...
def _parse_focused_summary(stdout: str, stderr: str) -> dict[str, int]:
    ...
    if type(count) is not int or count <= 0:
        raise _error("E0-MCALH focused-test count is not frozen")
...
def run_final_calibration_observed_risk_precision_patch_verification(
    *, expected_schema_preflight: Mapping[str, Any] | None = None
    ...
    if (
        ...
        raise _error("E0-MCALH schema changed before verification")
...
def check_only() -> dict[str, Any]:
    schema = patch.preflight_final_calibration_observed_risk_precision_patch_schema()
    prelock = (
        patch.collect_final_calibration_observed_risk_precision_patch_prelock_state(
            verify_remote=True
        )
    )
    return {
...
def execute_lock() -> dict[str, Any]:
    ...
    verification = run_final_calibration_observed_risk_precision_patch_verification(
        expected_schema_preflight=schema
    )
    after = patch.collect_final_calibration_observed_risk_precision_patch_prelock_state(
        verify_remote=True
    )
    if before != after:
        raise _error("E0-MCALH prelock state changed during verification")
    payload = patch.build_final_calibration_observed_risk_precision_patch_lock_payload(
...
def check_effective() -> dict[str, Any]:
    return patch.load_effective_final_calibration_observed_risk_precision_patch_authority(
        verify_remote=True
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except patch.FinalCalibrationObservedRiskPrecisionPatchError as exc:
