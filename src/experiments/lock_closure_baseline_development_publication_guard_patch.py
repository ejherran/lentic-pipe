#!/usr/bin/env python
"""Create the scientific-egress-closed E0-MQ bundle with read-only Git alignment."""
...
from src.experiments.closure_baseline_development_publication_guard_patch import (  # noqa: E402
    ...
    TYPE_CHECK_COMMAND,
    BaselineDevelopmentPublicationGuardPatchError,
    _canonical_json,
    _expected_companion,
    _file_record,
    build_baseline_development_publication_guard_patch_lock_payload,
    collect_baseline_development_publication_guard_patch_prelock_state,
    load_effective_baseline_development_publication_guard_authority,
    preflight_baseline_development_publication_guard_patch_schema,
    validate_baseline_development_publication_guard_patch_lock_payload,
)


PROJECT_ROOT = Path(__file__).resolve().parents[2]
OUTPUT_GUARD_DIRECTORY = PROJECT_ROOT / "tmp" / "closure_v1_e0_mq_locker"
OUTPUT_GUARD_PATH = OUTPUT_GUARD_DIRECTORY / "baseline_development_publication_guard_patch_lock.guard"
...
def _require_marker(stdout: str, stderr: str, marker: str, *, context: str) -> None:
    if marker not in stdout + "\n" + stderr:
        raise BaselineDevelopmentPublicationGuardPatchError(f"{context} success marker is absent")
...
def _require_publication_guard_success(stdout: str, stderr: str) -> None:
    ...
    if stderr.strip() or observed_lines != expected_lines:
        raise BaselineDevelopmentPublicationGuardPatchError(
            "publication guard did not emit the single exact success result"
        )


def _require_empty_output(stdout: str, stderr: str, *, context: str) -> None:
    if stdout.strip() or stderr.strip():
        raise BaselineDevelopmentPublicationGuardPatchError(f"{context} unexpectedly produced output")


def _parse_focused_summary(stdout: str, stderr: str) -> dict[str, Any]:
    if FOCUSED_TEST_COUNT <= 0:
        raise BaselineDevelopmentPublicationGuardPatchError(
            "E0-MQ focused-test count must be frozen before execute-lock"
        )
    ...
    if (
        ...
        raise BaselineDevelopmentPublicationGuardPatchError("Focused pytest summary is not one clean exact result")
    match = pattern.fullmatch(matches[0])
    if match is None or int(match.group("count")) != FOCUSED_TEST_COUNT:
        raise BaselineDevelopmentPublicationGuardPatchError("Focused pytest count drifted")
    return {
...
def run_baseline_development_publication_guard_patch_verification(
    *,
    expected_schema_preflight: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    """Run only repository checks; never DVC, science, auditors, or network."""
    schema_preflight = preflight_baseline_development_publication_guard_patch_schema()
    if expected_schema_preflight is not None and dict(expected_schema_preflight) != schema_preflight:
        raise BaselineDevelopmentPublicationGuardPatchError("E0-MQ schema changed before verification")
...
def _repo_relative(path: Path, *, context: str) -> Path:
    ...
    try:
        ...
        raise BaselineDevelopmentPublicationGuardPatchError(f"{context} escaped the repository") from exc
    if any(component in {"", ".", ".."} for component in relative.parts):
        raise BaselineDevelopmentPublicationGuardPatchError(f"{context} is not a closed path")
    return relative
...
def _open_real_directory(path: Path, *, context: str) -> int:
    ...
    try:
        ...
        if (
            ...
            raise BaselineDevelopmentPublicationGuardPatchError(f"{context} identity drifted")
        return descriptor
...
def _open_repo_directory(relative: Path, *, context: str) -> int:
    if relative.is_absolute() or any(
        ...
        raise BaselineDevelopmentPublicationGuardPatchError(f"{context} escaped the repository root")
    descriptor = _open_real_directory(
        Path(os.path.abspath(PROJECT_ROOT)),
        context="E0-MQ repository root",
    )
    try:
...
def _write_all(descriptor: int, payload: bytes, *, context: str) -> None:
    ...
    while offset < len(payload):
        try:
            ...
            raise BaselineDevelopmentPublicationGuardPatchError(f"{context} write failed") from exc
        if written <= 0:
            raise BaselineDevelopmentPublicationGuardPatchError(f"{context} produced a short write")
        offset += written
    os.fsync(descriptor)
...
def _owner_state(owner: OwnedFile | OutputGuard) -> str:
    try:
        ...
        current_descriptor = _open_repo_directory(
            ...
            context="E0-MQ owned-file parent",
        )
    except OSError as exc:
        raise BaselineDevelopmentPublicationGuardPatchError(
            "E0-MQ owned-file descriptors could not be inspected"
        ) from exc
    except BaselineDevelopmentPublicationGuardPatchError:
        return "foreign"
    try:
        ...
        raise BaselineDevelopmentPublicationGuardPatchError(
            "E0-MQ owned-file identity could not be inspected"
        ) from exc
    finally:
...
def _close_owner(owner: OwnedFile | OutputGuard) -> None:
    ...
    if errors:
        raise BaselineDevelopmentPublicationGuardPatchError(
            "E0-MQ owned-file descriptors could not be closed"
        ) from errors[0]
...
def _rollback_if_owned(owner: OwnedFile | OutputGuard, *, context: str) -> None:
    ...
    if state == "foreign":
        raise BaselineDevelopmentPublicationGuardPatchError(f"{context} preserved a foreign replacement")
    if state == "owned":
        try:
            os.unlink(owner.path.name, dir_fd=owner.directory_file_descriptor)
            os.fsync(owner.directory_file_descriptor)
        except OSError as exc:
            raise BaselineDevelopmentPublicationGuardPatchError(f"{context} rollback failed") from exc
...
def _acquire_guard() -> OutputGuard:
    expected = Path(os.path.abspath(PROJECT_ROOT)) / "tmp" / "closure_v1_e0_mq_locker"
    if Path(os.path.abspath(OUTPUT_GUARD_DIRECTORY)) != expected:
        raise BaselineDevelopmentPublicationGuardPatchError("E0-MQ guard directory escaped ignored tmp/")
    relative_parent = _repo_relative(
        OUTPUT_GUARD_DIRECTORY,
        context="E0-MQ guard directory",
    )
    ...
    try:
        ...
        raise BaselineDevelopmentPublicationGuardPatchError("E0-MQ locker guard already exists") from exc
    except BaseException:
        os.close(directory_descriptor)
        raise
    ...
    try:
        ...
        if not stat.S_ISREG(metadata.st_mode):
            raise BaselineDevelopmentPublicationGuardPatchError("E0-MQ guard is not regular")
        guard = OutputGuard(
        ...
        if not _guard_is_owned(guard):
            raise BaselineDevelopmentPublicationGuardPatchError("E0-MQ guard identity drifted")
        _write_all(
            descriptor,
            f"pid={os.getpid()}\n".encode("ascii"),
            context="E0-MQ guard",
        ...
        if guard is not None:
            try:
                _rollback_if_owned(guard, context="E0-MQ guard")
        ...
        for owned_descriptor in (descriptor, directory_descriptor):
            try:
                ...
                cleanup_errors.append(cleanup_exc)
        if cleanup_errors:
            cleanup_error = BaselineDevelopmentPublicationGuardPatchError(
                "E0-MQ guard creation cleanup failed closed"
            )
...
def _release_guard(guard: OutputGuard) -> None:
    ...
    try:
        if _owner_state(guard) != "owned":
            raise BaselineDevelopmentPublicationGuardPatchError(
                "E0-MQ guard ownership was lost; foreign state was preserved"
            )
        ...
        if _owner_state(guard) != "absent":
            raise BaselineDevelopmentPublicationGuardPatchError(
                "E0-MQ guard release did not establish absence"
            )
    except BaseException as exc:
        error = exc
    try:
...
def _closed_output(path: Path, expected: Path, *, context: str) -> tuple[Path, Path]:
    ...
    if lexical != required:
        raise BaselineDevelopmentPublicationGuardPatchError(f"{context} must use {expected.as_posix()}")
...
def _write_temp(path: Path, expected: Path, payload: bytes) -> OwnedFile:
    lexical, relative_parent = _closed_output(path, expected, context="E0-MQ temporary")
    directory_descriptor = _open_repo_directory(
        relative_parent,
        context="E0-MQ temporary parent",
    )
    flags = (
        os.O_RDWR
        | os.O_CREAT
    ...
    try:
        ...
        os.close(directory_descriptor)
        raise BaselineDevelopmentPublicationGuardPatchError(
            f"Refusing to clobber E0-MQ temporary: {expected.as_posix()}"
        ) from exc
    except BaseException:
        os.close(directory_descriptor)
        raise
    ...
    try:
        ...
        if not stat.S_ISREG(metadata.st_mode):
            raise BaselineDevelopmentPublicationGuardPatchError("E0-MQ temporary is not regular")
        _write_all(descriptor, payload, context="E0-MQ temporary")
        if _owner_state(owner) != "owned":
            raise BaselineDevelopmentPublicationGuardPatchError("E0-MQ temporary identity drifted")
        return owner
    except BaseException as exc:
        cleanup_errors: list[BaseException] = []
        if owner is not None:
            try:
                _rollback_if_owned(owner, context="E0-MQ temporary")
            except BaseException as cleanup_exc:
                cleanup_errors.append(cleanup_exc)
        for owned_descriptor in (descriptor, directory_descriptor):
            try:
        ...
        if cleanup_errors:
            cleanup_error = BaselineDevelopmentPublicationGuardPatchError(
                "E0-MQ temporary creation cleanup failed closed"
            )
            cleanup_error.add_note(
                "; ".join(f"{type(item).__name__}: {item}" for item in cleanup_errors)
            )
...
def _publish_temp(
    ...
    final, relative_parent = _closed_output(final_path, expected, context="E0-MQ output")
    if not _guard_is_owned(guard):
        raise BaselineDevelopmentPublicationGuardPatchError("E0-MQ guard changed before publication")
    if _owner_state(temp) != "owned":
        raise BaselineDevelopmentPublicationGuardPatchError("E0-MQ temporary ownership drifted")
    destination_descriptor = _open_repo_directory(
        relative_parent,
        context="E0-MQ output parent",
    )
    ...
    try:
        ...
        if _owner_state(owner) != "owned":
            raise BaselineDevelopmentPublicationGuardPatchError("E0-MQ published output identity drifted")
        if not _guard_is_owned(guard):
            raise BaselineDevelopmentPublicationGuardPatchError("E0-MQ guard changed during publication")
        _rollback_if_owned(temp, context="E0-MQ published temporary")
        _close_owner(temp)
        return owner
    except FileExistsError as exc:
        ...
        raise BaselineDevelopmentPublicationGuardPatchError(
            f"Refusing to clobber E0-MQ output: {expected.as_posix()}"
        ) from exc
    except BaseException as exc:
        cleanup_errors: list[BaseException] = []
        if owner is not None:
            try:
                _rollback_if_owned(owner, context="E0-MQ output")
            except BaseException as cleanup_exc:
                cleanup_errors.append(cleanup_exc)
        ...
        if cleanup_errors:
            cleanup_error = BaselineDevelopmentPublicationGuardPatchError(
                "E0-MQ publication rollback could not be completed safely"
            )
            cleanup_error.add_note(
                "; ".join(f"{type(item).__name__}: {item}" for item in cleanup_errors)
            )
...
def _read_owned_bytes(owner: OwnedFile) -> bytes:
    if _owner_state(owner) != "owned":
        raise BaselineDevelopmentPublicationGuardPatchError("E0-MQ output changed before verification")
    ...
    if _owner_state(owner) != "owned":
        raise BaselineDevelopmentPublicationGuardPatchError("E0-MQ output changed during verification")
    return payload
...
def _publish_lock_bundle(payload: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    try:
        ...
        lock_record = _logical_record(
            lock_path,
            "baseline_development_publication_guard_patch_lock",
            lock_bytes,
        )
        ...
        if _read_owned_bytes(published_lock) != lock_bytes:
            raise BaselineDevelopmentPublicationGuardPatchError("Published E0-MQ lock bytes drifted")
        if _read_owned_bytes(published_manifest) != companion_bytes:
            raise BaselineDevelopmentPublicationGuardPatchError("Published E0-MQ companion bytes drifted")
    ...
    if active_error is not None:
        for owner in reversed(published):
            try:
                _rollback_if_owned(owner, context="E0-MQ published output")
            except BaseException as exc:
    ...
    if active_error is None and not cleanup_errors:
        for owner in published:
            try:
                if _owner_state(owner) != "owned":
                    raise BaselineDevelopmentPublicationGuardPatchError(
                        "E0-MQ published output changed after guard release"
                    )
            except BaseException as exc:
                cleanup_errors.append(exc)
        if cleanup_errors:
    ...
    if cleanup_errors:
        cleanup_error = BaselineDevelopmentPublicationGuardPatchError(
            "E0-MQ lock resource cleanup failed closed"
        )
        cleanup_error.add_note(
            "; ".join(f"{type(item).__name__}: {item}" for item in cleanup_errors)
        )
    ...
    if active_error is not None:
        raise active_error
    if result is None:
        raise BaselineDevelopmentPublicationGuardPatchError("E0-MQ publication produced no result")
    return result


def check_only() -> dict[str, Any]:
    schema_preflight = preflight_baseline_development_publication_guard_patch_schema()
    prelock = collect_baseline_development_publication_guard_patch_prelock_state()
    return {
...
def execute_lock() -> dict[str, Any]:
    schema_preflight = preflight_baseline_development_publication_guard_patch_schema()
    prelock = collect_baseline_development_publication_guard_patch_prelock_state()
    verification = run_baseline_development_publication_guard_patch_verification(
        expected_schema_preflight=schema_preflight
    )
    if collect_baseline_development_publication_guard_patch_prelock_state() != prelock:
        raise BaselineDevelopmentPublicationGuardPatchError("E0-MQ prelock state changed during verification")
    payload = build_baseline_development_publication_guard_patch_lock_payload(
        prelock,
        verification,
        created_at_utc=datetime.now(timezone.utc).isoformat(),
    )
    validate_baseline_development_publication_guard_patch_lock_payload(payload)
