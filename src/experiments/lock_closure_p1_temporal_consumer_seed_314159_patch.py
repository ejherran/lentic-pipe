#!/usr/bin/env python
"""Create and verify the additive Closure V1 E0-MO lock bundle."""
...
from src.experiments.closure_p1_temporal_consumer_seed_314159_patch import (  # noqa: E402
...
OUTPUT_GUARD_DIRECTORY = PROJECT_ROOT / "tmp" / "closure_v1_e0_mo_locker"
OUTPUT_GUARD_NAMES = (
    "p1_temporal_consumer_seed_314159_patch_lock.guard",
    "p1_temporal_consumer_seed_314159_patch_lock_manifest.guard",
)
...
def _require_success_marker(stdout: str, stderr: str, marker: str) -> None:
    if marker not in stdout + "\n" + stderr:
        raise P1TemporalConsumerSeed314159PatchError(
            f"E0-MO verification marker is absent: {marker}"
        )
...
def _require_empty_output(stdout: str, stderr: str, *, context: str) -> None:
    if stdout.strip() or stderr.strip():
        raise P1TemporalConsumerSeed314159PatchError(
            f"E0-MO expected empty output from {context}"
        )


def _preflight_schema() -> dict[str, Any]:
    """Prove the physical MO schema uses the repository's supported subset."""
    observed = preflight_p1_temporal_consumer_seed_314159_patch_schema()
    if not isinstance(observed, Mapping) or set(observed) != SCHEMA_PREFLIGHT_FIELDS:
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO schema-subset preflight evidence is malformed"
        )
    result = dict(observed)
    digest = result.get("schema_sha256")
    size = result.get("schema_bytes")
    if (
        result.get("gate") != "E0-MO"
...
def _parse_focused_summary(stdout: str, stderr: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO focused pytest duration is malformed"
        ) from exc
    try:
        expected_clock = str(timedelta(seconds=int(duration)))
    except OverflowError as exc:
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO focused pytest duration is out of range"
        ) from exc
    if duration < Decimal(60):
        if clock is not None:
            raise P1TemporalConsumerSeed314159PatchError(
                "E0-MO sub-minute pytest summary must not include a clock"
            )
        summary_format = "pytest_short"
    else:
        if clock != expected_clock:
            raise P1TemporalConsumerSeed314159PatchError(
                "E0-MO long pytest summary clock drifted"
            )
...
def _dvc_terminal_status(
    ...
    *,
    ...
    if len(terminals) != 1 or (
        ...
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO targeted DVC push is not one closed idempotent terminal"
        )
    if any(
        ...
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO targeted DVC push exceeded one object"
        )
    return terminals[0]


def run_p1_temporal_consumer_seed_314159_patch_verification(
    *,
    expected_schema_preflight: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    """Run the closed E0-MO verification only after a live schema preflight."""
    ...
    if (
        ...
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO schema changed before verification"
        )
    if FOCUSED_TEST_COUNT <= 0:
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO focused-test count has not been finalized"
        )
    try:
...
def _guard_paths(*, create_directory: bool) -> tuple[Path, Path]:
    ...
    if OUTPUT_GUARD_DIRECTORY != tmp_root / "closure_v1_e0_mo_locker":
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO coordination directory escaped ignored tmp/"
        )
    if create_directory:
        try:
            hardened._ensure_real_directory(
                tmp_root,
                parent=PROJECT_ROOT,
                context="E0-MO tmp root",
            )
            hardened._ensure_real_directory(
                OUTPUT_GUARD_DIRECTORY,
                parent=tmp_root,
                context="E0-MO guard directory",
            )
        except hardened.DevelopmentRuntimeTemporalConsumerPatchError as exc:
            raise _translate(exc) from exc
    return (
...
def _refuse_existing_outputs(output: Path, companion: Path) -> None:
    ...
    if existing:
        raise P1TemporalConsumerSeed314159PatchError(
            f"Refusing to overwrite an existing E0-MO lock bundle: {existing}"
        )
...
def _acquire_guards(
    ...
    if len(guards) != 2:
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO requires exactly two guards"
        )
...
def _execute_lock(output: Path, companion: Path) -> dict[str, Any]:
    ...
    try:
        ...
        prelock = collect_p1_temporal_consumer_seed_314159_patch_prelock_state(
            verify_remote=True
        )
        verification = run_p1_temporal_consumer_seed_314159_patch_verification(
            expected_schema_preflight=schema_preflight
        )
        p1_consumer_namespace_absence()
        repeated = collect_p1_temporal_consumer_seed_314159_patch_prelock_state(
            verify_remote=True
        )
        if repeated != prelock:
            raise P1TemporalConsumerSeed314159PatchError(
                "E0-MO authority changed during verification"
            )
        repeated_schema_preflight = _preflight_schema()
        if repeated_schema_preflight != schema_preflight:
            raise P1TemporalConsumerSeed314159PatchError(
                "E0-MO schema changed during verification"
            )
        hardened._assert_lock_namespace(lock_path, companion_path, guards, owners)
        timestamp = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
        payload = build_p1_temporal_consumer_seed_314159_patch_lock_payload(
            prelock,
            verification,
            created_at_utc=timestamp,
        )
        schema = _load_regular_json(DEFAULT_PATCH_LOCK_SCHEMA, context="E0-MO schema")
        validate_p1_temporal_consumer_seed_314159_patch_lock_payload(
            payload,
            schema,
    ...
    try:
        ...
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO lock transaction cleanup failed closed"
        ) from exc
    if active_error is not None:
        if isinstance(active_error, P1TemporalConsumerSeed314159PatchError):
            raise active_error
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO lock transaction failed"
        ) from active_error
    if result is None:
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO lock transaction produced no result"
        )
    return result
...
def _check_only(output: Path, companion: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "historical_e0_mm": prelock["base_authorities"]["e0_mm"],
        "historical_e0_mn": prelock["base_authorities"]["e0_mn"],
        "p1_314159_publication": prelock["p1_314159_publication"],
...
def _check_effective(output: Path, companion: Path) -> dict[str, Any]:
    ...
    summary = require_p1_temporal_consumer_seed_314159_patch_authorized(
        model_id="P1",
        base_seed=314_159,
        device="cpu",
    )
    ...
    if before != after:
        raise P1TemporalConsumerSeed314159PatchError(
            "E0-MO lock bundle changed during preflight"
        )
    return {
