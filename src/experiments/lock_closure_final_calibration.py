#!/usr/bin/env python
"""Check, publish, and audit the outcome-closed E0-MCAL authority."""
...
def _parse_focused_summary(stdout: str, stderr: str) -> dict[str, int]:
    count = calibration.FOCUSED_TEST_COUNT
    if type(count) is not int or count <= 0:
        raise _error("E0-MCAL focused-test count is not frozen")
...
def run_final_calibration_verification(
    *, expected_schema_preflight: Mapping[str, Any] | None = None
) -> dict[str, Any]:
    """Run repository checks without calibration, E7 fitting, DVC, or egress."""
    schema_preflight = calibration.preflight_final_calibration_schema()
    if expected_schema_preflight is not None and dict(expected_schema_preflight) != schema_preflight:
        raise _error("E0-MCAL schema changed before verification")

    full_type_check, stdout, stderr = _run_command(calibration.TYPE_CHECK_COMMAND)
    _require_marker(stdout, stderr, "All checks passed!", context="full type check")

    focused_tests, stdout, stderr = _run_command(
        calibration.FOCUSED_TEST_COMMAND, sanitize_pytest_environment=True
    )
    focused_tests.update(_parse_focused_summary(stdout, stderr))

    poetry_check, stdout, stderr = _run_command(calibration.POETRY_CHECK_COMMAND)
    _require_marker(stdout, stderr, "All set!", context="Poetry check")

    publication_guard, stdout, stderr = _run_command(calibration.PUBLICATION_GUARD_COMMAND)
    _require_publication_guard_success(stdout, stderr)

    diff_check, stdout, stderr = _run_command(calibration.DIFF_CHECK_COMMAND)
...
def check_only() -> dict[str, Any]:
    schema = calibration.preflight_final_calibration_schema()
    prelock = calibration.collect_final_calibration_prelock_state(verify_remote=True)
    return {
...
def execute_lock() -> dict[str, Any]:
    schema = calibration.preflight_final_calibration_schema()
    before = calibration.collect_final_calibration_prelock_state(verify_remote=True)
    verification = run_final_calibration_verification(expected_schema_preflight=schema)
    after = calibration.collect_final_calibration_prelock_state(verify_remote=True)
    if before != after:
        raise _error("E0-MCAL prelock state changed during verification")
    payload = calibration.build_final_calibration_lock_payload(
        prelock=before, verification=verification
    )
    calibration.validate_final_calibration_lock_payload(payload)
    lock, companion = calibration.publish_final_calibration_lock_bundle(payload)
    return {
...
def check_effective() -> dict[str, Any]:
    return calibration.load_effective_final_calibration_authority(verify_remote=True)
