#!/usr/bin/env python
"""Check, publish, and audit the E0-MCALL coordination authority."""
...
def _error(
    ...
    return patch.FinalCalibrationR8CoordinationNamespaceRevalidationPatchError(
        message
    )
...
def _parse_focused_summary(stdout: str, stderr: str) -> dict[str, int]:
    ...
    if type(count) is not int or count <= 0:
        raise _error("E0-MCALL focused-test count is not frozen")
...
def run_final_calibration_r8_coordination_namespace_revalidation_patch_verification(
    *, expected_schema_preflight: Mapping[str, Any] | None = None
) -> dict[str, Any]:
    """Run the closed repository checks without R science, DVC, or egress."""

    schema_preflight = (
        patch.preflight_final_calibration_r8_coordination_namespace_revalidation_patch_schema()
    )
    if (
        expected_schema_preflight is not None
        and dict(expected_schema_preflight) != schema_preflight
    ):
        raise _error("E0-MCALL schema changed before verification")
...
def check_only() -> dict[str, Any]:
    ...
    prelock_before = (
        patch.collect_final_calibration_r8_coordination_namespace_revalidation_patch_prelock_state(
            verify_remote=True
        )
    )
    prelock_after = (
    ...
    if prelock_before != prelock_after:
        raise _error(
            "E0-MCALL prelock or coordination namespace changed during check-only"
        )
    if physical_before != physical_after:
        raise _error(
            "E0-MCALL physical input or immutable R8 identity changed during check-only"
        )
    return {
...
def execute_lock() -> dict[str, Any]:
    ...
    physical_before = patch._physical_snapshot(PROJECT_ROOT)
    verification = (
        run_final_calibration_r8_coordination_namespace_revalidation_patch_verification(
    ...
    if physical_before != physical_after:
        raise _error(
            "E0-MCALL physical input or immutable R8 identity changed during verification"
        )
    after = (
        patch.collect_final_calibration_r8_coordination_namespace_revalidation_patch_prelock_state(
            verify_remote=True
    ...
    if before != after:
        raise _error(
            "E0-MCALL prelock, namespace, or immutable R8 state changed during verification"
        )
    payload = (
        patch.build_final_calibration_r8_coordination_namespace_revalidation_patch_lock_payload(
            prelock=before,
            verification=verification,
        )
    )
    patch.validate_final_calibration_r8_coordination_namespace_revalidation_patch_lock_payload(
        payload,
        verify_remote=True,
    )
    lock, companion = (
