#!/usr/bin/env python
"""Check, publish, and audit the E0-MCALK R8-manifest authority."""
...
def _error(
    ...
    return patch.FinalCalibrationR8ManifestReproducibilityPatchError(message)
...
def _parse_focused_summary(stdout: str, stderr: str) -> dict[str, int]:
    ...
    if type(count) is not int or count <= 0:
        raise _error("E0-MCALK focused-test count is not frozen")
...
def check_only() -> dict[str, Any]:
    ...
    prelock = (
        patch.collect_final_calibration_r8_manifest_reproducibility_patch_prelock_state(
            verify_remote=True
        )
    )
    return {
...
def execute_lock() -> dict[str, Any]:
    ...
    verification = (
        run_final_calibration_r8_manifest_reproducibility_patch_verification(
    ...
    if physical_before != physical_after:
        raise _error(
            "E0-MCALK physical input or immutable R8 identity changed during verification"
        )
    after = (
        patch.collect_final_calibration_r8_manifest_reproducibility_patch_prelock_state(
            verify_remote=True
    ...
    if before != after:
        raise _error("E0-MCALK prelock or immutable R8 state changed during verification")
    payload = (
        patch.build_final_calibration_r8_manifest_reproducibility_patch_lock_payload(
            prelock=before,
            verification=verification,
        )
    )
    patch.validate_final_calibration_r8_manifest_reproducibility_patch_lock_payload(
        payload,
        verify_remote=True,
    )
    lock, companion = (
        patch.publish_final_calibration_r8_manifest_reproducibility_patch_lock_bundle(
            payload
        )
    )
    return {
...
def check_effective() -> dict[str, Any]:
    return (
        patch.load_effective_final_calibration_r8_manifest_reproducibility_patch_authority(
            verify_remote=True
        )
    )
