"""Close the exact Platt-parameter dialect under E0-MCALJ.

The producer has always emitted ``coefficient``, ``intercept``, and the
scientifically meaningful ``input=raw_probability`` marker.  E0-MCALJ adopts
that exact three-key record without changing the fit or any scientific data,
then delegates every other calibrator invariant to published E0-MCALI.
"""
...
from src.experiments import (
    closure_final_calibration_owned_run_guard_revalidation_patch as mcali,
)

mcal = mcali.mcal
mt = mcali.mt
TARGET_FILTER_EVIDENCE_CONTRACT = mcali.TARGET_FILTER_EVIDENCE_CONTRACT
INFERENCE_ROLE_CONTRACT = mcali.INFERENCE_ROLE_CONTRACT
RAW_SCORE_CANDIDATE_VALUES = mcali.RAW_SCORE_CANDIDATE_VALUES
RAW_EXCLUSION_EVIDENCE_CONTRACT = mcali.RAW_EXCLUSION_EVIDENCE_CONTRACT
ORDINAL_LABEL_REPRESENTATION_CONTRACT = (
    mcali.ORDINAL_LABEL_REPRESENTATION_CONTRACT
)
OBSERVED_RISK_PRECISION_CONTRACT = mcali.OBSERVED_RISK_PRECISION_CONTRACT


PROJECT_ROOT = Path(__file__).resolve().parents[2]
BASE_P_MCALI_COMMIT = "fbbb9ebb8260c43146ce6407d6629c20ce8cf4d9"
H_MCALI_COMMIT = "495ef14b2f110477318276755e7f6fc7d9ad2229"
H_MCALI_PARENT = mcali.BASE_P_MCALH_COMMIT
PATCH_GATE = "E0-MCALJ"
FINAL_CALIBRATION_GATE = PATCH_GATE
EXPERIMENT_ID = "closure_v1"
LOCK_SCHEMA_VERSION = "closure_final_calibration_platt_parameter_dialect_patch_lock_v1"
COMPANION_SCHEMA_VERSION = (
    "closure_final_calibration_platt_parameter_dialect_patch_lock_manifest_v1"
)
...
DEFAULT_PATCH_LOCK_PATH = Path(
    "reports/closure_v1/00_protocol/final_calibration_platt_parameter_dialect_patch_lock.json"
)
DEFAULT_PATCH_LOCK_MANIFEST_PATH = Path(
    "reports/closure_v1/00_protocol/"
    "final_calibration_platt_parameter_dialect_patch_lock_manifest.json"
)
...
LOCKER_GUARD_PATH = Path(
    "tmp/closure_v1_e0_mcalj/final_calibration_platt_parameter_dialect_patch_lock.guard"
)

H_MCALI_SUPERSEDED_PATHS = tuple(
    sorted(
        {
...
H_MCALI_PRESERVED_PATHS = tuple(
    path for path in mcali.PATCH_PATHS if path not in H_MCALI_SUPERSEDED_PATHS
)
P_MCALI_PATHS = (
    mcali.DEFAULT_PATCH_LOCK_PATH.as_posix(),
    mcali.DEFAULT_PATCH_LOCK_MANIFEST_PATH.as_posix(),
)
PATCH_PATHS = tuple(
    sorted(
        {
...
FINAL_CALIBRATION_H_STAGED_SCOPE = {
    path: ("M" if path in H_MCALI_SUPERSEDED_PATHS else "A")
...
FINAL_CALIBRATION_R_STAGED_SCOPE = dict(mcali.FINAL_CALIBRATION_R_STAGED_SCOPE)
R_OUTPUT_PATHS = mcali.R_OUTPUT_PATHS
...
OWNED_RUN_GUARD_REVALIDATION_CONTRACT = mcali.OWNED_RUN_GUARD_REVALIDATION_CONTRACT
PLATT_PARAMETER_DIALECT_CONTRACT = {
    ...
    "error_prefix": PATCH_GATE,
}

TYPE_CHECK_COMMAND = mcali.TYPE_CHECK_COMMAND
...
POETRY_CHECK_COMMAND = mcali.POETRY_CHECK_COMMAND
PUBLICATION_GUARD_COMMAND = mcali.PUBLICATION_GUARD_COMMAND
DIFF_CHECK_COMMAND = mcali.DIFF_CHECK_COMMAND
UNPUBLISHED_AUTHORIZATIONS = dict(mcali.UNPUBLISHED_AUTHORIZATIONS)
EMPTY_SHA256 = hashlib.sha256(b"").hexdigest()


class FinalCalibrationPlattParameterDialectPatchError(
    mcali.FinalCalibrationOwnedRunGuardRevalidationPatchError
):
    """Raised when the E0-MCALJ correction authority is not exact."""


FinalCalibrationError = mcali.FinalCalibrationError
P = ParamSpec("P")
R = TypeVar("R")


def _error(message: str) -> FinalCalibrationPlattParameterDialectPatchError:
    return FinalCalibrationPlattParameterDialectPatchError(message)


def _translate_predecessor_error(message: str) -> str:
    """Translate one leading historical gate without substring duplication."""
...
def _error_boundary(function: Callable[P, R]) -> Callable[P, R]:
    ...
    def wrapped(*args: P.args, **kwargs: P.kwargs) -> R:
        ...
        except FinalCalibrationPlattParameterDialectPatchError:
            raise
        except mcali.FinalCalibrationError as exc:
...
def _git_record_at_commit(
    ...
    if mode != "100644":
        raise _error(f"E0-MCALJ historical Git mode drifted: {path}")
    return {
        "role": role,
        "path": path,
        "bytes": len(payload),
...
def _p_mcali_authority(*, repo_root: Path) -> dict[str, Any]:
    """Reconstruct published H/P-E0-MCALI and its transitive authority."""

    if (
        mcal._single_parent(repo_root, H_MCALI_COMMIT, context="H-E0-MCALI")
        != H_MCALI_PARENT
    ):
        raise _error("E0-MCALJ historical H-E0-MCALI parent drifted")
    h_scope = mcal._git_scope(repo_root, H_MCALI_PARENT, H_MCALI_COMMIT)
    if h_scope != {
        "added": 5,
        "modified": 4,
        "deleted": 0,
        "path_count": 9,
        "paths": list(mcali.PATCH_PATHS),
    }:
        raise _error("E0-MCALJ historical H-E0-MCALI scope drifted")
    if (
        mcal._single_parent(repo_root, BASE_P_MCALI_COMMIT, context="P-E0-MCALI")
        != H_MCALI_COMMIT
    ):
        raise _error("E0-MCALJ historical P-E0-MCALI parent drifted")
    p_scope = mcal._git_scope(repo_root, H_MCALI_COMMIT, BASE_P_MCALI_COMMIT)
    if p_scope != {
        "added": 2,
        "modified": 0,
        "deleted": 0,
        "path_count": 2,
        "paths": sorted(P_MCALI_PATHS),
    }:
        raise _error("E0-MCALJ historical P-E0-MCALI scope drifted")

    h_components = [
        _git_record_at_commit(
            path,
            role="final_calibration_owned_run_guard_revalidation_patch_component",
            commit=H_MCALI_COMMIT,
            repo_root=repo_root,
        )
        for path in mcali.PATCH_PATHS
    ]
    preserved: list[dict[str, Any]] = []
    for path in H_MCALI_PRESERVED_PATHS:
        record = _git_record_at_commit(
            path,
            role="preserved_h_mcali_component",
            commit=H_MCALI_COMMIT,
            repo_root=repo_root,
        )
        ...
        if physical != record:
            raise _error(f"E0-MCALJ preserved H-MCALI component drifted: {path}")
        preserved.append(record)
    historical = [
        {
            **_git_record_at_commit(
                path,
                role="superseded_h_mcali_component",
                commit=H_MCALI_COMMIT,
                repo_root=repo_root,
            ),
            "commit": H_MCALI_COMMIT,
        }
        for path in H_MCALI_SUPERSEDED_PATHS
    ]
    p_components: list[dict[str, Any]] = []
    for path in P_MCALI_PATHS:
        role = (
            "published_p_mcali_lock"
            if path == mcali.DEFAULT_PATCH_LOCK_PATH.as_posix()
            else "published_p_mcali_lock_manifest"
        )
        record = _git_record_at_commit(
            path,
            role=role,
            commit=BASE_P_MCALI_COMMIT,
            repo_root=repo_root,
        )
        physical = mcal._git_artifact_record(
            Path(path),
        ...
        if physical != record:
            raise _error(f"E0-MCALJ published P-MCALI component drifted: {path}")
        p_components.append(record)

    lock_bytes = mcal._git_blob_bytes(
        repo_root, BASE_P_MCALI_COMMIT, mcali.DEFAULT_PATCH_LOCK_PATH
    )
    companion_bytes = mcal._git_blob_bytes(
        repo_root,
        BASE_P_MCALI_COMMIT,
        mcali.DEFAULT_PATCH_LOCK_MANIFEST_PATH,
    )
    try:
        lock_payload = mcal._parse_json_bytes(
            lock_bytes, context="published P-E0-MCALI lock"
        )
        companion = mcal._parse_json_bytes(
            companion_bytes, context="published P-E0-MCALI companion"
        )
    except mcali.FinalCalibrationError as exc:
        raise _error("E0-MCALJ historical P-E0-MCALI JSON drifted") from exc
    if (
        not isinstance(lock_payload, Mapping)
        or not isinstance(companion, Mapping)
        or lock_bytes != mcali._canonical_json_bytes(lock_payload)
        or companion_bytes != mcali._canonical_json_bytes(companion)
        or lock_payload.get("gate") != mcali.PATCH_GATE
        or cast(Mapping[str, Any], lock_payload.get("repository", {})).get(
            "h_patch_head"
        )
        != H_MCALI_COMMIT
    ):
        raise _error("E0-MCALJ historical P-E0-MCALI payload drifted")
    try:
        prior_schema = mcal._load_json_object(
            mcali.DEFAULT_PATCH_LOCK_SCHEMA, repo_root=repo_root
        )
        mcal.validate_json_schema(lock_payload, prior_schema)
        mcali._validate_timestamp(lock_payload.get("generated_at_utc"))
        mcali._validate_verification(
            lock_payload.get("verification"), repo_root=repo_root
        )
        prior_state = mcali._state_for_h(
            repository={
                "base_p_mcalh_commit": mcali.BASE_P_MCALH_COMMIT,
                "h_patch_head": H_MCALI_COMMIT,
                "branch": "main",
                "remote_head": H_MCALI_COMMIT,
                "scope": h_scope,
            },
            h_patch={
                "gate": "H-E0-MCALI",
        ...
        expected_prior = mcali.build_final_calibration_owned_run_guard_revalidation_patch_lock_payload(
            prior_state,
            cast(Mapping[str, Any], lock_payload["verification"]),
            generated_at_utc=cast(str, lock_payload["generated_at_utc"]),
        )
    except (KeyError, mcali.FinalCalibrationError) as exc:
        raise _error("E0-MCALJ historical P-E0-MCALI authority drifted") from exc
    if _canonical_json_bytes(lock_payload) != _canonical_json_bytes(expected_prior):
        raise _error("E0-MCALJ historical P-E0-MCALI reconstruction drifted")
    lock_record = {
        "role": "final_calibration_owned_run_guard_revalidation_patch_lock",
        "path": mcali.DEFAULT_PATCH_LOCK_PATH.as_posix(),
        "bytes": len(lock_bytes),
        "sha256": _sha256_bytes(lock_bytes),
    }
    if companion_bytes != mcali._canonical_json_bytes(
        mcali._expected_companion(lock_payload, lock_record)
    ):
        raise _error("E0-MCALJ historical P-E0-MCALI companion drifted")
    transitive = mcali._p_mcalh_authority(repo_root=repo_root)
    if _canonical_json_bytes(lock_payload.get("p_mcalh_authority")) != (
        _canonical_json_bytes(transitive)
    ):
        raise _error("E0-MCALJ transitive P-E0-MCALH authority drifted")
    sealed_runtime = lock_payload.get("runtime")
    if not isinstance(sealed_runtime, Mapping):
        raise _error("E0-MCALJ historical P-E0-MCALI runtime binding is absent")
    return {
        "gate": "P-E0-MCALI",
        "commit": BASE_P_MCALI_COMMIT,
        "parent_h_mcali": H_MCALI_COMMIT,
        "h_mcali_parent": H_MCALI_PARENT,
        "h_scope": h_scope,
        "p_scope": p_scope,
        "h_component_count": 9,
...
def _candidate_status_is_exact(repo_root: Path) -> bool:
    ...
    return all(
        ...
        in (
            {" M", "M ", "MM"}
            if path in H_MCALI_SUPERSEDED_PATHS
            else {"??", "A "}
        )
        for path in PATCH_PATHS
    )
...
def _h_patch_authority(
    *, repo_root: Path, verify_remote: bool
    ...
    if type(verify_remote) is not bool:
        raise _error("E0-MCALJ remote policy must be an exact boolean")
    head = mcal._git_head(repo_root)
    branch = cast(str, mcal._git(repo_root, "branch", "--show-current")).strip()
    if branch != "main":
        raise _error("E0-MCALJ requires branch main")
    expected_scope = {
        "added": 5,
        "modified": 4,
        "deleted": 0,
    ...
    candidate = head == BASE_P_MCALI_COMMIT
    if candidate:
        if not _candidate_status_is_exact(repo_root):
            raise _error("E0-MCALJ candidate workspace is not exact 4M+5A")
        component_commit: str | None = None
        h_head = BASE_P_MCALI_COMMIT
        scope = expected_scope
    else:
        if (
            mcal._single_parent(repo_root, head, context="H-E0-MCALJ")
            != BASE_P_MCALI_COMMIT
        ):
            raise _error("E0-MCALJ published H parent drifted")
        scope = mcal._git_scope(repo_root, BASE_P_MCALI_COMMIT, head)
        if scope != expected_scope or mcal._workspace_status_records(repo_root):
            raise _error("E0-MCALJ published H scope/worktree drifted")
        component_commit = head
        h_head = head
    components = [
        mcal._git_artifact_record(
            Path(path),
            role="final_calibration_platt_parameter_dialect_patch_component",
            repo_root=repo_root,
    ...
    expected_ref = BASE_P_MCALI_COMMIT if candidate else head
    if tracking != expected_ref:
        raise _error("E0-MCALJ H tracking ref drifted")
    remote = mcal._live_remote_main_head(repo_root) if verify_remote else tracking
    if remote != expected_ref:
        raise _error("E0-MCALJ H remote ref drifted")
    return (
        {
            "base_p_mcali_commit": BASE_P_MCALI_COMMIT,
            "h_patch_head": h_head,
            "branch": branch,
            "remote_head": remote,
            "scope": scope,
...
def _namespace_paths() -> tuple[Path, ...]:
    ...
    temporary_bases = (
        *forbidden_finals,
        mcali.mcale.mcalp.DEFAULT_PATCH_LOCK_PATH,
        mcali.mcale.mcalp.DEFAULT_PATCH_LOCK_MANIFEST_PATH,
        mcali.mcale.mcalc.DEFAULT_PATCH_LOCK_PATH,
        mcali.mcale.mcalc.DEFAULT_PATCH_LOCK_MANIFEST_PATH,
        mcali.mcale.mcald.DEFAULT_PATCH_LOCK_PATH,
        mcali.mcale.mcald.DEFAULT_PATCH_LOCK_MANIFEST_PATH,
        mcali.mcale.DEFAULT_PATCH_LOCK_PATH,
        mcali.mcale.DEFAULT_PATCH_LOCK_MANIFEST_PATH,
        mcali.DEFAULT_PATCH_LOCK_PATH,
        mcali.DEFAULT_PATCH_LOCK_MANIFEST_PATH,
    )
    return (
        *forbidden_finals,
        *(mcal._temporary_path(path) for path in temporary_bases),
        ...
        mcali.mcale.LOCKER_GUARD_PATH,
        mcali.LOCKER_GUARD_PATH,
        LOCKER_GUARD_PATH,
...
def _require_prelock_namespace(*, repo_root: Path) -> None:
    ...
    if missing_base:
        raise _error(f"E0-MCALJ base P-MCALI authority is absent: {missing_base}")
    occupied = [
        path.as_posix()
    ...
    if occupied:
        raise _error(f"E0-MCALJ prelock namespace is occupied: {occupied}")
    if mcal._entry_exists(Path(mcal.mze.OUTCOME_ACCESS_LOG), repo_root=repo_root):
        raise _error("E0-MCALJ outcome access log must remain absent")
    if any(
        mcal._entry_exists(Path(path), repo_root=repo_root)
        for path in mcal.mze.E0_M_PATHS
    ):
        raise _error("E0-MCALJ final E0-M namespace must remain absent")


@_error_boundary
def preflight_final_calibration_platt_parameter_dialect_patch_schema(
    *, repo_root: Path | None = None
) -> dict[str, Any]:
    root = _root(repo_root)
    schema = mcali.mcale.mcalp.mcal._load_json_object(
        DEFAULT_PATCH_LOCK_SCHEMA, repo_root=root
    )
    validator = getattr(
        mcali.mcale.mcalp.mcal.closure_contract, "_assert_supported_json_schema", None
    )
    if validator is None:
        raise _error("E0-MCALJ closed schema preflight is unavailable")
    try:
        validator(schema)
    except mcali.mcale.mcalp.mcal.ClosureContractError as exc:
    ...
    return {
        ...
        "schema_count": 1,
        "schemas": [
            mcali.mcale.mcalp.mcal._file_record(
                DEFAULT_PATCH_LOCK_SCHEMA,
                role="final_calibration_platt_parameter_dialect_patch_lock_schema",
                repo_root=root,
            )
        ],
...
def _state_for_h(
    *,
    ...
    schema_preflight = preflight_final_calibration_platt_parameter_dialect_patch_schema(
        repo_root=repo_root
    )
    runtime = mcal.load_and_validate_final_calibration_runtime(repo_root=repo_root)
    p_mcali = _p_mcali_authority(repo_root=repo_root)
    ...
    sealed_runtime = cast(Mapping[str, Any], p_mcali["sealed_runtime"])
    ...
    runtime_contract = {
        ...
        "platt_parameter_dialect_contract": _deep_copy(
            PLATT_PARAMETER_DIALECT_CONTRACT
        ),
    }
    boundary = {
...
@_error_boundary
def collect_final_calibration_platt_parameter_dialect_patch_prelock_state(
...
@_error_boundary
def build_final_calibration_platt_parameter_dialect_patch_lock_payload(
    ...
    *,
    ...
    if set(prelock) != required:
        raise _error("E0-MCALJ prelock dialect drifted")
    ...
    return {
        ...
        "p_mcali_authority": _deep_copy(prelock["p_mcali_authority"]),
        ...
        "platt_parameter_dialect_contract": _deep_copy(
            prelock["platt_parameter_dialect_contract"]
        ),
...
def _validate_timestamp(value: Any) -> None:
    if not isinstance(value, str):
        raise _error("E0-MCALJ generated timestamp is absent")
    try:
        parsed = datetime.fromisoformat(value.replace("Z", "+00:00"))
    except ValueError as exc:
        raise _error("E0-MCALJ generated timestamp is malformed") from exc
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise _error("E0-MCALJ generated timestamp must be timezone-aware")
...
def _validate_verification(value: Any, *, repo_root: Path) -> None:
    ...
    if not isinstance(value, Mapping) or set(value) != {
        ...
        raise _error("E0-MCALJ verification evidence dialect drifted")
    if _canonical_json_bytes(value["schema_preflight"]) != _canonical_json_bytes(
        preflight_final_calibration_platt_parameter_dialect_patch_schema(repo_root=repo_root)
    ):
        raise _error("E0-MCALJ schema verification evidence drifted")
    for key, command in (
    ...
    if (
        ...
        raise _error("E0-MCALJ focused verification evidence drifted")
    try:
        mcal._validate_command_evidence(
...
@_error_boundary
def validate_final_calibration_platt_parameter_dialect_patch_lock_payload(
    ...
    *,
    ...
    if not isinstance(payload, Mapping):
        raise _error("E0-MCALJ lock payload must be an object")
    ...
    if payload.get("authorizations") != UNPUBLISHED_AUTHORIZATIONS:
        raise _error("E0-MCALJ unpublished authorizations drifted")
    state = collect_final_calibration_platt_parameter_dialect_patch_prelock_state(
        verify_remote=verify_remote, repo_root=root
    )
    expected = build_final_calibration_platt_parameter_dialect_patch_lock_payload(
        state,
    ...
    if _canonical_json_bytes(payload) != _canonical_json_bytes(expected):
        raise _error("E0-MCALJ lock semantic reconstruction drifted")
    return dict(payload)


_OwnedOutput = mcali._OwnedOutput
...
def _publish_bytes_no_clobber(
    ...
    try:
        return mcali._publish_bytes_no_clobber(
...
def _rollback_owned_output(output: _OwnedOutput) -> None:
    try:
        mcali._rollback_owned_output(output)
    except mcali.FinalCalibrationError as exc:
        raise _error(_translate_predecessor_error(str(exc))) from exc
...
def _close_owned_output(output: _OwnedOutput) -> None:
    try:
        mcali._close_owned_output(output)
    except mcali.FinalCalibrationError as exc:
        raise _error(_translate_predecessor_error(str(exc))) from exc
...
def _expected_companion(
    ...
    base = cast(Mapping[str, Any], payload["p_mcali_authority"])
    ...
    if (
        ...
        raise _error("E0-MCALJ companion physical input set drifted")
...
def _require_publication_verification(
    ...
    if verification == _default_unrun_verification():
        raise _error(
            "E0-MCALJ publication requires the exact frozen verification evidence"
        )
...
def _physical_snapshot(
    ...
    records = [
        dict(record)
        for record in mcali._physical_snapshot(
            repo_root, scientific_inventory=scientific_inventory
        )
    ]
    known = {cast(str, record["path"]) for record in records}
    current_paths = [*H_MCALI_PRESERVED_PATHS, *P_MCALI_PATHS, *PATCH_PATHS]
...
def _require_physical_snapshot(
    ...
    *,
    ...
    if _canonical_json_bytes(expected) != _canonical_json_bytes(
        ...
        raise _error(f"E0-MCALJ physical authority changed {context}")
...
def _validate_owned_output_bytes(
    ...
    *,
    ...
    try:
        mcali._validate_owned_output(output)
        payload, metadata = mcal._read_regular_bytes_and_metadata(
            output.path, repo_root=repo_root
        )
    except Exception as exc:
        raise _error(f"E0-MCALJ owned output validation failed {context}") from exc
    if (
        payload != expected
        ...
        raise _error(
            f"E0-MCALJ owned output drifted {context}: {output.path.as_posix()}"
        )
    return metadata
...
def _rollback_outputs_best_effort(
...
) -> FinalCalibrationPlattParameterDialectPatchError | None:
    ...
    if not errors:
        return None
    error = _error("E0-MCALJ owned-output rollback was incomplete")
...
def _require_owned_guard_identity(guard: Any) -> None:
    try:
        ...
        raise _error("E0-MCALJ publication guard identity check failed") from exc
    if (
        ...
        raise _error("E0-MCALJ publication guard identity drifted")
...
def _require_publication_boundary(
    *, repo_root: Path, owned_guard: Any | None, outputs_present: bool
    ...
    if occupied:
        raise _error(f"E0-MCALJ publication namespace is occupied: {occupied}")
    if mcal._entry_exists(Path(mcal.mze.OUTCOME_ACCESS_LOG), repo_root=repo_root):
        raise _error("E0-MCALJ outcome access log must remain absent")
    if any(
        mcal._entry_exists(Path(path), repo_root=repo_root)
...
def _require_repository_checkpoint(
    *,
    ...
    if (
        ...
        raise _error(f"E0-MCALJ repository changed {context}")
    if verify_remote and mcal._live_remote_main_head(repo_root) != expected_head:
        raise _error(f"E0-MCALJ live remote changed {context}")
...
@_error_boundary
def publish_final_calibration_platt_parameter_dialect_patch_lock_bundle(
    ...
    try:
        ...
        raise _error("E0-MCALJ publication payload is not canonical JSON") from exc
    if (
        not isinstance(frozen_payload, dict)
        or _canonical_json_bytes(frozen_payload) != lock_bytes
    ):
        raise _error("E0-MCALJ publication payload must be a canonical object")
    payload = frozen_payload
    repository = payload.get("repository")
    if (
        not isinstance(repository, Mapping)
        or repository.get("h_patch_head") == BASE_P_MCALI_COMMIT
    ):
        raise _error("E0-MCALJ H must be published before P publication")
    ...
    if not isinstance(inventory, Mapping):
        raise _error("E0-MCALJ scientific inventory is absent")
    ...
    if initial_head != repository.get("h_patch_head"):
        raise _error("E0-MCALJ H refs drifted before publication")
    validate_final_calibration_platt_parameter_dialect_patch_lock_payload(
        payload, repo_root=root, verify_remote=True
    )
    ...
    try:
        try:
            guard = mt._acquire_publication_guard(
                LOCKER_GUARD_PATH,
                b"E0-MCALJ final calibration owned run guard lock publication\n",
                repo_root=root,
            )
        except Exception as exc:
            raise _error("E0-MCALJ publication guard acquisition failed") from exc
        _require_physical_snapshot(
            snapshot,
            scientific_inventory=inventory,
            repo_root=root,
        ...
        try:
            ...
            raise _error("E0-MCALJ publication guard release failed") from exc
        guard = None
        ...
        for pass_index in (1, 2):
            ...
            for output, expected in publication_set:
                ...
                if _metadata_identity(observed) != _metadata_identity(
                    ...
                    raise _error(
                        "E0-MCALJ owned output changed before ownership transfer"
                    )
        ...
        for output, expected in publication_set:
            ...
            if _metadata_identity(observed) != _metadata_identity(
                ...
                raise _error("E0-MCALJ owned output changed at ownership transfer")
        mcali._require_owned_identity_set(
            [output for output, _ in publication_set],
        ...
        if isinstance(exc, (KeyboardInterrupt, SystemExit)):
            raise
        if isinstance(exc, FinalCalibrationPlattParameterDialectPatchError):
            raise
        raise _error("E0-MCALJ lock bundle publication failed") from exc
    finally:
...
def _validate_calibration_filter_evidence(value: Any) -> None:
    ...
    if not isinstance(value, list) or len(value) != 5:
        raise _error("E0-MCALJ calibration filter evidence cardinality drifted")
    target = value[0]
    integer_keys = {
    ...
    if (
        ...
        raise _error("E0-MCALJ target filter evidence drifted")
    ...
    for raw, expected in zip(
        ...
        if (
            ...
            raise _error(f"E0-MCALJ raw exclusion evidence drifted: {model_id}")
...
def _validate_calibrator_specs(
    ...
    if not isinstance(value, Mapping) or value.get("gate") != PATCH_GATE:
        raise _error("E0-MCALJ calibrator spec gate drifted")
    try:
        adapted = _deep_copy(value)
    except (TypeError, ValueError, OverflowError, RecursionError) as exc:
        raise _error("E0-MCALJ calibrator specs are not canonical JSON") from exc
    records = adapted.get("bloom_calibrators")
    if not isinstance(records, list):
        raise _error("E0-MCALJ calibrator specs dialect drifted")
    for record in records:
        if not isinstance(record, dict) or record.get("selected_method") != (
            PLATT_PARAMETER_DIALECT_CONTRACT["method"]
        ):
            continue
        refit_spec = record.get("refit_spec")
        parameters = (
            refit_spec.get("parameters")
            if isinstance(refit_spec, dict)
            else None
        )
        if (
            not isinstance(parameters, dict)
            or set(parameters)
            != set(cast(Sequence[str], PLATT_PARAMETER_DIALECT_CONTRACT["parameter_keys"]))
            or type(parameters.get("coefficient")) is not float
            or not math.isfinite(cast(float, parameters["coefficient"]))
            or type(parameters.get("intercept")) is not float
            or not math.isfinite(cast(float, parameters["intercept"]))
            or type(parameters.get("input")) is not str
            or parameters.get("input")
            != PLATT_PARAMETER_DIALECT_CONTRACT["input_value"]
        ):
            raise _error("E0-MCALJ Platt parameters drifted")
        historical_parameters = dict(parameters)
        del historical_parameters["input"]
        refit_spec["parameters"] = historical_parameters
    adapted["gate"] = mcali.PATCH_GATE
    try:
        historical_index = mcali._validate_calibrator_specs(adapted)
    except mcali.FinalCalibrationError as exc:
        raise _error(_translate_predecessor_error(str(exc))) from exc
    original_records = cast(Sequence[Mapping[str, Any]], value["bloom_calibrators"])
    original_index = {
        (
            cast(str, record["model_id"]),
            cast(int, record["model_seed"]),
            cast(int, record["horizon_months"]),
        ): record
        for record in original_records
    }
    if list(original_index) != list(historical_index):
        raise _error("E0-MCALJ calibrator identity reconstruction drifted")
...
def _require_exact_output_group(
    ...
    *,
    ...
    if present and len(present) != len(paths):
        raise _error(f"E0-MCALJ {context} output bundle is partial")
    if not present:
        return 0
    ...
    for path in paths:
        ...
        if path.suffix == ".json":
            ...
            if payload != _canonical_json_bytes(value):
                raise _error(
                    f"E0-MCALJ {context} JSON output is not canonical: "
                    f"{path.as_posix()}"
                )
            json_values[path] = value
    manifest = json_values.get(manifest_path)
    if not isinstance(manifest, Mapping):
        raise _error(f"E0-MCALJ {context} manifest is absent")
    output_records = [
        {
            "path": path.as_posix(),
    ...
    if manifest.get("outputs") != output_records:
        raise _error(f"E0-MCALJ {context} manifest output bindings drifted")
    ...
    if (
        ...
        raise _error(f"E0-MCALJ {context} scientific boundary drifted")
    if context == "calibration":
        ...
        if (
            ...
            raise _error("E0-MCALJ calibration manifest scientific dialect drifted")
        evidence = manifest.get("input_filter_evidence")
        if not isinstance(evidence, list):
            raise _error("E0-MCALJ calibration input-filter evidence is absent")
        ...
        if (
            ...
            or not mcali._torch_policy_has_exact_integer_fields(
                execution.get("torch_cpu_execution_policy")
            )
        ):
            raise _error("E0-MCALJ calibration execution policy drifted")
        try:
        ...
        if (
            ...
            raise _error("E0-MCALJ E7 manifest scientific dialect drifted")
        ...
        if (
            ...
            raise _error("E0-MCALJ E7 execution policy drifted")
        try:
            mcali._require_exact_e7_sample_evidence_types(
        ...
        raise _error("E0-MCALJ output group context drifted")
    for path in paths:
        payload, observed = mcal._read_regular_bytes_and_metadata(
            path, repo_root=repo_root
        )
        if payload != payloads[path] or _metadata_identity(observed) != (
            ...
            raise _error(f"E0-MCALJ {context} output changed during validation")
    return len(paths)
...
def _parse_canonical_json_with_metadata(
    ...
    try:
        ...
        raise _error(f"E0-MCALJ published JSON is absent: {path.as_posix()}") from exc
    if not isinstance(value, dict) or payload != _canonical_json_bytes(value):
        raise _error(f"E0-MCALJ published JSON is not canonical: {path.as_posix()}")
    return value, payload, metadata


def _published_h_state(h_head: str, *, repo_root: Path) -> dict[str, Any]:
    if h_head == BASE_P_MCALI_COMMIT or re.fullmatch(r"[0-9a-f]{40}", h_head) is None:
        raise _error("E0-MCALJ published H commit is absent")
    if (
        mcal._single_parent(repo_root, h_head, context="H-E0-MCALJ")
        != BASE_P_MCALI_COMMIT
    ):
        raise _error("E0-MCALJ published H parent drifted")
    scope = mcal._git_scope(repo_root, BASE_P_MCALI_COMMIT, h_head)
    expected_scope = {
        "added": 5,
        "modified": 4,
        "deleted": 0,
    ...
    if scope != expected_scope:
        raise _error("E0-MCALJ published H scope drifted")
    components = [
        mcal._git_artifact_record(
            Path(path),
            role="final_calibration_platt_parameter_dialect_patch_component",
...
def _validate_published_lock_payload(
    ...
    if payload.get("authorizations") != UNPUBLISHED_AUTHORIZATIONS:
        raise _error("E0-MCALJ published authorizations drifted")
    repository = payload.get("repository")
    if not isinstance(repository, Mapping) or not isinstance(
        repository.get("h_patch_head"), str
    ):
        raise _error("E0-MCALJ published H binding is absent")
    ...
    if _canonical_json_bytes(payload) != _canonical_json_bytes(expected):
        raise _error("E0-MCALJ published lock reconstruction drifted")
...
def _validate_p_publication(
    ...
    if any(code != "??" or path not in allowed_r_paths for code, path in workspace):
        raise _error("E0-MCALJ published P worktree drifted")
    if (
        cast(str, mcal._git(repo_root, "branch", "--show-current")).strip()
        != "main"
        or mcal._single_parent(repo_root, head, context="P-E0-MCALJ") != h_head
        ...
        raise _error("E0-MCALJ published P topology drifted")
    tracking = mcal._git_head(repo_root, "origin/main")
    remote = mcal._live_remote_main_head(repo_root) if verify_remote else tracking
    if tracking != head or remote != head:
        raise _error("E0-MCALJ published P refs drifted")
    for path in (DEFAULT_PATCH_LOCK_PATH, DEFAULT_PATCH_LOCK_MANIFEST_PATH):
        ...
        if mode != "100644" or physical != mcal._git_blob_bytes(
            ...
            raise _error(
                f"E0-MCALJ published P physical/Git binding drifted: {path.as_posix()}"
            )
...
def _require_static_effective_boundary(*, repo_root: Path) -> None:
    ...
    if missing:
        raise _error(f"E0-MCALJ effective authority is absent: {missing}")
    forbidden = (
        mcal.DEFAULT_PATCH_LOCK_PATH,
        mcal.DEFAULT_PATCH_LOCK_MANIFEST_PATH,
        mcal._temporary_path(mcal.DEFAULT_PATCH_LOCK_PATH),
        mcal._temporary_path(mcal.DEFAULT_PATCH_LOCK_MANIFEST_PATH),
        mcal._temporary_path(mcali.mcale.mcalp.DEFAULT_PATCH_LOCK_PATH),
        mcal._temporary_path(mcali.mcale.mcalp.DEFAULT_PATCH_LOCK_MANIFEST_PATH),
        mcal._temporary_path(mcali.mcale.mcalc.DEFAULT_PATCH_LOCK_PATH),
        mcal._temporary_path(mcali.mcale.mcalc.DEFAULT_PATCH_LOCK_MANIFEST_PATH),
        mcal._temporary_path(mcali.mcale.mcald.DEFAULT_PATCH_LOCK_PATH),
        mcal._temporary_path(mcali.mcale.mcald.DEFAULT_PATCH_LOCK_MANIFEST_PATH),
        mcal._temporary_path(mcali.mcale.DEFAULT_PATCH_LOCK_PATH),
        mcal._temporary_path(mcali.mcale.DEFAULT_PATCH_LOCK_MANIFEST_PATH),
        mcal._temporary_path(mcali.DEFAULT_PATCH_LOCK_PATH),
        mcal._temporary_path(mcali.DEFAULT_PATCH_LOCK_MANIFEST_PATH),
    ...
    if occupied:
        raise _error(f"E0-MCALJ effective P namespace drifted: {occupied}")
    if mcal._entry_exists(Path(mcal.mze.OUTCOME_ACCESS_LOG), repo_root=repo_root):
        raise _error("E0-MCALJ outcome access log appeared")
    if any(
        mcal._entry_exists(Path(path), repo_root=repo_root)
        for path in mcal.mze.E0_M_PATHS
    ):
        raise _error("E0-MCALJ final E0-M outputs appeared")
...
def _validate_owned_capability(
    ...
    *,
    ...
    if not isinstance(capability, Mapping) or set(capability) != _OWNED_CAPABILITY_KEYS:
        raise _error(f"E0-MCALJ {context} capability dialect drifted")
    path = capability.get("path")
    if type(path) is not str or path != expected_path.as_posix():
        raise _error(f"E0-MCALJ {context} capability path drifted")
    if any(
        type(capability.get(field)) is not int
        for field in _OWNED_CAPABILITY_INTEGER_FIELDS
    ):
        raise _error(f"E0-MCALJ {context} capability identity types drifted")
    if (
        ...
        raise _error(f"E0-MCALJ {context} capability identity drifted")
    sha256 = capability.get("sha256")
    if type(sha256) is not str or re.fullmatch(r"[0-9a-f]{64}", sha256) is None:
        raise _error(f"E0-MCALJ {context} capability digest drifted")
    try:
        payload, metadata = mcal._read_regular_bytes_and_metadata(
            expected_path,
            repo_root=repo_root,
        ...
        raise _error(f"E0-MCALJ {context} owned name is not stable") from exc
    observed = {
    ...
    if any(capability[key] != value for key, value in observed.items()):
        raise _error(f"E0-MCALJ {context} capability no longer owns the name")
    if expected_payload is not None and payload != expected_payload:
        raise _error(f"E0-MCALJ {context} payload drifted")
...
def _validate_owned_guard_capability(
    ...
    if runner is not None:
        if runner not in allowed:
            raise _error("E0-MCALJ owned guard runner drifted")
        expected_path = allowed[runner]
    else:
        matches = [path for path in allowed.values() if path.as_posix() == path_value]
        if len(matches) != 1:
            raise _error("E0-MCALJ owned guard path is not registered")
    ...
    if (
        ...
        raise _error("E0-MCALJ owned guard sealed payload drifted")
...
def _expected_owned_output_paths(runner: str) -> tuple[Path, ...]:
    ...
    raise _error("E0-MCALJ owned output runner drifted")
...
def _validate_owned_output_capabilities(
    ...
    if isinstance(capabilities, (str, bytes, bytearray, Mapping)):
        raise _error("E0-MCALJ owned outputs must be a capability sequence")
    expected = _expected_owned_output_paths(runner)
    if len(capabilities) != len(expected):
        raise _error("E0-MCALJ owned output capability count drifted")
    by_path: dict[str, Mapping[str, Any]] = {}
    for capability in capabilities:
        if not isinstance(capability, Mapping) or type(capability.get("path")) is not str:
            raise _error("E0-MCALJ owned output capability dialect drifted")
        path = cast(str, capability["path"])
        if path in by_path:
            raise _error("E0-MCALJ owned output capability path is duplicated")
        by_path[path] = capability
    if set(by_path) != {path.as_posix() for path in expected}:
        raise _error("E0-MCALJ owned output capability set drifted")
...
def _validate_effective_namespace(
    *, repo_root: Path, owned_guard: Mapping[str, Any] | None = None
    ...
    if occupied:
        raise _error(
            "E0-MCALJ effective coordination/temporary namespace is occupied: "
            f"{occupied}"
        )
    ...
    if (calibration_count, e7_count) not in {(0, 0), (6, 0), (6, 2)}:
        raise _error("E0-MCALJ R bundle order drifted")
    lifecycle = {
...
def _require_effective_loading_checkpoint(
    *,
    ...
    if _canonical_json_bytes(observed) != _canonical_json_bytes(expected_namespace):
        raise _error(f"E0-MCALJ R namespace changed {context}")


def _load_effective_final_calibration_platt_parameter_dialect_patch_authority(
    *,
    verify_remote: bool,
    repo_root: Path | None,
    ...
    if _canonical_json_bytes(companion) != _canonical_json_bytes(
        ...
        raise _error("E0-MCALJ published companion drifted")
    ...
    if initial_head != publication["p_patch_head"]:
        raise _error("E0-MCALJ P refs do not match the captured baseline")
    _validate_published_lock_payload(payload, repo_root=root)
    recaptured = _validate_p_publication(
        payload, verify_remote=verify_remote, repo_root=root
    )
    if recaptured != publication:
        raise _error("E0-MCALJ P publication changed during baseline recapture")
    ...
    result = {
        ...
        **publication,
        ...
        "companion": mcal._file_record(
            DEFAULT_PATCH_LOCK_MANIFEST_PATH,
            role="final_calibration_platt_parameter_dialect_patch_lock_manifest",
            repo_root=root,
        ),
        "authority_binding_sha256": authority_sha256,
        "platt_parameter_dialect_correction": _deep_copy(
            PLATT_PARAMETER_DIALECT_CONTRACT
        ),
    ...
    if (
        ...
        raise _error("E0-MCALJ P authority changed during effective loading")
    _require_effective_loading_checkpoint(
        repo_root=root,
        expected_head=initial_head,
        verify_remote=verify_remote,
    ...
    if (
        ...
        raise _error("E0-MCALJ P authority changed at effective linearization")
    ...
    if _canonical_json_bytes(terminal_namespace) != _canonical_json_bytes(namespace):
        raise _error("E0-MCALJ R namespace changed at effective linearization")
    return result


@_error_boundary
def load_effective_final_calibration_platt_parameter_dialect_patch_authority(
    *, verify_remote: bool = True, repo_root: Path | None = None
) -> dict[str, Any]:
    """Load published authority; every active run guard remains forbidden."""

    return _load_effective_final_calibration_platt_parameter_dialect_patch_authority(
        verify_remote=verify_remote,
        repo_root=repo_root,
        owned_guard=None,
    )
...
def _require_lifecycle_authority(
    ...
    if (
        ...
        raise _error(f"E0-MCALJ {context} lifecycle authority drifted")
...
def revalidate_final_calibration_owned_run_publication(
    ...
    *,
    ...
    if type(runner) is not str or runner not in {"calibration", "e7"}:
        raise _error("E0-MCALJ owned publication runner drifted")
    if type(phase) is not str or phase not in {"active_guard", "post_release"}:
        raise _error("E0-MCALJ owned publication phase drifted")
    if type(verify_remote) is not bool:
        raise _error("E0-MCALJ owned publication remote policy drifted")
    if not isinstance(captured, Mapping):
        raise _error("E0-MCALJ captured authority must be an object")
    try:
        ...
        raise _error("E0-MCALJ owned publication capabilities are not canonical") from exc
    root = _root(repo_root)
    transitions = cast(
    ...
    if phase == "active_guard":
        if frozen_guard is None:
            raise _error("E0-MCALJ active phase requires the owned guard capability")
        _validate_owned_guard_capability(
            frozen_guard, repo_root=root, runner=runner
        )
    else:
        if frozen_guard is not None:
            raise _error("E0-MCALJ post-release phase forbids a guard capability")
        if mcal._entry_exists(expected_guard_path, repo_root=root):
            raise _error("E0-MCALJ post-release owned guard is still present")
    current = (
    ...
    if phase == "active_guard":
        ...
        raise _error("E0-MCALJ owned guard reappeared after release")
    _require_lifecycle_authority(
        current,
    ...
    if _canonical_json_bytes(_without_lifecycle_authority(frozen_captured)) != (
        ...
        raise _error("E0-MCALJ non-lifecycle authority changed during publication")
    return current
...
def require_final_calibration_authority(
    *, verify_remote: bool = True, repo_root: Path | None = None
    ...
    return load_effective_final_calibration_platt_parameter_dialect_patch_authority(
...
def require_final_calibration_run_namespace(
    *, runner: str, repo_root: Path | None = None
    ...
    root = _root(repo_root)
    if type(runner) is not str or runner not in {"calibration", "e7"}:
        raise _error("E0-MCALJ run namespace requires calibration or e7")
    ...
    if namespace["r_lifecycle_state"] != required_state:
        raise _error(f"E0-MCALJ {runner} one-shot namespace is not ready")
    return {
        "gate": PATCH_GATE,
        "runner": runner,
...
def __getattr__(name: str) -> Any:
    """Delegate unchanged scientific/runtime surfaces to predecessor E0-MCALI."""

    return getattr(mcali, name)
