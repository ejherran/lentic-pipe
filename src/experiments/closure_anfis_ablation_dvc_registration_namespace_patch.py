"""Repair transaction-prefix namespace validation under E0-MZB.

E0-MZB is an additive governance overlay over published P-E0-MZA.  It records
that the first R-E0-MZA DVC add completed, then the post-command family audit
rejected its allowed first pointer because the exact prediction tree still
listed only the ten payloads.  Rollback restored the repository-visible
baseline.  H/P-E0-MZB never fit, rewrite, register, or push anything.  Only
published P-E0-MZB can authorize the unchanged exact R scope.
"""
...
from src.experiments import closure_anfis_ablation_dvc_registration_order_patch as mza
...
PROJECT_ROOT = mza.PROJECT_ROOT
H_MZA_COMMIT = "4265b0a958761e7dabc410957932828c771b8e4c"
P_MZA_COMMIT = "b1f346f7191349901635fa7fa52807ea7031c39c"
BASE_COMMIT = P_MZA_COMMIT
PATCH_GATE = "E0-MZB"
SCHEMA_VERSION = "closure_anfis_ablation_dvc_registration_namespace_patch_lock_v1"
COMPANION_VERSION = (
    "closure_anfis_ablation_dvc_registration_namespace_patch_lock_manifest_v1"
)
...
LOCKER_GUARD_PATH = Path(
    "tmp/closure_v1_e0_mzb_locker/registration_namespace_patch.lock"
)
...
PATCH_COMPONENT_ROLES = {
    "configs/closure_v1/anfis_ablation_dvc_registration_namespace_patch_lock.schema.json": (
        "anfis_ablation_dvc_registration_namespace_patch_schema"
    ),
    "docs/closure_v1/E0_M_ANFIS_ABLATION_DVC_REGISTRATION_NAMESPACE_PATCH_1.md": (
        "anfis_ablation_dvc_registration_namespace_patch_protocol"
    ),
    ...
    "src/experiments/closure_anfis_ablation_dvc_registration_namespace_patch.py": (
        "anfis_ablation_dvc_registration_namespace_patch_validator"
    ),
    "src/experiments/lock_closure_anfis_ablation_dvc_registration_namespace_patch.py": (
        "anfis_ablation_dvc_registration_namespace_patch_locker"
    ),
    "tests/test_closure_anfis_ablation_dvc_registration_namespace_patch.py": (
        "anfis_ablation_dvc_registration_namespace_patch_test"
    ),
...
SUPERSEDED_MZA_PATHS = PATCH_MODIFIED_PATHS
PRESERVED_MZA_PATHS = tuple(
    path for path in mza.PATCH_PATHS if path not in set(SUPERSEDED_MZA_PATHS)
)
PATCH_COMPONENT_GIT_MODES = {
    path: "100755" if path == "src/data/prepare_commit_artifacts.py" else "100644"
    for path in PATCH_PATHS
}
ANFIS_ABLATION_H_MZB_STAGED_SCOPE = {
    path: ("A" if path in set(PATCH_ADDED_PATHS) else "M") for path in PATCH_PATHS
}
ANFIS_ABLATION_P_MZB_STAGED_SCOPE = {
    DEFAULT_PATCH_LOCK_PATH.as_posix(): "A",
    DEFAULT_PATCH_LOCK_MANIFEST_PATH.as_posix(): "A",
}
BASE_MZA_LOCK_PATH = mza.DEFAULT_PATCH_LOCK_PATH
BASE_MZA_COMPANION_PATH = mza.DEFAULT_PATCH_LOCK_MANIFEST_PATH
BASE_MZA_LOCK_BYTES = 47_562
BASE_MZA_LOCK_SHA256 = "7d25b46733d834f6f23bd1b8d7862ebb26ab6c980e92d6690cc9c293b64ab583"
BASE_MZA_COMPANION_BYTES = 9_556
BASE_MZA_COMPANION_SHA256 = "1009ecb6cf1b41b1c744e7b546f0c959d983724f0a9b0eac6afc8c5338852965"
...
OUTCOME_ACCESS_LOG = mza.OUTCOME_ACCESS_LOG
E0_M_PATHS = mza.E0_M_PATHS
ORDERED_SLOTS = mza.ORDERED_SLOTS
SLOT_ROLE_ORDER = (
    "model",
    "checkpoint",
    "preprocessor",
...
EXPECTED_HISTORICAL_INPUT_COUNT = 19
...
LOCK_SEALS = {
    ...
    "published_p_mza_reconstructed": True,
    "historical_h_mza_reconstructed": True,
    "blocked_r_mza_attempt_rolled_back": True,
...
class AnfisAblationDvcRegistrationNamespacePatchError(RuntimeError):
    """Raised when E0-MZB authority, artifacts, or topology drift."""


def _translate(exc: BaseException) -> AnfisAblationDvcRegistrationNamespacePatchError:
    return AnfisAblationDvcRegistrationNamespacePatchError(str(exc))
...
def _mzb_error_boundary(function: Callable[_P, _R]) -> Callable[_P, _R]:
    @wraps(function)
    def wrapped(*args: _P.args, **kwargs: _P.kwargs) -> _R:
        try:
            return function(*args, **kwargs)
        except AnfisAblationDvcRegistrationNamespacePatchError:
            raise
...
def _permissions_from_git_mode(value: str) -> int:
    if re.fullmatch(r"100[0-7]{3}", value) is None:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            f"E0-MZB unsupported regular-file Git mode: {value!r}"
        )
...
def _open_anchored_parent(path: Path, *, repo_root: Path) -> tuple[int, str]:
    if path.is_absolute() or not path.parts or any(
        ...
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            f"E0-MZB path must remain repository-relative: {path.as_posix()}"
        )
    ...
    *,
    ...
    try:
        ...
        if (
            ...
            raise AnfisAblationDvcRegistrationNamespacePatchError(
                f"E0-MZB path is not one stable regular {expected_mode:04o} file: "
                f"{path.as_posix()}"
            )
        chunks: list[bytes] = []
        while True:
        ...
        if identity_before != identity_after or identity_after != named_identity:
            raise AnfisAblationDvcRegistrationNamespacePatchError(
                f"E0-MZB path changed while read: {path.as_posix()}"
            )
        ...
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            f"Required stable E0-MZB input is unavailable: {path.as_posix()}"
        ) from exc
    finally:
...
def _git_blob_bytes(repo_root: Path, commit: str, path: Path) -> bytes:
    ...
    if result.returncode != 0 or result.stderr:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            f"E0-MZB cannot reconstruct Git blob {commit}:{path.as_posix()}"
        )
...
def _load_json(path: Path, *, repo_root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            f"E0-MZB JSON cannot be decoded: {path.as_posix()}"
        ) from exc
    if not isinstance(value, dict):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            f"E0-MZB JSON must be an object: {path.as_posix()}"
        )
    return value
...
def _validate_timestamp(value: Any) -> None:
    if not isinstance(value, str):
        raise AnfisAblationDvcRegistrationNamespacePatchError("E0-MZB timestamp is absent")
    try:
        parsed = datetime.fromisoformat(value.replace("Z", "+00:00"))
    except ValueError as exc:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB timestamp is malformed"
        ) from exc
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB timestamp must be timezone-aware"
        )
...
def _slot_paths(model_id: str, base_seed: int) -> dict[str, Path]:
    ...
    if set(paths) != set(SLOT_ROLE_ORDER):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            f"E0-MZB slot path roles drifted: {model_id}/{base_seed}"
        )
    return paths
...
def _family_physical_snapshot(repo_root: Path) -> tuple[dict[str, Any], ...]:
    ...
    if len(snapshot) != FAMILY_FINAL_COUNT:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB physical family snapshot must contain exact80 identities"
        )
...
def _require_family_physical_snapshot(
    ...
    if list(expected) != list(_family_physical_snapshot(repo_root)):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            f"E0-MZB exact80 physical family changed {context}"
        )
        ...
        sorted(
            {
                ...
                *(Path(path) for path in PRESERVED_MZA_PATHS),
                BASE_MZA_LOCK_PATH,
                BASE_MZA_COMPANION_PATH,
                DVC_CONFIG_PATH,
...
def _require_publication_state(
    ...
    *,
    ...
    if list(physical_snapshot) != list(_publication_input_snapshot(repo_root)):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            f"E0-MZB physical governance input changed {context}"
        )
    if not _exact_equal(git_snapshot, _publication_git_snapshot(repo_root)):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            f"E0-MZB Git publication state changed {context}"
        )
    _validate_family_namespace(
        registered=False,
        repo_root=repo_root,
...
def _dvc_configuration_contract(repo_root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB DVC repository/local configuration drifted"
        )
...
def _forbidden_family_namespace_paths() -> tuple[Path, ...]:
    return tuple(
        sorted(
            {
                *mza._forbidden_family_namespace_paths(),
    ...
    *,
    ...
    for path in expected_paths:
        try:
            ...
            raise AnfisAblationDvcRegistrationNamespacePatchError(
                f"E0-MZB {context} expected path escaped its root"
            ) from exc
        if relative == Path("."):
            raise AnfisAblationDvcRegistrationNamespacePatchError(
                f"E0-MZB {context} expected file path is malformed"
            )
    ...
    def walk(descriptor: int, relative_parent: Path) -> None:
        ...
        if not stat.S_ISDIR(before.st_mode):
            raise AnfisAblationDvcRegistrationNamespacePatchError(
                f"E0-MZB {context} walk escaped its directory root"
            )
        try:
            entries = sorted(os.listdir(descriptor))
        except OSError as exc:
            raise AnfisAblationDvcRegistrationNamespacePatchError(
                f"E0-MZB cannot walk exact {context} root"
            ) from exc
        for name in entries:
            relative = (
                Path(name)
            ...
            if stat.S_ISDIR(metadata.st_mode):
                if relative not in expected_directories:
                    raise AnfisAblationDvcRegistrationNamespacePatchError(
                        f"E0-MZB {context} contains an unexpected directory: {relative}"
                    )
                ...
                try:
                    ...
                    if identity(opened) != identity(metadata):
                        raise AnfisAblationDvcRegistrationNamespacePatchError(
                            f"E0-MZB {context} directory changed while opened: {relative}"
                        )
                    ...
                    if identity(os.fstat(child)) != identity(named_after):
                        raise AnfisAblationDvcRegistrationNamespacePatchError(
                            f"E0-MZB {context} directory changed while walked: {relative}"
                        )
                finally:
                    os.close(child)
                ...
                if (
                    ...
                    raise AnfisAblationDvcRegistrationNamespacePatchError(
                        f"E0-MZB {context} contains a foreign/non-0644 file: {relative}"
                    )
                file_descriptor = os.open(name, file_flags, dir_fd=descriptor)
                try:
                    ...
                    if (
                        ...
                        raise AnfisAblationDvcRegistrationNamespacePatchError(
                            f"E0-MZB {context} file changed while inspected: {relative}"
                        )
                ...
                raise AnfisAblationDvcRegistrationNamespacePatchError(
                    f"E0-MZB {context} contains a symlink/nonregular entry: {relative}"
                )
        if identity(before) != identity(os.fstat(descriptor)):
            raise AnfisAblationDvcRegistrationNamespacePatchError(
                f"E0-MZB {context} directory changed while walked: {relative_parent}"
            )
    ...
    try:
        ...
        if identity(root_before) != identity(named_before):
            raise AnfisAblationDvcRegistrationNamespacePatchError(
                f"E0-MZB {context} root changed while opened"
            )
        ...
        if (
            ...
            raise AnfisAblationDvcRegistrationNamespacePatchError(
                f"E0-MZB {context} root changed while walked"
            )
    except AnfisAblationDvcRegistrationNamespacePatchError:
        raise
    except OSError as exc:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            f"E0-MZB cannot open exact {context} root without following links"
        ) from exc
    finally:
    ...
    if observed_files != expected_files or observed_directories != expected_directories:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            f"E0-MZB {context} tree does not contain its exact closed namespace"
        )
...
def _validate_family_namespace(
    *,
    ...
    if type(registered) is not bool or type(allow_locker_guard) is not bool:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB namespace policy must use exact booleans"
        )
    if registration_transaction is not None and not registered:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB transaction coordination is post-registration only"
        )
    ...
    if occupied:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            f"E0-MZB ignored/coordination namespace is occupied: {occupied}"
        )
...
def _base_models_dvc_bytes() -> bytes:
    ...
    if len(payload) != BASE_MODELS_DVC_BYTES or _sha256_bytes(payload) != BASE_MODELS_DVC_SHA256:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB sealed base models.dvc constants drifted"
        )
    return payload
...
def _coordination_identity_record(
    ...
    *,
    ...
    if payload != expected_payload:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            f"E0-MZB transaction coordination bytes drifted: {path.as_posix()}"
        )
    return {
        "path": path.as_posix(),
        "device": int(metadata.st_dev),
...
def _coordination_directory_record(
    ...
    try:
        ...
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            f"E0-MZB coordination directory is absent/unreadable: {path.as_posix()}"
        ) from exc
    finally:
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            f"E0-MZB coordination directory identity drifted: {path.as_posix()}"
        )
    return {
        "path": path.as_posix(),
...
def _validate_registration_transaction_record(
    ...
    if not isinstance(value, Mapping) or set(value) != {
        ...
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB private transaction record dialect drifted"
        )
    mode = value.get("mode")
    if mode not in {"in_place", "atomic_replace"}:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB private transaction mode drifted"
        )
    expected_guard = _coordination_identity_record(
        REGISTRATION_GUARD_PATH,
        expected_mode=0o600,
        expected_payload=b"E0-MZB exact ANFIS-ablation DVC registration\n",
        repo_root=repo_root,
    )
    ...
    if not _exact_equal(value.get("guard"), expected_guard) or not _exact_equal(
        ...
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB private transaction identity record drifted"
        )
    ...
    if not _exact_equal(
        ...
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB private DVC config-isolation record drifted"
        )
    allowed = {
        REGISTRATION_GUARD_PATH,
        REGISTRATION_MODELS_BYTES_BACKUP_PATH,
    ...
    if mode == "in_place":
        if value.get("anchor") is not None or _lexists(
            ...
            raise AnfisAblationDvcRegistrationNamespacePatchError(
                "E0-MZB in-place transaction retained a hardlink anchor"
            )
    else:
        ...
        if not _exact_equal(value.get("anchor"), expected_anchor):
            raise AnfisAblationDvcRegistrationNamespacePatchError(
                "E0-MZB atomic transaction anchor record drifted"
            )
...
def _expected_registration_records(repo_root: Path) -> dict[str, Any]:
    ...
    if len(models) != EXPECTED_MODELS_DVC_BYTES or _sha256_bytes(models) != EXPECTED_MODELS_DVC_SHA256:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB expected models.dvc dialect drifted"
        )
    return {
...
def _registration_inventory(repo_root: Path) -> dict[str, Any]:
    try:
        return mza._registration_inventory(repo_root)
    except Exception as exc:
        raise _translate(exc) from exc
...
def _base_models_owner(repo_root: Path) -> dict[str, Any]:
    try:
        return mza._base_models_owner(repo_root)
    except Exception as exc:
        raise _translate(exc) from exc
...
def _h_components(head: str, repo_root: Path) -> list[dict[str, Any]]:
    ...
    for record in records:
        ...
        if (
            ...
            raise AnfisAblationDvcRegistrationNamespacePatchError(
                f"E0-MZB H component differs from Git: {record['path']}"
            )
    return records


def _historical_inputs(repo_root: Path) -> list[dict[str, Any]]:
    base_mza = _base_mza_authority(repo_root)
    inherited = [
        dict(record)
        for record in cast(
            Sequence[Mapping[str, Any]], base_mza["historical_inputs"]
        )
    ]
    superseded = [
        _historical_git_blob_record(
            path,
            role=f"superseded_h_mza_{mza.PATCH_COMPONENT_ROLES[path]}",
            commit=H_MZA_COMMIT,
            repo_root=repo_root,
        )
        for path in SUPERSEDED_MZA_PATHS
    ]
    ...
    if len(records) != EXPECTED_HISTORICAL_INPUT_COUNT or len(identities) != len(records):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB historical companion inputs must be exact19"
        )
    return records


def _historical_h_mza_authority(repo_root: Path) -> dict[str, Any]:
    expected_scope = {
        "added": 5,
        "modified": 5,
        "deleted": 0,
        "paths": list(mza.PATCH_PATHS),
    }
    if (
        _single_parent(repo_root, H_MZA_COMMIT, context="historical H-E0-MZA")
        != mza.BASE_COMMIT
        or _git_scope(repo_root, mza.BASE_COMMIT, H_MZA_COMMIT) != expected_scope
    ):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB historical H-E0-MZA topology drifted"
        )
    _require_exact_git_modes(
        repo_root,
        H_MZA_COMMIT,
        mza.PATCH_COMPONENT_GIT_MODES,
        context="historical H-E0-MZA",
    )
    preserved: list[dict[str, Any]] = []
    for path in PRESERVED_MZA_PATHS:
        role = mza.PATCH_COMPONENT_ROLES[path]
        current = _file_record(Path(path), role=role, repo_root=repo_root)
        historical = _historical_git_blob_record(
            path, role=role, commit=H_MZA_COMMIT, repo_root=repo_root
        )
        if current["bytes"] != historical["bytes"] or current["sha256"] != historical["sha256"]:
            raise AnfisAblationDvcRegistrationNamespacePatchError(
                f"E0-MZB preserved H-E0-MZA component drifted: {path}"
            )
    ...
    if len(preserved) != 4 or len(superseded) != 6:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB H-E0-MZA preserved/superseded partition drifted"
        )
    return {
        "gate": "E0-MZA",
        "head": H_MZA_COMMIT,
        "parent": mza.BASE_COMMIT,
...
def _base_mza_authority(repo_root: Path) -> dict[str, Any]:
    if (
        _single_parent(repo_root, P_MZA_COMMIT, context="P-E0-MZA") != H_MZA_COMMIT
        or _git_scope(repo_root, H_MZA_COMMIT, P_MZA_COMMIT)
        != {
            "added": 2,
            "modified": 0,
            "deleted": 0,
            "paths": sorted(
                (BASE_MZA_LOCK_PATH.as_posix(), BASE_MZA_COMPANION_PATH.as_posix())
            ),
        }
    ):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB base P-E0-MZA topology drifted"
        )
    lock = _file_record(
        BASE_MZA_LOCK_PATH,
    ...
    _require_exact_git_modes(
        ...
        P_MZA_COMMIT,
        {
            BASE_MZA_LOCK_PATH.as_posix(): "100644",
            BASE_MZA_COMPANION_PATH.as_posix(): "100644",
        },
        context="P-E0-MZA authority",
    )
    for path, record in (
        (BASE_MZA_LOCK_PATH, lock),
        (BASE_MZA_COMPANION_PATH, companion),
    ):
        git_payload = _git_blob_bytes(repo_root, P_MZA_COMMIT, path)
        if record["bytes"] != len(git_payload) or record["sha256"] != _sha256_bytes(git_payload):
            raise AnfisAblationDvcRegistrationNamespacePatchError(
                f"E0-MZB P-E0-MZA physical input differs from Git: {path.as_posix()}"
            )
    if (
        lock["bytes"] != BASE_MZA_LOCK_BYTES
        or lock["sha256"] != BASE_MZA_LOCK_SHA256
        or companion["bytes"] != BASE_MZA_COMPANION_BYTES
        or companion["sha256"] != BASE_MZA_COMPANION_SHA256
    ):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB sealed P-E0-MZA output hashes drifted"
        )
    lock_payload = _load_json(BASE_MZA_LOCK_PATH, repo_root=repo_root)
    companion_payload = _load_json(BASE_MZA_COMPANION_PATH, repo_root=repo_root)
    historical_inputs = companion_payload.get("historical_inputs")
    if (
        lock_payload.get("gate") != "E0-MZA"
        ...
        or len(historical_inputs) != mza.EXPECTED_HISTORICAL_INPUT_COUNT
        or not _exact_equal(historical_inputs, mza._historical_inputs(repo_root))
    ):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB P-E0-MZA semantic/history binding drifted"
        )
    return {
        "gate": "E0-MZA",
        "p_head": P_MZA_COMMIT,
...
def _namespace_correction() -> dict[str, Any]:
    targets = _selection_payload_paths()
    pointers = _selection_pointer_paths()
    if len(targets) != 10 or len(pointers) != 10:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB sealed first-pointer namespace incident drifted"
        )
    return {
        "blocked_gate": "E0-MZA",
        "published_p_mza_head": P_MZA_COMMIT,
        "status": "rolled_back_after_first_pointer_namespace_rejection",
        "registration_attempted": True,
        "dvc_add_command_count": 1,
        "completed_target_count": 1,
        "first_target": targets[0].as_posix(),
        "first_pointer": pointers[0].as_posix(),
        "pointer_count_at_failure": 1,
        "failure_phase": "post_add_family_namespace_validation",
        "root_cause": "prediction_tree_omitted_allowed_prefix_pointer",
...
def _companion_physical_inputs(
    *,
    ...
    base_mza: Mapping[str, Any],
    historical_h_mza: Mapping[str, Any],
) -> list[dict[str, Any]]:
    records = [
        dict(cast(Mapping[str, Any], base_mza["lock"])),
        dict(cast(Mapping[str, Any], base_mza["companion"])),
        *(
            dict(record)
            for record in cast(
                Sequence[Mapping[str, Any]], historical_h_mza["preserved_components"]
            )
        ),
    ...
    if len(records) != EXPECTED_COMPANION_INPUT_COUNT or len(identities) != len(records):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB companion physical inputs must be exact17"
        )
    return records
...
def _expected_untracked_light_status(repo_root: Path) -> list[str]:
    ...
    if tracked != expected_tracked or len(tracked) != FAMILY_TRACKED_LIGHT_COUNT:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB all fifty light outputs must remain tracked"
        )
    return []
...
@_mzb_error_boundary
def preflight_anfis_ablation_dvc_registration_namespace_patch_schema(
    *, repo_root: Path | None = None
    ...
    if unsupported:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            f"E0-MZB schema exceeds the supported subset: {unsupported}"
        )
    ...
    try:
        ...
        raise AnfisAblationDvcRegistrationNamespacePatchError(str(exc)) from exc
...
@_mzb_error_boundary
def collect_anfis_ablation_dvc_registration_namespace_patch_prelock_state(
    *,
    ...
    if type(verify_remote) is not bool or type(_allow_locker_guard) is not bool:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB prelock policies must be exact booleans"
        )
    root = _root(repo_root)
    preflight_anfis_ablation_dvc_registration_namespace_patch_schema(repo_root=root)
    head = _git_head(root)
    expected_scope = {
        "added": 5,
        "modified": 6,
    ...
    if (
        _single_parent(root, head, context="H-E0-MZB") != BASE_COMMIT
        or _git_scope(root, BASE_COMMIT, head) != expected_scope
    ):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "H-E0-MZB must be the exact 6M+5A child of published P-E0-MZA"
        )
    _require_exact_git_modes(
        root, head, PATCH_COMPONENT_GIT_MODES, context="H-E0-MZB"
    )
    ...
    if branch != "main" or tracking != head or remote != head:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "H-E0-MZB refs are not aligned with main"
        )
    status = [
        line
    ...
    if status != expected_status:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            f"E0-MZB prelock worktree must be clean with all fifty lights tracked: {status}"
        )
    ...
    if occupied_outputs:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            f"E0-MZB prelock output namespace is occupied: {occupied_outputs}"
        )
    ...
    return {
        ...
        "base_mza_authority": base_mza,
        "historical_h_mza": historical_h_mza,
        "namespace_correction": namespace,
...
@_mzb_error_boundary
def build_anfis_ablation_dvc_registration_namespace_patch_lock_payload(
    ...
    *,
    ...
    if set(prelock) != required:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB prelock bundle dialect drifted"
        )
    return {
...
def _validate_command_evidence(
    ...
    *,
    ...
    if not isinstance(value, Mapping) or set(value) != keys:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            f"E0-MZB {context} evidence dialect drifted"
        )
    if (
        ...
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            f"E0-MZB {context} command/result drifted"
        )
    if (
        ...
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            f"E0-MZB {context} digest/line evidence drifted"
        )
    if exact_stdout is not None and (
        ...
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            f"E0-MZB {context} stdout evidence drifted"
        )
...
def _validate_verification(value: Any, *, repo_root: Path) -> None:
    if not isinstance(value, Mapping):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB verification evidence is absent"
        )
    required = {
        "schema_preflight",
        "type_check",
    ...
    if set(value) != required:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB verification evidence key set drifted"
        )
    expected_schema = preflight_anfis_ablation_dvc_registration_namespace_patch_schema(
        repo_root=repo_root
    )
    if not _exact_equal(value.get("schema_preflight"), expected_schema):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB schema-preflight evidence drifted"
        )
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB focused pytest evidence drifted"
        )
    ...
    if not isinstance(stdout_text, str):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB focused pytest stdout text is absent"
        )
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB focused pytest stdout binding drifted"
        )
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB family semantic audit evidence drifted"
        )
    ...
    if not _exact_equal(boundaries, expected_boundaries):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB verification execution boundaries drifted"
        )


def _validate_anfis_ablation_dvc_registration_namespace_patch_lock_payload(
    ...
    *,
    ...
    if type(allow_registered_state) is not bool:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB registered-state policy must be an exact boolean"
        )
    if _registration_transaction is not None and not allow_registered_state:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB private transaction record requires registered-state audit"
        )
    root = _root(repo_root)
    schema = _load_json(DEFAULT_PATCH_LOCK_SCHEMA, repo_root=root)
    try:
    ...
    if payload.get("schema_version") != SCHEMA_VERSION or payload.get("gate") != PATCH_GATE:
        raise AnfisAblationDvcRegistrationNamespacePatchError("E0-MZB lock identity drifted")
    if not _exact_equal(payload.get("authorizations"), UNPUBLISHED_AUTHORIZATIONS):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB unpublished authorizations drifted"
        )
    if not _exact_equal(payload.get("seals"), LOCK_SEALS):
        raise AnfisAblationDvcRegistrationNamespacePatchError("E0-MZB seals drifted")
    repository = payload.get("repository")
    if not isinstance(repository, Mapping) or not isinstance(repository.get("head"), str):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB repository binding is absent"
        )
    h_head = str(repository["head"])
    if (
        SHA1_RE.fullmatch(h_head) is None
        or repository.get("parent") != BASE_COMMIT
        or _single_parent(root, h_head, context="H-E0-MZB") != BASE_COMMIT
        or _git_scope(root, BASE_COMMIT, h_head)
        != {"added": 5, "modified": 6, "deleted": 0, "paths": list(PATCH_PATHS)}
    ):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB repository/H topology drifted"
        )
    ...
    if not _exact_equal(h_patch, expected_h):
        raise AnfisAblationDvcRegistrationNamespacePatchError("E0-MZB H binding drifted")
    base_mza = _base_mza_authority(root)
    if not _exact_equal(payload.get("base_mza_authority"), base_mza):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB published P-E0-MZA reconstruction drifted"
        )
    historical_h_mza = _historical_h_mza_authority(root)
    if not _exact_equal(payload.get("historical_h_mza"), historical_h_mza):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB historical H-E0-MZA reconstruction drifted"
        )
    namespace = _namespace_correction()
    if not _exact_equal(payload.get("namespace_correction"), namespace):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB blocked R-E0-MZA namespace incident binding drifted"
        )
    inventory = _registration_inventory(root)
    if not _exact_equal(payload.get("artifact_inventory"), inventory):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB registration inventory binding drifted"
        )
    family = _family_records(
        root,
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB completed family binding drifted"
        )
    models_before = (
    ...
    if not _exact_equal(payload.get("models_owner_transition"), expected_transition):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB models.dvc transition drifted"
        )
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB exact registration plan drifted"
        )
    ...
    if not _exact_equal(payload.get("companion_contract"), expected_contract):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB companion contract drifted"
        )
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB prelock evidence drifted"
        )
    _validate_verification(payload.get("verification"), repo_root=root)


@_mzb_error_boundary
def validate_anfis_ablation_dvc_registration_namespace_patch_lock_payload(
    payload: Mapping[str, Any],
    *,
    allow_registered_state: bool = False,
    repo_root: Path | None = None,
...
def _expected_companion(
    ...
    *,
    ...
    root = _root(repo_root)
    h_patch = payload.get("h_patch")
    base_mza = payload.get("base_mza_authority")
    historical_h_mza = payload.get("historical_h_mza")
    if (
        not isinstance(h_patch, Mapping)
        or not isinstance(base_mza, Mapping)
        or not isinstance(historical_h_mza, Mapping)
    ):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "Cannot construct E0-MZB companion"
        )
    components = h_patch.get("components")
    if not isinstance(components, list):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "Cannot construct E0-MZB companion component list"
        )
    ...
    if script is None:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB companion script is absent"
        )
    output = dict(lock_record)
    if (
        output.get("path") != DEFAULT_PATCH_LOCK_PATH.as_posix()
        or output.get("role") != "anfis_ablation_dvc_registration_namespace_patch_lock"
    ):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB companion output record drifted"
        )
    return {
...
def _run_command(
    ...
    *,
    ...
    if result.returncode != 0:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            f"E0-MZB verification command failed: {list(command)}"
        )
...
def _parse_focused_summary(stdout: str, stderr: str) -> dict[str, int]:
    if stderr or FORBIDDEN_FOCUSED_SUMMARY_RE.search(stdout):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB focused pytest result is not one clean pass"
        )
    ...
    if len(matched) != 1 or lines[-1] != matched[0].group(0):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB focused pytest summary is not unique and terminal"
        )
    return {
...
@_mzb_error_boundary
def run_anfis_ablation_dvc_registration_namespace_patch_verification(
    *,
    ...
    schema = preflight_anfis_ablation_dvc_registration_namespace_patch_schema(repo_root=root)
    _require_family_physical_snapshot(
...
) -> AnfisAblationDvcRegistrationNamespacePatchError | None:
    first_error: AnfisAblationDvcRegistrationNamespacePatchError | None = None
    for output in reversed(published):
        try:
            mt._rollback_owned_output(output)
        except Exception as exc:
            if first_error is None:
                first_error = AnfisAblationDvcRegistrationNamespacePatchError(
                    "E0-MZB owned-output rollback failed for "
...
def _close_published_outputs_best_effort(
    ...
    for output in published:
        try:
            ...
            if first_error is None:
                first_error = AnfisAblationDvcRegistrationNamespacePatchError(
                    "E0-MZB owned-output descriptor close failed for "
                    f"{output.path.as_posix()}: {exc}"
                )
    ...
    *,
    ...
    try:
        ...
        if (
            ...
            raise AnfisAblationDvcRegistrationNamespacePatchError(
                f"E0-MZB owned output identity/metadata drifted {context}: "
        ...
        if (
            ...
            raise AnfisAblationDvcRegistrationNamespacePatchError(
                f"E0-MZB owned output bytes drifted {context}: "
                f"{output.path.as_posix()}"
            )
        mt._validate_owned_output(output)
    except AnfisAblationDvcRegistrationNamespacePatchError:
...
@_mzb_error_boundary
def execute_and_publish_anfis_ablation_dvc_registration_namespace_patch_lock_bundle(
    *, repo_root: Path | None = None
    ...
    before = collect_anfis_ablation_dvc_registration_namespace_patch_prelock_state(
        verify_remote=True, repo_root=root
    )
    verification = run_anfis_ablation_dvc_registration_namespace_patch_verification(
        family_snapshot=family_snapshot,
        publication_input_snapshot=input_snapshot,
        publication_git_snapshot=git_snapshot,
        repo_root=root,
    )
    after = collect_anfis_ablation_dvc_registration_namespace_patch_prelock_state(
        verify_remote=True, repo_root=root
    )
    if not _exact_equal(before, after):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB prelock state changed during verification"
        )
    _require_family_physical_snapshot(
    ...
    payload = build_anfis_ablation_dvc_registration_namespace_patch_lock_payload(
        before, verification, created_at_utc=datetime.now(timezone.utc).isoformat()
    )
    validate_anfis_ablation_dvc_registration_namespace_patch_lock_payload(payload, repo_root=root)
    controlled = (
    ...
    if occupied:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            f"E0-MZB lock namespace is occupied: {occupied}"
        )
    ...
    try:
        guard = mt._acquire_publication_guard(
            LOCKER_GUARD_PATH,
            b"E0-MZB lock bundle publication in progress\n",
            repo_root=root,
        )
        _require_family_physical_snapshot(
        ...
        if not _exact_equal(
            ...
            raise AnfisAblationDvcRegistrationNamespacePatchError(
                "E0-MZB guarded prelock state drifted"
            )
        ...
        lock_record = {
            "role": "anfis_ablation_dvc_registration_namespace_patch_lock",
        ...
        if isinstance(exc, AnfisAblationDvcRegistrationNamespacePatchError):
            translated = exc
...
@_mzb_error_boundary
def publish_anfis_ablation_dvc_registration_namespace_patch_lock_bundle(
    *, repo_root: Path | None = None
) -> tuple[dict[str, Any], dict[str, Any]]:
    return execute_and_publish_anfis_ablation_dvc_registration_namespace_patch_lock_bundle(
        repo_root=repo_root
    )
...
def _validate_p_publication(
    ...
    if not isinstance(repository, Mapping) or not isinstance(repository.get("head"), str):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB H repository binding is absent"
        )
    h_head = str(repository["head"])
    head = _git_head(repo_root)
    tracking = _git_head(repo_root, "origin/main")
    ...
    if (
        ...
        or _single_parent(repo_root, head, context="P-E0-MZB") != h_head
        or tracking != head
        or remote != head
        ...
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "Published P-E0-MZB topology/refs drifted"
        )
    _require_exact_git_modes(
        repo_root,
        head,
    ...
    for raw_path in expected_paths:
        ...
        if physical != _git_blob_bytes(repo_root, head, path):
            raise AnfisAblationDvcRegistrationNamespacePatchError(
                f"Published P-E0-MZB physical bytes differ from Git: {raw_path}"
            )
...
def _validate_registered_state(
    ...
    for record in pointers:
        ...
        if len(payload) != record["bytes"] or _sha256_bytes(payload) != record["sha256"]:
            raise AnfisAblationDvcRegistrationNamespacePatchError(
                f"E0-MZB registered pointer drifted: {path.as_posix()}"
            )
    ...
    if len(payload) != models["bytes"] or _sha256_bytes(payload) != models["sha256"]:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB registered models.dvc drifted"
        )
    return {
...
def _load_effective_anfis_ablation_dvc_registration_namespace_patch_authority(
    *,
    ...
    if type(audit_current_unpublished) is not bool or verify_remote is not True:
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB effective authority requires exact audit mode and live remote"
        )
    root = _root(repo_root)
    ...
    try:
        ...
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "Published E0-MZB lock is not valid UTF-8 JSON"
        ) from exc
    if not isinstance(payload, dict) or payload_bytes != _canonical_json(payload):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "Published E0-MZB lock is not canonical JSON"
        )
    ...
    try:
        ...
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "Published E0-MZB companion is not valid UTF-8 JSON"
        ) from exc
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "Published E0-MZB companion drifted"
        )
    ...
    return {
        ...
        **publication,
        ...
        "companion": _file_record(
            DEFAULT_PATCH_LOCK_MANIFEST_PATH,
            role="anfis_ablation_dvc_registration_namespace_patch_lock_manifest",
            repo_root=root,
        ),
        "family_records_sha256": FAMILY_RECORDS_SHA256,
...
@_mzb_error_boundary
def load_effective_anfis_ablation_dvc_registration_namespace_patch_authority(
    *,
    audit_current_unpublished: bool = False,
    verify_remote: bool = True,
    repo_root: Path | None = None,
    ...
    return _load_effective_anfis_ablation_dvc_registration_namespace_patch_authority(
        audit_current_unpublished=audit_current_unpublished,
...
@_mzb_error_boundary
def _load_effective_anfis_ablation_dvc_registration_namespace_patch_during_registration(
    *,
    transaction_record: Mapping[str, Any],
    verify_remote: bool = True,
    repo_root: Path | None = None,
) -> dict[str, Any]:
    """Internal R-E0-MZB audit while durable rollback coordination is owned."""
    if not isinstance(transaction_record, Mapping):
        raise AnfisAblationDvcRegistrationNamespacePatchError(
            "E0-MZB internal transaction record is absent"
        )
    return _load_effective_anfis_ablation_dvc_registration_namespace_patch_authority(
...
@_mzb_error_boundary
def require_anfis_ablation_dvc_registration_namespace_patch_authority(
    *,
    audit_current_unpublished: bool = False,
    verify_remote: bool = True,
    repo_root: Path | None = None,
) -> dict[str, Any]:
    return load_effective_anfis_ablation_dvc_registration_namespace_patch_authority(
        audit_current_unpublished=audit_current_unpublished,
        verify_remote=verify_remote,
        repo_root=repo_root,
    )
...
def require_anfis_ablation_dvc_registration_authority(
    *, verify_remote: bool = True, repo_root: Path | None = None
    ...
    return require_anfis_ablation_dvc_registration_namespace_patch_authority(
...
@_mzb_error_boundary
def load_effective_anfis_ablation_dvc_registration_namespace_patch(
    *,
    audit_current_unpublished: bool = False,
    verify_remote: bool = True,
    repo_root: Path | None = None,
...
__all__ = [
    "AnfisAblationDvcRegistrationNamespacePatchError",
    "ANFIS_ABLATION_H_MZB_STAGED_SCOPE",
    "ANFIS_ABLATION_P_MZB_STAGED_SCOPE",
    "ANFIS_ABLATION_R_MZB_STAGED_SCOPE",
    "BASE_COMMIT",
    ...
    "collect_anfis_ablation_dvc_registration_namespace_patch_prelock_state",
    "execute_and_publish_anfis_ablation_dvc_registration_namespace_patch_lock_bundle",
    "load_effective_anfis_ablation_dvc_registration_namespace_patch_authority",
    "load_effective_anfis_ablation_dvc_registration_namespace_patch",
    ...
    "require_anfis_ablation_dvc_registration_namespace_patch_authority",
