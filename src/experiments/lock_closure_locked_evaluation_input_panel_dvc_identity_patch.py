#!/usr/bin/env python
"""Check, publish, and audit the E0-MIC panel-DVC identity authority."""
...
def _error(
    message: str,
) -> patch.ClosureLockedEvaluationInputPanelDvcIdentityPatchError:
    return patch.ClosureLockedEvaluationInputPanelDvcIdentityPatchError(message)
...
def _parse_focused_summary(stdout: str, stderr: str) -> dict[str, int]:
    ...
    if type(count) is not int or count <= 0:
        raise _error("E0-MIC focused-test count is not frozen")
...
def run_closure_locked_evaluation_input_panel_dvc_identity_patch_verification(
    *, expected_schema_preflight: Mapping[str, Any] | None = None
) -> dict[str, Any]:
    """Run the exact H checks without input-bundle science, DVC, or outcomes."""

    schema_preflight = (
        patch.preflight_closure_locked_evaluation_input_panel_dvc_identity_patch_schema()
    )
    if (
        expected_schema_preflight is not None
        and dict(expected_schema_preflight) != schema_preflight
    ):
        raise _error("E0-MIC schema changed before verification")
...
def check_only() -> dict[str, Any]:
    """Capture H and the panel hardlink contract twice without writes."""

    schema = (
        patch.preflight_closure_locked_evaluation_input_panel_dvc_identity_patch_schema()
    )
    source_before = patch._source_identity_snapshot(PROJECT_ROOT)
    physical_before = patch._physical_snapshot(PROJECT_ROOT)
    prelock_before = (
        patch.collect_closure_locked_evaluation_input_panel_dvc_identity_patch_prelock_state(
            verify_remote=True,
    ...
    physical_after = patch._physical_snapshot(PROJECT_ROOT)
    source_after = patch._source_identity_snapshot(PROJECT_ROOT)
    if prelock_before != prelock_after:
        raise _error("E0-MIC topology, namespace, or panel identity changed")
    if physical_before != physical_after:
        raise _error("E0-MIC physical authority inputs changed during check-only")
    if source_before != source_after:
        raise _error("E0-MIC source identities changed during check-only")
    return {
        ...
        "source_identity_count": len(source_before),
        ...
        "r_tracked_output_count": len(patch.R_TRACKED_OUTPUT_PATHS),
        "r_physical_output_count": len(patch.R_PHYSICAL_OUTPUT_PATHS),
        "panel_dvc_identity_verified": True,
        "physical_snapshot_revalidated": True,
        "source_identity_snapshot_revalidated": True,
...
def execute_lock() -> dict[str, Any]:
    ...
    physical_before = patch._physical_snapshot(PROJECT_ROOT)
    verification = (
        run_closure_locked_evaluation_input_panel_dvc_identity_patch_verification(
            expected_schema_preflight=schema
        )
    )
    physical_after = patch._physical_snapshot(PROJECT_ROOT)
    if physical_before != physical_after:
        raise _error("E0-MIC physical authority inputs changed during verification")
    after = (
        patch.collect_closure_locked_evaluation_input_panel_dvc_identity_patch_prelock_state(
            verify_remote=True,
            repo_root=PROJECT_ROOT,
    ...
    if before != after:
        raise _error("E0-MIC topology, namespace, or panel identity changed")
    if source_before != source_after:
        raise _error("E0-MIC source identities changed during verification")
    payload = (
        patch.build_closure_locked_evaluation_input_panel_dvc_identity_patch_lock_payload(
            prelock=before,
            verification=verification,
        )
    )
    patch.validate_closure_locked_evaluation_input_panel_dvc_identity_patch_lock_payload(
        payload,
        verify_remote=True,
        repo_root=PROJECT_ROOT,
    )
    lock, companion = (
        patch.publish_closure_locked_evaluation_input_panel_dvc_identity_patch_lock_bundle(
            payload,
            repo_root=PROJECT_ROOT,
        )
    )
...
def check_effective() -> dict[str, Any]:
    return (
        patch.load_effective_closure_locked_evaluation_input_panel_dvc_identity_patch_authority(
            verify_remote=True,
            repo_root=PROJECT_ROOT,
        )
    )
