#!/usr/bin/env python
"""Validate the additive Closure V1 P1/20260614 temporal-consumer authority.

E0-MM is a fail-closed successor to the published P1/20260614 sequence
bundle.  It reconstructs E0-MK and E0-ML from immutable Git history, binds
the new sequence auditor without running it during H-E0-MM, and can authorize
only the P1/20260614/CPU consumer after a future two-file P-E0-MM publication.
...
from src.experiments import audit_closure_p1_seed_20260614_sequence_bundle as p1_audit
...
LOCK_VERSION = "closure_p1_temporal_consumer_seed_20260614_patch_lock_v1"
PATCH_GATE = "E0-MM"
PATCH_ID = "p1_temporal_consumer_seed_20260614_authority_patch_1"
PATCH_STATUS = "locked"
...
P1_BUNDLE_COMMIT = "9b40b2bea49084aab1ba37a1a5e4b87261a83fae"
PATCH_BASE_COMMIT = P1_BUNDLE_COMMIT

DEFAULT_PATCH_LOCK_PATH = Path(
    "reports/closure_v1/00_protocol/"
    "p1_temporal_consumer_seed_20260614_patch_lock.json"
)
DEFAULT_PATCH_MANIFEST_PATH = Path(
    "reports/closure_v1/00_protocol/"
    "p1_temporal_consumer_seed_20260614_patch_lock_manifest.json"
)
...
PATCH_COMPONENT_ROLES = {
    DEFAULT_PATCH_LOCK_SCHEMA.as_posix(): (
        "p1_temporal_consumer_seed_20260614_patch_lock_schema"
    ),
    "docs/closure_v1/E0_M_P1_TEMPORAL_CONSUMER_SEED_20260614_PATCH_1.md": (
        "p1_temporal_consumer_seed_20260614_patch_protocol"
    ),
    "src/experiments/closure_p1_temporal_consumer_seed_20260614_patch.py": (
        "p1_temporal_consumer_seed_20260614_patch_validator"
    ),
    "src/experiments/lock_closure_p1_temporal_consumer_seed_20260614_patch.py": (
        "p1_temporal_consumer_seed_20260614_patch_locker"
    ),
    "src/experiments/train_closure_pipe.py": "p1_temporal_consumer_mm_gate_routing",
    "tests/test_closure_p1_temporal_consumer_seed_20260614_patch.py": (
        "p1_temporal_consumer_seed_20260614_patch_tests"
    ),
    "tests/test_train_closure_pipe.py": "p1_temporal_consumer_mm_gate_tests",
}
...
MK_SUPERSEDED_PATHS = PATCH_MODIFIED_PATHS
MK_PRESERVED_PATHS = tuple(
    path for path in e0_mk.PATCH_PATHS if path not in MK_SUPERSEDED_PATHS
)
...
PATCH_SEALS = {
    ...
    "mk_preserved_component_count": 5,
    "mk_superseded_component_count": 2,
    "ml_preserved_component_count": 7,
...
# Exact closed collection for H/P-E0-MM plus the live P1/20260614 auditor.
...
AUDITOR_MODULE = "src.experiments.audit_closure_p1_seed_20260614_sequence_bundle"
AUDITOR_NAME = "audit_p1_seed_20260614_sequence_bundle"
AUDITOR_QUALNAME = "audit_p1_seed_20260614_sequence_bundle"
AUDITOR_SOURCE_PATH = p1_audit.AUDITOR_PATH.as_posix()


class P1TemporalConsumerSeed20260614PatchError(RuntimeError):
    """Raised when E0-MM cannot prove its closed authority."""
...
def _assert_schema_definition_supported(schema: Mapping[str, Any]) -> None:
    ...
    if not callable(validator):
        raise P1TemporalConsumerSeed20260614PatchError(
            "E0-MM closure-contract schema-definition validator is unavailable"
        )
    try:
        validator(schema)
...
def preflight_p1_temporal_consumer_seed_20260614_patch_schema(
    ...
    if schema_path != DEFAULT_PATCH_LOCK_SCHEMA:
        raise P1TemporalConsumerSeed20260614PatchError(
            "E0-MM schema preflight requires the closed default path"
        )
    schema = _load_regular_json(schema_path, context="E0-MM schema preflight")
    ...
    if minimum_count or format_count:
        raise P1TemporalConsumerSeed20260614PatchError(
            "E0-MM schema contains keywords outside the closed contract subset: "
            f"minimum={minimum_count}, format={format_count}"
        )
    _assert_schema_definition_supported(schema)
    record = _file_record(
        schema_path,
        role="p1_temporal_consumer_seed_20260614_patch_lock_schema",
    )
    if type(record["bytes"]) is not int or int(record["bytes"]) <= 0:
        raise P1TemporalConsumerSeed20260614PatchError(
            "E0-MM schema must be a non-empty regular JSON file"
        )
    return {
        "gate": PATCH_GATE,
...
def patch_git_diff_payload(patch_head: str) -> dict[str, Any]:
    patch_head = _require_commit(patch_head, context="H-E0-MM")
    ancestry = _git("rev-list", "--parents", "-n", "1", patch_head).split()
    if ancestry != [patch_head, PATCH_BASE_COMMIT]:
        raise P1TemporalConsumerSeed20260614PatchError(
            "H-E0-MM must be the direct non-merge child of P1/20260614"
        )
    expected = [
        {
    ...
    if observed != expected:
        raise P1TemporalConsumerSeed20260614PatchError(
            f"H-E0-MM diff differs from its closed 2M+5A allowlist: {observed}"
        )
    return {
...
def _historical_e0_mk_authority(*, execution_head: str) -> dict[str, Any]:
    """Reconstruct E0-MK with trainer and trainer tests explicitly superseded."""
    ...
    try:
        validate_json_schema(payload, schema, instance_path="$.historical_e0_mk_lock")
    except ClosureContractError as exc:
        raise _translate(exc) from exc
    fixed = {
    ...
    if repository.get("head") != E0_MK_H_COMMIT:
        raise P1TemporalConsumerSeed20260614PatchError(
            "Historical E0-MK H commit drifted"
        )
    git_diff = e0_mk.patch_git_diff_payload(E0_MK_H_COMMIT)
    ...
    if payload.get("git_diff") != git_diff or payload.get("patch_components") != components:
        raise P1TemporalConsumerSeed20260614PatchError(
            "Historical E0-MK Git authority drifted"
        )
    lock_record, companion_record = _published_lock_records(
        h_commit=E0_MK_H_COMMIT,
        p_commit=E0_MK_P_COMMIT,
        lock_path=e0_mk.DEFAULT_PATCH_LOCK_PATH,
        companion_path=e0_mk.DEFAULT_PATCH_MANIFEST_PATH,
        lock_role="external_p1_temporal_consumer_seed_20260613_patch_lock",
        companion_role="p1_temporal_consumer_seed_20260613_patch_companion",
    ...
    if companion != e0_mk._expected_companion(payload, lock_record=lock_record):
        raise P1TemporalConsumerSeed20260614PatchError(
            "Historical E0-MK companion drifted"
        )
    records = _component_records(cast(Mapping[str, Any], components))
    by_path = {str(record["path"]): record for record in records}
    if set(by_path) != set(e0_mk.PATCH_PATHS):
        raise P1TemporalConsumerSeed20260614PatchError(
            "Historical E0-MK component paths drifted"
        )
    superseded = [by_path[path] for path in MK_SUPERSEDED_PATHS]
    preserved = [by_path[path] for path in MK_PRESERVED_PATHS]
    for record in records:
        ...
        if record != expected:
            raise P1TemporalConsumerSeed20260614PatchError(
                f"Historical E0-MK Git record drifted: {record['path']}"
            )
...
def _historical_e0_ml_authority(*, execution_head: str) -> dict[str, Any]:
    ...
    payload = _load_regular_json(e0_ml.DEFAULT_PATCH_LOCK_PATH, context="P-E0-ML lock")
    schema = _load_regular_json(e0_ml.DEFAULT_PATCH_LOCK_SCHEMA, context="H-E0-ML schema")
    try:
        validate_json_schema(payload, schema, instance_path="$.historical_e0_ml_lock")
    except ClosureContractError as exc:
        raise _translate(exc) from exc
    fixed = {
    ...
    if repository.get("head") != E0_ML_H_COMMIT:
        raise P1TemporalConsumerSeed20260614PatchError(
            "Historical E0-ML H commit drifted"
        )
    git_diff = e0_ml.patch_git_diff_payload(E0_ML_H_COMMIT)
    components = e0_ml.patch_component_bundle(E0_ML_H_COMMIT)
    if payload.get("git_diff") != git_diff or payload.get("patch_components") != components:
        raise P1TemporalConsumerSeed20260614PatchError(
            "Historical E0-ML Git authority drifted"
        )
    lock_record, companion_record = _published_lock_records(
        h_commit=E0_ML_H_COMMIT,
        p_commit=E0_ML_P_COMMIT,
        lock_path=e0_ml.DEFAULT_PATCH_LOCK_PATH,
        companion_path=e0_ml.DEFAULT_PATCH_MANIFEST_PATH,
        lock_role="external_p1_sequence_seed_20260614_patch_lock",
        companion_role="p1_sequence_seed_20260614_patch_companion",
        execution_head=execution_head,
        context="P-E0-ML",
    )
    companion = _load_regular_json(
    ...
    if companion != e0_ml._expected_companion(payload, lock_record=lock_record):
        raise P1TemporalConsumerSeed20260614PatchError(
            "Historical E0-ML companion drifted"
        )
    records = _component_records(cast(Mapping[str, Any], components))
    if {str(record["path"]) for record in records} != set(e0_ml.PATCH_PATHS):
        raise P1TemporalConsumerSeed20260614PatchError(
    ...
    if not isinstance(base_authorities, Mapping) or set(base_authorities) != {
        ...
        raise P1TemporalConsumerSeed20260614PatchError(
            "Historical E0-ML base-authority dialect drifted"
        )
    sealed_mj = cast(Mapping[str, Any], base_authorities["e0_mj"])
    sealed_mk = cast(Mapping[str, Any], base_authorities["e0_mk"])
    if (
        sealed_mj.get("gate") != "E0-MJ"
        or sealed_mj.get("patch_head") != e0_ml.E0_MJ_H_COMMIT
        or sealed_mj.get("lock_commit") != e0_ml.E0_MJ_P_COMMIT
        or sealed_mj.get("effective_loader_called") is not False
        or sealed_mj.get("future_outcomes_accessed") is not False
        or sealed_mk.get("gate") != "E0-MK"
        or sealed_mk.get("patch_head") != E0_MK_H_COMMIT
        or sealed_mk.get("lock_commit") != E0_MK_P_COMMIT
        or sealed_mk.get("effective_loader_called") is not False
        or sealed_mk.get("future_outcomes_accessed") is not False
    ):
        raise P1TemporalConsumerSeed20260614PatchError(
            "Historical E0-ML nested E0-MJ/E0-MK authorities drifted"
        )
    if payload.get("p1_1729_publication") != e0_ml._published_p1_1729_bundle(
        execution_head=execution_head
    ):
        raise P1TemporalConsumerSeed20260614PatchError(
            "Historical E0-ML sealed P1/1729 publication drifted"
        )
    if payload.get(
        "p1_20260612_publication"
    ) != e0_ml._published_p1_20260612_bundle(execution_head=execution_head):
        raise P1TemporalConsumerSeed20260614PatchError(
            "Historical E0-ML sealed P1/20260612 publication drifted"
        )
    if payload.get(
        "p1_20260613_publication"
    ) != e0_ml._published_p1_20260613_bundle(execution_head=execution_head):
        raise P1TemporalConsumerSeed20260614PatchError(
            "Historical E0-ML sealed P1/20260613 publication drifted"
        )
    builder = payload.get("current_runtime_builder_record")
    if builder != {**P1_ARTIFACT_BUILDER_RECORD, "role": "current_runtime_builder"}:
        raise P1TemporalConsumerSeed20260614PatchError(
            "Historical E0-ML runtime builder drifted"
        )
    return {
        "gate": "E0-ML",
        ...
        "publication_topology_verified": True,
        "historical_e0_mj_and_e0_mk_seals_verified": True,
        "historical_authority_verified": True,
        "effective_loader_called": False,
        "future_outcomes_accessed": False,
    }
...
def _published_p1_20260614_bundle(*, execution_head: str) -> dict[str, Any]:
    ...
    if ancestry != [P1_BUNDLE_COMMIT, E0_ML_P_COMMIT]:
        raise P1TemporalConsumerSeed20260614PatchError(
            "P1/20260614 publication topology drifted"
        )
    versioned_roles = {
        "data/closure_v1/development/sequences/P1/seed_20260614.parquet.dvc": (
            "p1_seed_20260614_sequence_dvc_pointer"
        ),
        "reports/closure_v1/01_surface/sequences/P1/seed_20260614_manifest.json": (
            "p1_seed_20260614_sequence_manifest"
        ),
        "reports/closure_v1/01_surface/sequences/P1/seed_20260614_summary.csv": (
            "p1_seed_20260614_sequence_summary"
        ),
        "src/experiments/audit_closure_p1_seed_20260614_sequence_bundle.py": (
            "p1_seed_20260614_sequence_auditor"
        ),
        "tests/test_audit_closure_p1_seed_20260614_sequence_bundle.py": (
            "p1_seed_20260614_sequence_auditor_tests"
        ),
    }
    expected_diff = [
        {"status": "A", "path": path} for path in sorted(versioned_roles)
    ]
    if _observed_diff_entries(E0_ML_P_COMMIT, P1_BUNDLE_COMMIT) != expected_diff:
        raise P1TemporalConsumerSeed20260614PatchError(
            "P1/20260614 publication scope drifted"
        )
    ...
    _assert_paths_untouched(
        ...
        context="P1/20260614 published bundle",
    )
    expected = {
        "data/closure_v1/development/sequences/P1/seed_20260614.parquet.dvc": (
            104,
...
def _auditor_identity_record() -> dict[str, Any]:
    callable_object = p1_audit.audit_p1_seed_20260614_sequence_bundle
...
def run_p1_bundle_audit_in_process() -> dict[str, Any]:
    """Invoke exactly the published P1/20260614 auditor in this process."""
    _auditor_identity_record()
    try:
        result = p1_audit.audit_p1_seed_20260614_sequence_bundle()
    except p1_audit.ClosureP1Seed20260614SequenceAuditError as exc:
        raise _translate(exc) from exc
...
def p1_consumer_namespace_absence() -> dict[str, Any]:
    ...
    if existing:
        raise P1TemporalConsumerSeed20260614PatchError(
            f"P1/20260614 consumer namespace is not empty: {existing}"
        )
    return {
...
def closure_progression_prelock() -> dict[str, Any]:
    ...
    if missing_required or unexpected_registered or forbidden_external:
        raise P1TemporalConsumerSeed20260614PatchError(
            "P1/20260614 progression prelock drifted: "
...
def collect_p1_temporal_consumer_seed_20260614_patch_prelock_state(
    *,
    verify_remote: bool,
) -> dict[str, Any]:
    ...
    if status:
        raise P1TemporalConsumerSeed20260614PatchError(
            f"H-E0-MM lock requires a clean worktree: {status}"
        )
    head = _require_commit(_git("rev-parse", "HEAD"), context="H-E0-MM HEAD")
    if _git("branch", "--show-current") != "main":
        raise P1TemporalConsumerSeed20260614PatchError(
            "H-E0-MM lock requires branch main"
        )
    published = _require_commit(_git("rev-parse", PUBLISHED_REF), context=PUBLISHED_REF)
    if published != head:
        raise P1TemporalConsumerSeed20260614PatchError(
            "H-E0-MM HEAD differs from origin/main"
        )
    remote = _remote_main_oid() if verify_remote else published
    if remote != head:
        raise P1TemporalConsumerSeed20260614PatchError(
            "H-E0-MM HEAD differs from live origin/main"
        )
    ...
    for record in cast(Sequence[Mapping[str, Any]], components["records"]):
        if _file_record(Path(str(record["path"])), role=str(record["role"])) != record:
            raise P1TemporalConsumerSeed20260614PatchError(
                f"Physical H-E0-MM component drifted: {record['path']}"
            )
    historical_mk = _historical_e0_mk_authority(execution_head=head)
    historical_ml = _historical_e0_ml_authority(execution_head=head)
    published_bundle = _published_p1_20260614_bundle(execution_head=head)
    return {
        "patch_repository": {
            "head": head,
        ...
        "p1_20260614_publication": published_bundle,
...
def build_p1_temporal_consumer_seed_20260614_patch_lock_payload(
...
def _validate_command_evidence(
    ...
    *,
    ...
    if set(evidence).difference(required | optional):
        raise P1TemporalConsumerSeed20260614PatchError(
            f"E0-MM {context} evidence has unexpected fields"
        )
    if not required.issubset(evidence) or evidence.get("command") != list(command):
        raise P1TemporalConsumerSeed20260614PatchError(
            f"E0-MM {context} evidence drifted"
        )
    if evidence.get("returncode") != 0:
        raise P1TemporalConsumerSeed20260614PatchError(
            f"E0-MM {context} did not pass"
        )
    for field in ("stdout_line_count", "stderr_line_count"):
        value = evidence.get(field)
        if type(value) is not int or value < 0:
            raise P1TemporalConsumerSeed20260614PatchError(
                f"E0-MM {context} {field} must be a non-negative integer"
            )
...
def _validate_focused_duration_evidence(evidence: Mapping[str, Any]) -> None:
    ...
    if not isinstance(duration_text, str):
        raise P1TemporalConsumerSeed20260614PatchError(
            "E0-MM focused pytest duration evidence is malformed"
        )
    try:
    ...
    if duration < Decimal(60):
        ...
        try:
            ...
            raise P1TemporalConsumerSeed20260614PatchError(
                "E0-MM focused pytest duration evidence is out of range"
            ) from exc
        valid = summary_format == "pytest_timedelta" and clock == expected_clock
    if not valid:
        raise P1TemporalConsumerSeed20260614PatchError(
            "E0-MM focused pytest duration/clock evidence drifted"
        )
...
def _validate_audit_evidence_shape(evidence: Mapping[str, Any]) -> None:
    ...
    if drifted:
        raise P1TemporalConsumerSeed20260614PatchError(
            f"E0-MM in-process audit evidence drifted: {drifted}"
        )
    identity = _auditor_identity_record()
    ...
    if (
        ...
        raise P1TemporalConsumerSeed20260614PatchError(
            "E0-MM canonical audit hash evidence is malformed"
        )


def validate_p1_temporal_consumer_seed_20260614_patch_verification(
    verification: Mapping[str, Any],
) -> None:
    expected_fields = {
    ...
    if set(verification) != expected_fields:
        raise P1TemporalConsumerSeed20260614PatchError(
            "E0-MM verification fields drifted"
        )
    ...
    for field, command in commands.items():
        ...
        if not isinstance(value, Mapping):
            raise P1TemporalConsumerSeed20260614PatchError(
                f"E0-MM {field} evidence is malformed"
            )
    ...
    if (
        ...
        raise P1TemporalConsumerSeed20260614PatchError(
            "E0-MM focused-test evidence drifted"
        )
    ...
    if first.get("terminal_status") not in {
        ...
        raise P1TemporalConsumerSeed20260614PatchError(
            "E0-MM first targeted DVC push evidence drifted"
        )
    if second.get("terminal_status") != "Everything is up to date.":
        raise P1TemporalConsumerSeed20260614PatchError(
            "E0-MM second targeted DVC push is not idempotent"
        )
    audit = verification.get("p1_bundle_audit")
    if not isinstance(audit, Mapping):
        raise P1TemporalConsumerSeed20260614PatchError(
            "E0-MM in-process audit evidence is malformed"
        )
    _validate_audit_evidence_shape(audit)


def validate_p1_temporal_consumer_seed_20260614_patch_lock_payload(
    ...
    *,
    ...
    try:
        validate_json_schema(
            payload,
            schema,
            instance_path="$.p1_temporal_consumer_seed_20260614_patch_lock",
        )
    except ClosureContractError as exc:
        raise _translate(exc) from exc
    fixed = {
    ...
    for field, expected in fixed.items():
        if payload.get(field) != expected:
            raise P1TemporalConsumerSeed20260614PatchError(
                f"E0-MM fixed field drifted: {field}"
            )
    created = payload.get("created_at_utc")
    if not isinstance(created, str):
        raise P1TemporalConsumerSeed20260614PatchError("E0-MM timestamp is invalid")
    try:
        timestamp = datetime.fromisoformat(created.replace("Z", "+00:00"))
    except ValueError as exc:
        raise P1TemporalConsumerSeed20260614PatchError(
            "E0-MM timestamp is invalid"
        ) from exc
    if timestamp.utcoffset() is None:
        raise P1TemporalConsumerSeed20260614PatchError(
            "E0-MM timestamp requires a timezone"
        )
    repository = cast(Mapping[str, Any], payload["patch_repository"])
    patch_head = _require_commit(str(repository.get("head", "")), context="H-E0-MM")
    if repository != {
        "head": patch_head,
        "parent": PATCH_BASE_COMMIT,
        "branch": "main",
    ...
    if payload.get("base_authorities") != expected_authorities:
        raise P1TemporalConsumerSeed20260614PatchError(
            "E0-MM historical authorities drifted"
        )
    if payload.get("p1_20260614_publication") != _published_p1_20260614_bundle(
        execution_head=execution_head
    ):
        raise P1TemporalConsumerSeed20260614PatchError(
            "E0-MM P1/20260614 publication drifted"
        )
...
def _expected_companion(
    ...
    *,
    ...
    mk_authority = cast(Mapping[str, Any], authorities["e0_mk"])
    ml_authority = cast(Mapping[str, Any], authorities["e0_ml"])
    publication = cast(Mapping[str, Any], payload["p1_20260614_publication"])
    ...
    inputs = [
        ...
        *[
            ...
            for record in cast(
                ...
                cast(Mapping[str, Any], mk_authority["preserved_components"])["records"],
            )
        ],
        dict(cast(Mapping[str, Any], mk_authority["lock"])),
        dict(cast(Mapping[str, Any], mk_authority["companion_manifest"])),
        *[
            dict(record)
            for record in cast(
                Sequence[Mapping[str, Any]],
                cast(Mapping[str, Any], ml_authority["preserved_components"])["records"],
            )
        ],
        dict(cast(Mapping[str, Any], ml_authority["lock"])),
        dict(cast(Mapping[str, Any], ml_authority["companion_manifest"])),
        *[
            dict(record)
    ...
    historical_inputs = [
        {
            **dict(record),
            "commit": E0_MK_H_COMMIT,
            "hash_source": "git_blob_at_commit",
        }
        for record in cast(
            Sequence[Mapping[str, Any]],
            cast(Mapping[str, Any], mk_authority["superseded_components"])["records"],
        )
    ]
    historical_inputs.sort(key=lambda record: str(record["path"]))
    return {
        "manifest_version": (
            "closure_p1_temporal_consumer_seed_20260614_patch_manifest_v1"
        ),
        "status": "completed",
        "experiment_id": EXPERIMENT_ID,
        "surface_id": SURFACE_ID,
...
def _validate_publication_bundle(
    ...
    *,
    ...
    if _observed_diff_entries(patch_head, lock_commit) != expected:
        raise P1TemporalConsumerSeed20260614PatchError(
            "P-E0-MM must add exactly lock plus companion"
        )
    if _git("branch", "--show-current") != "main":
        raise P1TemporalConsumerSeed20260614PatchError(
            "E0-MM effective authority requires branch main"
        )
    if execution_head != lock_commit:
        raise P1TemporalConsumerSeed20260614PatchError(
            "E0-MM execution HEAD must equal the exact P-E0-MM lock commit"
        )
    ...
    if set(refs.values()) != {lock_commit}:
        raise P1TemporalConsumerSeed20260614PatchError(
            f"E0-MM local/tracking refs diverged: {refs}"
        )
    if verify_remote and _remote_main_oid() != lock_commit:
        raise P1TemporalConsumerSeed20260614PatchError(
            "E0-MM execution HEAD differs from live origin/main"
        )
    return (
        lock_commit,
        _git_record(
...
def load_and_validate_p1_temporal_consumer_seed_20260614_patch_lock(
    ...
    *,
    ...
    if (
        ...
        raise P1TemporalConsumerSeed20260614PatchError(
            "E0-MM requires closed default paths"
        )
    payload = _load_regular_json(lock_path, context="E0-MM lock")
    schema = _load_regular_json(lock_schema, context="E0-MM schema")
    ...
    companion = _load_regular_json(companion_path, context="E0-MM companion")
    if companion != _expected_companion(payload, lock_record=lock_record):
        raise P1TemporalConsumerSeed20260614PatchError("E0-MM companion drifted")
    ...
    if require_published:
        ...
        if status:
            raise P1TemporalConsumerSeed20260614PatchError(
                f"E0-MM execution requires a clean worktree: {status}"
            )
        ...
        if execution_head != patch_head:
            raise P1TemporalConsumerSeed20260614PatchError(
                "Unpublished E0-MM validation must run at H-E0-MM"
            )
        validate_p1_temporal_consumer_seed_20260614_patch_lock_payload(
            payload,
            schema,
    ...
    ml_lock_record = dict(cast(Mapping[str, Any], ml_authority["lock"]))
    ml_companion_record = dict(
        cast(Mapping[str, Any], ml_authority["companion_manifest"])
    )
    ...
    summary = {
        "status": (
            "published_p1_temporal_consumer_seed_20260614_patch_valid"
            if effective
...
def require_p1_temporal_consumer_seed_20260614_patch_authorized(
    *,
    model_id: str,
    base_seed: int,
    device: str,
) -> dict[str, Any]:
    """Require published E0-MM for exactly the P1/20260614/CPU consumer."""
    if (
        model_id != AUTHORIZED_MODEL_ID
        ...
        raise P1TemporalConsumerSeed20260614PatchError(
            "E0-MM authorizes only the P1 seed 20260614 CPU temporal consumer"
        )
    _, summary = load_and_validate_p1_temporal_consumer_seed_20260614_patch_lock(
    ...
    if failed:
        raise P1TemporalConsumerSeed20260614PatchError(
            f"E0-MM authorization predicates failed: {failed}"
        )
    required_false = (
    ...
    if drifted:
        raise P1TemporalConsumerSeed20260614PatchError(
            f"E0-MM fail-closed seals drifted: {drifted}"
        )
    if summary.get("fit_availability") != FIT_AVAILABILITY:
        raise P1TemporalConsumerSeed20260614PatchError(
            "E0-MM fit availability drifted"
        )
    if summary.get("schema_subset_preflight_evidence") != (
        preflight_p1_temporal_consumer_seed_20260614_patch_schema()
    ):
        raise P1TemporalConsumerSeed20260614PatchError(
            "E0-MM schema-subset preflight summary drifted"
        )
    if summary.get("state_consumer_authority") is not None:
        raise P1TemporalConsumerSeed20260614PatchError(
            "E0-MM must not apply the historical seed-1729 ANFIS exception"
        )
...
__all__ = [
    ...
    "validate_p1_temporal_consumer_seed_20260614_patch_lock_payload",
    "validate_p1_temporal_consumer_seed_20260614_patch_verification",
]
