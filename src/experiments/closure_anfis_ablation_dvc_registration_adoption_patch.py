"""Adopt the published 45-light-output commit under fail-closed E0-MZ.

E0-MZ is an additive governance overlay over the valid historical H-E0-MY
commit and its accidental, exact 45-file light-output child.  It does not
reinterpret that child as R-E0-MY, rewrite any family final, or authorize DVC.
H/P-E0-MZ remain administrative and development-only.  Only a separately
published P-E0-MZ can authorize the remaining closed 10-pointer plus
``models.dvc`` registration transaction.
"""
...
PATCH_GATE = "E0-MZ"
SCHEMA_VERSION = "closure_anfis_ablation_dvc_registration_adoption_patch_lock_v1"
COMPANION_VERSION = (
    "closure_anfis_ablation_dvc_registration_adoption_patch_lock_manifest_v1"
)
...
LOCKER_GUARD_PATH = Path(
    "tmp/closure_v1_e0_mz_locker/registration_adoption_patch.lock"
)
...
PATCH_COMPONENT_ROLES = {
    "configs/closure_v1/anfis_ablation_dvc_registration_adoption_patch_lock.schema.json": (
        "anfis_ablation_dvc_registration_adoption_patch_schema"
    ),
    "docs/closure_v1/E0_M_ANFIS_ABLATION_DVC_REGISTRATION_ADOPTION_PATCH_1.md": (
        "anfis_ablation_dvc_registration_adoption_patch_protocol"
    ),
    ...
    "src/experiments/closure_anfis_ablation_dvc_registration_adoption_patch.py": (
        "anfis_ablation_dvc_registration_adoption_patch_validator"
    ),
    "src/experiments/lock_closure_anfis_ablation_dvc_registration_adoption_patch.py": (
        "anfis_ablation_dvc_registration_adoption_patch_locker"
    ),
    "tests/test_closure_anfis_ablation_dvc_registration_adoption_patch.py": (
        "anfis_ablation_dvc_registration_adoption_patch_test"
    ),
    "tests/test_closure_anfis_ablation_dvc_registration_patch.py": (
        "anfis_ablation_dvc_registration_patch_adoption_regression_test"
    ),
...
SUPERSEDED_MY_PATHS = PATCH_MODIFIED_PATHS
PRESERVED_MY_PATHS = tuple(
    path for path in my.PATCH_PATHS if path not in set(SUPERSEDED_MY_PATHS)
)
INHERITED_MY_HISTORICAL_PATHS = tuple(my.PATCH_MODIFIED_PATHS)
PATCH_COMPONENT_GIT_MODES = {
    path: "100755" if path == "src/data/prepare_commit_artifacts.py" else "100644"
    for path in PATCH_PATHS
}
ANFIS_ABLATION_H_MZ_STAGED_SCOPE = {
    path: ("A" if path in set(PATCH_ADDED_PATHS) else "M") for path in PATCH_PATHS
}
ANFIS_ABLATION_P_MZ_STAGED_SCOPE = {
    DEFAULT_PATCH_LOCK_PATH.as_posix(): "A",
    DEFAULT_PATCH_LOCK_MANIFEST_PATH.as_posix(): "A",
}
SUPERSEDED_P_MY_LOCK_PATH = my.DEFAULT_PATCH_LOCK_PATH
SUPERSEDED_P_MY_COMPANION_PATH = my.DEFAULT_PATCH_LOCK_MANIFEST_PATH
...
ADOPTED_LIGHT_PATHS = tuple(
    sorted(
        path
        for path, status in my.ANFIS_ABLATION_R_MY_STAGED_SCOPE.items()
        if status == "A" and path.startswith("reports/closure_v1/02_models/")
    )
)
...
LOCK_SEALS = {
    ...
    "registration_requires_published_p_mz": True,
    "historical_h_my_reconstructed": True,
    "adopted_light_commit_reconstructed": True,
    "adopted_light_commit_is_not_r_my": True,
    "superseded_p_my_absent": True,
...
class AnfisAblationDvcRegistrationAdoptionPatchError(RuntimeError):
    """Raised when E0-MZ authority, artifacts, or topology drift."""


def _translate(exc: BaseException) -> AnfisAblationDvcRegistrationAdoptionPatchError:
    return AnfisAblationDvcRegistrationAdoptionPatchError(str(exc))
...
def _mz_error_boundary(function: Callable[_P, _R]) -> Callable[_P, _R]:
    @wraps(function)
    def wrapped(*args: _P.args, **kwargs: _P.kwargs) -> _R:
        try:
            return function(*args, **kwargs)
        except AnfisAblationDvcRegistrationAdoptionPatchError:
            raise
...
def _permissions_from_git_mode(value: str) -> int:
    if re.fullmatch(r"100[0-7]{3}", value) is None:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            f"E0-MZ unsupported regular-file Git mode: {value!r}"
        )
...
def _open_anchored_parent(path: Path, *, repo_root: Path) -> tuple[int, str]:
    if path.is_absolute() or not path.parts or any(
        ...
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            f"E0-MZ path must remain repository-relative: {path.as_posix()}"
        )
    ...
    *,
    ...
    try:
        ...
        if (
            ...
            raise AnfisAblationDvcRegistrationAdoptionPatchError(
                f"E0-MZ path is not one stable regular {expected_mode:04o} file: "
                f"{path.as_posix()}"
            )
        chunks: list[bytes] = []
        while True:
        ...
        if identity_before != identity_after or identity_after != named_identity:
            raise AnfisAblationDvcRegistrationAdoptionPatchError(
                f"E0-MZ path changed while read: {path.as_posix()}"
            )
        ...
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            f"Required stable E0-MZ input is unavailable: {path.as_posix()}"
        ) from exc
    finally:
...
def _git_blob_bytes(repo_root: Path, commit: str, path: Path) -> bytes:
    ...
    if result.returncode != 0 or result.stderr:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            f"E0-MZ cannot reconstruct Git blob {commit}:{path.as_posix()}"
        )
...
def _load_json(path: Path, *, repo_root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            f"E0-MZ JSON cannot be decoded: {path.as_posix()}"
        ) from exc
    if not isinstance(value, dict):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            f"E0-MZ JSON must be an object: {path.as_posix()}"
        )
    return value
...
def _validate_timestamp(value: Any) -> None:
    if not isinstance(value, str):
        raise AnfisAblationDvcRegistrationAdoptionPatchError("E0-MZ timestamp is absent")
    try:
        parsed = datetime.fromisoformat(value.replace("Z", "+00:00"))
    except ValueError as exc:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ timestamp is malformed"
        ) from exc
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ timestamp must be timezone-aware"
        )
...
def _slot_paths(model_id: str, base_seed: int) -> dict[str, Path]:
    ...
    if set(paths) != set(SLOT_ROLE_ORDER):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            f"E0-MZ slot path roles drifted: {model_id}/{base_seed}"
        )
    return paths
...
def _family_physical_snapshot(repo_root: Path) -> tuple[dict[str, Any], ...]:
    ...
    if len(snapshot) != FAMILY_FINAL_COUNT:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ physical family snapshot must contain exact80 identities"
        )
...
def _require_family_physical_snapshot(
    ...
    if list(expected) != list(_family_physical_snapshot(repo_root)):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            f"E0-MZ exact80 physical family changed {context}"
        )
        ...
        sorted(
            {
                ...
                *(Path(path) for path in PRESERVED_MY_PATHS),
                BASE_MX_LOCK_PATH,
...
def _require_publication_state(
    ...
    *,
    ...
    if list(physical_snapshot) != list(_publication_input_snapshot(repo_root)):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            f"E0-MZ physical governance input changed {context}"
        )
    if not _exact_equal(git_snapshot, _publication_git_snapshot(repo_root)):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            f"E0-MZ Git publication state changed {context}"
        )
    _validate_family_namespace(
        registered=False,
        repo_root=repo_root,
...
def _dvc_configuration_contract(repo_root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ DVC repository/local configuration drifted"
        )
        ...
        sorted(
            {
                ...
                *_training_guard_paths(),
                SUPERSEDED_P_MY_LOCK_PATH,
                SUPERSEDED_P_MY_COMPANION_PATH,
                _temporary_path(SUPERSEDED_P_MY_LOCK_PATH),
                _temporary_path(SUPERSEDED_P_MY_COMPANION_PATH),
    ...
    *,
    ...
    for path in expected_paths:
        try:
            ...
            raise AnfisAblationDvcRegistrationAdoptionPatchError(
                f"E0-MZ {context} expected path escaped its root"
            ) from exc
        if relative == Path("."):
            raise AnfisAblationDvcRegistrationAdoptionPatchError(
                f"E0-MZ {context} expected file path is malformed"
            )
    ...
    def walk(descriptor: int, relative_parent: Path) -> None:
        ...
        if not stat.S_ISDIR(before.st_mode):
            raise AnfisAblationDvcRegistrationAdoptionPatchError(
                f"E0-MZ {context} walk escaped its directory root"
            )
        try:
            entries = sorted(os.listdir(descriptor))
        except OSError as exc:
            raise AnfisAblationDvcRegistrationAdoptionPatchError(
                f"E0-MZ cannot walk exact {context} root"
            ) from exc
        for name in entries:
            relative = (
                Path(name)
            ...
            if stat.S_ISDIR(metadata.st_mode):
                if relative not in expected_directories:
                    raise AnfisAblationDvcRegistrationAdoptionPatchError(
                        f"E0-MZ {context} contains an unexpected directory: {relative}"
                    )
                ...
                try:
                    ...
                    if identity(opened) != identity(metadata):
                        raise AnfisAblationDvcRegistrationAdoptionPatchError(
                            f"E0-MZ {context} directory changed while opened: {relative}"
                        )
                    ...
                    if identity(os.fstat(child)) != identity(named_after):
                        raise AnfisAblationDvcRegistrationAdoptionPatchError(
                            f"E0-MZ {context} directory changed while walked: {relative}"
                        )
                finally:
                    os.close(child)
                ...
                if (
                    ...
                    raise AnfisAblationDvcRegistrationAdoptionPatchError(
                        f"E0-MZ {context} contains a foreign/non-0644 file: {relative}"
                    )
                file_descriptor = os.open(name, file_flags, dir_fd=descriptor)
                try:
                    ...
                    if (
                        ...
                        raise AnfisAblationDvcRegistrationAdoptionPatchError(
                            f"E0-MZ {context} file changed while inspected: {relative}"
                        )
                ...
                raise AnfisAblationDvcRegistrationAdoptionPatchError(
                    f"E0-MZ {context} contains a symlink/nonregular entry: {relative}"
                )
        if identity(before) != identity(os.fstat(descriptor)):
            raise AnfisAblationDvcRegistrationAdoptionPatchError(
                f"E0-MZ {context} directory changed while walked: {relative_parent}"
            )
    ...
    try:
        ...
        if identity(root_before) != identity(named_before):
            raise AnfisAblationDvcRegistrationAdoptionPatchError(
                f"E0-MZ {context} root changed while opened"
            )
        ...
        if (
            ...
            raise AnfisAblationDvcRegistrationAdoptionPatchError(
                f"E0-MZ {context} root changed while walked"
            )
    except AnfisAblationDvcRegistrationAdoptionPatchError:
        raise
    except OSError as exc:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            f"E0-MZ cannot open exact {context} root without following links"
        ) from exc
    finally:
    ...
    if observed_files != expected_files or observed_directories != expected_directories:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            f"E0-MZ {context} tree does not contain its exact closed namespace"
        )
...
def _validate_family_namespace(
    *,
    ...
    if type(registered) is not bool or type(allow_locker_guard) is not bool:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ namespace policy must use exact booleans"
        )
    if registration_transaction is not None and not registered:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ transaction coordination is post-registration only"
        )
    ...
    if occupied:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            f"E0-MZ ignored/coordination namespace is occupied: {occupied}"
        )
...
def _base_models_dvc_bytes() -> bytes:
    ...
    if len(payload) != BASE_MODELS_DVC_BYTES or _sha256_bytes(payload) != BASE_MODELS_DVC_SHA256:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ sealed base models.dvc constants drifted"
        )
    return payload
...
def _coordination_identity_record(
    ...
    *,
    ...
    if payload != expected_payload:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            f"E0-MZ transaction coordination bytes drifted: {path.as_posix()}"
        )
    return {
        "path": path.as_posix(),
        "device": int(metadata.st_dev),
...
def _coordination_directory_record(
    ...
    try:
        ...
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            f"E0-MZ coordination directory is absent/unreadable: {path.as_posix()}"
        ) from exc
    finally:
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            f"E0-MZ coordination directory identity drifted: {path.as_posix()}"
        )
    return {
        "path": path.as_posix(),
...
def _validate_registration_transaction_record(
    ...
    if not isinstance(value, Mapping) or set(value) != {
        ...
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ private transaction record dialect drifted"
        )
    mode = value.get("mode")
    if mode not in {"in_place", "atomic_replace"}:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ private transaction mode drifted"
        )
    expected_guard = _coordination_identity_record(
        REGISTRATION_GUARD_PATH,
        expected_mode=0o600,
        expected_payload=b"E0-MZ exact ANFIS-ablation DVC registration\n",
        repo_root=repo_root,
    )
    ...
    if not _exact_equal(value.get("guard"), expected_guard) or not _exact_equal(
        ...
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ private transaction identity record drifted"
        )
    ...
    if not _exact_equal(
        ...
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ private DVC config-isolation record drifted"
        )
    allowed = {
        REGISTRATION_GUARD_PATH,
        REGISTRATION_MODELS_BYTES_BACKUP_PATH,
    ...
    if mode == "in_place":
        if value.get("anchor") is not None or _lexists(
            ...
            raise AnfisAblationDvcRegistrationAdoptionPatchError(
                "E0-MZ in-place transaction retained a hardlink anchor"
            )
    else:
        ...
        if not _exact_equal(value.get("anchor"), expected_anchor):
            raise AnfisAblationDvcRegistrationAdoptionPatchError(
                "E0-MZ atomic transaction anchor record drifted"
            )
...
def _expected_registration_records(repo_root: Path) -> dict[str, Any]:
    ...
    if len(models) != EXPECTED_MODELS_DVC_BYTES or _sha256_bytes(models) != EXPECTED_MODELS_DVC_SHA256:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ expected models.dvc dialect drifted"
        )
    return {
...
def _registration_inventory(repo_root: Path) -> dict[str, Any]:
    try:
        return my._registration_inventory(repo_root)
    except Exception as exc:
        raise _translate(exc) from exc
...
def _base_models_owner(repo_root: Path) -> dict[str, Any]:
    try:
        return my._base_models_owner(repo_root)
    except Exception as exc:
        raise _translate(exc) from exc
...
def _h_components(head: str, repo_root: Path) -> list[dict[str, Any]]:
    ...
    for record in records:
        ...
        if (
            ...
            raise AnfisAblationDvcRegistrationAdoptionPatchError(
                f"E0-MZ H component differs from Git: {record['path']}"
            )
    return records
...
def _historical_inputs(repo_root: Path) -> list[dict[str, Any]]:
    inherited = [
        _historical_git_blob_record(
            path,
            role=f"inherited_{my.PATCH_COMPONENT_ROLES[path]}",
            commit=P_MX_COMMIT,
            repo_root=repo_root,
        )
        for path in INHERITED_MY_HISTORICAL_PATHS
    ]
    superseded = [
        _historical_git_blob_record(
            path,
            role=f"superseded_h_my_{my.PATCH_COMPONENT_ROLES[path]}",
            commit=H_MY_COMMIT,
            repo_root=repo_root,
        )
        for path in SUPERSEDED_MY_PATHS
    ]
    ...
    if len(records) != EXPECTED_HISTORICAL_INPUT_COUNT or len(identities) != len(records):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ historical companion inputs must be exact8"
        )
    return records


def _historical_h_my_authority(repo_root: Path) -> dict[str, Any]:
    expected_scope = {
        "added": 5,
        "modified": 4,
        "deleted": 0,
        "paths": list(my.PATCH_PATHS),
    }
    if (
        _single_parent(repo_root, H_MY_COMMIT, context="historical H-E0-MY")
        != P_MX_COMMIT
        or _git_scope(repo_root, P_MX_COMMIT, H_MY_COMMIT) != expected_scope
    ):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ historical H-E0-MY topology drifted"
        )
    _require_exact_git_modes(
        repo_root,
        H_MY_COMMIT,
        my.PATCH_COMPONENT_GIT_MODES,
        context="historical H-E0-MY",
    )
    preserved: list[dict[str, Any]] = []
    for path in PRESERVED_MY_PATHS:
        role = my.PATCH_COMPONENT_ROLES[path]
        current = _file_record(Path(path), role=role, repo_root=repo_root)
        historical = _historical_git_blob_record(
            path, role=role, commit=H_MY_COMMIT, repo_root=repo_root
        )
        if current["bytes"] != historical["bytes"] or current["sha256"] != historical["sha256"]:
            raise AnfisAblationDvcRegistrationAdoptionPatchError(
                f"E0-MZ preserved H-E0-MY component drifted: {path}"
            )
    ...
    if len(preserved) != 5 or len(superseded) != 4:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ H-E0-MY preserved/superseded partition drifted"
        )
    return {
        "gate": "E0-MY",
        "head": H_MY_COMMIT,
...
def _adopted_light_publication(repo_root: Path) -> dict[str, Any]:
    expected_scope = {
        "added": EXPECTED_ADOPTED_LIGHT_COUNT,
        "modified": 0,
        "deleted": 0,
        "paths": list(ADOPTED_LIGHT_PATHS),
    }
    if (
        _single_parent(repo_root, ADOPTED_LIGHT_COMMIT, context="adopted light commit")
        != H_MY_COMMIT
        or _git_scope(repo_root, H_MY_COMMIT, ADOPTED_LIGHT_COMMIT) != expected_scope
    ):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ adopted light-output topology drifted"
        )
    _require_exact_git_modes(
        repo_root,
        ADOPTED_LIGHT_COMMIT,
        {path: "100644" for path in ADOPTED_LIGHT_PATHS},
        context="adopted light commit",
    )
    records: list[dict[str, Any]] = []
    for path in ADOPTED_LIGHT_PATHS:
        current = _file_record(
            Path(path), role="adopted_light_family_output", repo_root=repo_root
        )
        historical = _historical_git_blob_record(
            path,
            role="adopted_light_family_output",
            commit=ADOPTED_LIGHT_COMMIT,
            repo_root=repo_root,
        )
        if current["bytes"] != historical["bytes"] or current["sha256"] != historical["sha256"]:
            raise AnfisAblationDvcRegistrationAdoptionPatchError(
                f"E0-MZ adopted light output differs from Git: {path}"
            )
        records.append(current)
    if len(records) != EXPECTED_ADOPTED_LIGHT_COUNT:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ adopted light-output record count drifted"
        )
    return {
        "commit": ADOPTED_LIGHT_COMMIT,
        "parent": H_MY_COMMIT,
        "scope": {"added": 45, "modified": 0, "deleted": 0},
...
def _base_mx_authority(repo_root: Path) -> dict[str, Any]:
    if (
        _single_parent(repo_root, P_MX_COMMIT, context="P-E0-MX") != BASE_H_MX_COMMIT
        or _git_scope(repo_root, BASE_H_MX_COMMIT, P_MX_COMMIT)
        != {
            "added": 2,
            "modified": 0,
            "deleted": 0,
        ...
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ base P-E0-MX topology drifted"
        )
    lock = _file_record(
        BASE_MX_LOCK_PATH,
    ...
    _require_exact_git_modes(
        ...
        P_MX_COMMIT,
        {
            BASE_MX_LOCK_PATH.as_posix(): "100644",
            BASE_MX_COMPANION_PATH.as_posix(): "100644",
        },
        context="P-E0-MX authority",
    )
    for path, record in (
        (BASE_MX_LOCK_PATH, lock),
        (BASE_MX_COMPANION_PATH, companion),
    ):
        git_payload = _git_blob_bytes(repo_root, P_MX_COMMIT, path)
        if record["bytes"] != len(git_payload) or record["sha256"] != _sha256_bytes(git_payload):
            raise AnfisAblationDvcRegistrationAdoptionPatchError(
                f"E0-MZ P-E0-MX physical input differs from Git: {path.as_posix()}"
            )
    return {
        "gate": "E0-MX",
        "p_head": P_MX_COMMIT,
...
def _companion_physical_inputs(
    *,
    ...
    base_mx: Mapping[str, Any],
    historical_h_my: Mapping[str, Any],
) -> list[dict[str, Any]]:
    records = [
        ...
        *(
            dict(record)
            for record in cast(
                Sequence[Mapping[str, Any]], historical_h_my["preserved_components"]
            )
        ),
    ...
    if len(records) != EXPECTED_COMPANION_INPUT_COUNT or len(identities) != len(records):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ companion physical inputs must be exact16"
        )
    return records
...
def _expected_untracked_light_status(repo_root: Path) -> list[str]:
    ...
    if tracked != expected_tracked or len(tracked) != FAMILY_TRACKED_LIGHT_COUNT:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ all fifty light outputs must remain tracked"
        )
    return []
...
@_mz_error_boundary
def preflight_anfis_ablation_dvc_registration_adoption_patch_schema(
    *, repo_root: Path | None = None
    ...
    if unsupported:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            f"E0-MZ schema exceeds the supported subset: {unsupported}"
        )
    ...
    try:
        ...
        raise AnfisAblationDvcRegistrationAdoptionPatchError(str(exc)) from exc
...
@_mz_error_boundary
def collect_anfis_ablation_dvc_registration_adoption_patch_prelock_state(
    *,
    ...
    if type(verify_remote) is not bool or type(_allow_locker_guard) is not bool:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ prelock policies must be exact booleans"
        )
    root = _root(repo_root)
    preflight_anfis_ablation_dvc_registration_adoption_patch_schema(repo_root=root)
    head = _git_head(root)
    expected_scope = {
        "added": 5,
        "modified": 4,
    ...
    if (
        _single_parent(root, head, context="H-E0-MZ") != BASE_COMMIT
        or _git_scope(root, BASE_COMMIT, head) != expected_scope
    ):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "H-E0-MZ must be the exact 4M+5A child of the adopted 45A commit"
        )
    _require_exact_git_modes(
        root, head, PATCH_COMPONENT_GIT_MODES, context="H-E0-MZ"
    )
    ...
    if branch != "main" or tracking != head or remote != head:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "H-E0-MZ refs are not aligned with main"
        )
    status = [
        line
    ...
    if status != expected_status:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            f"E0-MZ prelock worktree must be clean with all fifty lights tracked: {status}"
        )
    ...
    if occupied_outputs:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            f"E0-MZ prelock output namespace is occupied: {occupied_outputs}"
        )
...
@_mz_error_boundary
def build_anfis_ablation_dvc_registration_adoption_patch_lock_payload(
    ...
    *,
    ...
    if set(prelock) != required:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ prelock bundle dialect drifted"
        )
    return {
...
def _validate_command_evidence(
    ...
    *,
    ...
    if not isinstance(value, Mapping) or set(value) != keys:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            f"E0-MZ {context} evidence dialect drifted"
        )
    if (
        ...
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            f"E0-MZ {context} command/result drifted"
        )
    if (
        ...
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            f"E0-MZ {context} digest/line evidence drifted"
        )
    if exact_stdout is not None and (
        ...
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            f"E0-MZ {context} stdout evidence drifted"
        )
...
def _validate_verification(value: Any, *, repo_root: Path) -> None:
    if not isinstance(value, Mapping):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ verification evidence is absent"
        )
    required = {
        "schema_preflight",
        "type_check",
    ...
    if set(value) != required:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ verification evidence key set drifted"
        )
    expected_schema = preflight_anfis_ablation_dvc_registration_adoption_patch_schema(
        repo_root=repo_root
    )
    if not _exact_equal(value.get("schema_preflight"), expected_schema):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ schema-preflight evidence drifted"
        )
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ focused pytest evidence drifted"
        )
    ...
    if not isinstance(stdout_text, str):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ focused pytest stdout text is absent"
        )
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ focused pytest stdout binding drifted"
        )
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ family semantic audit evidence drifted"
        )
    ...
    if not _exact_equal(boundaries, expected_boundaries):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ verification execution boundaries drifted"
        )


def _validate_anfis_ablation_dvc_registration_adoption_patch_lock_payload(
    ...
    *,
    ...
    if type(allow_registered_state) is not bool:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ registered-state policy must be an exact boolean"
        )
    if _registration_transaction is not None and not allow_registered_state:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ private transaction record requires registered-state audit"
        )
    root = _root(repo_root)
    schema = _load_json(DEFAULT_PATCH_LOCK_SCHEMA, repo_root=root)
    try:
    ...
    if not _exact_equal(h_patch, expected_h):
        raise AnfisAblationDvcRegistrationAdoptionPatchError("E0-MZ H binding drifted")
    base_mx = _base_mx_authority(root)
    if not _exact_equal(payload.get("base_mx_authority"), base_mx):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ P-E0-MX reconstruction drifted"
        )
    historical_h_my = _historical_h_my_authority(root)
    if not _exact_equal(payload.get("historical_h_my"), historical_h_my):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ historical H-E0-MY reconstruction drifted"
        )
    adopted_light = _adopted_light_publication(root)
    if not _exact_equal(payload.get("adopted_light_publication"), adopted_light):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ adopted 45A publication binding drifted"
        )
    inventory = _registration_inventory(root)
    if not _exact_equal(payload.get("artifact_inventory"), inventory):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ registration inventory binding drifted"
        )
    family = _family_records(
        root,
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ completed family binding drifted"
        )
    models_before = (
    ...
    if not _exact_equal(payload.get("models_owner_transition"), expected_transition):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ models.dvc transition drifted"
        )
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ exact registration plan drifted"
        )
    ...
    if not _exact_equal(payload.get("companion_contract"), expected_contract):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ companion contract drifted"
        )
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ prelock evidence drifted"
        )
    _validate_verification(payload.get("verification"), repo_root=root)


@_mz_error_boundary
def validate_anfis_ablation_dvc_registration_adoption_patch_lock_payload(
    payload: Mapping[str, Any],
    *,
    allow_registered_state: bool = False,
    repo_root: Path | None = None,
...
def _expected_companion(
    ...
    *,
    ...
    historical_h_my = payload.get("historical_h_my")
    if (
        not isinstance(h_patch, Mapping)
        or not isinstance(base_mx, Mapping)
        or not isinstance(historical_h_my, Mapping)
    ):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "Cannot construct E0-MZ companion"
        )
    components = h_patch.get("components")
    if not isinstance(components, list):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "Cannot construct E0-MZ companion component list"
        )
    ...
    if script is None:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ companion script is absent"
        )
    output = dict(lock_record)
    if (
        output.get("path") != DEFAULT_PATCH_LOCK_PATH.as_posix()
        or output.get("role") != "anfis_ablation_dvc_registration_adoption_patch_lock"
    ):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ companion output record drifted"
        )
    return {
...
def _run_command(
    ...
    *,
    ...
    if result.returncode != 0:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            f"E0-MZ verification command failed: {list(command)}"
        )
...
def _parse_focused_summary(stdout: str, stderr: str) -> dict[str, int]:
    if stderr or FORBIDDEN_FOCUSED_SUMMARY_RE.search(stdout):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ focused pytest result is not one clean pass"
        )
    ...
    if len(matched) != 1 or lines[-1] != matched[0].group(0):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ focused pytest summary is not unique and terminal"
        )
    return {
...
@_mz_error_boundary
def run_anfis_ablation_dvc_registration_adoption_patch_verification(
    *,
    ...
    schema = preflight_anfis_ablation_dvc_registration_adoption_patch_schema(repo_root=root)
    _require_family_physical_snapshot(
...
) -> AnfisAblationDvcRegistrationAdoptionPatchError | None:
    first_error: AnfisAblationDvcRegistrationAdoptionPatchError | None = None
    for output in reversed(published):
        try:
            mt._rollback_owned_output(output)
        except Exception as exc:
            if first_error is None:
                first_error = AnfisAblationDvcRegistrationAdoptionPatchError(
                    "E0-MZ owned-output rollback failed for "
...
def _close_published_outputs_best_effort(
    ...
    for output in published:
        try:
            ...
            if first_error is None:
                first_error = AnfisAblationDvcRegistrationAdoptionPatchError(
                    "E0-MZ owned-output descriptor close failed for "
                    f"{output.path.as_posix()}: {exc}"
                )
    ...
    *,
    ...
    try:
        ...
        if (
            ...
            raise AnfisAblationDvcRegistrationAdoptionPatchError(
                f"E0-MZ owned output identity/metadata drifted {context}: "
        ...
        if (
            ...
            raise AnfisAblationDvcRegistrationAdoptionPatchError(
                f"E0-MZ owned output bytes drifted {context}: "
                f"{output.path.as_posix()}"
            )
        mt._validate_owned_output(output)
    except AnfisAblationDvcRegistrationAdoptionPatchError:
...
@_mz_error_boundary
def execute_and_publish_anfis_ablation_dvc_registration_adoption_patch_lock_bundle(
    *, repo_root: Path | None = None
    ...
    before = collect_anfis_ablation_dvc_registration_adoption_patch_prelock_state(
        verify_remote=True, repo_root=root
    )
    verification = run_anfis_ablation_dvc_registration_adoption_patch_verification(
        family_snapshot=family_snapshot,
        publication_input_snapshot=input_snapshot,
        publication_git_snapshot=git_snapshot,
        repo_root=root,
    )
    after = collect_anfis_ablation_dvc_registration_adoption_patch_prelock_state(
        verify_remote=True, repo_root=root
    )
    if not _exact_equal(before, after):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ prelock state changed during verification"
        )
    _require_family_physical_snapshot(
    ...
    payload = build_anfis_ablation_dvc_registration_adoption_patch_lock_payload(
        before, verification, created_at_utc=datetime.now(timezone.utc).isoformat()
    )
    validate_anfis_ablation_dvc_registration_adoption_patch_lock_payload(payload, repo_root=root)
    controlled = (
    ...
    if occupied:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            f"E0-MZ lock namespace is occupied: {occupied}"
        )
    ...
    try:
        guard = mt._acquire_publication_guard(
            LOCKER_GUARD_PATH,
            b"E0-MZ lock bundle publication in progress\n",
            repo_root=root,
        )
        _require_family_physical_snapshot(
        ...
        if not _exact_equal(
            ...
            raise AnfisAblationDvcRegistrationAdoptionPatchError(
                "E0-MZ guarded prelock state drifted"
            )
        ...
        if isinstance(exc, AnfisAblationDvcRegistrationAdoptionPatchError):
            translated = exc
...
@_mz_error_boundary
def publish_anfis_ablation_dvc_registration_adoption_patch_lock_bundle(
    *, repo_root: Path | None = None
) -> tuple[dict[str, Any], dict[str, Any]]:
    return execute_and_publish_anfis_ablation_dvc_registration_adoption_patch_lock_bundle(
        repo_root=repo_root
    )
...
def _validate_p_publication(
    ...
    if not isinstance(repository, Mapping) or not isinstance(repository.get("head"), str):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ H repository binding is absent"
        )
    h_head = str(repository["head"])
    head = _git_head(repo_root)
    tracking = _git_head(repo_root, "origin/main")
    ...
    if (
        ...
        or _single_parent(repo_root, head, context="P-E0-MZ") != h_head
        or tracking != head
        or remote != head
        ...
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "Published P-E0-MZ topology/refs drifted"
        )
    _require_exact_git_modes(
        repo_root,
        head,
    ...
    for raw_path in expected_paths:
        ...
        if physical != _git_blob_bytes(repo_root, head, path):
            raise AnfisAblationDvcRegistrationAdoptionPatchError(
                f"Published P-E0-MZ physical bytes differ from Git: {raw_path}"
            )
...
def _validate_registered_state(
    ...
    for record in pointers:
        ...
        if len(payload) != record["bytes"] or _sha256_bytes(payload) != record["sha256"]:
            raise AnfisAblationDvcRegistrationAdoptionPatchError(
                f"E0-MZ registered pointer drifted: {path.as_posix()}"
            )
    ...
    if len(payload) != models["bytes"] or _sha256_bytes(payload) != models["sha256"]:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ registered models.dvc drifted"
        )
    return {
...
def _load_effective_anfis_ablation_dvc_registration_adoption_patch_authority(
    *,
    ...
    if type(audit_current_unpublished) is not bool or verify_remote is not True:
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ effective authority requires exact audit mode and live remote"
        )
    root = _root(repo_root)
    ...
    try:
        ...
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "Published E0-MZ lock is not valid UTF-8 JSON"
        ) from exc
    if not isinstance(payload, dict) or payload_bytes != _canonical_json(payload):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "Published E0-MZ lock is not canonical JSON"
        )
    ...
    try:
        ...
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "Published E0-MZ companion is not valid UTF-8 JSON"
        ) from exc
    ...
    if (
        ...
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "Published E0-MZ companion drifted"
        )
...
@_mz_error_boundary
def load_effective_anfis_ablation_dvc_registration_adoption_patch_authority(
    *,
    audit_current_unpublished: bool = False,
    verify_remote: bool = True,
    repo_root: Path | None = None,
    ...
    return _load_effective_anfis_ablation_dvc_registration_adoption_patch_authority(
        audit_current_unpublished=audit_current_unpublished,
...
@_mz_error_boundary
def _load_effective_anfis_ablation_dvc_registration_adoption_patch_during_registration(
    *,
    transaction_record: Mapping[str, Any],
    verify_remote: bool = True,
    repo_root: Path | None = None,
) -> dict[str, Any]:
    """Internal R-E0-MZ audit while durable rollback coordination is owned."""
    if not isinstance(transaction_record, Mapping):
        raise AnfisAblationDvcRegistrationAdoptionPatchError(
            "E0-MZ internal transaction record is absent"
        )
    return _load_effective_anfis_ablation_dvc_registration_adoption_patch_authority(
...
@_mz_error_boundary
def require_anfis_ablation_dvc_registration_adoption_patch_authority(
    *,
    audit_current_unpublished: bool = False,
    verify_remote: bool = True,
    repo_root: Path | None = None,
) -> dict[str, Any]:
    return load_effective_anfis_ablation_dvc_registration_adoption_patch_authority(
        audit_current_unpublished=audit_current_unpublished,
        verify_remote=verify_remote,
        repo_root=repo_root,
    )
...
def require_anfis_ablation_dvc_registration_authority(
    *, verify_remote: bool = True, repo_root: Path | None = None
    ...
    return require_anfis_ablation_dvc_registration_adoption_patch_authority(
...
def require_anfis_ablation_dvc_registration_adoption_authority(
    *, verify_remote: bool = True, repo_root: Path | None = None
    ...
    """MZ-specific lazy-loader spelling consumed by the registration helper."""
    return require_anfis_ablation_dvc_registration_adoption_patch_authority(
        audit_current_unpublished=False,
        verify_remote=verify_remote,
        repo_root=repo_root,
...
@_mz_error_boundary
def load_effective_anfis_ablation_dvc_registration_adoption_patch(
    *,
    audit_current_unpublished: bool = False,
    verify_remote: bool = True,
    repo_root: Path | None = None,
...
__all__ = [
    "AnfisAblationDvcRegistrationAdoptionPatchError",
    "ANFIS_ABLATION_H_MZ_STAGED_SCOPE",
    "ANFIS_ABLATION_P_MZ_STAGED_SCOPE",
    "ANFIS_ABLATION_R_MZ_STAGED_SCOPE",
    "ADOPTED_LIGHT_COMMIT",
    ...
    "collect_anfis_ablation_dvc_registration_adoption_patch_prelock_state",
    "execute_and_publish_anfis_ablation_dvc_registration_adoption_patch_lock_bundle",
    "load_effective_anfis_ablation_dvc_registration_adoption_patch_authority",
    "load_effective_anfis_ablation_dvc_registration_adoption_patch",
    ...
    "require_anfis_ablation_dvc_registration_adoption_patch_authority",
