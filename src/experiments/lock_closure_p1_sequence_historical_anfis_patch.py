#!/usr/bin/env python
"""Create and verify the additive Closure V1 E0-MC lock bundle."""
...
from src.experiments.closure_p1_sequence_historical_anfis_patch import (  # noqa: E402
...
OUTPUT_GUARD_DIRECTORY = PROJECT_ROOT / "tmp" / "closure_v1_e0_mc_locker"
OUTPUT_GUARD_NAMES = (
    "p1_sequence_historical_anfis_patch_lock.guard",
    "p1_sequence_historical_anfis_patch_lock_manifest.guard",
)
...
def _run_command(
    ...
    *,
    ...
    try:
        ...
        raise P1SequenceHistoricalAnfisPatchError(str(exc)) from exc
...
def _parse_focused_summary(stdout: str, stderr: str) -> int:
    ...
    if (
        ...
        raise P1SequenceHistoricalAnfisPatchError(
            "E0-MC focused pytest summary is not one exact clean result"
        )
...
def _require_success_marker(stdout: str, stderr: str, marker: str) -> None:
    if marker not in stdout + "\n" + stderr:
        raise P1SequenceHistoricalAnfisPatchError(
            f"E0-MC verification marker is absent: {marker}"
        )
...
def _require_empty_output(stdout: str, stderr: str, *, context: str) -> None:
    if stdout.strip() or stderr.strip():
        raise P1SequenceHistoricalAnfisPatchError(
            f"E0-MC expected empty output from {context}"
        )


def run_p1_sequence_historical_anfis_patch_verification() -> dict[str, Any]:
    """Run the closed H-E0-MC verification set; intentionally heavy."""
    if FOCUSED_TEST_COUNT <= 0:
        raise P1SequenceHistoricalAnfisPatchError(
            "E0-MC focused-test count has not been finalized"
        )
    try:
...
def _guard_paths(*, create_directory: bool) -> tuple[Path, Path]:
    ...
    if OUTPUT_GUARD_DIRECTORY != tmp_root / "closure_v1_e0_mc_locker":
        raise P1SequenceHistoricalAnfisPatchError(
            "E0-MC coordination directory escaped tmp/"
        )
    for path, context in (
        (tmp_root, "E0-MC tmp root"),
        (OUTPUT_GUARD_DIRECTORY, "E0-MC guard directory"),
    ):
    ...
    if create_directory:
        try:
            hardened._ensure_real_directory(
                tmp_root,
                parent=PROJECT_ROOT,
                context="E0-MC tmp root",
            )
            hardened._ensure_real_directory(
                OUTPUT_GUARD_DIRECTORY,
                parent=tmp_root,
                context="E0-MC guard directory",
            )
        except hardened.DevelopmentRuntimeTemporalConsumerPatchError as exc:
            raise P1SequenceHistoricalAnfisPatchError(str(exc)) from exc
    return (
...
def _refuse_existing_outputs(output: Path, companion: Path) -> None:
    ...
    if existing:
        raise P1SequenceHistoricalAnfisPatchError(
            f"Refusing to overwrite an existing E0-MC lock bundle: {existing}"
        )
...
def _release_guard_strict(guard: Any) -> None:
    ...
    try:
        ...
        try:
            ...
            pass
        raise P1SequenceHistoricalAnfisPatchError(
            "E0-MC guard ownership could not be verified before release"
        ) from exc
    ...
    if not owned_before or residual or release_error is not None:
        error = P1SequenceHistoricalAnfisPatchError(
            "E0-MC guard release lost ownership or left a residual path"
        )
...
def _anchored_owner_state(owner: Any) -> str:
    try:
        ...
        raise P1SequenceHistoricalAnfisPatchError(
            "E0-MC anchored output ownership could not be inspected"
        ) from exc
    if not stat.S_ISDIR(directory.st_mode):
        raise P1SequenceHistoricalAnfisPatchError(
            "E0-MC anchored output parent is no longer a directory"
        )
    if (
...
def _rollback_owner_anchored(owner: Any) -> None:
    ...
    if state == "foreign":
        raise P1SequenceHistoricalAnfisPatchError(
            "E0-MC rollback preserved a foreign replacement"
        )
    if state == "owned":
        try:
            ...
            raise P1SequenceHistoricalAnfisPatchError(
                "E0-MC anchored output rollback failed"
            ) from exc
    if _anchored_owner_state(owner) != "absent":
        raise P1SequenceHistoricalAnfisPatchError(
            "E0-MC rollback did not establish output absence"
        )
    try:
        os.fsync(owner.directory_file_descriptor)
    except OSError as exc:
        raise P1SequenceHistoricalAnfisPatchError(
            "E0-MC rollback could not synchronize its anchored parent"
        ) from exc
...
def _publish_guarded_bytes(
    ...
    if not guard_owned:
        raise P1SequenceHistoricalAnfisPatchError(
            "E0-MC output guard changed before publication"
        )
    try:
        ...
        while offset < len(payload):
            ...
            if written <= 0:
                raise P1SequenceHistoricalAnfisPatchError(
                    "Short write while preparing an E0-MC output"
                )
            offset += written
        os.fsync(guard.file_descriptor)
        if not hardened._guard_is_owned(guard):
            raise P1SequenceHistoricalAnfisPatchError(
                "E0-MC output guard changed while writing"
            )
    except hardened.DevelopmentRuntimeTemporalConsumerPatchError as exc:
        raise P1SequenceHistoricalAnfisPatchError(str(exc)) from exc
    except OSError as exc:
        raise P1SequenceHistoricalAnfisPatchError(
            "E0-MC output preparation failed"
        ) from exc

    try:
        destination_descriptor = hardened._open_repo_directory(
            expected.parent,
            context="E0-MC final-output parent",
        )
    except hardened.DevelopmentRuntimeTemporalConsumerPatchError as exc:
        raise P1SequenceHistoricalAnfisPatchError(str(exc)) from exc
    try:
        ...
        raise P1SequenceHistoricalAnfisPatchError(
            f"Refusing to clobber an E0-MC output: {destination}"
        ) from exc
    except BaseException:
        os.close(destination_descriptor)
        raise
    ...
    try:
        ...
        if not hardened._owned(owner):
            raise P1SequenceHistoricalAnfisPatchError(
                "E0-MC published output identity drifted"
            )
        return owner
        ...
        except (P1SequenceHistoricalAnfisPatchError, OSError) as exc:
        ...
        try:
            ...
            cleanup_errors.append(exc)
        if cleanup_errors:
            cleanup_error = P1SequenceHistoricalAnfisPatchError(
                "E0-MC output rollback could not be completed safely"
            )
        ...
        if isinstance(error, P1SequenceHistoricalAnfisPatchError):
            raise error
        raise P1SequenceHistoricalAnfisPatchError(
            "E0-MC output publication failed"
        ) from error
...
def _cleanup_lock_resources(
    ...
    *,
    ...
    if succeeded and not guard_release_failed:
        for owner in owners:
            try:
                if not hardened._owned(owner):
                    raise P1SequenceHistoricalAnfisPatchError(
                        "E0-MC published output changed after guard release"
                    )
            except (
                P1SequenceHistoricalAnfisPatchError,
    ...
    if cleanup_errors:
        cleanup_error = P1SequenceHistoricalAnfisPatchError(
            "E0-MC lock resource cleanup failed closed"
        )
        cleanup_error.add_note(
            "Cleanup failures: "
            + "; ".join(
...
def _execute_lock(output: Path, companion: Path) -> dict[str, Any]:
    ...
    try:
        ...
        prelock = collect_p1_sequence_historical_anfis_patch_prelock_state(
            verify_remote=True
        )
        verification = run_p1_sequence_historical_anfis_patch_verification()
        p1_sequence_namespace_absence()
        closure_progression_namespace_absence()
        repeated = collect_p1_sequence_historical_anfis_patch_prelock_state(
            verify_remote=True
        )
        if repeated != prelock:
            raise P1SequenceHistoricalAnfisPatchError(
                "E0-MC authority changed during verification"
            )
        hardened._assert_lock_namespace(lock_path, companion_path, guards, owners)

        timestamp = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
        payload = build_p1_sequence_historical_anfis_patch_lock_payload(
            prelock,
            verification,
    ...
    try:
        ...
        raise P1SequenceHistoricalAnfisPatchError(
            "E0-MC lock transaction cleanup failed closed"
        ) from exc
    if active_error is not None:
        if isinstance(active_error, P1SequenceHistoricalAnfisPatchError):
            raise active_error
        raise P1SequenceHistoricalAnfisPatchError(
            "E0-MC lock transaction failed"
        ) from active_error
    if result is None:
        raise P1SequenceHistoricalAnfisPatchError(
            "E0-MC lock transaction produced no result"
        )
    return result
...
def _open_effective_bundle_parent() -> int:
    try:
        return hardened._open_repo_directory(
            ...
            context="E0-MC effective bundle parent",
        )
    except hardened.DevelopmentRuntimeTemporalConsumerPatchError as exc:
        raise P1SequenceHistoricalAnfisPatchError(str(exc)) from exc
...
def _require_effective_parent_current(directory_descriptor: int) -> None:
    try:
        current_descriptor = hardened._open_repo_directory(
            DEFAULT_PATCH_LOCK_PATH.parent,
            context="E0-MC current effective bundle parent",
        )
    except hardened.DevelopmentRuntimeTemporalConsumerPatchError as exc:
        raise P1SequenceHistoricalAnfisPatchError(str(exc)) from exc
    try:
        ...
        raise P1SequenceHistoricalAnfisPatchError(
            "E0-MC effective bundle parent could not be inspected"
        ) from exc
    finally:
    ...
    if (
        ...
        raise P1SequenceHistoricalAnfisPatchError(
            "E0-MC effective bundle parent identity changed"
        )
...
def _anchored_entry_exists(directory_descriptor: int, name: str) -> bool:
    try:
        ...
        raise P1SequenceHistoricalAnfisPatchError(
            "E0-MC effective bundle residual could not be inspected"
        ) from exc
    return True
...
def _effective_bundle_identities(
    ...
    if existing:
        raise P1SequenceHistoricalAnfisPatchError(
            f"E0-MC effective preflight found temporary or guard paths: {existing}"
        )
    return (
...
def _require_anchored_authorization_inputs(
    ...
    if (
        ...
        raise P1SequenceHistoricalAnfisPatchError(
            "E0-MC effective authority inputs drifted"
        )
    records = {
    ...
    if set(records) != {path for path, _role, _identity in expected}:
        raise P1SequenceHistoricalAnfisPatchError(
            "E0-MC effective authority input paths drifted"
        )
    for path, role, identity in expected:
        ...
        if (
            ...
            raise P1SequenceHistoricalAnfisPatchError(
                f"E0-MC anchored authority input differs from loader: {path}"
            )
...
def _check_effective(output: Path, companion: Path) -> dict[str, Any]:
    ...
    if lock_path.parent != companion_path.parent:
        raise P1SequenceHistoricalAnfisPatchError(
            "E0-MC lock and companion escaped their shared parent"
        )
    ...
    try:
        ...
        summary = require_p1_sequence_historical_anfis_authorized(
    ...
    if before != after:
        raise P1SequenceHistoricalAnfisPatchError(
            "E0-MC lock bundle changed during effective preflight"
        )
