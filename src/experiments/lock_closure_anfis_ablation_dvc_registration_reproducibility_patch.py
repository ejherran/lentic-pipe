#!/usr/bin/env python
"""Check and publish the outcome-closed E0-MZE manifest-provenance authority."""
...
def _count(value: Any, name: str) -> int:
    ...
    raise patch.AnfisAblationDvcRegistrationReproducibilityPatchError(
        f"E0-MZE check-only summary is missing exact integer: {name}"
    )
...
def _missing_pointer_validation(value: Any) -> dict[str, Any]:
    ...
    if not isinstance(value, Mapping) or not patch._exact_equal(dict(value), expected):
        raise patch.AnfisAblationDvcRegistrationReproducibilityPatchError(
            "E0-MZE missing-pointer validation contract drifted"
        )
    return expected


def _in_progress_namespace_validation(value: Any) -> dict[str, Any]:
    """Require the guarded exact-prefix policy preserved from E0-MZD."""
    ...
    if not isinstance(value, Mapping) or not patch._exact_equal(dict(value), expected):
        raise patch.AnfisAblationDvcRegistrationReproducibilityPatchError(
            "E0-MZE in-progress namespace validation contract drifted"
        )
    return expected


def _porcelain_scope_validation(value: Any) -> dict[str, Any]:
    """Require strict exact-map validation at every Git porcelain frontier."""

    expected = patch._porcelain_scope_validation()
    if not isinstance(value, Mapping) or not patch._exact_equal(dict(value), expected):
        raise patch.AnfisAblationDvcRegistrationReproducibilityPatchError(
            "E0-MZE porcelain scope-validation contract drifted"
        )
    return expected


def _manifest_provenance_correction(value: Any) -> dict[str, Any]:
    """Require the exact blocked-R manifest/Git provenance incident."""

    expected = patch._manifest_provenance_correction(PROJECT_ROOT)
    if not isinstance(value, Mapping) or not patch._exact_equal(dict(value), expected):
        raise patch.AnfisAblationDvcRegistrationReproducibilityPatchError(
            "E0-MZE manifest provenance incident/correction contract drifted"
        )
    return expected


def _manifest_script_provenance_validation(value: Any) -> dict[str, Any]:
    """Require the closed map10 and exact-two-finding adoption policy."""

    expected = patch._manifest_script_provenance_validation(PROJECT_ROOT)
    if not isinstance(value, Mapping) or not patch._exact_equal(dict(value), expected):
        raise patch.AnfisAblationDvcRegistrationReproducibilityPatchError(
            "E0-MZE manifest script provenance validation contract drifted"
        )
    return expected


def check_only() -> dict[str, Any]:
    """Run the schema-first, non-writing H-E0-MZE preflight."""

    schema = patch.preflight_anfis_ablation_dvc_registration_reproducibility_patch_schema()
    prelock = patch.collect_anfis_ablation_dvc_registration_reproducibility_patch_prelock_state(
        verify_remote=True
    )
    ...
    provenance = _manifest_provenance_correction(
        prelock.get("manifest_provenance_correction")
    )
    ...
    porcelain_scope_validation = _porcelain_scope_validation(
        registration.get("porcelain_scope_validation")
        if isinstance(registration, Mapping)
        else None
    )
    manifest_script_provenance_validation = _manifest_script_provenance_validation(
        registration.get("manifest_script_provenance_validation")
        if isinstance(registration, Mapping)
        else None
    )
    counts = {
    ...
    if counts != expected_counts:
        raise patch.AnfisAblationDvcRegistrationReproducibilityPatchError(
            f"E0-MZE check-only counts drifted: {counts}"
        )
    return {
        ...
        "base_p_mzd_commit": patch.P_MZD_COMMIT,
        "missing_pointer_validation": missing_pointer_validation,
        "in_progress_namespace_validation": in_progress_namespace_validation,
        "porcelain_scope_validation": porcelain_scope_validation,
        "manifest_provenance_correction": provenance,
        "manifest_script_provenance_validation": (
            manifest_script_provenance_validation
        ),
        **counts,
...
def check_effective() -> dict[str, Any]:
    """Inspect the exact published manifest-provenance patch authority."""

    return patch.load_effective_anfis_ablation_dvc_registration_reproducibility_patch_authority(
        verify_remote=True
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except patch.AnfisAblationDvcRegistrationReproducibilityPatchError as exc:
