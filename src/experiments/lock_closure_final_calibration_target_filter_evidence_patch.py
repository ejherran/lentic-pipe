#!/usr/bin/env python
"""Check, publish, and audit the E0-MCALE target-filter-evidence authority."""
...
def _error(message: str) -> patch.FinalCalibrationTargetFilterEvidencePatchError:
    return patch.FinalCalibrationTargetFilterEvidencePatchError(message)
...
def _parse_focused_summary(stdout: str, stderr: str) -> dict[str, int]:
    ...
    if type(count) is not int or count <= 0:
        raise _error("E0-MCALE focused-test count is not frozen")
...
def check_only() -> dict[str, Any]:
    schema = patch.preflight_final_calibration_target_filter_evidence_patch_schema()
    prelock = (
        patch.collect_final_calibration_target_filter_evidence_patch_prelock_state(
            verify_remote=True
        )
    )
    return {
...
def execute_lock() -> dict[str, Any]:
    ...
    verification = run_final_calibration_target_filter_evidence_patch_verification(
        expected_schema_preflight=schema
    )
    after = patch.collect_final_calibration_target_filter_evidence_patch_prelock_state(
        verify_remote=True
    )
    if before != after:
        raise _error("E0-MCALE prelock state changed during verification")
...
def check_effective() -> dict[str, Any]:
    return patch.load_effective_final_calibration_target_filter_evidence_patch_authority(
        verify_remote=True
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except patch.FinalCalibrationTargetFilterEvidencePatchError as exc:
